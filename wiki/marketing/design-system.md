...
**갱신: 2026-07-11 (컨셉 전환 반영).** 원출처: [proposals/2026-07-07-brand-kit.md](./proposals/2026-07-07-brand-kit.md) §5 → 얼굴 **비노출** 확정([naming.md](./naming.md) §2) + **AI 콘텐츠 비서 컨셉**([ADR-005](../decisions/005-brand-naming.md))에 맞춰 조정. 팔레트·타이포·규격·파이프라인은 유지, 로고·태그라인·모티프만 비서로 교체.
...
| 역할 | hex | 용도 |
|---|---|---|
| Base (dark) | `#0B0F1A` | 배너·썸네일 기본 배경 (사장님 퇴근 후에도 돌아가는 "심야 근무 비서" 무드 — 프리미엄·신뢰) |
...
## 3. 로고 (얼굴 비노출 + 비서 컨셉, 2026-07-11 교체 — 회장 veto 가능)

> 구 "SJ 커서 블록 모노그램"·"`> sj` 워드마크"는 개발자 페르소나 유물 — 폐기.

1. **발행 스탬프 모노그램** (프로필 아바타 정본): 원형 결재 도장 형태 안에 "OSMU"의 O + 체크마크(✓), amber `#F59E0B` on `#0B0F1A` — "비서가 처리 완료 도장을 찍었다"는 은유(사장님 문화의 결재/도장 코드). 전 플랫폼 동일 이미지. 원형 크롭·64px 축소에서 판독 생존 필수.
2. **`osmu ✓` 워드마크**: 워터마크·썸네일 코너 뱃지·카드뉴스 하단 서명("비서가 처리함" 서명 기능). JetBrains Mono 소문자, 보조 그레이(`#94A3B8`) 기본, 강조 시 amber.
...
- safe zone 안: 좌측 60% = 태그라인(7~12단어, Pretendard ExtraBold, 2줄 이내) + 업로드 요일 1줄(JetBrains Mono, mint — "매주 목요일 운영 보고").
- 우측 40% = OSMU 대시보드(발행 큐/보고 화면) 스크린샷 15° 기울여 배치.
- 배경: `#0B0F1A` + 미세 그리드 패턴(불투명도 2%). 이모지·장식 아이콘 금지.
- 확정 태그라인: **"사장님 SNS는 제가 굴립니다 — 매주 목요일 운영 보고"** (표시 이름 낙점 시 naming.md 기준 재확인)
...
4. 하단 좌측 코너 `osmu ✓` 뱃지 고정 (시리즈 인지 + "비서가 처리함" 서명).
...
SOURCES/MODEL: [Fable 5] · 원출처 brand-kit §5(WebSearch 벤치마크 7건 기반: snappa/b2w/adobe 배너 규격 등) · 조정 = 얼굴 비노출 확정(ADR-005) 반영, AI 프롬프트 가이드 신규(§7 — store-visual 스킬 파이프라인 규율 계승) · 2026-07-11 = 로고("SJ 커서"→"발행 스탬프")·태그라인·모티프를 AI 비서 컨셉으로 교체(naming.md·brand.md 정합, 회장 veto 가능)
