...
정본 계층: 태그가 참조하는 번호 체계 = [hook-bank.md](./hook-bank.md)(H-XX)·[content-calendar.md](./content-calendar.md)(S1–S4·Day)·[growth-log.md](./growth-log.md)(실험 EXP-1~10)·[gtm-plan.md](./gtm-plan.md)(GA4 이벤트) — **이 파일은 그 문서들을 수정하지 않고 연결만 한다.** 워딩 = [naming.md](./naming.md) §2 공장 세계관(출고/생산/라인/리포트).
...
| `sig=pain`·`sig=feature` 신호 주 3건+ 동일 주제 누적 | [product/vision.md](../product/vision.md) 반영 제안문 (신호 verbatim 인용 포함) | 하위모델 | **회장** (기획 반영 = 제품 정본) |
| IG 카드뉴스 저장·도달이 특정 표지 유형에서 우위 | [design-system.md](./design-system.md) 카드 규격 개정 제안 | 하위모델 | **회장** (디자인 정본) |
