...
## Explicitly open production observations

The following are **user-reported/production-observed open issues**. They are not downgraded by local code or a successful HTTP health check:
...
4. OSMU was observed returning a 502. A later `/` and health 200 does not identify or clear the incident.

These need a future browser + provider + production-log investigation; no fix is claimed here.

## Source and self-check

**Sources:** `tasks/osmu-full-ui-code-audit.output`; `wiki/ops/session-state.md` (2026-08-06 audit/open-issue section); current `dashboard/src/app`, `components/layout/Sidebar.tsx`, channel, Studio, Settings, Inbox and Calendar code.
