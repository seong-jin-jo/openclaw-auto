# Launch Pack — 2026-07-16 (OSMU 팩토리, Instagram+Threads v1)

**갱신: 2026-07-16 (워딩 전환: 비서→공장 "OSMU 팩토리" — 회장 직접 낙점, naming.md §2)** · 작성: content-growth-marketer (series-content-planner + hook-angle-lab + social-post-packager + openclaw-creative-brief 스킬 / 공장 워딩 개정 = hook-angle-lab)
정본 계층: 이름·핸들 확정값 = [naming.md](../../1-team-brand/naming.md) §2, [ADR-005](../../3-operations/decisions/005-brand-naming.md) · 톤·필러 = [brand.md](../../1-team-brand/brand.md) · 후크 원문 = [hook-bank.md](./hook-bank.md) · 시리즈 구조 = [content-calendar.md](./content-calendar.md) · 채널 실행 = [channels/instagram.md](./channels/instagram.md), [channels/threads.md](./channels/threads.md).

이 문서는 **실행 문서** — 아이데이션이 아니라 회장 확정(결정일 2026-07-14, 출시일 2026-07-16) 이후의 발행 준비 큐다. 여기 실린 모든 콘텐츠는 `wiki/5-hubs/hub-mkt/social-launch-v1.json`에 기계판으로 미러링돼 있으며, **전부 `status: draft`, `requires_manual_approval: true`** — 이 문서·JSON 어디에도 자동 발행 트리거는 없다.
...
**2026-07-16 워딩 재전파(최종) 변경 요약**: 회장 재지시("비서보다 공장 워딩") + "OSMU 팩토리" 직접 낙점(프리뷰 3안 중 선택)에 따라 프로필·bio·고정글·T-01~T-04·IG 캐러셀/카드 전 카피를 **공장 어휘(출고/생산/라인/가동/주간 리포트)·주어 절제 리포트체**로 교체. 핸들 2순위 @osmu.secretary→@osmu.factory. 발행 정책(전부 draft·수동 승인·허위 희소성 금지·raw 미사용)·JSON schema 구조는 무변경. JSON↔MD exact-substring·char_count 재검증 완료(§9 재실행, 이 세션).
...
| 항목 | 확정값 |
|---|---|
| 표시 이름 | **OSMU 팩토리** (naming.md §2, 2026-07-16 회장 직접 낙점) |
| 핸들 1순위 | `@osmu.official` — 리네임 화면에서 회장이 가용 확인 후 실행 |
| 핸들 2순위 | `@osmu.official` 확보 실패 시 `@osmu.factory` (구 @osmu.secretary는 워딩 전환으로 폐기) |
| 얼굴 노출 | 없음 (공장 = 시설 인격) |
| 프로필 이미지 | **v1 제작 완료 (2026-07-15, 회장 확정 대기)** — `scratchpad/osmu-launch-assets/profile-osmu-v1.png`(1080×1080, 씰+체크 모티프 — 공장 "출고 완료 도장" 컨셉과 정합, design-system §3). O+체크 문자적 결합 대신 48px 판독 우선한 실루엣 채택(트레이드오프, 회장 확인 필요). 확정 후 리네임 화면 업로드는 회장 수동(§0-1) |

### 1.1 Instagram Bio (150자 한도, 실측 약 90자 — [channels/instagram.md](./channels/instagram.md) §2 그대로)
...
### 1.2 Threads Bio (150자 한도, 실측 약 90자 — [channels/threads.md](./channels/threads.md) §2 그대로)
...
이 계정도 이 공장 라인에서 출고됩니다. 공장이 제대로 도는지 궁금하면 이 계정이 꾸준히 올라오는지만 보면 됩니다.
...
콘텐츠 맡길 공장이 필요한 사장님은 프로필 링크로 가입/문의 주세요. 지금은 공개 베타 운영 중입니다.
```
글자수: 실측 397자(500자 한도 내, §9 검증 명령 결과 — 2026-07-16 공장판 재실측). content_id = `T-PIN-01`.
...
### T-01 — Day 1 공장 가동 개시 선언 (H-37, family=receipt, CTA=C-05)
...
```
숫자출처: `{N}` = 리네임 완료 직후 실측 팔로워 수(baseline 스냅샷) — 리네임 전 발행 금지. 글자수 실측 260자.
...
사람이 쓴 글 어느 건지 찍어보시길.
```
**사용 조건(hook-bank H-11 원 규정)**: 이 글은 반드시 실제 자동 생성·큐 경유 발행일 때만 사용 — 수동 작성 시 이 후크를 발행 금지. 숫자출처: `{T}` = 큐 타임스탬프 실측, [캡처: 예약 큐 화면] 필수. 글자수 실측 192자.

### T-04 — 첫 주간 리포트 (H-36, family=receipt, CTA=C-05)
...
```
숫자출처: 전 항목 = 출고 로그·insights 집계(치환 불가 시 해당 줄 삭제, 3숫자 전부 없으면 발행 불가 — S1 규칙). 발행 예정일 = Day 8(2026-07-23, 목).
...
굴립니다.
```
2. "여기는 사장님의 콘텐츠 공장, OSMU 팩토리입니다."
3. "사장님이 장사하는 동안 SNS를 만들고, 고치고, 올리고, 반응을 리포트합니다."
...
**Alt text (접근성 — Meta 접근성 가이드 반영, JSON `alt_text` 배열과 exact text 동일):**
- 슬라이드1: "다크 배경에 흰 텍스트 '이 계정, 사람이 안 굴립니다' 3줄 카드뉴스 커버"
- 슬라이드2: "'여기는 사장님의 콘텐츠 공장, OSMU 팩토리입니다' 텍스트 카드"
- 슬라이드3: "'사장님이 장사하는 동안 SNS를 만들고, 고치고, 올리고, 반응을 리포트합니다' 텍스트 카드"
- 슬라이드4: "'브랜드 위키를 읽고 씁니다. 지어내지 않습니다' 텍스트 카드"
- 슬라이드5: "'맡기고 싶은 사장님은 프로필 링크로' CTA 텍스트 카드"

해시태그(3~5개, 풀 내): `#자영업자 #소상공인 #사장님 #콘텐츠자동화 #마케팅자동화`
...
[슬라이드3 입력] 실제 발행 copy(JSON slide 3 text와 정확히 동일):
```
공장에 먼저 넣는 것 — 가게 사실 위키: 메뉴 / 가격 / 산지 / 말투
```
...
```
같은 주문을 두 번 했음. "우리 카페 신메뉴 글 써줘." 일반 AI vs 가게 사실을 아는 콘텐츠 공장 — 차이는 4번째 장. 샘플 위키로 돌린 데모임(실고객 아님). 우리 가게도 되나 싶으면 DM.
```
**Alt text (접근성 — Meta 공식 Instagram Help Center 접근성/대체 텍스트 가이드 반영, JSON `alt_text` 배열과 exact text 동일):**
- 슬라이드1: "'우리 가게를 모르는 AI vs 아는 AI' 대비 텍스트 카드뉴스 커버"
- 슬라이드2: "'AI한테 홍보글 시켰다가 접은 이유' 문제 제기 텍스트 카드"
- 슬라이드3: "'공장에 먼저 넣는 것 — 가게 사실 위키: 메뉴·가격·산지·말투' 입력 항목 텍스트 카드"
...
[캡션 — 첫 125자 안에 훅] 실제 발행 copy(JSON caption과 정확히 동일, 한 줄):
```
이번 주 숫자만 딱: 출고 {N}건 · 팔로워 {±M} · 비용 ₩{X}. 좋았던 것도 아쉬웠던 것도 숨기지 않고 그대로 올림. 자세한 얘기는 프로필 링크 블로그에 있음.
```
**Alt text (접근성 — Meta 공식 Instagram Help Center 접근성/대체 텍스트 가이드 반영, JSON `alt_text` 배열과 exact text 동일):**
- 슬라이드1: "출고 건수·팔로워 증감·비용 3개 숫자를 amber 강조한 주간 지표 카드"

숫자출처: 출고 로그·growth-log·가동비 실측 3종 — 전부 확보 전엔 발행 불가(§0-4). content_id = `IG-FOLLOW-02`. 예정일: Day 14(2026-07-30, 수) — 첫 주간 리포트(T-04) 이후 지표가 쌓인 시점.
...
- **방어 수정**: T-01은 가동 개시 선언이지 성과 주장이 아니다. 성과 주장(H-36 주간 리포트)은 실측 출고 로그가 쌓인 Day 8 이후에만 배치했다 — 순서 자체가 방어.
- **공격 3 (공장 워딩 전환분)**: "공장이라니 — 내 가게 글을 찍어내는 공산품 취급인가? 몰개성 대량생산 아냐?"
- **방어 수정**: 공장 워딩의 짝은 항상 그라운딩이다 — 고정글·T-02·IG-02가 "브랜드 위키를 읽고, 거기 없는 건 라인에 올리지 않음"을 반복 노출해 '대량생산=몰개성' 연상을 '위키 기반 맞춤 생산'으로 재프레임. H-27("가게 이름 지워보셈")이 오히려 몰개성 홍보글을 공격하는 우리 각이라 방어와 공격이 같은 축에 있다.
...
| risk=raw 미사용 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 표기(OSMU/오스무, OpenClaw 미언급, 주어 절제 리포트체·자칭 OSMU 팩토리만·"저/제가" 0회) | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 필러 소속 명확 | 5+2 | 3 | 2 | 1+5 | 1+2 | 1+3 | 1+5 |
| 글자수 한도(Threads 500/IG bio 150·캡션 2200 첫125접힘) | 실측260자 OK | 실측273자 OK | 실측192자 OK | 실측256자(placeholder 원문 기준, 실측 치환 후 재확인 — §9) | 캡션 실측381자, 훅 첫125안 OK | 캡션 실측112자 OK | 캡션 실측95자 OK |
...
**공장 워딩 재전파 후 재실행 결과(2026-07-16 워딩 전환 세션)**: 1) IDs unique·draft·approval·safety flags PASS / 2) char_count 전항목 실측 일치 PASS(T-PIN-01 397, T-01 260, T-02 273, T-03 192, T-04 256, IG-PIN-01 caption 381, IG-FOLLOW-01 112, IG-FOLLOW-02 95) / 3) 허위 희소성 0건 PASS / 5) alt_text 계약 PASS(전항목 <100자) / 추가: 전 라이브 copy·slide·alt·DM에서 "비서" 0건 PASS.
...
⛔ 회수 필요:
1. ~~프로필 이미지(씰+체크 심볼) 제작~~ **2026-07-15 v1 완료** — `scratchpad/osmu-launch-assets/profile-osmu-v1.png` (공장 "출고 완료 도장" 컨셉과 정합, 워딩 전환에도 재제작 불요). 남은 것: 회장이 "O+체크 문자성 생략, 씰+체크 실루엣만" 트레이드오프를 확정할지 판단(§1 표 참조), 확정 후 실제 리네임 화면 업로드는 여전히 회장 수동.
...
RUBRIC_SCORE: hook=4/5 detail=5/5 rhythm=4/5 voice=5/5 slop=5/5 total=23/25 (2026-07-16 공장 워딩판 재채점 — T-01 "팔로워 {N}명에서 가동 시작"·T-PIN "개업 인사 대신 가동 안내" 첫 줄 생존, 리포트체 voice 정합, 블랙리스트 0)
WEAKEST_LINE: "다음 주 라인 예약분 {M}건은 이미 큐에 들어가 있음." (T-04) — 문장 자체는 정보 전달이나 플레이스홀더 밀도가 높아 실제 숫자 치환 전에는 리듬이 기계적으로 읽힘(치환 후 재확인 대상, §9. 구판 감사 지적 계승 — IG-FOLLOW-01 슬라이드4 alt 잠정 상태도 캡처 확보 시 재작성 필요, §0-4 블로킹 조건과 연동).

CoVe 자문(공장 워딩 재전파, 결론 직전 자가검증): "이 재전파가 틀렸다면 가장 그럴듯한 이유는?" → ①JSON↔MD exact-substring 불일치(3차 재작업에서 걸렸던 바로 그 오류) 재발 가능성 — 이번엔 JSON을 스크립트로 먼저 확정하고 MD를 그 원문에 맞춰 미러링, §9 재실행으로 char_count 전항목 일치 확인. ②"굴립니다" 어휘가 공장 컨셉과 충돌한다는 오독 — naming.md §2·brand.md §5가 "이 계정도 공장이 굴림"을 필러 2 확정 표현으로 명시(주어가 공장일 때 허용), IG 커버 "이 계정, 사람이 안 굴립니다"는 유지가 정본. ③확정 정책(draft·수동 승인·DM·희소성) 훼손 가능성 — §9 재실행에서 status/approval/safety flags/희소성 전부 PASS, DM 템플릿 카피는 무변경. ④공장 워딩이 '몰개성 대량생산' 반감을 살 가능성 — §6 공격 3으로 방어 수정 반영(그라운딩 반복 노출로 재프레임). 0번 게이트 자문: 매칭 스킬(hook-angle-lab)을 실제 Skill 툴로 호출했고, 산출 전 writing/marketing/branding 헌법과 naming.md §2·ADR-005를 Read했다.

SKILLS_USED: hook-angle-lab(공장 어휘 후크·T-01~T-04·H1 계열 재작성 — 실제 Skill 툴 호출, family·심리축 보존) / openclaw-creative-brief·series-content-planner·social-post-packager(구판 골격 — 이번 판은 그 구조를 보존한 워딩 층 교체)
SKILLS_SKIPPED: 없음(구조 변경 없는 워딩 재전파라 hook-angle-lab 1개로 충분 — 시리즈·브리프 구조는 기존 정본 재사용)
SOURCES/MODEL: claude-fable-5 (Fable 5) · 품질헌법 = ~/.claude/standards/{writing,marketing,branding}.md(2026-07-16 Read) · 내부 = wiki/1-team-brand/naming.md(2026-07-16 공장 확정판)·ADR-005(재개정판)·brand.md·channels/{threads,instagram}.md·hook-bank.md·content-calendar.md(전부 이 세션 공장판)·social-launch-v1.json(스크립트 수정·검증) · 외부(WebSearch 2026-07-16) = contentfac.com("The Content Factory" 팩토리 네이밍 카테고리 성립), gogumafarm.kr(제조 코드 감성 자산화 — 곰표) · 계승 = Meta Instagram Help Center 접근성 가이드(alt_text 규격), 구판 벤치마크 전체(각 문서 SOURCES 참조)
