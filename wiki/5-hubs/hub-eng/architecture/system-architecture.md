...
**지금 어디에 저장되는가.** 고객이 올리거나 생성한 이미지는 대시보드 컨테이너의 영속 볼륨
`osmu-data:/app/data` 아래 테넌트별 경로(`tenants/<tenant_id>/images`)에 파일로 저장된다.
객체 저장소가 아니다. 배포로 지워지지 않도록 compose project 와 무관한 고정 이름 볼륨을 쓴다.
...
큐에 예약된 이미지는 업로드 시점에 절대 HTTPS 서명 URL 을 발급받아 저장한다. 며칠 뒤 예약
발행이 실행될 때 그 URL 을 그대로 쓰기 때문에, 짧은 만료 토큰을 영속 저장하면 발행 시점에
이미 죽어 있다.

**따라서 객체 저장소(R2)는 발행의 전제 조건이 아니다.** 서명 배달 경로가 이미 그 역할을 한다.
R2 는 내구성과 규모를 위한 선택지이지, 없다고 이미지 발행이 막히는 구조가 아니다.
2026-08-31 에 "R2 미설정이라 아무것도 생성되지 않는다"고 서술한 보고가 있었으나 사실이 아니다.
그때의 실제 원인은 생성 경로에 LLM 이 붙어 있지 않았던 것과 학습 정보 모달이 화면을 덮은 것이다.

**R2 현황.** 2026-09-01 에 Cloudflare 계정에 `osmu-media` 버킷을 만들었다. 아직 제품이 쓰지 않는다.
쓰려면 공개 개발 URL 또는 커스텀 도메인 연결이 필요하고 그 설정은 Cloudflare 콘솔에서만 된다.
현재 보유한 API 토큰은 터널 전용이라 R2 권한이 없다.
