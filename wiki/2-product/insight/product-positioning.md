...
개봉 장면 1컷: 연결 버튼이 초록으로 바뀌었는데 실제로는 토큰이 붙지 않은 화면. 우리 제품에서 실제로 보고된 OAuth 거짓 성공이다(근거: `wiki/4-reference/channel-status.md` "OAuth false-success" 미해결 항목). 그날부터 우리는 "됐습니다"라는 문구를 팔지 않고 증거를 팔기로 했다.
...
- 판정: **진입자격. 차별점 아님.** 텍스트 발행 코드 목록은 8개, 영상 직접 발행 경로는 2개, 발행 extension은 15개지만 최신 실제 공개 채널 발행은 미검증이다([채널 상태](../../4-reference/channel-status.md)). 구현 수를 Live 채널 수로 말하지 않는다.
...
- 우리 증거: 시장 공백은 실재한다. 2026년 조사에서 사회적 활동과 매출을 명확히 연결할 수 있다고 답한 팀은 35%뿐이다. 저장소에는 GA 분석과 Slack 설정·테스트·미리보기·전송 코드가 존재하지만, Awareness 밖의 퍼널 제품 범위와 운영 end-to-end는 미검증이다. 계획 이력은 [GA4·Slack 중앙 성과 통합](../build/plan-ga4-slack-central.md)에 남아 있다.
...
### 성과 측정 범위 확정 — Awareness 집중 (회장 R-14, 2026-08-12)

회장이 성과 측정 범위를 못박았다. 우리는 AARRR 다섯 칸 전부를 대신 돌리는 척하지 않는다. **첫 칸 Acquisition, 그중에서도 Awareness(노출·반응)만** 관리·측정한다.

- **측정하는 것 = 플랫폼 API가 주는 노출·반응 지표뿐**: 노출/조회수, 저장, 좋아요, 댓글, 팔로우, 프로필 방문. 콘텐츠가 얼마나 퍼졌고 사람들이 얼마나 반응했는지까지가 우리 성과의 정의다.
- **제외(후순위 옵션) = 웹사이트 유입·전환 측정**: GA4, 전환 퍼널, 검색 콘솔, 검색 어드바이저, Clarity 류. 이건 웹/앱을 운영하는 IT 사업자에게만 의미가 있다. 우리 타겟에는 웹사이트가 없는 자영업자(음식점 사장, 헬스장 사장)가 포함되므로 기본 제품 범위에서 뺀다. 요구가 쌓이면 웹사이트 가진 고객에 한해 옵션으로 붙인다.
- **유지(우리 핵심 입력) = 트렌드·키워드 도구**: 네이버/구글 트렌드, 키워드 플래너는 유입 측정이 아니라 **콘텐츠 소재 발굴 = 생성 입력**이라 그대로 둔다. 자체 블로그 SEO도 우리 마케팅 자산이라 유지한다.
- **판정 근거**: 이 확정으로 위 4층 판정의 4층(AARRR 퍼널)은 "비전 섹션 전용"에서 **"기본 제품 범위 밖, 웹 고객 옵션"으로 한 칸 더 내려간다.** GA4·슬랙 중앙 성과 통합(`wiki/2-product/build/plan-ga4-slack-central.md`)은 후순위 옵션 트랙으로 재분류한다.

아래 표의 상태는 이 스코프 확정 이전 매핑이며, Acquisition/Retention 실행 자동화 + Activation 근거 정리라는 정직한 범위는 유지된다. Revenue·Referral·전환 추적은 위 확정에 따라 후순위다.
...
전환 연결(광고 클릭에서 신청까지의 추적)은 GA4·슬랙 중앙 성과 통합 계획에 설계만 있고, R-14 확정으로 **후순위 옵션(웹 고객 한정)** 트랙이 됐다. 따라서 현재 우리가 정직하게 말할 수 있는 범위는 **Awareness(노출·반응) 실행 자동화 + Retention의 반복 발행 + Activation의 근거 정리**까지다. Revenue·Referral과 유입-전환 측정은 제품 기본 범위가 아니라 사업 계획·옵션이다.
...
| 주장 | 금지 이유 | 해금 조건 |
|---|---|---|
| "16개 채널 자동 발행" | 코드 목록, 영상 경로, extension 수가 서로 다르고 최신 실채널 발행은 미검증 | 채널별 credential 검증과 실 발행 증거 후 그 채널명만 특정해 표기 |
...
- `/Users/sj/sj_code_master/openclaw-auto/wiki/2-product/_index.md`
- `/Users/sj/sj_code_master/openclaw-auto/wiki/4-reference/channel-status.md`
