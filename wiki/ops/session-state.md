## 2026-09-03 07:20 KST | Codex code-builder | 승인 인박스 공백 본문 운영 결함 수정 착수
...
- 승인 범위: 승인 인박스의 공백 본문과 선택적 제목 판정, 승인 API의 같은 방어, 계약 테스트, 구현현황과 QA 기록만 수정한다. 머지와 배포는 하지 않는다.
- 직접 관찰: 운영 캡처 `docs/qa/osmu-live-walk-20260903/v70-inbox-1024.png`에서 판단할 본문 없이 승인 단추가 활성이고 본문 누락 경고가 없는 상태를 확인했다.
- 코드 대조: 화면에는 이미 `text?.trim()`과 공백 세 칸 테스트가 있으나 승인 API에는 본문 검증이 없다. 화면의 선택적 제목, 주제, 해시태그, 채널 라벨도 공백 정규화 계약이 없다.
- 다음 액션: 공용 판정 함수를 추가해 화면과 승인 API를 같은 기준으로 잠그고, 공백과 줄바꿈 본문, 공백 제목, API 상태 불변 계약을 추가한 뒤 지정 TypeScript와 Vitest를 실행한다.
