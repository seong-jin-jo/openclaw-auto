## [2026-08-28 작업 공간 간 테넌트 격리 공격 build 착수]
...
- **현재 상태:** QA 트래커에 교차 작업 공간 읽기·쓰기·삭제와 무토큰·만료 토큰·몸통 위조의
  최신 실앱 증거가 없다는 NG와 종료조건을 등록했다. 기존 더티 변경은 건드리지 않는다.
- **정확한 다음 작업:** 기반 산출물과 전체 테넌트 API inventory를 읽고 실제 두 작업 공간과
  토큰을 만든 뒤 수정 전 공격표를 얻는다. 누출만 최소 수정하고 회귀 테스트, 실앱 재공격,
  전체 Vitest, TypeScript, production build, 문서 갱신, 커밋으로 닫는다.
