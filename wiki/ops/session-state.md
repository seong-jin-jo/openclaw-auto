## [2026-08-28 Opus 교차검수 잔여 결함 재검증 진행]

**핸드오프 기준: 회장이 명시한 `docs/audit/osmu-cross-review-2026-08-28-opus.md`와 기반 산출물 3종. 관련 tmux pane은 없음.**

- build 허용: `pipeline-state.osmu.md`에서 소스·테스트·로컬 실행은 허용된다. 새 스키마·API 결정은 이번 세션이 임의 확정하지 않는다.
- 기존 수정 확인: M1, I1~I3, C1~C3, F1, F4 대응 커밋이 HEAD에 있다. D2 자동저장 코드도 복구돼 있다.
- 재검증에서 남은 결함: M2 내부 멱등 삽입 충돌, C4 응답 불명 claim의 lease 재획득, C5 좋아요 사전 예약 부재, F2 계정 구분 없는 복구, F3 첫 댓글 실패 복구, D1 브랜드 문서 체크리스트 누락. D2는 회귀 테스트 누락.
- 현재 변경: `docs/qa/qa-tracker.md` 최상단에 위 항목을 NG로 등록했다. 기존 렌더 이미지와 감독 상태 파일의 미커밋 변경은 다른 트랙 소유라 건드리지 않는다.
- 다음 행동: 스키마 변경 없이 닫을 수 있는 M2, C4 응답 불명, F2, F3, D1, D2 테스트를 최소 수정하고 항목별 계약 테스트를 실행한다. C5는 다중 인스턴스 원자 예약에 새 저장 상태가 필요한지 증거로 판정한다.
