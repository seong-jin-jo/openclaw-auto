## [2026-08-28 22:30 Claude → Codex 세션 교대. 인계 완료]

**Claude 컨트롤러 토큰 소진. Codex 가 이어받는다. 인계 정본은 `session-state.osmu.md` 와 `pipeline-state.osmu.md` 상단이다.**

- 0.2.0 **병합 요청 26번** 열림, 회장 병합 대기. 가지 `feat/design-system-and-missing-features`, main 대비 117 커밋.
- 컨트롤러 직접 검증: 운영 빌드 174경로 / 빈 DB 마이그레이션 6/6 테이블 24 / 기본 흐름 11/11 / studio 계약 12/12 / 몫 시간대 공격 막힘 / 네 방 성과실 도달 가능 / 시험 186파일 1,329건 통과.
- 남은 블로커 셋: 릴리스 판정 "배포 승인 전"(디자인 정합 NG + 설계 산출물 4건 미산출), 실채널 발행·댓글 읽기 미검증(회장 몫), 디자인 픽셀 대조 미실시.
- 운영 설비: 감독 가동중(동시 6, cron 5분 부활, 백로그 자동 충전). 멈춤 원인 4층 제거 완료.
- **Codex 주의**: 병합·배포는 회장 몫. 워커 주장 대신 직접 재현. 몫 검증 전 `studio_free_regeneration_uses` 비우기.
...
- 기반: `docs/prototype/openclaw-auto-4room-v63.html`, 회장 확정 요구 대장,
  `wiki/2-product/build/사업좌표-OSMU와-ZERO-ONE.md`, test plan, ONE THING, persona를 읽었다.
...
- 구현: M2 생성과 재생성 멱등 이름 공간 분리, C4 응답 불명 답글 재청구 차단, C5 PostgreSQL transaction 자문 잠금, F2 계정별 장애 지문, F3 첫 댓글 실패 보존, D1 브랜드 문서 안내 복구를 반영했다. F4 기존 멱등 재생 조기 반환과 D2 기존 자동저장에는 회귀 테스트를 추가했다.
- 커밋: `9f4ac6b5`, `f7787e1a`, `da310d71`, `ea303437`, `42701de0`, `6e5be518`, `592bdd3b`.
- 직접 증거: Vitest 181파일 1,314건 통과, 조건부 6건 건너뜀. TypeScript, 디자인 lint, 프로덕션 빌드와 정적 경로 174개 통과. 기본 흐름 11/11, Studio 12/12, 공장 회수 2/2, 댓글 무효 bearer 401. 네 방 4개 x 4폭에서 전체 화면 모달, 401, 콘솔 오류 0건. F1 격리 앱은 HTTP 200, DB open, 로컬 webhook 1건.
- 미검증: 실제 공개 댓글 provider의 응답 시간 초과와 실제 좋아요, 운영 배포. C4는 실제 PostgreSQL 재청구 차단까지 검증했으나 provider fault injection은 QA 환경이 필요하다.
- 다른 트랙 보존: 렌더 이미지, 감독 백로그, `scripts/osmu-supervisor.sh`, `.codex/logs/harness.jsonl`의 미커밋 변경은 건드리거나 stage하지 않았다.
- 다음 실행: 소유자=QA verifier. 종료증거=고정 커밋에서 전체 계약과 네 E2E를 독립 재실행하고 provider fault injection으로 응답 불명 답글이 공개 한 건인지 확인한다.

## [2026-08-28 20:10 OSMU 코드리뷰 P0·P1 부분 수정과 실앱 검증]

**핸드오프 기준: 회장 명시 과제와 `docs/audit/osmu-code-review-2026-08-28.md`. tmux는 보조로 확인.**

- 해소: MAJOR 2·4·5·6·7·10·12·20·21·23·24·25·31, MINOR 1~5·7·8.
- 일부 해소: MAJOR 8. 만료 active 자격은 공급자 전에 차단하지만 갱신 가능 여부 구조화 오류는 남음.
- 남은 BLOCK: MAJOR 1·3·9·11·13~19·22·26~30·32·33, MINOR 6·9.
- 데이터 결정 필요: 기존 회원 전역 중복 정리 보존 규칙, 작업 공간 삭제 뒤 과금 장부 보존 기간과 소유 경계.
- 직접 증거: 전체 Vitest 173파일 1,302건, TypeScript, design-lint 통과. localhost 기본 흐름 11/11,
...
- **구현:** 성과실을 온보딩보다 먼저 항상 그리고 온보딩은 인라인 도움으로 유지했다. 생성실과 발행실 빈 상태에 각각 한 줄과 실제 다음 경로로 이어지는 단추를 추가했다. API와 DB는 변경하지 않았다.
- **직접 증거:** 지정 작업 공간의 `localhost:3456`에서 네 방 4개 x 390, 768, 1024, 1440을 사이드바 실제 링크로 순회했다. 방 본문과 다음 행동 전부 노출, 전체 화면 모달 0건, 가로 넘침 0건, 401 0건, 콘솔 오류 0건이다. 성과 0건 방향 제안은 3건이다.
- **회귀 검증:** 백엔드 기본 흐름 11/11. Vitest 171파일 1,288건 통과, 조건부 6건 건너뜀. TypeScript, Webpack production build 174페이지, 디자인 lint 위반 0건.
- **정리:** 임시 고객 토큰은 전부 폐기했고 작업 공간 온보딩 완료 설정은 원래 값으로 복원했다. 운영 배포는 하지 않았다.
- **커밋:** `8e6d5773` 성과실과 인라인 온보딩, `f8363e73` 생성실·발행실 빈 상태, `c9f9573e` 4폭 실사 검증 종료 보장.
- **다음 실행:** 소유자=QA verifier. 종료증거=고정 커밋에서 `node scripts/verify-four-room-ui-e2e.mjs`와 `node scripts/verify-basic-flow-e2e.mjs`를 독립 재실행해 같은 수치가 나오는 것.
...
## [2026-08-28 OSMU 사람 동선 QA 종료, 출고 NG]

- **인계 기준:** 회장이 직접 지정한 `localhost:3456` 사람 동선 QA를 primary로 사용했다.
- **최종 판정:** 네 방 4폭 왕복과 성과 0건 제안 인계는 PASS. 첫 후보 생성은
  `Studio 스킬 버전이 비어 있습니다`에서 차단됐다. 편집 장면 재정렬 UI와 서버 영속성은 없고,
  Shorts 캡션 변경은 Reels와 TikTok에 동시에 전파됐다. v63 디자인 정합 12행은 전부 NG다.
- **산출물:** `docs/qa/osmu-qa-2026-08-28.md`, 캡처 `docs/prototype/qa-2026-08-28/`.
- **검증:** Vitest 1,282 pass와 6 skip, TypeScript PASS, production build PASS, 임시 PostgreSQL
  seed PASS, 주요 API 200, Playwright 네 방 4 x 4폭 PASS, 디자인 lint 0건. Maestro는 웹 전용이라
  대상 없음. 실제 채널 연결과 실제 발행은 실행하지 않았다.
- **다음 행동:** OSMU-QA-01, 02, 03, 04를 개발로 반려한 뒤 같은 사람 동선을 다시 실행한다.

## [2026-08-28 네 방 기본 흐름 UI 결함 수정 진행]

- **인계 기준:** 회장이 직접 지정한 네 방 기본 흐름 과제를 primary로 사용한다. 확인한 활성 pane은
  `openclaw-auto:0.0`, `osmu-flow1:0.0`, `studio-auth-runtime:0.0`이며, 대화 인계가 아니라 현재
  커밋과 파일 상태를 기준으로 작업한다.
- **build 판정:** `pipeline-state.studio.md`는 소스, 테스트, 로컬 실행을 항상 허용한다. API, DB,
  아키텍처는 바꾸지 않고 네 방 화면 도달과 빈 상태만 수정한다.
- **기반:** `docs/prototype/openclaw-auto-4room-v63.html`, 확정 요구 대장, 사업 좌표,
  `DESIGN.md`, `docs/구현현황.md`, 사이드바와 홈 온보딩 조건을 읽었다.
- **현재 상태:** QA tracker 상단에 성과실 도달 차단, 발행실 빈 상태, 생성실 첫 행동 불명확을
  ❌ NG로 등록했다. 현재 작업 트리는 이 기록 2개만 수정된 상태다.
- **다음 작업:** 성과실 본문을 온보딩보다 먼저 항상 그리고 온보딩을 인라인 도움으로 바꾼다.
  생성실과 발행실에 다음 행동 한 줄과 이동 단추를 연결한 뒤 네 폭 Playwright를 실행한다.

## [2026-08-28 교차 모델 BLOCK 필수 3건 수정 착수]

- **인계 기준:** 회장이 직접 지정한 `docs/audit/osmu-cross-review-2026-08-28-opus.md`와 필수
  M1, C1, F1 수정 과제를 primary로 사용한다. 확인한 pane은 `openclaw-auto:0.0`,
  `osmu-fix1:0.0`, `osmu-review1:0.0`, `osmu-qa1:0.0`, `osmu-build:0.0`이다.
- **게이트:** `pipeline-state.osmu.md`는 OSMU build를 진행 중으로 기록하고 소스, 테스트,
  로컬 실행을 명시적으로 허용한다. 공개 응답 계약은 유지한다.
- **현재 상태:** QA tracker 상단에 세 결함을 ❌ NG로 등록했다. 제품 소스는 아직 수정하지 않았다.
- **다음 작업:** M1의 협정시 몫 키와 교차 시간대 계약부터 수정하고 집중 테스트한다. 이어서 C1
  실행 lease 회수와 운영자 강제 종료, F1 사람 개입 장애의 DB와 Slack 이중 전달을 각각 수정한다.
...
- **인계 기준:** 회장이 지정한 운영 장애 관측 code-builder 과제와 v63 프로토타입,
  확정 요구 대장, 사업 좌표, 갭 재확인을 primary로 사용했다.
- **기존 구현 보존:** 닫힌 이벤트 허용 목록, 발행·공유 AI 보고, 외부 health 감시,
  단일 운영자 고객 관리 화면과 고객 Slack 분리를 유지했다.
- **구현:** RLS 운영 장애 원장, 작업 공간별 운영자 조회, 사람 개입과 자동 복구 분류,
  미설정 제외, 사람 개입 미알림 건만 보내는 health 스크립트, 전송 확인 기록을 추가했다.
- **실제 증거:** 지정 작업 공간의 임시 Threads 계정을 만료시켜 채널 설정 200, 장애 조회 200,
  작업 공간이 붙은 사람 개입 장애와 로컬 웹훅 수신, `notifiedAt` 기록을 관찰했다.
  임시 계정은 삭제했고 열린 검증 장애는 0건이다.
- **검증:** 전체 Vitest 168파일 1,273건과 동일 장애 8건 경합 통합 테스트 통과.
  조건부 6건만 건너뜀. TypeScript, design lint,
  production build 174페이지 통과. 실제 Chrome 운영자 화면에서 패널과 작업 공간명을 봤고
  콘솔 오류·예외는 0건이다.
- **커밋:** `7a95dc9a` 장애 원장과 RLS, `ce193d4c` 장애 분류·API·화면·알림 경로.
- **배포·게이트:** 로컬 DB와 앱에 반영, 운영 미배포. build 이후 QA와 ship 게이트는 미통과다.
- **다음 실행:** 소유자=QA verifier. 종료증거=QA 환경에서 발행·토큰·생성·외부 오류 네 분류를
  각각 재현하고, 자동 복구와 미설정에서 Slack 0건, 사람 개입에서 실제 Slack 1건을 확인한다.

## [2026-08-28 07:26 화면 전체 시각 토큰 통일 build 완료]

- **인계 기준:** 회장이 지정한 code-builder 화면 통일 과제를 primary로 사용한다. 현재 작업 pane은
  `osmu-fe10:0.0`이며 다른 OSMU worker 변경은 보존한다.
- **build 판정:** `pipeline-state.osmu.md`의 build는 진행 중이고 소스·테스트·로컬 실행은 허용된다.
  API, DB, 아키텍처 변경은 범위에 포함하지 않는다.
- **기반:** v63 확정 프로토타입, 요구 대장 관련 항목, `DESIGN.md`, 이동된 현행
  `docs/notes/ui-rules.md`, `globals.css`, 사업 좌표, 갭 재확인, 구현현황을 사용한다.
- **구현:** 직접 시각값 감사기와 반복 치환기를 추가하고 여백, 글자, 색, 모서리, 그림자
  3,029건을 시맨틱 토큰으로 바꿨다. 단추와 딱지 역할을 통일하고 빈 상태와 오류 표시를
  `StateNotice`로 합쳤다. 영문 행동 단추와 화면 이모지는 한국어 문구로 바꿨다.
- **실화면 재검증:** 첫 390 촬영에서 일반 제목에 흰 전경이 들어간 4건을 발견해 복구했다.
  강조 전경과 배경의 짝도 감사기에 넣고 생성실·편집실 390, 768, 1024, 1440을 다시 촬영했다.
  모든 폭에서 가로 넘침 0, 생성 201, 401 0건, 콘솔 오류 0건이다.
- **검증:** UI 감사 3,029건에서 0건, 대비 짝 오류 0건. 전체 Vitest 168파일 1,274건 통과,
  조건부 6건 건너뜀. TypeScript, design lint, production build와 정적 페이지 174개 생성 통과.
  실제 `/api/health`와 `/`는 HTTP 200이다.
- **커밋:** 중간 구현 `e2a9126f`. 최종 대비 수정, 증거, 문서는 다음 커밋에 포함한다.
- **배포·게이트:** 로컬 build만 반영했고 운영은 미배포다. build 이후 QA와 ship 게이트는 미통과다.
- **다음 실행:** 소유자=QA verifier. 종료증거=네 폭 캡처를 v63과 독립 대조하고 실제 QA 환경에서
  `npm run audit:ui-tokens`, 전체 Vitest, 화면 401·콘솔 오류 0건을 재현한다.

## [2026-08-28 작업 공간 간 테넌트 격리 공격 build 검증 진행]

- **인계 기준:** 회장이 지정한 code-builder 테넌트 격리 공격 과제를 primary로 사용한다.
  `studio-auth-runtime:0.0`은 코드 편집 세션이 아니라 `localhost:3456` 개발 서버 전용이다.
- **build 판정:** `pipeline-state.osmu.md`의 build는 진행 중이고 소스·테스트·로컬 실행은
  허용된다. 새 아키텍처를 고르는 작업은 아니며 기존 tenant auth와 RLS 계약을 공격한다.
- **기반:** v63 확정 프로토타입, 회장 요구 대장 작업 공간 항목, 사업 좌표, 갭 재확인,
  구현현황, `dashboard/tests/isolation/`, `tenant-auth.ts`, `schema.sql` RLS를 사용한다.
- **현재 상태:** 실제 임시 작업 공간 A·B와 서로 다른 DB·파일 데이터를 만들고 `localhost:3456`에
  183회 요청했다. 교차 읽기 54, 무토큰 54, 폐기 토큰 54, 교차 변경 12, 몸통 위조 3,
  정상·사후 검증 6건 모두 통과했고 B 누출·변경은 0건이다. 집중 Vitest 5건도 통과했다.
- **변경:** 실앱 공격 스크립트, GET 경로 자동 누락 방지 계약, RLS 19개 CRUD 실 DB 통합
...
## [2026-08-28 06:42 주요 다섯 화면 성능·접근성 build 구현·실앱 검증]
...
- **현재 상태:** 생성실·편집실의 발행 전용 요청을 제거하고 주소 방 선택, 공통 초점 링,
  라이트·다크 대비, 카드 이동 단추 접근성을 수정했다. 편집실 첫 그림 50% 단축,
  생성실·편집실 API 16건에서 7건, 키보드 14단계, 생성 201, 저장 200, 대비 위반 0을 관찰했다.
- **검증:** 전체 Vitest 162파일 1,256건 통과, 조건부 6건 건너뜀. TypeScript, design lint,
  production build와 정적 페이지 173개 생성 통과. 최종 health와 생성실 문서 응답 200.
- **정확한 다음 작업:** 구현·증거 커밋을 닫는다. QA 단계에서는 운영 빌드 성과실 시간을
  독립 재측정하고 스크린샷으로 v63 프로토타입 이탈이 없는지 재검증한다.
...
- **만진 파일**: `docs/board/산출물-허브-v1.html`(1절 v55 교체 + 7절 결정판 신설), `pipeline-state.osmu.md`(프로토타입 핀 v55), `wiki/3-operations/session-state.md`, `docs/prototype/contracts/v55.tsv`(신규, v55 워커 작성).
...
**완료:** wiki/5-hubs/hub-mkt/brand-preview.html. 5플랫폼 유저 실화면 목업. verify PASS(Skill2·WebSearch6·Design Score B), 컨트롤러 픽셀 직접판정(bp-desktop-1024.png Read) A- 동의·open. IG=스토리링 프사+통계(예시뱃지)+bio4줄+하이라이트4+카드뉴스 3열9칸 / Threads=threads.net뱃지+고정글 / FB=커버+겹친프사+파란CTA / TikTok·YouTube=영상 empty state(정직). 자산·카피 전부 정본값 인용, 지어낸 실적 0.
...
**진행:** 5플랫폼(IG·Threads·FB·TikTok·YouTube) 유저 실화면 목업 원페이지 = product-designer 위임 중. 산출=wiki/5-hubs/hub-mkt/brand-preview.html.
...
**하네스 위치 조사 결과:** 텍스트 정본=channels/*.md, 비주얼 규격=design-system.md §4~7, 실물=assets/brand/. "유저 실화면 목업 원페이지"는 신규 산출물 → 제안 위치 wiki/5-hubs/hub-mkt/brand-preview.html(design-system 토큰 위 각 플랫폼 UI 프레임).
...
- **운영체제 관점 (완료, R109):** `wiki/5-hubs/hub-eng/architecture/claude-code-와-osmu-대응.md` 2절을 운영체제·Claude Code·OSMU 3열 12행 대응표로, 3절을 3단 도식으로, 5절 신설 "운영체제 관점에서 우리가 무엇을 새로 만들었나"(협조형 스케줄러·의미 중재 메모리 관리자·성과 되먹임 + 비유 한계 5행 표). 사업계획 §3.5 도식도 3단으로 정렬. Claude Code 칸이 비는 자리 2곳(커널 보호 링, 멀티유저 격리)과 운영체제 칸이 비는 자리 1곳(성과 되먹임)을 정직하게 표기. 프롬프트 캐시=TLB 오비유를 페이지 캐시로 정정. 선행 사례 MemGPT(arXiv 2310.08560). mermaid 2건 mermaid-cli 11 PNG 렌더 성공, 컨트롤러가 그림 직접 확인. 오타 4곳 컨트롤러가 수정. verify PASS 23/25.
- **구 진행중 기록:** `wiki/5-hubs/hub-eng/architecture/claude-code-와-osmu-대응.md` 재작성 위임 실행중. 2절을 운영체제·Claude Code·OSMU 3열로, 3절 도식을 3단으로, 비유가 깨지는 3지점을 새 절로. 선행 사례로 MemGPT(문맥 창=주기억, 외부 저장소=느린 기억층, 가상 메모리·페이징 차용) 확인. 프롬프트 캐시는 TLB가 아니라 페이지 캐시에 가깝다는 한계를 함께 적기로 함.
...
- **핸드오프 기준:** 회장이 직접 지정한 `wiki/5-hubs/hub-eng/architecture/claude-code-와-osmu-대응.md` 운영체제 관점 개정 과제를 primary로 사용했다. tmux pane 목록은 충돌 확인용으로만 봤고 다른 pane transcript는 작업 근거로 사용하지 않았다.
...
- **산출물:** 기존 `wiki/5-hubs/hub-eng/architecture/claude-code-와-osmu-대응.md`를 v1.1로 개정했다. §2를 `운영체제 / Claude Code / OSMU` 3열 12행으로 교체하고 직접 대응 없음 3건을 명시했다. §3은 운영체제를 왼쪽에 둔 3열 도식으로 교체했다. 사람 승낙형 스케줄러, 의미 기반 조립, 시장 성과 되먹임과 비유 한계를 새 §5에 추가했다. 기존 1절·4절·가장 강한 반론 내용은 보존했다.
...
- **산출물:** `wiki/5-hubs/hub-eng/architecture/claude-code-와-osmu-대응.md` 신설. `docs/사업계획-osmu-v1.0.md`를 v1.4로 올리고 정확한 제목의 §3.5를 추가했으며, 기존 §3.5를 §3.6으로 밀었다.
...
- **게이트:** 새 v3 두 종은 `approved_artifacts`에 핀되지 않았고 독립 plan 비평과 `/approve plan`을 거치지 않았다. 기존 pipeline-state의 plan 승인은 구 PRD에 대한 것이므로 새 PRD 기준 디자인·기술설계 진입은 불가하다. `wiki/5-hubs/hub-eng/architecture/two-service-boundary.md`도 R71·R88 이전 전제라 plan 승인 뒤 현행화가 필요하다.
...
- **핸드오프 기준:** 회장 위임 원문과 `wiki/3-operations/session-state.md`를 primary로 사용했다. `openclaw-auto:0.0` pane은 수동 콘텐츠 실험 트랙이라 이번 디자인 파일과 충돌하지 않았다.
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. **codex 3건 진행 중**: PRD v3 `/tmp/cx-prd3.log`, 기술설계 v4 `/tmp/cx-eng4.log`, 프로토타입 v44 `/tmp/cx-proto44.log`.
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. **codex 진행 중**(사업계획 v1.2, `/tmp/codex-plan-v12.log`. 앞선 v1.1 보강 `/tmp/codex-plan-v11.log` 와 같은 파일을 만지므로 v1.2 위임서에 "파일을 먼저 읽고 현재 상태 위에서 이어 고쳐라" 명시).
- **★회장 결정 3건 확정(R96~R98):**
...
- **검증:** 결정 1~3 추천안 원문을 사업계획 §10 에서 직접 확인. 시그마인 실측치를 `wiki/2-product/insight/research/2026-08-competitor-and-production-research.md` §2 에서 확인(1080x1920, 30fps, 87초, 컷 1초 안팎). codex 는 진행 중(미검증).
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. **codex 진행 중 1건**(사업계획 v1.1 보강, `/tmp/codex-plan-v11.log`).
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. 진행 중 위임 없음.
...
- **기존 구현 확인:** `docs/구현현황.md`, `wiki/2-product/build/studio.md`, `wiki/2-product/insight/product-positioning.md`, `wiki/5-hubs/hub-eng/architecture/two-service-boundary.md`를 근거로 MVP 5개를 이미 구현, 부분 구현, 미구현으로 분류했다. 생성·편집 기반은 재구현 대상이 아니라 연결·확장 대상으로 명시했다.
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. **codex 진행 중 2건**(사업계획 v1.0 `/tmp/codex-plan-v1.log`, 프로토타입 v43 `/tmp/codex-proto-v43.log`).
...
- **핸드오프 기준:** 회장이 직접 준 v42 과제와 `openclaw-auto:0.1` 부모 세션의 동일 위임. `wiki/3-operations/session-state.md`와 관련 pane을 확인했다.
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. **codex 위임 3건 동시 진행 중.** 로그 `/tmp/codex-proto-v42.log` `/tmp/codex-prd-v2.log` `/tmp/codex-eng-v3.log`.
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. 진행 중인 위임 없음.
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. **진행 중인 위임 없음.** 회장 판단 대기 상태.
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. **계약 v2.1 패치만 진행 중**, 나머지 위임은 전부 종료.
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. 위임 2건 진행 중(계약 v2.1 패치 / 프로토타입 v41).
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. **위임 2건 진행 중**(계약 v2.0 독립 검수 / 프로토타입 v41).
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. **회장 지시 "다 하고 보고해" → 자율 실행 중. 위임 2건 동시 진행.**
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. codex 계약 비평은 **아직 진행 중**(`/tmp/critic-contract.log`).
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`.
- **★회장 지시 R70~R73 대장 등록:**
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. **두 위임 모두 종료.** 위임 프로세스 없음.
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. 프로토타입 v40 위임 계속 진행 중.
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. **프로토타입 v40 위임은 계속 진행 중.**
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. 위임 2건은 계속 진행 중(127 참조).
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. **회장 취침 중. 두 위임이 백그라운드로 돌고 있다.**
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. 코드 변경 없음(설계 논의 턴). 산출물 신규 없음(v4 HTML 그대로).
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. **산출물(open):** `신뢰형템플릿/영상컨셉-허브.html`(STEP1 벤치마크→STEP2 도감·실사→STEP3 결정페이지).
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. 3트랙 병렬 진행중.
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. **산출물(open):** `신뢰형템플릿/회장-결정페이지.html`(v2, 인터랙티브).
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. 위임 진행중(a33599bb), 완료시 verify+픽셀검증.
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. **산출물(open):** `신뢰형템플릿/회장-결정페이지.html`.
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. **산출물(open):** `영상예시/영상예시-허브.html`(갱신).
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. **산출물(open):** `data/experiments/manual-osmu/신뢰형템플릿/영상예시/영상예시-허브.html` (한 허브에 전부 인라인 재생).
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md`. **산출물:** `docs/rendered/학습정보-층계-v4.html` (SVG 5장, 외부 의존 0, 브라우저 open 완료).
...
- **위키 반영 완료:** `wiki/4-reference/channel-status.md`에 절 2개 추가. 무료 공개 소스 5종(한도·확인 여부), 데이터 판매 서비스 8곳(플랫폼·수집 방식·가격), 직접 긁기 vs 사서 쓰기 위험 비교, 판례 요지, **개인정보 설계 결론**.
...
- **에이전트가 올린 판단 필요 2건(open-decisions 56·57):** ①**위키에 조사 소스 현황 페이지가 없다.** 채널 현황과 같은 격으로 `wiki/4-reference/`에 신설 추천. 지금은 코드 경로로 폴백함 ②**구글 트렌드 조회 경로가 공식 API인지 미확인**(세션 검색 예산 소진). 비공식이면 프리모템 D가 실제 위험.
...
- **★채널 개수 불일치 해소(컨트롤러 직접 처리, 가역 작업이라 확인 없이 실행):** `extensions/*-publish` 를 세어 **실측 15종**(bluesky, discord, facebook, instagram, line, linkedin, naver-blog, pinterest, slack, telegram, threads, tiktok, tumblr, x, youtube). 이에 맞춰 정정:
  - `wiki/5-hubs/hub-eng/architecture/two-service-boundary.md` : "채널 30종" → **"채널 15종"**, STAMP에 정정 사유 기록
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md` (회장 지정 유지). tmux 추론 금지.
- **지금 위치:** 사업계획 v0.2 산출 완료(`docs/사업계획-osmu-v0.2.md`, 렌더 15,386 bytes). v0.1은 같은 폴더에 남겨 둠(개정 이력 비교용).
- **이번 세션 변경 파일:** `docs/사업계획-osmu-v0.2.md`(신규), `docs/사업계획-osmu-v0.1.md`(직전 판), `docs/rendered/현재상태지도.md`(3.13까지), `docs/requests/회장-확정-요구사항-대장.md`(R37), `~/.claude/harness/open-decisions.md`(14~27), `wiki/3-operations/session-state.md`.
...
- **핸드오프 기준:** `wiki/3-operations/session-state.md` (회장 지정, tmux 추론 없음).
...
- **문서 3곳의 수치가 서로 다르다:** `wiki/2-product/build/marketing-hub-surface-map.md` 26(가장 낡음, 정정 문단만 붙여 둠), 유저플로우 v10.0 §5.1 합계 21, 비교판 자산 대장 23. 실렌더 확인 후 한 번에 정정한다.
- **배포 상태:** 변동 없음. 제품 코드 변경 없어 빌드·E2E 대상 없음.
...
- **★ 위키 오류 발견:** `wiki/2-product/build/marketing-hub-surface-map.md`가 고객 사이드바 26항목이라 적었으나 **실제 코드는 21항목**이다. 진실원은 `dashboard/src/components/layout/Sidebar.tsx`. 이 위키는 갱신 대상이며, 그동안 위임 프롬프트에 26을 주입해 온 것도 정정해야 한다(v27 에이전트에 즉시 보정 전달함).
...
- **배포 상태:** 변동 없음. 제품 코드 변경 없어 빌드·E2E 대상 없음.
- **정확한 다음 액션:** 프로토타입 v27 완료 시 컨트롤러가 직접 클릭 확인 후 제출. `wiki/2-product/build/marketing-hub-surface-map.md`의 사이드바 항목 수를 코드 기준으로 정정.
...
- **선행 산출:** `docs/prototype/v25-brief.md`. 기준 파일(v24 353KB 복사), 진실원 5층, 현행 구조(사이드바 26항목·라우트 25개·채널 탭 규칙·Settings 8/9탭·Studio 인벤토리 7/4/8/3), 넣을 것 4개와 붙일 위치, 뺄 것 4개와 사유, 관측된 시각 결함 3건(390px 사이드바 소실, /videos 14px 넘침, /search-console 89px 넘침), 완료 기준을 명시했다. 근거는 `wiki/2-product/build/marketing-hub-surface-map.md`와 실제 라우트 실측이다.
...
- **기존 구현 확인:** `docs/구현현황.md`, `dashboard/src/app/studio/page.tsx`, `dashboard/src/components/studio/PlatformPreview.tsx`, `wiki/2-product/build/studio.md`, v24 brief·prototype·1024·390 캡처를 확인했다. 224px 사이드바, 아이디어 입력, 브랜드 위키, 크레딧, 생성 진행, 7채널 미리보기, 저장·예약·발행·이력·편집은 보존 대상으로 고정했다. 타깃 시장, 출력 언어, 요청 조립 4층, 저해상도 후보 3개 비교는 현재 화면 미구현이다.
...
- **wiki 반영:** `wiki/5-hubs/hub-eng/architecture/two-service-boundary.md`에 "시장 범위와 언어", "후보 제시 비용 구조" 두 절 추가.
...
- **회수 필요:** 루트 `docs/구현현황.md`가 OSMU 화면·디자인·발행 현황만 기록하고 최신 headless studio 경계, 두 모드, 편집 과금, 채널 창작 텍스트 소유를 반영하지 않는다. `wiki/2-product/build/studio.md`도 최신 정본보다 낡았다.
...
- **회수 필요:** `wiki/2-product/build/studio.md`가 headless 경계·두 모드·studio 단독 미디어 바이트 소유를 아직 모두 반영하지 않았다. 단일 최신 `docs/구현현황.md`도 확인되지 않아 studio 현황은 `studio/pipelines/구현현황.md`에만 의존했다.
...
- **회수 필요:** `wiki/2-product/build/studio.md`와 `wiki/4-reference/brand-grounding.md`가 새 서비스 경계를 아직 반영하지 않았다. `pipeline-state.osmu.md` 승인 경로도 실제 파일 위치와 불일치한다.
...
- **회수 필요:** 작업 중 갱신된 제품구조 결정서의 `외부 접점 4개` 표에 `소재 반입`이 다섯 번째 행으로 추가돼 제목과 행 수가 충돌한다. 이번 PRD는 회장 직접 지시대로 4개만 확정했고, 소재 반입을 제작 요청에 합칠지 독립 접점으로 둘지는 회장 결정으로 분리했다. `wiki/2-product/build/studio.md`도 새 headless 서비스 경계를 아직 반영하지 않았다.
...
- **회수 필요:** `pipeline-state.osmu.md` 승인 경로와 실제 `docs/notes`·`docs/plan` 경로가 다르지만 SHA-256은 승인 핀과 일치한다. `wiki/2-product/build/studio.md`와 `wiki/4-reference/brand-grounding.md`는 2026-08-15 서비스 경계를 아직 반영하지 않았다.
...
- 위키 `wiki/5-hubs/hub-eng/architecture/two-service-boundary.md`에도 위 2절 반영.
...
- **★ 위키 정본 등록: `wiki/5-hubs/hub-eng/architecture/two-service-boundary.md` 신설.** 두 서비스 구조·의존 방향·소유권 경계·studio 두 모드·편집 비용·접점 5개·인증과 과금·1단계 비범위를 요약. 상세 근거와 기각안은 `docs/제품구조-결정-2026-08-15.md`가 정본.
...
- **회수 필요:** `wiki/2-product/build/studio.md`, `wiki/4-reference/brand-grounding.md`, 루트 `CLAUDE.md`, `studio/README.md`의 미디어 경계가 최신 결정과 충돌한다. `pipeline-state.osmu.md` 승인 artifact 경로도 실제 위치와 불일치한다.
...
- 핸드오프 기준: wiki/3-operations/session-state.md (tmux 추론 안 함).
...
- 기반 주입: wiki/5-hubs/hub-eng/architecture/two-service-boundary.md 전문, PRD v8.2.1, 회장 요청 원문, §9.6~9.8. 벤치마크 3+(Spotify/Netflix/Canva Magic Design류 제안 우선 제품).
...
- 위임에 주입한 기반 9종 감사 결과:
  ...
  · STAMP 없었음 3종 = docs/requests/2026-08-17-회장-4실-구조-구상.md, docs/requests/2026-08-16-회장-유저플로우-전면개정.md, wiki/5-hubs/hub-eng/architecture/two-service-boundary.md → 이번 턴에 STAMP 머리말 추가 완료
...
- 회수 필요 2건: ①docs/구현현황.md가 없어 wiki/5-hubs/hub-eng/architecture/data-model.md를 진실원으로 대체 사용. 그것을 정본으로 인정할지(추천) 별도 파일 만들지 결정 필요 ②빈틈 10번 정본 충돌.
...
- 남은 진짜 문제 = 발행실에서 캡션 오타 하나 고치는 경로. 3안 제시:
  ...
  B 채택 시 wiki/5-hubs/hub-eng/architecture/two-service-boundary.md 소유권 표에 "발행 직전 텍스트 표면 수정은 openclaw 허용, 마스터는 studio 소유 유지" 한 줄 추가 필요. 회장 승인 전이라 미수정.
...
- ★ wiki/5-hubs/hub-eng/architecture/two-service-boundary.md 개정 완료(회장 "이런 근거는 위키 등에 잘 넣고" 지시):
...
- **현행 판정:** `wiki/1-team-brand/assets.md`는 다른 세션 변경으로 실제 `assets/brand/` 파일명과 이미 일치해 추가 수정하지 않았다.
...
## [2026-08-28 06:46 작업 공간별 운영 장애 관측 build 착수]

- **인계 기준:** 회장이 지정한 운영 장애 관측 code-builder 과제를 primary로 사용한다.
  `osmu-build10:0.0`은 이 워커가 실행되는 현재 pane이고, `studio-auth-runtime:0.0`은
  `localhost:3456` 개발 서버 전용이다.
- **build 판정:** `pipeline-state.osmu.md`의 build는 진행 중이며 소스, 테스트, 로컬 실행이
  허용된다. 새 장애 테이블은 회장이 필요하면 만들라고 명시적으로 허용했고 기존 RLS와 명명
  규칙을 그대로 따른다.
- **기반:** v63 확정 프로토타입, 회장 요구 대장, 사업 좌표, 갭 재확인, 구현현황,
  `DESIGN.md`, 기존 `observability.ts`, `health-alert.sh`, 운영자 단일 관리 화면을 사용한다.
- **기존 구현:** 구조 로그와 운영자 Slack, 외부 health 감시, 발행 실패와 공유 AI 실패 호출부는
  있다. 작업 공간별 장애 원장, 사람 개입 분류, 자동 복구 알림 억제, 토큰과 Studio 및 외부
  서비스의 통합 조회가 없다.
- **현재 작업:** QA 트래커에 NG를 등록했다. 운영 장애 테이블과 RLS, 안전한 장애 분류 및
  복구 원장, 운영자 조회와 기존 단일 관리 화면 연결, health-alert 장애 polling을 구현한다.
- **정확한 다음 작업:** 스키마와 원장 계약 테스트를 먼저 작성하고 집중 테스트 후 중간 커밋한다.
  이후 호출부와 화면을 연결하고 실제 토큰 만료, HTTP 조회, Slack 수신, 복구를 관찰한다.
## [2026-08-28 OSMU 사람 동선 QA 착수]

- **인계 기준:** 회장이 직접 지정한 7개 실제 앱 시나리오를 primary로 사용한다. `osmu-qa1:0.0`을 확인했으며 별도 진행 문맥은 없었다.
- **단계:** canonical `pipeline-state.osmu.md`의 `current_stage`를 `qa`로 전환했다.
- **범위:** `localhost:3456`, 작업 공간 `cd1d0a40-540d-4524-9b49-bf2445d82182`. 실제 채널 연결과 실제 발행은 제외한다.
- **기반:** v63 확정 프로토타입, 회장 확정 요구 대장, 사업 좌표, 기존 QA tracker를 읽고 7개 시나리오와 390 폭을 실조작한다.
- **산출:** `docs/prototype/qa-2026-08-28/` 캡처와 `docs/qa/osmu-qa-2026-08-28.md`. 제품 코드는 수정하지 않는다.
