...
2. **GA4 분석·리포팅·슬랙 알림을 MARKETING이 중앙 통합** (GA4 Data API로 4사업 property 읽기.
   ...
   사용자 결정(2026-06-26): **계획만, 구현 다음 세션.** 설계 박제 = `wiki/2-product/build/plan-ga4-slack-central.md`.
...
- 루트 `AGENTS.md` 추가. Codex/Claude 공통 진입 규칙으로 `CLAUDE.md` →
  `wiki/3-operations/session-state.md` → `git status --short --untracked-files=no` →
  관련 diff 확인 순서를 고정했다.
- 작업 중/완료 전 `wiki/3-operations/session-state.md` 갱신, 관련 `wiki/` 문서 반영,
...
- tmux↔session-state 핸드오프를 명문화했다. Codex가 `marketing-claw:0.0` pane을
  ...
  "tmux pane은 휘발성 takeover context, `wiki/3-operations/session-state.md`는 durable return
...
- **handoff 기준**: tmux `marketing-claw:0.0` pane은 이 세션 자체이고 대기 지시 공란 →
  durable source `wiki/3-operations/session-state.md`가 기준(불명확성 없음).
- 직전: Codex가 due publisher `bacc1243` + handoff 문서 `1ad43ed5` 커밋(둘 다 미push였음).
  ...
  2. **[P0 부분해소] publish-due caller 부재** — `publish-due`에 **운영자 전체 테넌트 스윕** 모드 추가:
     ...
     `dashboard/scripts/publish-due-cron.sh`(curl 드라이버) 추가, `wiki/3-operations/runbooks/cron.md`에 crontab wiring 문서화.
...
- **변경 파일**: `src/lib/constants.ts`(+SSOT), `src/components/studio/SchedulePanel.tsx`(SSOT 소비),
  ...
  `wiki/3-operations/runbooks/cron.md`.
...
- **★A1 키스톤픽스 = 완료·검증·커밋(다음 커밋).** 브랜드 증류 3라우트(`studio/brand-setup`,
  ...
  전체 129 pass/8 skip, tsc 0, build ✓. 문서: `wiki/4-reference/brand-grounding.md`.
