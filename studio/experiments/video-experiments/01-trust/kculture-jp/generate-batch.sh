...
run_one() {
  ...
  ext=${result_url##*.}
  [[ "$ext" == "mp4" ]] || ext=mp4
  curl -fL "$result_url" -o "$OUTPUT_DIR/kculture-jp-$id-v1-gpt-codex.$ext"
  print -r -- "$(date -Iseconds)\t$id\tdownload\tCOMPLETED\t$job_id\t$before\t$quote\t$model\t$prompt_file\t$result_url" >> "$ATTEMPTS"
}

run_one vf-jp-001a veo3_1_lite 12 "$PROMPT_DIR/kculture-jp-vf-jp-001a-jp-scene-veo-v1-gpt-codex.txt" --aspect_ratio 9:16 --duration 8 --generate_audio true || exit $?
run_one vf-jp-001b veo3_1_lite 12 "$PROMPT_DIR/kculture-jp-vf-jp-001b-en-scene-veo-v1-gpt-codex.txt" --aspect_ratio 9:16 --duration 8 --generate_audio true || exit $?
run_one vf-jp-001c veo3_1_lite 12 "$PROMPT_DIR/kculture-jp-vf-jp-001c-jp-explain-veo-v1-gpt-codex.txt" --aspect_ratio 9:16 --duration 8 --generate_audio true || exit $?
run_one vf-jp-001d kling3_0 16 "$PROMPT_DIR/kculture-jp-vf-jp-001d-jp-scene-kling-v1-gpt-codex.txt" --aspect_ratio 9:16 --duration 8 --mode std --sound on || exit $?
run_one vf-jp-001e veo3_1_lite 12 "$PROMPT_DIR/kculture-jp-vf-jp-001e-jp-scene-veo-repeat-v1-gpt-codex.txt" --aspect_ratio 9:16 --duration 8 --generate_audio true || exit $?
run_one vf-jp-001f seedance_2_0 36 "$PROMPT_DIR/kculture-jp-vf-jp-001f-jp-scene-seedance-v1-gpt-codex.txt" --aspect_ratio 9:16 --duration 8 --mode std --resolution 720p --bitrate_mode standard --generate_audio true --genre auto || exit $?
