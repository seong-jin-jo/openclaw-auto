...
basis: docs/사업계획-osmu-v1.0.md 내부 v1.3 §3.4.1~3.4.3, studio/docs/prd-studio-생성-v3.0.md, DESIGN.md v22, docs/user-flow.md v44, R01~R99, two-service-boundary, 기존 DB와 Studio 구현
evidence_urls: https://docs.stripe.com/webhooks, https://docs.aws.amazon.com/prescriptive-guidance/latest/cloud-design-patterns/transactional-outbox.html, https://www.postgresql.org/docs/current/ddl-rowsecurity.html, https://github.com/cloudevents/spec/blob/main/cloudevents/spec.md, https://docs.confluent.io/platform/current/schema-registry/fundamentals/schema-evolution.html
deliberation: 기존 v4 시도가 build 차단 미결 5건을 없다고 선언한 결함을 바로잡았다. 미결은 0절에만 격리하고, 본문은 선택지와 무관하게 유지되는 정식 계약과 선택 뒤 채울 자리를 분리했다.
-->

# Studio 기능 설계 문서 v4.0

> 한 줄 결론: Studio는 자기 회원과 자기 `S0 S1 U2 U3 X4 L5 R6`을 가진 독립 서비스이며, 무상태 조립·제작 엔진에 판이 고정된 봉투만 전달한다. 최신 v44 순환 11단계와 세부 흐름 32단계는 엔드포인트·프론트엔드 구성요소·DB 테이블·시험에 모두 연결됐지만, 되돌리기 비싼 정책 5건과 eng-design 승인이 남아 build 진입은 불가하다.

| 항목 | 값 |
|---|---|
| 상태 | 기술설계 v4.0, 미결 선택 5건의 확정과 eng-design 승인 대기 |
| 상류 정본 | `docs/사업계획-osmu-v1.0.md` 내부 v1.3 §3.4.1~3.4.3 |
| plan 입력 | `studio/docs/prd-studio-생성-v3.0.md`, `studio/docs/prd-studio-service-v1.2.1-gpt-codex.md` |
| design 입력 | `DESIGN.md` v22, `docs/user-flow.md` v44 |
...
| 대응 API 계약 | `studio/docs/api-contract-studio-v4.0.md` |
| user-flow 매핑 | v44 순환 11/11, 세부 흐름 32/32, endpoint·component·table·test 빈칸 0 |
| 매핑 gap | 0 |
| 정책 gap | 5, 전부 0절에만 표시 |
| Mermaid | 10개, Mermaid CLI 11.16.0 전량 SVG 렌더 통과 |
...
이 절만 미결을 담는다. 1절부터 7절은 사업계획 v1.3에서 확정된 층 이름, 책임 경계, 봉투 구조와 user-flow를 구현 계약으로 쓴다. 아래 선택이 확정되기 전 개발자는 관련 migration, 권한, 삭제 스케줄, 외부 스킬 실행 정책을 구현하지 않는다.

| ID | 무엇을 정하나 | 선택지 | 추천안 | 확정 전 차단 범위 |
|---|---|---|---|---|
| D-01 | 기존 openclaw 회원과 Studio 단독 회원 연결 | 자동 병합 / 사용자가 작업 공간·학습 정보를 확인한 뒤 연결 | 사용자 확인 연결. 잘못된 계정 결합은 private 작업 공간 노출로 이어진다 | `member_external_mappings`, 조정 API, 병합 E2E |
| D-02 | 생성 요청 원문 보유 기간 | 기본 30일 / 기본 90일 | 30일. 재현 정보는 비식별 꼬리표로 남기고 원문 노출 기간을 줄인다 | 원문 삭제 job, 개인정보 안내, 복구·분쟁 시험 |
| D-03 | 사용자가 올린 외부 `X4` 실행 권한 | 파일·망·비밀값 차단 / 심사 뒤 선택 권한 허용 | 첫 출시는 전부 차단. 스킬 장터보다 안전한 결과 한 편 완주가 우선이다 | sandbox profile, 권한 심사, secret alias 정책 |
| D-04 | 옛 층 자료와 v4 자료의 동시 수용 기간 | 즉시 전환 / 30일 읽기 호환 | 30일 읽기 호환. 새 쓰기는 v4만 허용한다 | legacy adapter 제거일, rollback, backfill 종료 기준 |
| D-05 | 정본 투영 실패를 사용자에게 알리는 시간 | 값 종류별 15분·24시간 / 전체 24시간 | `S0`·`U3` 금지·권리·정책은 15분 안에 멈춤, 나머지는 24시간 경고 | stale classifier, 알림 SLA, 생성 차단 기준 |
...
기존 FDD v4 시도의 “build를 막는 미결정 없음” 판정은 위 D-01부터 D-05를 누락한 것이므로 폐기한다. 매핑 gap은 0이지만 정책 gap이 5이므로 build gate는 열지 않는다.
...
상세 JSON 요청·응답, 오류, 멱등성, openclaw 교환 봉투와 Studio 단독 모드 차이는 `studio/docs/api-contract-studio-v4.0.md`가 정본이다. 이 절은 user-flow와 구현 모듈이 호출할 표면을 고정한다.
...
### 5.2 최신 v44 한 편 순환 1:1 매핑

`docs/user-flow.md` 최상단 v44 Happy path 11단계를 먼저 고정한다. 한 단계가 여러 서버 동작을 포함해도 사용자 단계의 대표 명령 엔드포인트 하나, 화면 구성요소 하나, 주 저장 대상 하나, 시험 하나를 지정한다. 하위 명령은 §5.4 전수 매핑에서 분해한다.

| v44 단계 | 사용자 또는 시스템 행위 | 대표 엔드포인트 | 대표 화면 구성요소 | 주 저장 대상 | 시험 |
|---|---|---|---|---|---|
| CYCLE-01 | 채널 연결 없이 글·영상 갈래와 이번 요청 선택 | `POST /v4/studio/productions` | `WorkspaceStartPicker` | `production_jobs`, `request_adjustments` | CYCLE-01 |
| CYCLE-02 | 안전·권리·사실·X4·L5·R6 읽기 | `POST /v4/studio/productions/{jobId}/assemble` | `AssemblyDisclosure` | `production_envelopes`, `production_conflicts` | CYCLE-02 |
| CYCLE-03 | 후보 3개, 선택 원본, 선택 관찰과 제작 정보 축적 | `POST /v4/studio/productions/{jobId}/selections` | `CandidateChooser` | `production_outputs`, `production_decisions`, `learning_observations` | CYCLE-03 |
| CYCLE-04 | 편집실이 선택 원본·편집 요청·규격 읽기 | `POST /v4/studio/edits` | `EditPreview` | `edit_jobs`, `production_recipes` | CYCLE-04 |
| CYCLE-05 | 완성 원본·채널별 문구·편집 기록 축적 | `POST /v4/studio/outputs/{outputId}/channel-packages` | `ChannelCopyEditor` | `channel_text_packages`, `channel_text_revisions`, `edit_instructions` | CYCLE-05 |
| CYCLE-06 | 발행실이 완성 원본·문구·취향·연결 상태 읽기 | `POST /v4/studio/handoffs` | `HandoffStatus` | Studio `handoff_records`, openclaw `handoff_inbox` | CYCLE-06 |
...
| CYCLE-09 | 비교 근거와 근거 부족 규칙 후보 생성 | `POST /v4/studio/sync/events` | `LearningCandidateCard` | `learning_observations`, `learning_candidates` | CYCLE-09 |
| CYCLE-10 | 후보 승낙 때만 L5 생성, 거절·보류는 미적용 | `POST /v4/studio/learning-candidates/{candidateId}/accept` | `LearningConsentAction` | `learning_candidates`, `layer_items`, `layer_revisions` | CYCLE-10 |
| CYCLE-11 | 승낙 L5와 새 R6로 다음 생성 시작 | `POST /v4/studio/productions` | `GenerationRoom` | `production_jobs`, `request_adjustments`, `production_envelopes` | CYCLE-11 |

판정: v44 11/11, endpoint 빈칸 0, frontend component 빈칸 0, DB table 빈칸 0, test 빈칸 0.

### 5.3 세부 흐름 1:1 매핑 원칙 <a id="매핑"></a>

`docs/user-flow.md` v44와 사업계획 v1.3의 네 방 흐름을 32개 구현 단계로 분해한다.
...
|---|---|---|---|---|---|
| UF-01 | Studio 세션 시작 | `POST /v4/studio/sessions` | `StudioAuthGate` | `studio_sessions` | FLOW-01 |
| UF-02 | 작업 공간 목록과 현재 공간 선택 | `GET /v4/studio/workspaces` | `WorkspaceSwitcher` | `workspaces`, `member_workspace_roles` | FLOW-02 |
| UF-03 | 작업 공간 생성, 처음에는 글·영상 갈래만 선택 | `POST /v4/studio/workspaces` | `WorkspaceStartPicker` | `workspaces`, `layer_items`, `layer_revisions` | FLOW-03 |
| UF-04 | 학습 정보 열람·수정 | `GET /v4/studio/workspaces/{workspaceId}/layers`, `POST /v4/studio/layer-items/{itemId}/revisions` | `LearningInfoPanel` | `layer_items`, `layer_revisions` | FLOW-04 |
| UF-05 | 소재·참고자료 반입 | `POST /v4/studio/material-imports` | `MaterialImportReview` | `material_imports`, 승인 뒤 `layer_items`, `layer_revisions` | FLOW-05 |
| UF-06 | 근거 있는 사례와 추천 3개 조회 | `POST /v4/studio/proposal-sets` | `DisplayProposalDeck` | `reference_views`, `proposal_sets`, `production_outputs` | FLOW-06 |
| UF-07 | 비용·시간 범위 확인 | `POST /v4/studio/production-estimates` | `CostTimeApproval` | `cost_entries` | FLOW-07 |
| UF-08 | 제작 작업과 `R6` 생성 | `POST /v4/studio/productions` | `GenerationProgress` | `production_jobs`, `request_adjustments` | FLOW-08 |
| UF-09 | 일곱 층 조립과 사실 충돌 판정 | `POST /v4/studio/productions/{jobId}/assemble` | `ConflictQuestion` | `production_envelopes`, `production_conflicts` | FLOW-09 |
| UF-10 | 저해상도 후보 3개 실행 | `POST /v4/studio/productions/{jobId}/execute-previews` | `DisplayLoadingState` | `production_attempts`, `production_outputs`, `cost_entries` | FLOW-10 |
| UF-11 | 세 후보 전부 거절 후 재요청 | `POST /v4/studio/proposal-sets/{setId}/retry` | `ProposalRetryAction` | `proposal_sets`, `production_attempts` | FLOW-11 |
| UF-12 | 후보 하나와 이유 선택 | `POST /v4/studio/productions/{jobId}/selections` | `CandidateChooser` | `production_decisions`, `learning_observations` | FLOW-12 |
| UF-13 | 선택 후보만 고해상도·고품질 승격 | `POST /v4/studio/productions/{jobId}/promotions` | `PromotionProgress` | `production_outputs`, `production_attempts`, `cost_entries` | FLOW-13 |
| UF-14 | 품질·금지 표현·사실·권리 검사 | `POST /v4/studio/outputs/{outputId}/inspections` | `QualityGateResult` | `output_inspections` | FLOW-14 |
| UF-15 | 확정·보관·다른 제안 선택 | `POST /v4/studio/productions/{jobId}/disposition` | `ResultDispositionBar` | `production_jobs`, `production_decisions` | FLOW-15 |
| UF-16 | 편집실에서 부모 결과 열기 | `POST /v4/studio/edits` | `EditPreview` | `edit_jobs`, `production_outputs`, `production_recipes` | FLOW-16 |
| UF-17 | 대화·선택·직접 조작 편집 지시 저장 | `POST /v4/studio/edits/{editId}/instructions` | `EditInstructionPanel` | `edit_instructions` | FLOW-17 |
| UF-18 | 영향 컷만 재생성 또는 로컬 재렌더 | `POST /v4/studio/edits/{editId}/render` | `EditRenderProgress` | `production_attempts`, `production_outputs`, `cost_entries` | FLOW-18 |
| UF-19 | 세부 채널 규격 받기, 계정 연결은 하지 않음 | `POST /v4/studio/channel-spec-snapshots` | `ChannelTargetPicker` | `channel_spec_projections` | FLOW-19 |
| UF-20 | 제목·소개·해시태그·첫 댓글 생성 | `POST /v4/studio/outputs/{outputId}/channel-packages` | `ChannelCopyEditor` | `channel_text_packages`, `channel_text_revisions` | FLOW-20 |
| UF-21 | 완성 원본·문구·제작 정보 인계 | `POST /v4/studio/handoffs` | `HandoffStatus` | `handoff_records`, `production_outputs` | FLOW-21 |
...
| UF-25 | 선택·수정·성과 관찰 밀기 | `POST /v4/studio/sync/events` | `LearningInfoBadge` | `sync_inbox`, `learning_observations` | FLOW-25 |
| UF-26 | 3회 선택 또는 5건 성과 뒤 후보 제시 | `GET /v4/studio/workspaces/{workspaceId}/learning-candidates` | `LearningCandidateCard` | `learning_candidates` | FLOW-26 |
| UF-27 | 후보 승낙, `L5` 판 생성 | `POST /v4/studio/learning-candidates/{candidateId}/accept` | `LearningConsentAction` | `learning_candidates`, `layer_items`, `layer_revisions` | FLOW-27 |
| UF-28 | 후보 거절, 관찰 보존 | `POST /v4/studio/learning-candidates/{candidateId}/reject` | `LearningRejectAction` | `learning_candidates`, `learning_observations` | FLOW-28 |
| UF-29 | 되돌리기, 원본 보존·앞 방 항목 추가 | `POST /v4/studio/productions/{jobId}/forks` | `RollbackBanner` | `production_jobs`, `provenance_events` | FLOW-29 |
| UF-30 | 새 브랜드·언어·취향용 작업 공간 복제 | `POST /v4/studio/workspaces/{workspaceId}/copies` | `WorkspaceCopyAction` | `workspaces`, `layer_items`, `layer_revisions` | FLOW-30 |
| UF-31 | 정본 판·삭제를 투영 서비스로 밀기 | `POST /v4/studio/sync/events` | `SyncStatusBadge` | `sync_outbox`, `sync_inbox`, `sync_mappings` | FLOW-31 |
| UF-32 | 작업 공간 삭제 전파와 새 작업 차단 | `DELETE /v4/studio/workspaces/{workspaceId}` | `WorkspaceDeleteConfirm` | `workspaces`, `sync_outbox`, `projection_sync_states` | FLOW-32 |
...
| Mermaid 문법 렌더 | 10/10, Mermaid CLI 11.16.0 SVG 렌더 | 충족 |
| 되돌리기 비싼 정책 D-01~D-05 | 미확정 5건 | 미충족 |
| eng-design 승인 | 미승인 | 미충족 |

**판정: build stage 진입 불가.** 매핑 gap 때문이 아니라 D-01부터 D-05까지의 정책 gap과 eng-design 미승인 때문이다.
...
이 결론이 틀렸다면 가장 그럴듯한 이유는 최신 v44의 11단계 순환을 오래된 32단계 표가 가렸기 때문이다. 이를 방지하려고 §5.2에 v44 11단계를 별도 전수 매핑하고, §5.4의 32단계는 하위 명령 분해로 명시했다.
...
WEAKEST_LINE: D-01부터 D-05까지는 회장 확정 전이므로 수치와 정책을 본문 상수로 고정하지 않았다.
