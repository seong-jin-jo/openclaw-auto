...
basis: 학습정보 층계 계약 v2.1 §5 §6 §15 §17, FDD v3.0, API 계약 v2.0, 기존 ERD v1.0, 현 dashboard PostgreSQL 스키마
...
deliberation: 기존 tenants, brand_guides, drafts, usage_events를 파괴하지 않고 새 Studio 정규화 저장소로 확장한 뒤 호환 뷰로 전환한다.
-->

# Studio 생성 데이터 설계 v2.0

> 한 줄 결론: Studio는 회원, 브랜드, 작업 공간, 층 항목과 판, 제작법, 동기화, 작업과 부분 결과를 자기 PostgreSQL에 저장하며 모든 범위 행을 회원과 브랜드까지 격리한다.
...
| 격리 | Studio 회원, 브랜드, 작업 공간 관계와 PostgreSQL 행 격리 |
...
erDiagram
  ...
  STUDIO_MEMBER ||--o{ MEMBER_BRAND_ROLE : receives
  STUDIO_MEMBER ||--o{ BRAND : owns
  BRAND ||--o{ MEMBER_BRAND_ROLE : grants
  BRAND ||--o{ WORKSPACE : contains
  STUDIO_MEMBER ||--o{ LAYER_ITEM : scopes
  BRAND ||--o{ LAYER_ITEM : scopes
  WORKSPACE ||--o{ LAYER_ITEM : scopes
  LAYER_ITEM ||--o{ LAYER_REVISION : versions
  LAYER_REVISION ||--o{ LAYER_REVISION_SOURCE : cites
  BRAND ||--o| VOICE_PROFILE : has
  VOICE_PROFILE ||--o{ VOICE_PROFILE_REVISION : versions
  RECIPE ||--o{ RECIPE_VERSION : versions
  RECIPE_VERSION ||--o{ RECIPE_INSPECTION : inspected_by
...
| authority_brand_id | text | nullable | 외부 정본 식별자 |
...
- `(authority_service, authority_brand_id)` where authority_brand_id is not null

### 3.4 `member_brand_roles`

| 열 | 타입 | 제약 | 설명 |
|---|---|---|---|
| studio_member_id | uuid | PK 일부, FK | 회원 |
| brand_id | uuid | PK 일부, FK | 브랜드 |
| role | text | NN, CHECK | owner, editor, viewer |
| granted_by | uuid | FK | 부여자 |
| created_at | timestamptz | NN | 생성 |

복합 PK `(studio_member_id, brand_id)`.

브랜드 owner는 최소 한 명이어야 한다.
...
|---|---|---|---|
| id | uuid | PK | 작업 공간 |
| studio_member_id | uuid | FK, NN | 소유 회원 범위 |
| brand_id | uuid | FK, NN | 정확히 한 브랜드 |
| name | text | NN | 표시명 |
| authority_service | text | NN | 정본 서비스 |
| authority_workspace_id | text | nullable | 외부 식별자 |
| status | text | NN, CHECK | active, held, deleting, deleted |
| created_at | timestamptz | NN | 생성 |
| updated_at | timestamptz | NN | 갱신 |
| deleted_at | timestamptz | nullable | 삭제 |
...
- `(brand_id, lower(name))` where status != deleted
- `(authority_service, authority_workspace_id)` where authority_workspace_id is not null

닫힌 외래키를 권고한다.

`(brand_id, studio_member_id)`가 brands의 같은 복합 후보키를 참조하게 해 다른 회원 브랜드 아래 작업 공간을 만들 수 없게 한다.
...
| brand_id | uuid | FK, nullable | 브랜드 범위 |
| workspace_id | uuid | FK, nullable | 작업 공간 범위 |
| layer_code | text | NN, CHECK | S0, S1, U2, U3, U4, L5 |
| scope_kind | text | NN, CHECK | system, personal, brand, workspace |
| item_kind | text | NN | brand_fact 등 |
...
| stopping_class | text | NN, CHECK | forbidden_phrase, brand_fact, material_rights, non_stopping |
...
- personal이면 brand_id와 workspace_id 둘 다 null
- brand이면 brand_id NN, workspace_id null
- workspace면 brand_id와 workspace_id 둘 다 NN
- U2는 personal
- U3는 brand
- U4는 workspace
- L5는 세 범위 중 하나
...
- `(brand_id, layer_code, status)` where brand_id is not null
...
| brand_id | uuid | FK, nullable | 조건 1 |
| workspace_id | uuid | FK, nullable | 조건 2 |
| language | text | NN | 조건 3 |
| channel | text | nullable | 조건 4 |
| format | text | NN | 조건 5 |
| metric | text | nullable | 조건 6 |
| measurement_from | timestamptz | nullable | 조건 7 |
| measurement_to | timestamptz | nullable | 조건 7 |
| sample_size | integer | NN, >=0 | 조건 8 |
| approval_scope | text | NN, CHECK | 조건 9 |
| confidence | numeric(5,4) | NN, 0..1 | 조건 10 |
| expires_at | timestamptz | nullable | 조건 11 |
| rollback_state | text | NN, CHECK | active, reverted, held |

인덱스 `(brand_id, workspace_id, language, channel, format, expires_at)` where rollback_state=active.
...
| brand_id | uuid | FK, NN, UQ |
| layer_item_id | uuid | FK, NN, UQ |
| current_revision | bigint | NN |
| created_at | timestamptz | NN |

### 5.2 `voice_profile_revisions`

목소리 판은 U3 layer_revision과 1:1이다.

| 열 | 타입 | 제약 | 채움 판정 |
|---|---|---|---|
| id | uuid | PK | 해당 없음 |
| voice_profile_id | uuid | FK, NN | 해당 없음 |
| layer_revision_id | uuid | FK, NN, UQ | 해당 없음 |
| revision | bigint | NN | 해당 없음 |
| v1_formality | text | nullable, CHECK | 허용값 하나면 true |
| v2_terms | text[] | NN, 기본 빈 배열 | trim 후 1개 이상이면 true |
| v3_intro | text | nullable, CHECK | intro와 ending 모두 있어야 true |
| v3_ending | text | nullable, CHECK | intro와 ending 모두 있어야 true |
| v4_sentence_min | integer | nullable | 네 범위가 유효해야 true |
| v4_sentence_max | integer | nullable | min <= max |
| v4_paragraph_min | integer | nullable | min <= max |
| v4_paragraph_max | integer | nullable | 양수 |
| v5_emoji | boolean | nullable | 세 값 모두 null 아니면 true |
| v5_arabic_digits | boolean | nullable | 명시 false도 채움 |
| v5_parentheses | boolean | nullable | 명시 false도 채움 |
| v6_self_reference | text | nullable | 두 문자열이 공백 아니면 true |
| v6_audience_reference | text | nullable | 두 문자열이 공백 아니면 true |
| free_note | text | nullable | 어떤 slot도 잠그지 않음 |
| filled_slots | bit(6) | NN | 서버 계산 스냅샷 |
| created_at | timestamptz | NN | 해당 없음 |

`filled_slots`는 입력값을 신뢰하지 않고 서버가 계산한다.

DB generated column을 쓰지 않는 이유는 배열 정리와 문자열 trim 규칙을 판 스키마에 맞춰 명시적으로 버전 관리하기 위해서다.

서비스는 저장 전 계산하고 CHECK 함수로 재검증한다.

### 5.3 자유 서술과 잠금

`free_note`가 있어도 filled_slots는 바뀌지 않는다.

이 불변조건은 시험과 저장 함수 양쪽에서 확인한다.
...
| brand_id | uuid | FK, nullable | 브랜드 전용 |
...
Studio 공용 제작법은 studio_member_id와 brand_id가 null이다.

사용자 제작법은 studio_member_id가 NN이다.
...
active recipe_version은 accepted 또는 승인된 downgraded 검사 결과를 요구한다.

### 6.4 `recipe_applications`
...
고유 `(production_envelope_id, recipe_version_id, field_path)`.
...
닫힌 FK로 brand와 workspace가 같은 member에 속함을 강제한다.
...
실행 공간에 회원이나 브랜드 간 공유 참조를 허용하지 않는다.
...
status는 pending, accepted, rejected, edited다.
...
| execution_version_manifest | jsonb NN | 모델과 제작법 판 |
...
3. 브랜드 삭제는 소속 U3, U4, 브랜드 조건 L5를 비활성화한다.
...
| brands, workspaces | 계정 수명 | deleted 상태 |
| layer_revisions | 계보 수명 | 민감 value 가림 가능 |
| request_raw_inputs | 추천 90일 | ciphertext 물리 삭제 |
...
브랜드 자료는 `member_brand_roles`를 통해 접근한다.

owner와 editor는 쓰기, viewer는 읽기다.

작업 생성은 editor 이상이다.

다른 브랜드 식별자 입력은 0행으로 처리된다.

### 11.3 필수 인덱스 목록

| 질의 | 인덱스 |
|---|---|
| 회원 브랜드 목록 | brands(member_id, status, created_at) |
| 브랜드 작업 공간 | workspaces(brand_id, status, created_at) |
| 봉투 층 조회 | layer_items(member_id, brand_id, workspace_id, layer_code, status) |
| 최신 판 | layer_revisions(layer_item_id, revision desc) |
| 조건 L5 | l5_conditions(brand_id, workspace_id, language, channel, format) |
...
| 브랜드 작업 목록 | production_jobs(brand_id, updated_at desc) |
...
- tenants.id -> 외부 member, brand, workspace 매핑 후보
- tenants.name -> 초기 브랜드 또는 작업 공간 표시명

자동으로 한 tenant를 한 브랜드로 확정하지 않는다.
...
목소리 여섯 칸은 자동 추출 결과를 `pending_review`로 두고 정본으로 확정하지 않는다.
...
2. brand_guides를 legacy U3 source로 적재한다.
...
1. workspace.brand_id는 null이 아니다.
2. workspace와 brand의 member가 같다.
3. U2는 personal 범위다.
4. U3는 brand 범위다.
5. U4는 workspace 범위다.
...
11. L5는 조건 열두 칸을 모두 가진다.
12. recipe_version은 선언 여덟 칸을 모두 가진다.
13. free_note는 filled_slots를 바꾸지 않는다.
14. 작업은 하나의 member, brand, workspace만 가진다.
15. 봉투 안 모든 브랜드 범위 항목은 작업 brand와 같다.
...
| 판단 대기 | 추천 기준으로 둔 열 또는 정책 |
|---|---|
| 층 이름 | layer_code CHECK |
| Studio 회원 분리 | studio_members, external mappings |
| 브랜드 정본 | authority_service |
| 계정 병합 | reconciliation과 held |
| 원문 보유 | retention_until 90일 추천 |
| 외부 제작법 권한 | permissions와 network_allowlist |
| 두 판 수용 | contract compatibility metadata |
| 목소리 추가 칸 | 현재 bit(6), minor 확장 시 별도 구조 필요 |
| 밀기 감지 창 | periodic_compare_due_at 계산 정책 |

목소리 칸이 늘면 bit(6)를 직접 늘리는 migration보다 `voice_slot_states` 자식 표로 바꾸는 선택이 필요할 수 있다.

현재는 여섯 칸만 확정한다는 추천안 때문에 bit(6)을 사용한다.
...
하지만 브랜드 격리, 과거 판, 부분 성공, 동기화 재시도는 JSONB 한 행으로는 제약하기 어렵다.
...
공격: `PostgreSQL 한 개에 회원과 브랜드를 같이 두면 물리 격리가 아니다.`

응답: 현재 사업 단계의 공유 DB 선택을 유지하되 비우회 애플리케이션 역할, 행 정책, 닫힌 FK, 브랜드 단위 시험을 강제한다. 고위험 등급의 물리 분리는 기존 private tier 패턴을 별도 ADR로 확장할 수 있다.
...
공격: `브랜드 복사가 계속 연결되어 원본 변경이 따라오면 싫다.`
...
- `docs/학습정보-층계-계약-v2.1.md` §5, §6.4.1, §15, §17, §18
...
WEAKEST_LINE: Studio 회원과 기존 tenants의 실제 계정 병합은 판단 대기라 external mapping과 held 상태까지만 확정했다.
