# studio-service PRD v1.2.0

| 항목 | 값 |
|---|---|
| 버전 | v1.2.0 |
| 생성 시각 | 2026-08-15 08:32 KST |
| 모델 | gpt-codex/GPT-5 |
| 에이전트 | prd-architect worker |
| 작업 라인 | studio-service |
| 상태 | Draft, plan gate 미승인 |
| 스킬 | pipeline |
| 근거 URL | https://sigmine.ai/pricing, https://sigmine.ai/, https://markety.co.kr/, https://trypencil.com/pricing, https://help.trypencil.com/en/articles/9669079-how-does-the-media-performance-score-work, https://support.anyword.com/understanding-the-insights-panel-in-anyword, https://www.anyword.com/pricing |
| 고민 한 줄 | 화면의 A/B/C 선택만으로는 쉽게 복제되므로, 질문의 근거와 반박, 입력 세 범주, 선택 전 확률성, 선택 후 결정론, 유저 소유 데이터 내보내기를 하나의 감사 가능한 결정 계보로 묶었다. |
| 상류 정본 | [제품구조 결정 2026-08-15](../../docs/제품구조-결정-2026-08-15.md), [참여형 결정 경험 벤치마킹 v3](../../docs/벤치마킹-참여형결정경험-2026-08-15-v3-gpt-codex.md), [두 서비스 경계 위키](../../wiki/architecture/two-service-boundary.md), [PRD v1.1.2](./prd-studio-service-v1.1.2-gpt-codex.md) |
...
- [3. 핵심 제품 설계](#3-핵심-제품-설계)
  - [3.7 질문 엔진 화법 규약](#37-질문-엔진-화법-규약)
  - [3.8 생성·편집 두 모드와 제작 지시서](#38-생성편집-두-모드와-제작-지시서)
  - [3.9 요청 입력 세 범주](#39-요청-입력-세-범주)
  - [3.10 확률적 생성과 결정론적 편집](#310-확률적-생성과-결정론적-편집)
  - [3.95 운영자 화면과 지원 범위](#395-운영자-화면과-지원-범위)
- [4. 페르소나](#4-페르소나)
...
- [9. 사용자 흐름과 취향 학습 루프](#9-사용자-흐름과-취향-학습-루프)
  - [9.5 회장 확정 운영·과금·소유권 정책](#95-회장-확정-운영과금소유권-정책)
...
**One Thing:** 마케팅 지식이 없는 사용자가 선택지·추천·근거·반박으로 A/B/C를 고르면 선택 전 확률적 생성이 선택 후 결정론적 편집으로 잠기고, 그 결정·수정·실제 결과가 다음 질문과 이미지·영상을 바꾸는 headless 제작 서비스.

이번 v1.2.0은 v1.1.2의 17개 본장과 부록을 삭제 없이 보존하면서 크게 개정된 제품구조 결정서와 참여형 결정 경험 벤치마킹 v3를 전 장에 연결했다. 질문 엔진의 기본 경로는 `선택지 + 추천 + 근거 + 필요한 반박`이며 자유입력은 보조다. 입력은 그 요청에만 쓰이는 제약, 저장되는 소재, 취향을 바꾸는 학습 신호로 분리한다. 확률성은 선택 전에 다양성을 만드는 데만 쓰고, 사용자가 고른 뒤에는 버전된 제작 지시서와 소재를 고정해 결정론적으로 편집한다. 자막·이펙트·컷 조정은 “무료”가 아니라 “추가 크레딧 없음”으로 표시하며, 나레이션 문구·소재 교체만 해당 컷 단위로 과금한다.

발행 방식은 테넌트 옵션이고 품질 게이트 반려분은 무과금이다. 체험은 연락처를 받은 뒤 크레딧을 지급한다. 결과물·소재·편집 지시서·사람이 읽을 수 있는 취향 프로파일·성과 이력은 전부 유저 소유이며 내보낼 수 있어야 한다. 내부 벤처 테넌트에는 `내부 테스트` 표시를 두어 공용 운영 집계에서 실제 고객과 분리한다. 경쟁 지형은 시그마인, Pencil, Anyword를 최근접 3종으로 보고, 마케티는 공개 상품·가격은 확인됐으나 상용 운영 실체는 미확인인 조건부 직접 경쟁으로 분류한다. 조사 범위의 빈자리는 생성 기능 하나가 아니라 비마케터 질문, 근거 계보, 입력 신호 분리, 생성 전 비용·시간, 선택 후 구조화 편집의 연결부다.
...
|---|---|---|---|---|
| OpenClaw Studio 화면, 초안 저장, 시각 미리보기 | 이미 구현 | 구현현황에 생성·저장·발행·예약·중지·편집/재생성 버튼, 두 PlatformPreview, 발행 이력 패널이 기록돼 있다. 다만 stage·prod는 미배포다 | studio 자체 UI로 복제하지 않는다. openclaw가 입력, 선택, 발행 화면을 계속 소유한다 | [루트 구현현황](../../docs/구현현황.md), [시스템 아키텍처](../../wiki/architecture/system-architecture.md) |
| 위키 기반 후보 생성 | 이미 구현 | 멀티테넌트 위키는 `wiki_path`와 `context_sources` 기반 후보 생성의 코드·시뮬레이션 검증을 기록한다 | 참고자료 반입의 첫 소비자로 연결한다 | [멀티테넌트 위키](../../wiki/ops/multi-tenant.md) |
...
| 입력 원천 4종과 8단계 선택 유도 | 부분 구현 | 업종, 위키 연결, 파일 입력 일부는 있으나 사례, 복수 추천, 비용·시간, 선택 누적은 없다 | 기존 입력 원천을 유지하고 §3.9의 제약·소재·학습 신호로 분류해 비어 있는 결정 보조 단계를 연결한다 | [멀티테넌트 위키](../../wiki/ops/multi-tenant.md), [제품구조 결정](../../docs/제품구조-결정-2026-08-15.md) |
...
| 질문 엔진 화법 규약 | 미구현 | 선택지·추천·근거·반박·자유입력 보조가 하나의 제품 계약으로 구현됐다는 위키·release·승인 산출물은 없다 | 질문 카드와 추천 근거 계보를 Phase 1 요구로 둔다 | [제품구조 결정 §3.7](../../docs/제품구조-결정-2026-08-15.md), [벤치마킹 v3 §14](../../docs/벤치마킹-참여형결정경험-2026-08-15-v3-gpt-codex.md) |
| 제약·소재·학습신호 입력 분리 | 미구현 | 위키는 소재와 신호를 구분하지만 채널 제약을 요청 파라미터로 받는 studio-service 계약은 없다 | 세 범주를 저장·학습·감사 정책까지 분리한다 | [두 서비스 경계 위키](../../wiki/architecture/two-service-boundary.md), [제품구조 결정 §3.9](../../docs/제품구조-결정-2026-08-15.md) |
| 확률적 생성 후 결정론적 편집 | 부분 구현 | EC0147은 고정 소재와 FFmpeg·PIL 렌더를 재현할 수 있으나 선택 전 후보 다양성에서 선택 후 지시서 고정으로 전환하는 서비스 원장은 없다 | 기존 렌더 규칙을 보존하고 선택 순간을 결정론 전환점으로 기록한다 | [파이프라인 README](../pipelines/README.md), [제품구조 결정 §3.10](../../docs/제품구조-결정-2026-08-15.md) |
| 운영자 화면과 지원 | 부분 구현 | openclaw 쪽 Admin·채널 상태는 구현돼 있으나 studio 테넌트·크레딧·원가·큐·품질 반려·지원 사건·내부 테스트 구분 화면은 없다 | openclaw Admin을 복제하지 않고 studio 운영 범위만 추가한다 | [루트 구현현황](../../docs/구현현황.md), [제품구조 결정 §3.95](../../docs/제품구조-결정-2026-08-15.md) |
| 발행 옵션·반려 무과금·체험·데이터 내보내기·내부 테스트 | 미구현 | 2026-08-15 회장 확정 정책이며 구현·배포 증거는 없다 | §9.5와 FR·AC·BM·운영 요구로 추적한다 | [제품구조 결정 §9.5](../../docs/제품구조-결정-2026-08-15.md) |

### 1.3 현재 상태 결함과 회수

⛔ 회수 필요: [Studio 위키](../../wiki/product/studio.md)가 빈 파일이다. 위키가 1차 진실원이어야 하지만 Studio 화면·headless 서비스 경계·두 모드·질문 엔진·입력 세 범주·운영자 범위·회장 확정 5건을 기록하지 않아 구현현황과 제품구조 결정서를 보조 근거로 사용했다. 이 PRD가 plan 승인되면 wiki compiler가 승인 버전과 [두 서비스 경계 위키](../../wiki/architecture/two-service-boundary.md)를 근거로 채워야 한다.

⛔ 회수 필요: [두 서비스 경계 위키](../../wiki/architecture/two-service-boundary.md)의 편집 비용은 “크레딧이 들지 않는다”와 “재렌더뿐”으로 적혀 있어 이번 확정 화법인 “추가 크레딧 없음”과 서버 렌더 원가 구분을 충분히 전달하지 못한다. PRD 승인 뒤 위키 표현을 정합화해야 한다.
...
| 회장 결정 | 자막·이펙트·컷·오버레이·비율의 로컬 편집은 고객에게 `추가 크레딧 없음`으로 표시하고, 나레이션 문구 수정과 소재 교체만 해당 컷 단위 과금한다. 단 서버 렌더 비용은 남는다 |
...
아래 네 가지는 사용자가 가져오는 원천 형태다. §3.9의 세 범주는 시스템이 각 값을 어떻게 저장·적용할지 정하는 의미 분류다. 예를 들어 기존 결과는 재사용 `소재`이고 “첫 3초를 더 빠르게”는 `학습 신호`이며, 채널 길이 상한은 같은 요청에 붙더라도 `제약`이다. 원천 4종과 의미 범주 3종을 같은 축으로 합치지 않는다.
...
질문 엔진은 사용자가 마케팅 명세를 백지에서 쓰게 하지 않는다. 각 질문은 `선택지 + 추천 + 근거 + 필요한 반박` 순서로 제시하며, 자유입력은 선택지에 없는 생각을 보충하는 보조 경로로만 둔다. 기본 화면이 빈 텍스트 영역이면 이 요구를 위반한 것이다.

| 구성 | 필수 내용 | 금지 | 수용 가능한 예 |
|---|---|---|---|
| 선택지 | 실제 결과 또는 결과에 직접 영향을 주는 2~3개 상호배타적 선택 | 추상적 형용사만 나열, 4개 이상 동시 노출 | “첫 방문 고객”과 “재구매 고객” 중 이번 1순위 |
| 추천 | 지금 목적에 맞는 기본안 1개 | 세 안을 같은 무게로 던짐, 추천 없는 백지 질문 | “이번 목표는 저장이므로 B를 추천합니다” |
| 근거 | 추천 계산에 실제 사용한 출처, 표본, 기간, 신뢰도 | 생성 뒤 붙인 사후 설명, 공급자 주장을 독립 사실처럼 표시 | “내 계정 최근 30일 12건, 저장률 기준, 신뢰도 낮음” |
| 반박 | 사용자의 선택이 목적·금지선·비용과 충돌할 때 구체 반론과 대안 | 무조건 동의, 선택 강제, 목적과 무관한 훈계 | “A는 친근하지만 금융 신뢰 목표와 충돌합니다. B를 비교해 보세요” |
| 자유입력 | `둘 다 아님`, `이번만`, 짧은 메모 | 자유입력을 첫 입력으로 강제 | “다른 생각이 있으면 적어 주세요”를 선택지 아래에 배치 |

질문 순서는 결과 영향이 크고 불확실성이 높은 축부터다. 이미 신뢰도가 높은 취향은 다시 묻지 않는다. 질문 다섯 개마다 남은 질문을 건너뛸 수 있고, 건너뛰면 추천 기본값으로 첫 결과가 나와야 한다. `이번만`은 현재 작업에만 적용하고 취향 프로파일을 바꾸지 않는다. `둘 다 아님`은 거절 이유를 보존하고 후보 생성 규칙을 재검토하게 한다.

반박은 사용자의 결정을 빼앗는 기능이 아니다. 시스템은 충돌과 대안을 제시한 뒤 최종 선택권을 사용자에게 돌려준다. 법적·브랜드 금지선처럼 넘을 수 없는 경계는 반박이 아니라 차단 사유와 해결 조건을 보여준다. 추천 근거에 데이터가 부족하면 점수를 만들지 않고 `가설`, `외부 사례`, `내 계정 데이터`, `우리 실험` 중 하나로 표시한다.
...
생성과 편집은 동일한 작업 계보를 공유하지만 서로 다른 동작 모드다. 생성 모드는 주제·목적·참고자료·취향을 바탕으로 새 A/B/C를 만들고 선택을 학습한다. 편집 모드는 선택된 기존 결과와 버전된 제작 지시서를 불러와 원본을 보존한 채 수정 지시만 반영한다. 편집 지시는 현재 작업에는 강하게 적용하되 취향 프로파일에는 약한 신호로 기록한다.

#### 3.8.1 편집 크레딧 판정표

| 편집 지시 | 실행 수단 | 고객 크레딧 | 실제 비용 처리 | 과금 단위 |
|---|---|---:|---|---|
| 자막 크기·글꼴·색·위치·문구 변경 | 로컬 FFmpeg·PIL과 기존 자막 레이어 재렌더 | 추가 크레딧 없음 | 서버 CPU·GPU·저장·전송 비용은 운영 원가로 측정 | 해당 없음 |
| 이펙트 추가 | 로컬 FFmpeg 필터와 기존 타임라인 재렌더 | 추가 크레딧 없음 | 서버 렌더 비용은 남음 | 해당 없음 |
| 컷 순서·길이 변경, 컷 삭제 | 기존 컷 자산과 타임라인 재조립 | 추가 크레딧 없음 | 서버 렌더 비용은 남음 | 해당 없음 |
| 오버레이·용어칩·말풍선 배치 변경 | 로컬 PIL·FFmpeg 합성 | 추가 크레딧 없음 | 서버 렌더 비용은 남음 | 해당 없음 |
| 채널별 비율 변경·크롭 | 기존 자산의 로컬 변환 | 추가 크레딧 없음 | 서버 렌더 비용은 남음 | 해당 없음 |
...
고객 표시는 반드시 `추가 크레딧 없음`이다. `무료`, `비용 0`, `크레딧 0`을 고객 문구로 쓰지 않는다. 기존 결과의 자막·이펙트·컷·오버레이·비율을 바꾸는 동안 생성 크레딧을 더 차감하지 않는다는 뜻이며 서버 렌더 원가는 남는다. BM과 원가 원장은 고객 크레딧, 외부 모델 직접비, 서버 렌더 원가를 분리해야 한다.
...
studio가 받는 입력은 `제약(constraint)`, `소재(asset)`, `학습 신호(signal)` 세 범주다. 같은 요청 본문에 들어오더라도 저장 위치와 수명, 취향 반영 여부가 다르므로 혼합 필드나 공통 `metadata` 하나로 뭉치면 안 된다.

| 범주 | 예 | 수명·저장 | 취향 반영 | 필수 판정 |
|---|---|---|---|---|
| 제약 | 비율, 길이, 글자수 상한, 해시태그 상한, 지원 필드, 안전영역 | 해당 제작 요청과 결과 계보에 적용 규격 버전만 보존. 범용 취향에는 누적하지 않음 | X | openclaw가 studio 호출 요청에 실어 보냄. studio가 openclaw를 역호출해 읽지 않음 |
| 소재 | 롱폼 원본, 이미지, 로고, 목소리 샘플, 참고 링크, 기존 studio 결과 | tenant 자산 저장소와 권리·보존·삭제 계보 | 직접 반영 X | 내용에서 취향 축을 추출해 별도 신호로 환원할 때만 취향 후보가 됨 |
| 학습 신호 | A/B/C 선택, 거절, 이번만, 자연어 수정, 추천 채택, 발행 성과 | tenant 신호 원장과 근거 기간 | 종류별 규칙에 따라 O·조건부·X | 선택·수정·성과를 분리하고, 성과를 취향으로 오인하지 않음 |

채널 제약은 요청에 실려 오는 파라미터이며 학습 신호가 아니다. 9:16 결과를 요청했다고 해서 사용자가 세로 화면을 영구 선호한다고 기록하면 안 된다. openclaw가 제약 스냅샷을 studio 요청에 실어 보내므로 의존 방향 `openclaw-service -> studio-service`가 유지된다. studio는 플랫폼 이름을 몰라도 비율·길이·텍스트 상한으로 결과를 만들 수 있어야 한다.

#### 3.9.1 테넌트와 private 데이터 경계
...
### 3.10 확률적 생성과 결정론적 편집

studio는 전 구간이 결정론적인 제품이 아니다. 연출층의 LLM과 이미지·영상·음성 공급자는 확률적이다. 이 확률성은 선택 전에 서로 다른 후보를 만들 때 필요하다. 사용자가 하나를 고른 순간 결과를 만드는 대본, 컷 순서, 소재 ID, 자막 스타일, 목소리, 레이아웃, 채널 파생 규칙을 버전된 제작 지시서로 고정하고 그 뒤 편집은 결정론적으로 수행한다.

| 구간 | 성질 | 허용되는 변화 | 금지 |
|---|---|---|---|
| 선택 전 연출 | 확률적 | 구조·후크·시각 문법·말투가 다른 A/B/C | 같은 결과의 색상만 바꾼 위장 후보 |
| 선택 전 소재 생성 | 확률적 | 승인 범위 안의 이미지·영상·음성 변형 | 사용자가 고른 뒤 영향 없는 소재를 묵시적으로 다시 생성 |
| 선택 순간 | 전환점 | 선택 후보와 파라미터를 지시서 버전으로 고정 | 완성 파일만 남기고 계보 폐기 |
| 선택 후 편집 | 결정론적 | 지시서 diff에 적힌 컷·레이어만 변경 | 에이전트가 임의로 다음 도구·스타일을 다시 선택 |

동일한 부모 결과와 지시서 버전, 동일한 수정 diff, 동일한 고정 소재가 주어지면 편집 결과의 바이트 또는 허용된 결정론 검증값이 같아야 한다. 나레이션 문구나 소재 내용이 바뀌어 해당 컷을 다시 생성하는 순간 그 컷만 확률 구간으로 돌아가며, 새 결과를 승인한 뒤 다시 결정론 구간으로 잠근다. 원가 우위는 이 전환점을 지킬 때만 성립한다.
...
Phase 1의 studio-service는 고객용 자체 화면은 없지만 운영자가 실패와 원가를 보지 못하는 headless 블랙박스여서는 안 된다. 고객이 보는 제작 화면은 openclaw가 소유하고, studio 운영자는 별도의 내부 운영 면에서 테넌트·작업·크레딧·공급자·품질·지원 사건을 조회하고 제한된 조치를 수행한다.

| 운영 면 | 반드시 보여줄 것 | 허용 조치 | 금지 |
|---|---|---|---|
| 테넌트 | 상태, `내부 테스트` 여부, 최근 작업, 잔액, 사용량, 연락처 존재 여부 | 정지·재개, 내부 테스트 표시 변경, 승인된 잔액 조정 | 다른 tenant의 콘텐츠 본문을 기본 목록에 노출 |
| 크레딧·결제 | 충전, 예약, 차감, 환불, 품질 반려 무과금, 조정 이력 | 사유가 있는 환불·조정 | 원장 없는 수동 잔액 변경 |
| 사용량·원가 | 공급자별 직접비, 서버 렌더 원가, 재시도, 견적 이탈 | 공급자·방식 중지, 한도 조정 제안 | 고객 크레딧과 내부 원가를 한 값으로 표시 |
| 작업 큐 | 대기·실행·선택대기·품질반려·준비·실패, 마지막 갱신 | 안전한 재시도, 취소, 사람 검수 배정 | 결과가 불명확한 작업을 완료 처리 |
| 품질·운영 루프 | 반려 이유, 방식별 반려율, 프롬프트 실패, 실제 고객과 내부 테스트 분리 집계 | 반려 규칙 검토, 공급자 차단 | 내부 테스트를 실제 고객 성과에 합산 |
| 지원 | 문의자, tenant, 관련 작업, 재현 정보, 응답 상태 | 사건 연결, 담당·상태·해결 기록 | 시크릿·원문 자격증명을 지원 메모에 복사 |

지원 경로는 openclaw 고객 화면의 연락 수단과 studio 작업 ID를 연결한다. 사용자가 생성 실패·비용·결과 수정 문제로 문의하면 운영자는 별도 설명을 다시 요구하기 전에 해당 작업의 입력 범주, 견적, 공급자 시도, 품질 반려, 지시서 버전, 크레딧 원장을 한 화면에서 확인할 수 있어야 한다. 운영자가 private 원문을 열어야 하는 경우에는 최소 권한, 목적, 열람 주체, 시각을 감사한다.

Phase 1에서 지원 채널 제공자, 응답시간 SLA, 역할별 권한 매트릭스는 확정하지 않는다. 비용과 운영 책임이 수반되므로 기술설계와 운영정책에서 회장 선택을 받아야 한다. 단, 작업과 지원 사건의 연결, 상태 이력, 비밀값 미노출은 MVP 필수다.
...
그녀는 추천을 그대로 따르는 수동적 사용자가 아니다. 추천이 자기 목적과 충돌하면 시스템이 무엇을 근거로 판단했는지 보고 반박하거나 기존 선택을 유지하고 싶다. 반대로 매 질문을 자유입력으로 설명하는 것도 원치 않으므로, 기본은 고를 수 있는 선택지와 한 개의 추천이어야 한다. 서비스를 떠날 때는 완성 파일만 받는 것으로 충분하지 않다. 어떤 소재와 지시서로 만들었고 왜 이런 취향 규칙이 생겼는지 사람이 읽을 수 있는 형태로 가져갈 수 있어야 자신의 작업이 서비스 안에 인질로 남지 않는다고 느낀다.
...
| 제작 지시서를 보존해 수정비를 낮추는 도구 | 실제 비용과 시간을 직접 줄임 | 자체 생성물에만 강하고 외부 영상에는 약함 | `추가 크레딧 없음`을 원가 없음으로 과장하면 원가·환불 사고 | 유력 |
...
**마케팅 지식이 없는 사용자가 선택지·추천·근거·반박으로 A/B/C를 고르면 선택 전 확률적 생성이 선택 후 결정론적 편집으로 잠기고, 그 결정·수정·실제 결과가 다음 질문과 이미지·영상을 바꾸는 headless 제작 서비스.**

### 5.3 One Thing 인과

`출처 있는 사례`가 무엇을 만들지 모르는 첫 번째 백지 부담을 줄인다. `선택지·추천·근거·필요한 반박`과 `비용·시간이 보이는 A/B/C`가 결정을 가능하게 한다. `선택 전 확률적 생성과 선택 후 결정론적 편집`이 후보 다양성과 국소 수정 재현성을 함께 만든다. `선택 이유와 약한 수정 신호, 실제 성과, 제작 지시서`가 다음 질문과 결과를 개선한다. 이 네 고리가 끊기면 studio는 기존 생성 API의 얇은 래퍼가 된다.
...
| Question contract completeness | 결정 질문에 선택지·추천·근거·필요한 반박이 모두 있는 비율 | 100% | 질문 엔진이 빈칸을 사용자에게 떠넘긴다 |
| Post-selection determinism | 같은 부모·같은 수정 diff에서 영향 밖 생성 호출이 없는 비율 | 100% | 선택 뒤에도 결과가 흔들려 편집 신뢰가 없다 |
| Quality-reject charge incident | 필수 품질 반려 결과에 고객 크레딧이 정산된 건수 | 0건 | 회장 확정 과금 정책이 깨졌다 |
| Portable export completeness | 사용자 소유 데이터 종류별 파일·설명·누락 사유가 제공된 비율 | 100% | 데이터 소유권이 선언에 그친다 |
| Publishing-option fidelity | tenant·채널·유형별 발행 설정과 실제 발행 동작의 일치율 | 100% | 사용자 통제 없이 잘못 발행된다 |
| Internal-test contamination | 실제 고객 기본 집계에 내부 테스트가 포함된 건수 | 0건 | 시장·품질 판단이 내부 표본에 오염된다 |

### 6.3 MVP 기능 5개

| MVP 기능 | One Thing 연결 | 포함 요구 | 제외 경계 |
|---|---|---|---|
| 1. 근거·소재 반입과 사례 추천 | 백지 부담을 줄임 | 제약·소재·학습신호 분리, 5접점, 출처 상태, 권리 메타데이터 | 범용 웹 크롤러, 권리 자동 판정 |
| 2. 질문 엔진·A/B/C·제작 방식 견적 | 선택을 가능하게 함 | 선택지, 추천 1개, 근거·표본·신뢰도, 필요한 반박, 자유입력 보조, 의미 있는 세 방향, 비용·시간 범위 | 무제한 질문, 무제한 변형, 모델 카탈로그 노출 |
| 3. 완성 미디어·채널 창작 텍스트와 품질 게이트 | studio 경계를 증명 | 이미지·영상·음성·자막·인코딩, 제목·소개·해시태그·첫 댓글, 공급자 근거, 품질 규칙 | 발행 실행, 채널 계정 관리 |
| 4. 선택 이유·취향 학습·결정론적 편집 | 다음 수정 부담을 줄임 | 선택 전 확률적 후보, 선택 후 지시서 고정, 원본 유지, 이유축, 약한 수정 신호, 적용 이력, 부분 재렌더, 추가 크레딧 없음 판정 | 외부 장편 역설계·자동 클리핑 |
| 5. 견적·정산·소유권·운영과 openclaw 인계 | 비용 신뢰와 실제 사용을 증명 | quote, reserve, settle, 반려 무과금, 체험 크레딧, 컷 단위 과금, 유저 데이터 내보내기, 내부 테스트 구분, 운영자 지원, 발행 방식 옵션 | 최종 판매가격·PG·SLA·역할 권한의 임의 확정 |
...
### FR-002 내부 테스트 테넌트 구분

| 필드 | 내용 |
|---|---|
| 설명 | 각 tenant를 실제 고객 또는 `내부 테스트`로 명시 분류하고 운영 루프 집계에서 기본 분리한다 |
| 근거 | 해줘단타·제로원 취향이 초기 표본을 과대 대표해 실제 고객에게 전이되는 것을 방지 |
| Fit criterion | 운영자 화면과 모든 집계에서 내부 테스트 포함·제외 표본 수가 보이고, 실제 고객 기본 집계에는 내부 테스트가 0건이다 |
| 우선순위 | Must |
| 현재 상태 | 미구현. 회장 확정 정책 |
...
### FR-013 제약·소재·학습신호 분리

| 필드 | 내용 |
|---|---|
| 설명 | 제작 요청의 채널 규격은 제약, 미디어·문서는 소재, 선택·수정·성과는 학습 신호로 분류한다 |
| 근거 | 일회성 채널 규격이 영구 취향으로 오염되는 것을 방지하고 단방향 의존을 유지 |
| Fit criterion | 제약은 요청에만 적용되고 취향 규칙을 변경하지 않는다. 소재는 신호 변환 근거 없이 취향에 직접 반영되지 않으며, 모든 학습 변경은 신호 원장까지 추적된다 |
| 우선순위 | Must |
| 현재 상태 | 미구현 |

### FR-014 유저 소유 데이터 내보내기

| 필드 | 내용 |
|---|---|
| 설명 | 결과물, 업로드·생성 소재, 편집 지시서, 사람이 읽을 수 있는 취향 프로파일, 선택과 성과 이력을 tenant가 일괄 내보낼 수 있게 한다 |
| 근거 | 회장 확정 데이터 소유권. 이동 가능한 형태로 줘야 실제 유저 소유다 |
| Fit criterion | 내보내기 결과에 원본 파일, 파생물, 지시서 버전과 계보, 축·가중치·근거를 설명한 취향 문서, 선택·성과 표가 포함되고 내부 식별자만으로 해석을 요구하지 않는다 |
| 우선순위 | Must. 외부 판매 전 필수 |
| 현재 상태 | 미구현 |
...
| 필드 | 내용 |
|---|---|
| 설명 | 각 제작 결정에 선택지, 추천 한 개, 실제 사용 근거, 목적 충돌 시 반박을 제공하고 자유입력은 보조로 둔다 |
| 근거 | 백지공포와 선택 피로를 함께 줄이는 핵심 경험 |
| Fit criterion | 필수 질문 100%에 선택지·추천·근거 유형·출처 또는 미확인·신뢰도·직접입력 보조가 있다. 목적 충돌 시험에서는 반박과 대안을 제시한 뒤 최종 선택권을 돌려준다 |
| 우선순위 | Must |
| 현재 상태 | 미구현 |
...
| 필드 | 내용 |
|---|---|
| 설명 | 로컬 FFmpeg·PIL로 끝나는 편집은 `추가 크레딧 없음`으로 처리하고, 나레이션 문구 수정과 소재 교체만 영향을 받는 컷 단위로 과금한다 |
| 근거 | 수정이 재생성이 아니라 재렌더라는 원가 우위의 실체와 과금 투명성 |
| Fit criterion | 자막·이펙트·컷·오버레이·비율 편집은 추가 생성 크레딧을 차감하지 않고 서버 렌더 원가만 별도 기록된다. 나레이션·소재 변경은 영향 컷 밖의 생성 호출과 추가 과금이 0이다 |
| 우선순위 | Must |
| 현재 상태 | 미구현 |

### FR-037 선택 전 확률성·선택 후 결정론

| 필드 | 내용 |
|---|---|
| 설명 | 선택 전에는 의미 있게 다른 후보를 확률적으로 생성하고, 선택 시점부터 고정 소재와 버전된 지시서 diff로 편집한다 |
| 근거 | 후보 다양성과 국소 수정 재현성을 동시에 확보 |
| Fit criterion | 선택된 후보는 지시서 버전으로 고정된다. 동일 부모·동일 지시서·동일 수정 diff 시험에서 영향 밖 생성 호출은 0이고 결정론 검증값이 일치한다 |
| 우선순위 | Must |
| 현재 상태 | 부분 구현. 로컬 렌더는 있으나 서비스 전환 원장 미구현 |
...
### FR-052 품질 게이트 반려분 무과금

| 필드 | 내용 |
|---|---|
| 설명 | 필수 품질 게이트가 반려한 결과에는 고객 크레딧을 정산하지 않고 예약액을 되돌린다 |
| 근거 | 고객이 고를 수 없는 결과에 과금할 근거가 없다는 회장 확정 |
| Fit criterion | quality_failed로 끝난 모든 후보의 고객 정산액이 0이고 예약액은 전액 환원된다. 공급자 비용은 studio 운영 손실과 반려율 원장에 남는다 |
| 우선순위 | Must |
| 현재 상태 | 미구현 |

### FR-053 연락처 기반 체험 크레딧

| 필드 | 내용 |
|---|---|
| 설명 | 상시 무료 등급 대신 연락 가능한 정보를 받고 명시된 체험 크레딧을 1회 지급한다 |
| 근거 | 원가 통제와 후속 리드 확보를 함께 달성하는 회장 확정 |
| Fit criterion | 연락처 수집 목적·이용·보존·철회 고지가 동의와 연결되고, 중복 지급이 차단되며 지급·사용·만료 원장이 남는다 |
| 우선순위 | Must. 외부 체험 전 필수 |
| 현재 상태 | 미구현 |
...
### FR-062 테넌트별 발행 방식 옵션

| 필드 | 내용 |
|---|---|
| 설명 | openclaw는 tenant가 승인 후 발행 또는 자율 발행을 고르게 하고 채널별·콘텐츠 유형별로 다르게 설정한다 |
| 근거 | 회장 확정. 모든 고객에게 하나의 자동화 강도를 강제하지 않음 |
| Fit criterion | 기본값은 승인 후 발행이며, 자율 발행은 tenant가 명시적으로 켠 채널·유형에만 적용된다. studio는 발행 여부를 결정하지 않고 품질 통과 패키지만 인계한다 |
| 우선순위 | Must. openclaw 연동 계약 |
| 현재 상태 | 미구현 |

### FR-070 운영자 화면과 지원 사건

| 필드 | 내용 |
|---|---|
| 설명 | 운영자가 tenant, 내부 테스트 여부, 작업 큐, 크레딧·환불, 공급자 원가, 품질 반려, 지원 사건을 연결해 본다 |
| 근거 | headless 서비스의 실패·원가·고객 막힘을 선제 파악 |
| Fit criterion | 한 작업 ID에서 요청 입력 범주, 견적, 공급자 시도, 품질 판정, 지시서, 크레딧, 지원 사건을 추적할 수 있고 모든 수동 조정에 주체·사유·시각이 남는다 |
| 우선순위 | Must |
| 현재 상태 | 부분 구현. openclaw Admin은 있으나 studio 운영 면은 미구현 |
...
- 추가 크레딧이 없는 편집도 렌더 시간, 사용 자원, 저장·전송량을 운영 원가에 남긴다.
...
### NFR-011 데이터 이동성과 유저 소유권

- 결과물·소재·편집 지시서·취향 프로파일·성과 이력을 제품 내부 전용 식별자만으로 잠그지 않는다.
- 취향 프로파일은 사람이 읽을 수 있는 축, 선호 방향, 가중치, 적용 범위, 근거 선택, 신뢰도, 마지막 갱신 시각을 포함한다.
- 내보내기는 tenant 경계를 지키고 요청자 권한·생성 시각·포함 범위·누락·실패를 감사한다.
- 내보낸 편집 지시서는 다른 도구가 완전히 재현할 수 있다고 과장하지 않되, 컷·소재·대본·자막·음성·레이아웃·변경 이력을 사람이 해석할 수 있어야 한다.

### NFR-012 운영자 접근과 지원 보안

- 운영자 목록은 고객 원문·소재 미리보기를 기본 노출하지 않고 예외 확인에만 최소 권한을 요구한다.
- 원문 열람, 잔액 조정, 환불, tenant 정지·재개, 내부 테스트 표시 변경은 주체·사유·시각을 감사한다.
- 지원 사건에는 API 키·OAuth 토큰·결제수단·원문 자격증명을 복사하지 않는다.
- 운영 집계는 실제 고객과 내부 테스트를 기본 분리하며 포함 집계에는 표본 구성을 명시한다.
...
```mermaid
flowchart TD
    A[openclaw에서 목적과 입력 선택] --> Z[요청 제약, 재사용 소재, 학습 신호 분리]
    Z --> B{입력 원천}
    B -->|주제| C[출처 있는 업계 사례 탐색]
    B -->|참고자료| D[출처와 사실 상태 검증]
    B -->|재사용 소재| E[권리와 형식 검역]
    B -->|기존 studio 결과| F[제작 지시서와 수정 영향 분석]
    C --> G[선택지, 추천, 근거, 필요한 반박]
    D --> G
    E --> G
    F --> H[재사용과 재생성 범위 및 증분 견적]
    G --> I[확률적 A/B/C와 제작 방식별 비용·시간 비교]
    I --> J[방향·방식·이유 선택과 지시서 잠금]
    H --> J
    J --> K[비용 예약]
    K --> L[선택 뒤 결정론적 제작 또는 영향 범위 편집]
    L --> M{사실·권리·포맷·시각·음성 품질 통과?}
    M -->|아니오| N[고객 과금 없이 예약 환원, 실패 원인과 수정 선택지]
    N --> H
...
2. openclaw는 대상 채널의 규격 스냅샷을 요청 제약으로 studio에 제공한다. studio는 제약·소재·학습 신호를 분리한다.
3. studio는 선택지, 추천 1개, 출처·표본·기간·신뢰도 근거, 필요한 반박을 결정 카드로 반환한다. 자유입력은 보조다.
4. 선택 전에는 의미축이 다른 A/B/C와 제작 방식별 비용·시간 범위를 확률적으로 탐색한다.
5. 사용자는 후보와 방식을 고르고 이유축 또는 메모를 남긴다. 이 선택이 제작 지시서를 고정하는 전환점이다.
6. studio는 비용을 예약하고 선택된 지시서에 따라 미디어를 만든 뒤 강제 품질 게이트를 실행한다. 필수 품질 반려분은 고객에게 과금하지 않는다.
7. studio는 품질 통과 결과에 대해 채널별 제목, 소개·본문, 해시태그, 첫 댓글을 창작한다.
8. openclaw는 파일과 문구를 임의 변경하지 않고 규격을 검증한다. tenant의 채널·유형별 옵션에 따라 사용자 승인 뒤 발행하거나 명시 범위만 자율 발행한다.
9. studio는 선택 이유를 강한 신호로, 수정 지시를 약한 신호로, 발행 성과를 보조 신호로 분리해 다음 취향 상태에 반영한다.
...
3. studio는 수정 지시를 영향 컷·레이어로 분해하고 `추가 크레딧 없음` 편집인지 컷 단위 재생성인지 판정한다.
4. 자막·이펙트·컷·오버레이·비율 편집이면 고객에게 `추가 크레딧 없음`을 표시한다. 동시에 서버 렌더 비용은 운영 원가로 남는다고 구분한다.
...
### 9.5 회장 확정 운영·과금·소유권 정책

아래 다섯 건은 미결정이 아니라 2026-08-15 회장 확정이다. 제품·기술설계·화면이 다른 기본값을 두면 정책 회귀다.

| 확정 정책 | 제품 동작 | 수용 경계 | 소유 서비스 |
|---|---|---|---|
| 발행 방식은 tenant 옵션 | 승인 후 발행과 자율 발행을 tenant가 고르고 채널별·유형별로 다르게 둔다 | 초기 기본값은 승인 후 발행. 명시적으로 켜지 않은 자율 발행은 금지 | 설정·실행은 openclaw, 품질 통과 패키지는 studio |
| 품질 게이트 반려분 무과금 | 필수 품질 반려 결과는 고객 정산액 0, 예약액 전액 환원 | 고객이 선택할 수 없는 결과에 과금 금지. 공급자 호출비는 운영 손실로 기록 | studio |
| 체험은 연락처 후 크레딧 지급 | 연락처 이용 고지를 받고 1회 체험 크레딧을 지급 | 상시 무료 등급으로 확장하지 않음. 중복 지급·보존·철회 정책 필요 | 계정·연락처는 openclaw, 지급·사용 원장은 studio와 정합 |
| 데이터는 전부 유저 소유 | 결과물·업로드와 생성 소재·편집 지시서·사람이 읽을 수 있는 취향 프로파일·선택과 성과 이력을 내보냄 | 내부 ID 덤프만 제공하면 불합격. 이동 후 해석 가능한 설명과 파일 포함 | 각 서비스가 자기 소유 데이터를 내보내고 openclaw가 사용자 요청을 조정 |
| `내부 테스트` tenant 표시 | 해줘단타·제로원과 실제 고객을 계정 단위로 구분 | 실제 고객 기본 운영 집계에서 내부 테스트 제외. 포함 시 표본 분리 표시 | 두 서비스에 같은 분류를 전달하되 각 DB에서 강제 |

#### 9.5.1 데이터 내보내기 최소 패키지

| 데이터 | 최소 내보내기 | 합격 기준 |
|---|---|---|
| 결과물 | 원본과 최종 파일, 채널 파생물, 파일 해시, 생성 시각 | 누락·손상 목록을 포함하고 tenant 밖 파일 0건 |
| 소재 | 업로드·생성 자산, 출처, 권리 메타데이터, 참조 관계 | 삭제·만료 자산은 상태와 누락 사유 표시 |
| 편집 지시서 | 부모·자식 버전, 컷, 소재 ID, 대본, 자막, 음성, 레이아웃, 변경 diff | 사람이 어느 부분이 어떻게 바뀌었는지 읽을 수 있음 |
| 취향 프로파일 | 축, 선호 방향, 가중치, 적용 범위, 신뢰도, 근거 선택, 교정 이력 | “밝은 톤 선호, 짧은 컷, 빠른 나레이션”처럼 사람이 읽고 수정 가능한 형태 |
| 선택·성과 | 후보·선택·이유·이번만·수정, 채널·목적·기간·관측 지표 | 취향과 성과 신호를 다른 열과 의미로 분리 |

#### 9.5.2 발행 옵션과 질문 엔진의 연결

질문 엔진은 발행 방식도 백지로 묻지 않는다. 초기에는 “품질을 확인하고 발행”을 추천하고 이유를 설명한다. tenant가 같은 채널·유형을 반복 승인한 뒤 자율 발행을 원하면 해당 범위만 켤 수 있다. 회장 확정은 옵션 제공이며, 동일 유형 승인 횟수나 반려율 같은 자동 승격 수치는 아직 가설이므로 기술설계에서 임의 확정하지 않는다.
...
| AC-017 | 기존 결과에서 자막 크기·글꼴·색·위치·문구, 이펙트, 컷 순서·길이·삭제, 오버레이, 비율 중 하나를 수정 | 편집 견적을 확인하고 실행 | 고객 화면에 `추가 크레딧 없음`이 표시되고 원본·부모 지시서는 보존되며 새 리비전과 서버 렌더 원가만 기록된다 | FR-034, FR-036, FR-043, NFR-010 |
...
| AC-020 | 목적·채널·비용이 정해지지 않은 신규 사용자가 첫 질문을 봄 | 질문 엔진이 결정을 요청 | 선택지 2~3개, 추천 1개, 추천 계산에 쓴 근거 유형·출처·표본·기간·신뢰도, 직접입력 보조가 한 결정 카드에 보인다 | FR-024 |
| AC-021 | 사용자가 목적과 충돌하는 후보를 선택 | 다음 단계로 진행 | 시스템이 구체 충돌과 대안을 반박으로 제시하고, 사용자는 기존 선택 유지 또는 대안 선택을 최종 결정한다 | FR-024 |
| AC-022 | openclaw가 9:16·60초·캡션 상한을 담은 제약과 소재·선택 신호를 함께 전송 | studio가 요청을 처리 | 제약은 해당 결과에만 적용되고 취향 프로파일 변경 0건, 소재는 자산으로, 선택은 신호로 각각 기록된다 | FR-013, NFR-010 |
| AC-023 | 사용자가 A/B/C 중 하나를 승인하고 지시서 버전이 고정됨 | 같은 부모와 같은 수정 diff를 두 번 적용 | 영향 밖 생성 호출은 0이고 결정론 검증값이 일치하며 새 리비전이 부모를 가리킨다 | FR-037, FR-043 |
| AC-024 | 품질 게이트가 필수 항목 하나를 반려 | 작업 정산이 실행 | 고객 정산액 0, 예약액 전액 환원, 공급자 비용은 운영 손실로 기록되고 openclaw 인계는 차단된다 | FR-040, FR-052 |
| AC-025 | 신규 사용자가 체험을 요청 | 연락처 이용 고지에 동의하고 연락처를 제공 | 체험 크레딧이 1회 지급되고 지급·만료·사용 원장이 남는다. 동일 연락처·계정의 중복 지급은 차단된다 | FR-053 |
| AC-026 | tenant가 데이터 내보내기를 요청할 권한이 있음 | 일괄 내보내기를 실행 | 결과물·소재·편집 지시서·사람이 읽을 수 있는 취향 프로파일·선택·성과 이력과 누락 사유가 제공되고 타 tenant 데이터는 0건이다 | FR-014, NFR-011 |
| AC-027 | 기본값이 승인 후 발행인 tenant가 특정 채널·유형만 자율 발행으로 켬 | 두 유형의 품질 통과 패키지가 인계됨 | 켠 범위만 자율 발행되고 나머지는 승인 대기다. studio는 발행 결정을 만들지 않는다 | FR-062 |
| AC-028 | 내부 테스트 tenant와 실제 고객 tenant가 모두 활동함 | 운영자 기본 집계를 조회 | 실제 고객 표본에서 내부 테스트 0건이며 별도 포함 집계에는 두 표본 수가 표시된다 | FR-002, FR-070, NFR-012 |
| AC-029 | 운영자가 품질 반려 문의를 열람 | 관련 작업 ID를 선택 | 입력 세 범주, 견적, 공급자 시도, 품질 판정, 지시서, 크레딧·환원, 지원 이력이 한 계보로 보이고 원문 열람은 별도 감사된다 | FR-070, NFR-012 |

### 10.2 요구사항 추적표

| 목표 | MVP | 기능 요구 | 수용기준 | 종료 증거 |
|---|---|---|---|---|
| 백지 부담 감소 | MVP-1, MVP-2 | FR-010, 011, 020, 021, 022, 023, 024 | AC-002~006, 020, 021 | 첫 근거 선택 20분 이하 |
| 반복 수정 감소 | MVP-4 | FR-032, 033, 034, 036, 037, 043 | AC-007~010, 017, 018, 023 | 수정 부담 30% 감소, 국소 편집의 영향 밖 생성 호출 0 |
| 품질과 경계 증명 | MVP-3, MVP-5 | FR-030, 040, 041, 042, 044, 060, 061 | AC-011, 012, 014, 019 | 파일 해시 동일, 채널 창작 텍스트 studio 소유, 실제 2트랙 발행 |
| 비용 신뢰 | MVP-2, MVP-4, MVP-5 | FR-022, 036, 050, 051, 052, 053 | AC-006, 012, 013, 017, 018, 024, 025 | 견적 적중 80% 이상, 추가 크레딧 없음과 서버 렌더 비용 분리, 반려 과금 0건 |
| private 데이터 보호와 이동성 | 전체 | FR-001, 002, 011, 013, 014, 070, NFR-001~003, 011, 012 | AC-001, 003, 015, 016, 022, 026, 028, 029 | 교차 사고 0건, 사용자 내보내기 완결, 내부 테스트 혼입 0건 |
| 발행 통제 | MVP-5 | FR-060, 061, 062 | AC-014, 019, 027 | tenant·채널·유형별 설정과 실제 발행 동작 일치 100% |
...
| 연락처 기반 체험 | 첫 결정 경험과 품질 확인 | 연락처 동의 후 1회 크레딧 지급 | 구조 채택, 지급량·만료 미결정 |
| 데이터 이동성 | 결과물뿐 아니라 수정 가능한 자산과 설명 가능한 취향을 보유 | 별도 잠금 요금 없음. 기본 권리 | 회장 확정 |
...
6. **quality reject:** 필수 품질 게이트 반려분은 고객 정산액 0, 예약액 전액 환원이다. 공급자 직접비는 운영 손실로 별도 기록한다.
...
|---|---:|---|---|---|
| 로컬 편집 | 추가 크레딧 없음 | 자막 크기·글꼴·색·위치·문구, 이펙트 추가, 컷 순서·길이·삭제, 오버레이 배치, 비율 변경 | “추가 크레딧 없음. 서버 재렌더는 실행됩니다” | 렌더 시간, CPU·GPU, 저장, 전송을 기록 |
...
`추가 크레딧 없음`과 `원가 없음`은 다르다. 로컬 편집은 고객 생성 크레딧을 더 차감하지 않지만 서버 렌더 비용이 발생한다. 초기 BM은 이 서버 렌더 비용을 기본 사용료나 생성 마진 안에서 흡수할 수 있는지 내부 20건으로 측정한다. 측정 전에는 `무제한 수정`을 판매 문구로 쓰지 않는다. 반복 편집으로 서버 원가가 과도해지면 횟수 제한을 임의 확정하지 말고 회장에게 사용료 구조 선택지를 다시 올린다.
...
| 체험 리드 관리 | 연락처 동의, 중복 지급, 만료, 철회 | 목적·보존 고지와 1회 지급 원장 | 고객지원·개인정보 책임자 |
| 운영자 예외 처리 | 품질 반려, 큐 적체, 잔액 조정, 공급자 장애 | 작업 계보와 사유 있는 제한 조치 | studio 운영 |
| 데이터 내보내기 | 대용량 파일, 지시서·취향 설명, 누락 처리 | 비동기 패키지와 완결성 보고 | studio·openclaw 공동 |
...
- 로컬 편집은 추가 크레딧이 없지만 렌더 원가를 누락하지 않는다. 작업별 렌더 시간과 자원 사용량이 없으면 BM 검증 표본에서 제외한다.
- 나레이션 문구 수정과 소재 교체는 해당 컷 밖의 공급자 호출이 0임을 원장으로 증명하지 못하면 컷 단위 과금 성공으로 세지 않는다.
- 품질 게이트 반려분은 고객 과금 0이어야 하며 공급자 비용은 반려율과 함께 운영 손실로 측정한다.
- 체험 크레딧은 연락처 동의와 1회 지급 원장 없이 발급하지 않는다.
- 내부 테스트 tenant를 실제 고객의 추천 채택률·반려율·원가 기본 집계에 섞지 않는다.
- 데이터 내보내기 실패·누락은 성공으로 표시하지 않고 종류별 상태와 재시도 경로를 제공한다.

---

## 12. 경쟁 벤치마크와 포지셔닝

### 12.1 v3 경쟁 지형과 빈자리 판정

[참여형 결정 경험 벤치마킹 v3](../../docs/벤치마킹-참여형결정경험-2026-08-15-v3-gpt-codex.md)는 국내외 15종을 선택 입력, 추천 근거, 선택 누적, 성과 학습, 비용 사전 표시, 결정 뒤 편집, 발행 연결의 일곱 축으로 비교했다. 결론은 “이미 있다. 다만 우리가 노리는 결합은 공개 근거에서 아직 한 제품으로 확인되지 않았다”이다. A/B/C, 브랜드 학습, 성과 개선, 좋아요·싫어요 학습 중 어느 하나도 단독 차별점이 아니다.

| 경쟁 순위 | 제품 | 이미 차지한 자리 | 우리가 정면으로 이기기 어려운 점 | 조사 범위의 연결부 |
|---:|---|---|---|---|
| 1 | Sigmine | 브랜드 자료, 대량 제작, 자동·승인 발행, 성과 분석과 다음 전략 | 전체 운영 흐름이 가장 가깝고 셀프서브 SaaS와 B2B 구독을 함께 공개 | 비마케터 질문, 추천 출처·표본·신뢰도, 선택·수정·성과 분리, 지시서 편집 |
| 2 | Pencil | 광고 후보 생성, 점수, 사람 승인, 광고 채널 실행, 실제 성과 학습 | 생성·사람 승인·실성과 학습을 이미 연결 | 초보자용 결정 질문, 결과 방식별 비용·시간, 선택 뒤 결정론적 편집 계보 |
| 3 | Anyword | 후보별 예측 점수, 오디언스·채널 기준, 과거 성과 벤치마크 | 추천 근거를 보여주는 결정면이 성숙 | 마케터 설정 UI가 아닌 선택지·추천·근거·반박 화법, 미디어 지시서 편집 |
| 조건부 | Markety | 사진 한 장에서 콘텐츠·영상 생성, Instagram·Facebook 발행, 가격·지원 표기 | 공개 기능 범위가 넓고 소상공인 약속이 직접적 | 사이트·상품 표시는 확인됐지만 상용 운영·실결제·실생성·실발행은 미확인 |

**이미 차지된 문장:** “AI가 콘텐츠를 만든다”, “여러 채널용으로 만든다”, “브랜드 톤을 학습한다”, “사람이 승인한다”, “성과로 좋아진다”, “점수로 추천한다”, “좋아요·싫어요로 취향을 학습한다”. 이 문장만으로 포지셔닝하면 후발 생성기다.

**빈자리 판정:** 빈자리는 카테고리가 아니라 연결부다. 공개 근거 범위에서 한 제품으로 확인하지 못한 조합은 비마케터에게 실제 결과를 선택하게 하는 질문, 추천 계산에 실제 사용한 출처·표본·기간·신뢰도, 생성 전 비용·시간 범위, 제약·소재·선택·수정·성과 신호 분리, 불확실성에 따른 다음 질문, 선택 후 제작 지시서 부분 재렌더, tenant별 자동·승인 발행 옵션의 결합이다. 전 세계 부존재를 뜻하지 않으며 로그인 뒤 기능은 미확인이다.

**해자 판정:** 질문 카드와 A/B/C 버튼은 쉽게 복제된다. 방어력은 고객 내부 지식, 선택·수정·성과 계보, 실제 제작 원가, 취향과 성과 충돌 교정, 재생성 없이 고친 지시서 이력, 질문별 이탈·추천 채택 데이터가 쌓일 때 생긴다. 따라서 포지셔닝 후보는 `AI 콘텐츠 제작기`가 아니라 `비마케터용 마케팅 결정 시스템`이다. 다만 구현·검증 전 외부 약속으로 쓰지 않는다.

#### 12.1.1 v1.2.0 공식 페이지 재검증

이번 워커는 2026-08-15에 한국어와 `국내`를 포함한 시그마인·마케티 검색, Pencil·Anyword 공식 검색의 네 질의를 실제 실행하고 공식 페이지를 열었다.

| 공식 근거 | 확인 사실 | 차용 | 비차용·주의 |
|---|---|---|---|
| https://sigmine.ai/ | 홈페이지 URL·브랜드 자료·기존 채널을 받아 자동 제작·발행하고 성과를 다음 전략에 반영한다고 공개한다. B2B 블로그·Threads 15만원, 숏폼 45만원, 묶음 75만원을 표시한다 | 브랜드 입력에서 발행 성과까지 잇는 운영 순환 | 성과 수치는 공급자 자기주장. 우리 차별점으로 “성과 학습” 단독 주장 금지 |
| https://sigmine.ai/pricing | Starter $20·2,000 credits, Pro 예시 $40·4,000 credits, Business $200·20,000 credits, 추가 채널 $5를 표시한다 | 제품 사용량과 운영대행 가격면 분리 | B2B 구독과 한 가격표로 합치지 않음 |
| https://markety.co.kr/ | 사진 한 장, 콘텐츠·영상 생성, Instagram·Facebook 발행, Starter 무료와 99,000·249,000·499,000·1,099,000원 플랜을 표시한다 | 시작 입력 단순화 | 페이지 존재와 상용 운영 실체를 같은 사실로 취급하지 않음 |
| https://trypencil.com/pricing | Core $14 월간, Growth $55 월간, 50·250 generations, 광고 채널 실행·추적, 점수, 브랜드·성과 데이터 학습을 표시한다 | 데이터 부족 시 점수 보류, 성과와 브랜드 점수 분해 | 공급자 성과 주장을 독립 효과로 인용하지 않음 |
| https://help.trypencil.com/en/articles/9669079-how-does-the-media-performance-score-work | 과거 광고 대비 CTR percentile 점수와 데이터 부족 시 `Insufficient data`를 설명한다 | 표본 부족을 점수로 위장하지 않음 | headline score 하나로 취향·성과·규격을 합치지 않음 |
| https://support.anyword.com/what-is-the-predictive-performance-score | 0~100 예측 점수와 채널별 평가를 설명한다 | 추천에 비교축을 제공 | 추천 계산에 쓰지 않은 데이터를 사후 근거로 붙이지 않음 |
| https://www.anyword.com/pricing | 월간 Starter $49, Data-Driven $99, 실시간 수동 편집 예측, 과거 캠페인 학습을 표시한다 | 수정 중 근거 변화와 과거 성과 비교 | 카피 성과 예측을 이미지·영상 결정론적 편집 증거로 확대 해석하지 않음 |

#### 12.1.2 v1.1.2 WebSearch 실조회 기록 보존
...
| Pencil | 광고 후보 생성·점수·사람 승인·실성과 학습 | 데이터 부족 시 점수 보류, 사람이 승인한 결과와 실제 성과 연결 | 광고 성과 점수 하나를 취향·규격·권리 판단으로 확대하지 않음 | 비마케터 질문, 비용·시간, 선택 후 지시서 편집을 연결 |
| Anyword | 후보별 예측 점수와 오디언스·브랜드·채널 근거 | 근거가 보이는 후보 비교와 편집 중 점수 변화 | 사후 설명이 아니라 추천 계산에 실제 쓴 출처·표본·기간·신뢰도를 공개 | 반박·교정·선택·수정·실성과 계보까지 확장 |
| Markety | 사진 한 장에서 다채널 콘텐츠와 영상 | 시작 입력을 단순화 | 한 장 자동생성 대신 출처·선택 이유를 요구 | 수량 약속보다 원본 유지 편집, 로컬 편집의 `추가 크레딧 없음`, 컷 단위 과금을 증명 |
...
| 시장 | 질문 엔진의 추천과 반박이 훈계처럼 느껴짐 | 이탈·추천 불신 | 추천 근거 공개, 선택 유지권, `이번만`·`둘 다 아님` 경로 | 반박 빈도와 이탈 상관 |
...
| BM | `추가 크레딧 없음` 편집을 원가 없음으로 오인해 서버 렌더 비용이 누적 | 편집 사용량이 늘수록 공헌이익 악화 | 고객 크레딧·외부 모델비·서버 렌더 원가 분리, 내부 20건 측정 | 기본료에 흡수할지 별도 사용료로 둘지 회장 결정 |
| BM | 품질 반려율이 높은 공급자를 계속 써 고객 과금 없이 직접비만 누적 | 반려가 늘수록 마진·현금흐름 악화 | 방식·공급자별 반려 원가, 자동 차단 문턱, 운영자 비교 | 차단 문턱과 예비 공급자 |
...
| 운영 | 연락처 기반 체험이 중복 지급과 광고성 연락 분쟁을 만듦 | 크레딧 남용·개인정보 민원 | 목적·보존·철회 고지, 1회 지급 원장, 마케팅 동의 분리 | 지급량·만료·보존기간 |
| 운영 | 운영자가 잔액·재시도·private 원문을 과도하게 조작·열람 | 부정 조정·개인정보 사고 | 최소 권한, 사유 필수, 불변 감사, 원문 별도 승인 | 역할별 권한 매트릭스 |
| 운영 | 자율 발행 옵션의 범위가 잘못 적용돼 승인 전 콘텐츠가 게시됨 | 브랜드·법적 사고 | 기본 승인 후 발행, tenant·채널·유형 범위 검증, 즉시 중지 | 자동발행 사고 대응 SLA |
...
| 데이터 | 채널 규격·길이·비율 같은 제약을 취향 신호로 저장 | 다음 채널 결과가 잘못 편향 | 요청 제약·재사용 소재·학습 신호를 저장·만료·적용에서 분리 | 규격 스냅샷 보존기간 |
| 데이터 | 유저 소유 내보내기가 내부 ID 덤프에 그치거나 대용량 파일을 누락 | 이동권 미달·락인 비판·지원 폭증 | 종류별 manifest, 사람이 읽을 수 있는 취향, 누락 사유, 재시도 | 전달 방식·완료 기한 |
| 데이터 | `내부 테스트`가 실제 고객 집계에 섞여 채택률·반려율이 좋아 보임 | 잘못된 시장·품질 판단 | 기본 집계 제외, 표본 수 병기, 분류 변경 감사 | 과거 집계 재분류 범위 |
...
**반대안 4, 선택과 반박을 없애고 사용자가 한 줄만 쓰면 자율 발행까지 맡긴다.** Sigmine과 Markety의 공개 약속처럼 생성과 발행을 한 번에 줄이면 첫 사용 장벽과 운영 클릭이 작아진다. 사용자가 바쁜 것이 핵심이라면 질문 엔진은 도구가 해야 할 판단을 다시 고객에게 떠넘기는 설문이 될 수 있다.

이 반대안은 시간 절감이라는 고객 목적을 정확히 공격한다. 그래서 질문 수를 늘리는 것이 아니라 결과를 바꾸는 load-bearing 결정만 묻고, 매 질문은 추천 한 개와 근거를 먼저 내야 한다. 사용자가 같은 범위를 반복 승인하면 tenant 옵션으로 자율 발행을 열되, 추천 불신·반려·법적 위험이 큰 범위까지 자동화하지 않는다. 질문 완료시간과 발행 사고가 개선되지 않으면 참여형 결정 경험을 축소해야 한다.
...
**시나리오 7, `추가 크레딧 없음` 편집이 인기를 끌수록 서버 렌더 적자가 커진다.** 고객에게 생성 크레딧을 더 받지 않는다는 이유로 FFmpeg·PIL 재렌더의 CPU·GPU·저장·전송 비용을 원가에서 빼 버렸다. 사용자는 자막과 컷을 여러 번 고치고, 외부 모델 직접비는 늘지 않지만 서버 비용과 대기열이 계속 늘어난다.

대응은 고객의 `추가 크레딧 없음`과 내부 원가 없음을 분리하는 것이다. 모든 편집에 렌더 시간과 자원 사용량을 기록하고 내부 20건에서 생성 마진으로 흡수 가능한지 본다. 흡수가 안 되면 무제한 수정 약속을 하지 않고 기본 사용료, 공정사용 한도, 렌더 사용량 중 어떤 BM을 택할지 회장 결정으로 다시 올린다.

**시나리오 8, 품질 반려 무과금이 공급자 실패를 숨기는 보조금이 된다.** 고객은 반려 결과에 돈을 내지 않아 신뢰를 유지하지만, 운영자는 반려율과 공급자 직접비를 한 화면에서 보지 못한다. 자동 재시도가 같은 불량을 반복하면서 매출 없는 비용과 대기열만 늘어난다.

대응은 반려 결과를 매출 0과 원가 손실로 동시에 기록하는 것이다. 공급자·방식·프롬프트 버전별 반려율과 손실을 운영자 면에 표시하고, 승인된 문턱을 넘으면 방식 중지나 사람 검수로 전환한다. 품질 기준을 낮춰 정산 가능한 결과로 바꾸는 우회는 금지한다.

**시나리오 9, 연락처 체험이 리드 수는 늘리지만 신뢰와 개인정보 준수를 잃는다.** 한 번 체험하려는 사용자에게 광고성 연락 동의까지 묶거나 보존기간을 설명하지 않았고, 중복 계정은 계속 크레딧을 받는다. 지원팀은 철회 요청과 부정 사용을 수동으로 처리한다.

대응은 체험 제공에 필요한 연락처 이용 고지와 선택적인 마케팅 수신 동의를 분리하는 것이다. 지급·만료·중복 판정·철회 이력을 남기고 연락처 보존기간은 법적 검토와 회장 결정 전 확정하지 않는다. 연락처 전환율만 성과로 보지 않고 중복 지급률과 철회 처리시간도 함께 본다.

**시나리오 10, 유저 소유권이 선언뿐이고 실제로는 떠날 수 없다.** 결과 파일은 내려받을 수 있지만 편집 지시서는 내부 식별자와 모델 파라미터뿐이고, 취향 프로파일은 해석 불가능한 벡터다. 대용량 소재 일부가 누락되어 다른 도구에서 작업을 이어갈 수 없다.

대응은 결과·소재·지시서·선택·성과·사람이 읽을 수 있는 취향을 종류별 manifest로 내보내는 것이다. 누락된 종류와 이유, 재시도 경로를 함께 제공하고 타 tenant 데이터 0건을 검증한다. 이동성 패키지 한 건도 끝까지 복원 검증하지 못하면 데이터 소유권을 제품 약속으로 내세우지 않는다.

**시나리오 11, 내부 테스트와 자율 발행이 성과를 왜곡하거나 사고를 낸다.** 내부 팀이 좋은 소재와 반복 피드백으로 만든 결과가 실제 고객 채택률에 섞여 제품이 잘된 것처럼 보인다. 다른 tenant에서는 채널·유형 범위가 잘못 적용돼 승인하지 않은 콘텐츠가 발행된다.

대응은 `내부 테스트`를 tenant 수준에서 표시하고 실제 고객 기본 집계에서 제외하는 것이다. 발행 기본값은 승인 후로 두고 자율 발행은 tenant·채널·유형의 명시 범위에만 적용한다. 내부 표본 혼입이나 범위 밖 자율 발행이 한 건이라도 관찰되면 관련 집계와 자동 발행을 즉시 중지한다.
...
**수정:** v1.2.0은 “모든 고객의 제작 동반자” 같은 범용 표현을 제거하고, 측정 가능한 내부 2트랙 증거를 먼저 요구하며 외부 고객·독립 UI·SDK를 뒤로 미뤘다. 비용 선공개와 export의 추가 크레딧 없음은 InVideo에도 있으므로 단독 차별점에서 제외했다. 벤치마킹 v3의 빈자리 판정에 맞춰 선택지·추천·근거·반박, 입력 세 범주, 선택 전 확률성, 선택 후 결정론, 사람이 읽을 수 있는 취향 내보내기를 하나의 데이터 계보로 묶었다. 이 연결이 실제 질문시간·수정부담·영향 밖 생성 호출을 줄이지 못하면 UI 신기함만 남으므로 가설을 축소한다.
...
| KC-12 렌더 원가 실패 | `추가 크레딧 없음` 편집 20건 | 서버 렌더 원가가 승인된 흡수 한도를 2주 연속 초과 | 해당 편집 범위 확장 중단, BM과 공정사용 정책 회장 재결정 |
| KC-13 질문 엔진 실패 | 유효 결정 카드 20건 | 필수 네 요소 누락 1건 또는 첫 근거 선택 중앙값 30분 초과 | 질문 수·순서 축소, 추천 근거와 자유입력 경로 재설계 |
| KC-14 품질 반려 손실 실패 | 품질 판정 20건 | 반려 결과 고객 과금 1건 또는 반려 직접비가 승인 한도를 2주 연속 초과 | 고객 과금 사고 환원, 해당 방식·공급자 중지, 품질선 재승인 |
| KC-15 데이터 이동성 실패 | 실제 내보내기 3건 | 필수 종류 누락·타 tenant 혼입·사람이 읽을 수 없는 취향 중 1건 | 외부 판매와 소유권 마케팅 중단, export 계약 재설계 |
| KC-16 표본 오염 실패 | 운영 집계 상시 | 실제 고객 기본 집계에 내부 테스트 1건 포함 | 지표 사용 중지, 과거 집계 재산출, 분류 경계 감사 |
| KC-17 발행 통제 실패 | 자율 발행 상시 | tenant·채널·유형 범위 밖 발행 1건 | 자율 발행 즉시 중단, 사고 대응 후 회장 재승인 |
...
### 15.0 이번 개정에서 확정되어 재결정하지 않는 항목

다음 다섯 건은 [§9.5](#95-회장-확정-운영과금소유권-정책)의 회장 확정 정책이다. 발행 방식의 tenant 옵션 제공, 필수 품질 반려분 무과금, 연락처 제공 뒤 체험 크레딧 지급, 결과·소재·편집 지시서·사람이 읽을 수 있는 취향 프로파일을 포함한 유저 데이터 전체 소유와 내보내기, `내부 테스트` tenant 표시는 다시 선택지로 올리지 않는다. 남은 결정은 이 정책을 바꾸는 것이 아니라 가격·보존기간·권한·SLA처럼 구현 비용과 법적 책임을 정하는 일이다.
...
| D-06 | 외부 크레딧 가격과 실패·환불 기준 | 미결정 | **부분 해결**. 필수 품질 반려분 무과금·예약 전액 환원은 확정, 판매가격과 시스템 실패·변심 정책은 미결정 | 20건 원가 실측 후 남은 quote/reserve/settle/refund 정책 승인 | 확정 정책을 지키며 고객 신뢰와 마진 통제 | 조기 확정 시 환불 분쟁·적자 |
...
| D-09 | 원본·파생물·취향·감사 원장 보존·삭제기간 | 미결정 | **부분 해결**. 데이터 전체 유저 소유와 내보내기는 확정, 보존·삭제기간과 법정 감사 예외는 미결정 | 데이터 종류별 보존·삭제 정책과 사용자 처리 결과표 승인 | 소유권·이동권을 지키며 개인정보 대응 가능 | 구현 후 역으로 삭제 계보를 붙이면 누락 위험 |
...
| D-13 | `추가 크레딧 없음` 로컬 편집의 서버 렌더 비용을 어떤 BM으로 흡수할지 | 내부 20건 원가 측정 전에는 생성 마진에 임시 귀속하고, 이후 기본 사용료·공정사용 한도·렌더 사용량 중 선택 | 고객 표시 원칙과 단위경제를 함께 유지 | 원가 없음으로 오인하면 사용량 증가가 적자로 연결되고, 조기 한도 확정은 원가 우위를 약화 | 내부 20건 정산 후, 외부 판매 전 |
| D-14 | 운영자 역할별 조회·조정·원문 열람 권한과 지원 응답시간 | 최소 권한 역할안과 위험별 응답시간을 실제 사건 10건 뒤 승인 | private 원문 접근과 잔액 조정의 책임이 명확 | 미결정 상태로 구현하면 과권한·감사 누락, 너무 이른 확정은 운영 현실과 불일치 | eng-design 전 권한 원칙, QA 전 구체 문턱 |
| D-15 | 체험 크레딧 지급량·만료와 연락처 보존·철회 정책 | 1회 체험 완료에 필요한 중앙값 원가와 법적 검토 후 승인 | 남용과 개인정보 부담을 제어 | 임의 지급량·보존기간은 부정 사용 또는 과잉 수집 위험 | 외부 체험 공개 전 |
...
3. **D-06, D-07, D-09:** 품질 반려 무과금과 유저 소유·내보내기는 재논의하지 않는다. 외부 가격·그 밖의 환불·PG·보존·삭제기간만 설계 전에 회장 결정과 법적 검토가 필요하다.
4. **D-08 제품명:** 내부 작업명은 유지하되 외부 출시 전 확정한다.
5. **D-13 서버 렌더 BM:** `추가 크레딧 없음` 편집 20건의 실제 렌더 원가를 본 뒤 회장이 흡수 방식을 결정한다. 지금 임의로 횟수 제한이나 추가 크레딧을 확정하지 않는다.
6. **D-14, D-15 운영·체험 세부:** 운영자 권한·지원시간과 체험 지급량·만료·연락처 보존기간은 비용·법적 책임이 있으므로 외부 공개 전에 회장이 정한다.
...
| 반대 검증 | Steelman 4개, 프리모텀 11개, 셀프심문, KC 17개 | 실측 전 가설 | plan gate 뒤 파일럿으로 검증 |
...
| 버전 | 변경 |
|---|---|
| v1.2.0 | v1.1.2 전체를 보존한 확장 개정. 질문 엔진 화법, 생성·편집 두 모드와 `추가 크레딧 없음` 표기, 입력 세 범주, 선택 전 확률성·선택 후 결정론, 운영자 화면·지원 범위, 회장 확정 5건을 FR·AC·BM·리스크·KC까지 연결. 참여형 결정 경험 벤치마킹 v3의 15개 사례와 최신 공식 페이지 재확인을 바탕으로 Sigmine·Pencil·Anyword를 최근접 3종, Markety를 조건부 직접 경쟁으로 분류하고 빈자리를 데이터 계보 연결부로 한정 |
...
| v1.1.2 | 시그마인·마케티·국내 Vrew·해외 InVideo를 네 번의 별도 WebSearch로 재조사. 생성·편집 두 모드를 기능요구와 사용자 흐름으로 분리하고 편집을 Phase 1 필수로 확정. 원본 유지·약한 수정 신호·로컬 편집의 당시 `크레딧 0` 표현·서버 렌더 원가·나레이션/소재의 컷 단위 과금을 BM·FR·AC·KC에 연결. 채널별 제목·소개·해시태그·첫 댓글은 studio, 채널 규격은 openclaw 소유로 정정 |

### 17.2 v1.0.0 유효 내용 복원 감사

| v1.0.0 유효 내용 | v1.1.0·v1.1.1 상태 | v1.2.0 누적 조치 |
|---|---|---|
...
| 편집 비용 원가 우위 | “저비용 수정” 수준으로만 표현 | 자막·이펙트·컷·오버레이·비율은 `추가 크레딧 없음`, 나레이션·소재는 해당 컷 과금, 서버 렌더 비용은 별도라는 판정표와 BM을 추가 |
...
| 질문 엔진 화법과 반박 | A/B/C와 이유 입력은 있었으나 추천 계약·반박·자유입력 보조가 분산 | §3.7, FR-024, AC-020·021, KPI, 리스크, Steelman, KC-13으로 연결 |
| 요청 입력과 학습 신호 경계 | 채널 규격과 취향 신호가 한 요청에 함께 들어오는 위험이 불명확 | §3.9에서 제약·소재·학습 신호를 분리하고 FR-013, AC-022, 데이터 리스크로 추적 |
| 확률적 생성과 결정론적 편집 | 부분 재렌더는 있었으나 선택 전후의 확률성 경계가 명시되지 않음 | §3.10, FR-037, AC-023, 성공지표로 선택을 상태 전이로 고정 |
| 운영자 화면과 지원 | 사람 검수·환불 부하는 있었으나 내부 운영 면의 조회·조치·금지가 없음 | §3.95, FR-070, NFR-012, AC-029, 운영·권한 리스크, D-14 추가 |
| 회장 확정 5건 | 개별 정책이 미결정 또는 미기록 | §9.5에서 확정값으로 분리하고 FR·AC·BM·프리모텀·KC까지 추적. 가격·보존기간·권한 같은 하위 결정만 §15에 남김 |
| 경쟁 지형과 빈자리 | 기능 목록 중심 비교 | 벤치마킹 v3의 15개 사례를 상류 근거로 두고 Sigmine·Pencil·Anyword 최근접, Markety 조건부 직접으로 분류. 빈자리를 질문 UI가 아니라 결정·수정·성과의 계보로 좁힘 |

### 17.3 plan gate 미통과 항목

- [ ] plan-critic 독립 비평과 verify-agent-quality 통과
- [ ] 회장 D-02, D-04·05, D-06의 잔여 가격·환불, D-07, D-08, D-09의 보존·삭제, D-13~15 결정 또는 다음 단계 이관 범위 승인
- [ ] `wiki/product/studio.md`와 구현현황에 headless 경계·두 모드·편집 비용·채널별 창작 텍스트 소유 갱신
- [x] Mermaid CLI 실제 렌더 확인. flowchart SVG 생성 성공
...
| quality_failed | 필수 품질 미달 | 고객 정산액 0, 예약액 전액 환원, 공급자 직접비는 운영 손실 | 입력 수정, 방식 변경, 사람 검수 또는 새 견적 |
...
| 사용자 지시 | 편집 모드 해석 | 재사용 | 재생성 | 고객 표시·과금 | 학습 신호 |
|---|---|---|---|---:|---|
| “자막을 위로 올려 줘” | 자막 좌표 변경 | 전체 원본 소재·음성 | 없음 | 추가 크레딧 없음 | 약함, 자막 위치 후보 |
| “자막 글꼴과 색을 바꿔 줘” | 자막 스타일 변경 | 전체 원본 소재·음성 | 없음 | 추가 크레딧 없음 | 약함, 시각 문법 후보 |
| “2번 컷을 0.5초 줄여 줘” | 타임라인 길이 변경 | 모든 컷 소재·음성 | 없음 | 추가 크레딧 없음 | 약함, 속도 후보 |
| “3번 컷을 삭제하고 4번을 앞으로” | 컷 삭제·순서 변경 | 남은 컷 전부 | 없음 | 추가 크레딧 없음 | 약함, 구조 후보 |
| “용어칩을 오른쪽 아래로” | 오버레이 배치 변경 | 전체 원본 소재·음성 | 없음 | 추가 크레딧 없음 | 약함, 레이아웃 후보 |
| “릴스 비율로 바꿔 줘” | 비율 변경·크롭 | 전체 원본 소재·음성 | 없음 | 추가 크레딧 없음 | 채널 국소 신호, 전역 취향 아님 |
...
| 블록 | 필수 내용 | 실패 예시 | 사용자 통제 |
|---|---|---|---|
| 질문 | 지금 결과를 바꾸는 결정 한 가지 | 목적·대상·톤·비용을 한 번에 질문 | 질문 미루기 또는 이전 단계 이동 |
| 선택지 | 의미축이 다른 2~3개와 각각의 결과 | 이름만 A/B/C이고 차이가 없음 | `둘 다 아님`, 직접입력 |
| 추천 | 현재 입력 기준 한 개와 예상 영향 | “AI 추천”만 표시 | 추천과 다른 선택 가능 |
| 근거 | 출처·표본·기간·신뢰도·적용 범위 | 시장 평균이라는 출처 없는 단정 | 근거 펼치기, 근거 제외 요청 |
| 반박 | 선택이 목적·권리·비용·품질과 구체 충돌할 때만 | 모든 비추천 선택을 경고 | 기존 선택 유지, 대안 선택 |
| 자유입력 | 선택지로 표현되지 않는 맥락 보조 | 기본 경로를 빈 입력창으로 시작 | 이번 작업에만 적용 여부 지정 |

### A.9 유저 소유 데이터 내보내기 계약

| 데이터 종류 | 사람이 이어서 쓸 수 있는 형태 | 완결성 증거 | 제외·예외 처리 |
|---|---|---|---|
| 결과물·소재 | 원본 파일 또는 접근 가능한 패키지, 형식·해시·권리 메타데이터 | manifest 수와 실제 파일 수 대조 | 누락·권리 제한·만료를 종류별 표시 |
| 제작·편집 지시서 | 부모·리비전·영향 컷·레이어·사용 소재가 읽히는 문서와 기계 판독 표현 | 모든 결과가 부모 계보 또는 최초 버전을 가짐 | 공급자 시크릿·내부 자격증명은 제외 |
| 취향 프로파일 | 규칙, 적용 범위, 신뢰도, 근거 선택, 사용자 교정 이력의 자연어 설명 | 활성 규칙마다 근거 또는 사용자 직접 설정 연결 | 불투명 임베딩만 제공하면 불합격 |
| 선택·수정·성과 | 시각, 작업, 선택지, 이유, 수정 diff, 채널·기간·관측값 | 작업·결과·성과 연결 누락률 0 | 다른 tenant·공용 운영 집계 제외 |
| 비용·품질 | 견적·예약·정산·환원·품질 판정과 사유 | 원장 합계와 작업 상태 정합 | 법정 보존이 필요한 감사정보는 예외와 기간 설명 |

### A.10 운영자 지원 사건 최소 계보

`tenant 표시 -> 문의자와 작업 ID -> 입력 세 범주 -> 견적·예약 -> 공급자 시도 -> 품질 판정 -> 결과·지시서 리비전 -> 고객 크레딧·운영 원가 -> openclaw 인계·발행 -> 지원 조치`가 한 사건에서 연결돼야 한다. 기본 목록은 본문·미디어 원문을 숨기고 상태·사유·금액·해시만 보여준다. 원문 열람은 별도 권한과 목적을 요구하며 열람 주체·시각을 감사한다.

---

## 부록 B. 경쟁 조사 증거 원장

v1.1.2 실제 검색 호출은 네 번이다. 1회는 시그마인, 2회는 마케티, 3회는 한국어와 “국내”를 포함한 Vrew 추가 발굴, 4회는 InVideo다. 각 검색 뒤 공급자 공식 페이지를 다시 열어 아래 사실을 대조했다. v1.2.0은 [참여형 결정 경험 벤치마킹 v3](../../docs/벤치마킹-참여형결정경험-2026-08-15-v3-gpt-codex.md)의 15개 사례와 빈자리 판정을 상류 근거로 사용하고, Sigmine·Markety·Pencil·Anyword 공식 페이지를 다시 검색·열람했다.
...
- 따라서 “생성 전에 비용을 보여준다”거나 “export에서 크레딧을 쓰지 않는다”라는 기능만으로 studio의 차별성을 주장하지 않는다. studio는 원본·지시서 유지, 국소 편집, 컷 단위 재생성, 서버 렌더 원가 분리를 함께 증명해야 한다.
...
### B.6 v1.2.0 최근접 경쟁과 빈자리 재확인

- [Sigmine 공식 홈](https://sigmine.ai/)은 브랜드 입력에서 콘텐츠 생성·발행·성과 피드백까지의 운영 루프와 원화 B2B 상품을 공개한다. [공식 가격](https://sigmine.ai/pricing)은 달러 셀프서비스 상품과 채널 추가 단위를 별도로 보여준다. 두 구매형태를 한 가격으로 합산하지 않는다.
- [Pencil 공식 가격](https://trypencil.com/pricing)은 후보 생성·점수·광고 성과 중심 상품을 제공한다. [Media Performance Score](https://help.trypencil.com/en/articles/9669079-how-does-the-media-performance-score-work)는 충분한 과거 광고 데이터가 없으면 점수를 제시하지 않는 `Insufficient data` 상태를 설명하고, [Custom Scores](https://help.trypencil.com/en/articles/11848392-how-can-i-set-up-custom-scores-in-pencil)는 고객 성과 기준을 따로 정의할 수 있음을 보여준다. studio는 데이터가 적을 때 자신감 있는 추천을 꾸며내지 않고 신뢰도와 표본 부족을 표시한다.
- [Anyword 가격](https://www.anyword.com/pricing)과 [Insights Panel](https://support.anyword.com/understanding-the-insights-panel-in-anyword)은 후보별 예측 성과와 오디언스·브랜드·채널 정책 근거를 비교하는 결정면을 보여준다. 점수와 근거 패널 자체는 빈자리가 아니므로 studio는 출처 계보, 반박, 사용자 교정, 편집·성과 신호 분리까지 연결해야 한다.
- [Markety 공식 홈](https://markety.co.kr/)은 사진 한 장에서 콘텐츠·영상 생성과 Instagram·Facebook 발행, 가격 단계를 공개한다. 다만 공개 페이지 내부 수량 표기 불일치와 운영 실체 미확인이 남아 `웹사이트 실존`과 `검증된 상용 운영`을 분리한다.
- 빈자리 판정은 “경쟁자가 질문하지 않는다”가 아니다. 조사 범위에서 공개 증거로 확인하지 못한 결합은 `비마케터용 선택 질문 + 추천의 출처·표본·신뢰도 + 생성 전 비용·시간 + 제약·소재·학습 신호 분리 + 선택 전 확률성 + 선택 후 결정론적 편집 + 선택·수정·실성과 계보`다. 경쟁사가 이 연결을 공개하거나 studio가 수정 부담·발행 안전·원가를 실측하지 못하면 빈자리 주장을 철회한다.
...
- `/Users/sj/sj_code_master/openclaw-auto/docs/벤치마킹-참여형결정경험-2026-08-15-v3-gpt-codex.md`
...
- https://help.trypencil.com/en/articles/9669079-how-does-the-media-performance-score-work
- https://help.trypencil.com/en/articles/11848392-how-can-i-set-up-custom-scores-in-pencil
- https://www.anyword.com/pricing
- https://support.anyword.com/understanding-the-insights-panel-in-anyword
- https://support.anyword.com/what-is-the-predictive-performance-score

### 품질 선언

SKILLS_USED: pipeline, 현재 plan 단계와 산출물·게이트 요건을 확인하고 `/approve plan` 전 정지하는 데 사용

SKILLS_SKIPPED: brand-positioning-kit은 브랜드 개념·톤·메시징을 새로 확정하는 과업이 아니라 승인 전 PRD의 제품 규칙·요구·추적성을 확장하는 개정이므로 사용하지 않음

RUBRIC_SCORE: 완결성=5/5 정밀성=5/5 벤치마크=5/5 추적성=5/5 전문성=4/5 total=24/25

WEAKEST_LINE: `추가 크레딧 없음` 편집의 서버 렌더 원가를 생성 마진에 흡수할지 별도 사용료로 둘지는 내부 20건 실측과 회장 결정 전이라 확정할 수 없다.
...
STAMP: 2026-08-15 08:32 KST / studio-service / prd-architect worker / pipeline / 제품구조 결정 v2026-08-15·PRD v1.1.2·참여형 결정 경험 벤치마킹 v3·Sigmine·Markety·Pencil·Anyword 공식 출처 / 고민: 질문 UI를 차별점으로 과장하지 않고 선택 전 확률성과 선택 후 결정론, 신호 계보, 유저 이동권까지 하나의 검증 가능한 순환으로 묶음 / plan gate unapproved
