...
basis: docs/사업계획-osmu-v1.0.md v1.2 §3.4, docs/requests/회장-확정-요구사항-대장.md R01~R99, studio/docs/prd-studio-service-v1.2.1-gpt-codex.md, DESIGN.md v21, docs/user-flow.md, wiki/architecture/two-service-boundary.md, 기존 FDD v3.0, 실제 dashboard와 studio/pipelines 구현
...
deliberation: 반려된 브랜드 층과 스킬 분리안을 제거하고 작업 공간 하나를 격리 단위로 고정했다. 정본 변경은 아웃박스로 밀고 제작 요청은 로컬 투영본만 읽게 해 서비스 장애 전파와 요청 지연을 줄였다.
-->

# Studio 생성 기능 설계 문서 v4.0

> 한 줄 결론: Studio는 자기 회원과 자기 일곱 층 저장소를 가진 독립 서비스다. 브랜드와 취향의 분리는 작업 공간으로 해결하고, `S0 S1 U2 U3 X4 L5 R6`을 조립한 불변 봉투만 무상태 제작 엔진에 전달한다.

| 항목 | 값 |
|---|---|
| 상태 | 기술설계안. 구현 전 승인 필요 |
| 상류 정본 | `docs/사업계획-osmu-v1.0.md` v1.2 §3.4, 요구 대장 R01~R99 |
| 제품 입력 | `studio/docs/prd-studio-service-v1.2.1-gpt-codex.md` |
| 화면 입력 | `DESIGN.md` v21, `docs/user-flow.md` |
| 승계 문서 | FDD v3.0, API v2.0, ERD v2.0, 시험 계획 v2.0 |
| 매핑 결과 | 32단계 중 엔드포인트, 화면 구성요소, 저장 대상, 시험 빈칸 0 |
| 구현 상태 | 기존 Studio 화면과 Python 제작 실험은 존재. 독립 studio-service와 일곱 층 계약은 미구현 |
...
본문에는 확정 설계만 둔다. 이번 판에서 build를 막는 미결정은 없다. 운영 수치 중 보유 기간, 주기 대조 창, 재시도 최대 횟수는 제품 동작을 바꾸는 고정 계약으로 만들지 않고 배포 설정으로 둔다. 기본값은 시험 계획의 부하·복구 시험을 통과한 뒤 운영자가 정한다. 금지 표현·브랜드 사실·소재 권리의 최신성 실패는 설정과 무관하게 항상 해당 작업만 멈춘다.

`wiki/architecture/two-service-boundary.md`의 채널별 문구 소유 설명은 R88 이전이다. 이 설계는 최신 정본을 따라 제목·소개·해시태그·첫 댓글을 Studio 편집실이 만들고, openclaw는 채널 규격 제공·검증·예약·발행만 담당한다.

### 회장이 정할 것

없음. R96~R99까지 사업계획 v1.2 본문과 확정 원장으로 이동되어 있다.

## 1. 목표와 범위 <a id="목표"></a>

### 1.1 목표

이 문서는 사업계획 v1.2 §3.4의 일곱 층, 네 방, 조립 순서와 두 서비스 상호작용을 실제 구현 구조로 내린다.
...
| 입력 | 이 문서에서 가져온 것 |
|---|---|
| 사업계획 v1.2 §3.4 | 일곱 층, 네 방, 조립 단계, 상호작용, 무료 영상 한 편 |
| 회장 확정 요구사항 R01~R99 | 최신 확정과 폐기 규칙. R88, R89, R92, R96~R99 우선 |
| Studio PRD v1.2.1 | 독립 서비스, 생성 단계, 후보, 비용, 실패, 수용 기준 |
...
| U3 작업 공간 정본 | Studio | Studio 정본, openclaw 투영 |
| L5 배운 규칙 정본 | Studio | Studio 정본, 승낙된 활성 판만 openclaw 투영 가능 |
...
| `U2`는 `member_id`만, `U3·L5`는 `workspace_id` 필수 | DB CHECK |
...
Studio가 직접 관리하는 회원·작업 공간·일곱 층의 정본은 Studio DB다. 합친 상품에서 openclaw가 발행·성과용 `S0 S1 U2 U3 L5`를 관리하면 필요한 항목만 Studio에 투영한다. 어느 쪽이 정본인지는 항목마다 `authority_service`로 고정한다.
...
`docs/user-flow.md` v43과 사업계획 v1.2의 네 방 흐름을 32개 구현 단계로 고정한다.
...
|---|---|---|---|---|---|
| UF-01 | Studio 세션 시작 | `POST /v3/studio/sessions` | `StudioAuthGate` | `studio_sessions` | FLOW-01 |
| UF-02 | 작업 공간 목록과 현재 공간 선택 | `GET /v3/studio/workspaces` | `WorkspaceSwitcher` | `workspaces`, `workspace_members` | FLOW-02 |
| UF-03 | 작업 공간 생성, 처음에는 글·영상 갈래만 선택 | `POST /v3/studio/workspaces` | `WorkspaceStartPicker` | `workspaces`, `layer_items`, `layer_revisions` | FLOW-03 |
| UF-04 | 학습 정보 열람·수정 | `GET /v3/studio/workspaces/{workspaceId}/layers`, `POST /v3/studio/layer-items/{itemId}/revisions` | `LearningInfoPanel` | `layer_items`, `layer_revisions` | FLOW-04 |
| UF-05 | 소재·참고자료 반입 | `POST /v3/studio/material-imports` | `MaterialImportReview` | `material_imports`, `material_rights` | FLOW-05 |
| UF-06 | 근거 있는 사례와 추천 3개 조회 | `POST /v3/studio/proposal-sets` | `DisplayProposalDeck` | `reference_views`, `proposal_sets`, `production_outputs` | FLOW-06 |
| UF-07 | 비용·시간 범위 확인 | `POST /v3/studio/production-estimates` | `CostTimeApproval` | `cost_entries` | FLOW-07 |
| UF-08 | 제작 작업과 `R6` 생성 | `POST /v3/studio/productions` | `GenerationProgress` | `production_jobs`, `request_adjustments` | FLOW-08 |
| UF-09 | 일곱 층 조립과 사실 충돌 판정 | `POST /v3/studio/productions/{jobId}/assemble` | `ConflictQuestion` | `production_envelopes`, `production_conflicts` | FLOW-09 |
| UF-10 | 저해상도 후보 3개 실행 | `POST /v3/studio/productions/{jobId}/execute-previews` | `DisplayLoadingState` | `production_attempts`, `production_outputs`, `cost_entries` | FLOW-10 |
| UF-11 | 세 후보 전부 거절 후 재요청 | `POST /v3/studio/proposal-sets/{setId}/retry` | `ProposalRetryAction` | `proposal_sets`, `production_attempts` | FLOW-11 |
| UF-12 | 후보 하나와 이유 선택 | `POST /v3/studio/productions/{jobId}/selections` | `CandidateChooser` | `production_decisions`, `learning_observations` | FLOW-12 |
| UF-13 | 선택 후보만 고해상도·고품질 승격 | `POST /v3/studio/productions/{jobId}/promotions` | `PromotionProgress` | `production_outputs`, `production_attempts`, `cost_entries` | FLOW-13 |
| UF-14 | 품질·금지 표현·사실·권리 검사 | `POST /v3/studio/outputs/{outputId}/inspections` | `QualityGateResult` | `output_inspections` | FLOW-14 |
| UF-15 | 확정·보관·다른 제안 선택 | `POST /v3/studio/productions/{jobId}/disposition` | `ResultDispositionBar` | `production_jobs`, `production_decisions` | FLOW-15 |
| UF-16 | 편집실에서 부모 결과 열기 | `POST /v3/studio/edits` | `EditPreview` | `edit_jobs`, `production_outputs`, `production_recipes` | FLOW-16 |
| UF-17 | 대화·선택·직접 조작 편집 지시 저장 | `POST /v3/studio/edits/{editId}/instructions` | `EditInstructionPanel` | `edit_instructions` | FLOW-17 |
| UF-18 | 영향 컷만 재생성 또는 로컬 재렌더 | `POST /v3/studio/edits/{editId}/render` | `EditRenderProgress` | `production_attempts`, `production_outputs`, `cost_entries` | FLOW-18 |
| UF-19 | 세부 채널 규격 받기, 계정 연결은 하지 않음 | `POST /v3/studio/channel-spec-snapshots` | `ChannelTargetPicker` | `channel_spec_projections` | FLOW-19 |
| UF-20 | 제목·소개·해시태그·첫 댓글 생성 | `POST /v3/studio/outputs/{outputId}/channel-packages` | `ChannelCopyEditor` | `channel_text_packages`, `channel_text_revisions` | FLOW-20 |
| UF-21 | 완성 원본·문구·제작 정보 인계 | `POST /v3/studio/handoffs` | `HandoffStatus` | `handoff_records`, `production_outputs` | FLOW-21 |
...
| UF-25 | 선택·수정·성과 관찰 밀기 | `POST /v3/studio/sync/events` | `LearningInfoBadge` | `sync_inbox`, `learning_observations` | FLOW-25 |
| UF-26 | 3회 선택 또는 5건 성과 뒤 후보 제시 | `GET /v3/studio/workspaces/{workspaceId}/learning-candidates` | `LearningCandidateCard` | `learning_candidates` | FLOW-26 |
| UF-27 | 후보 승낙, `L5` 판 생성 | `POST /v3/studio/learning-candidates/{candidateId}/accept` | `LearningConsentAction` | `learning_candidates`, `layer_items`, `layer_revisions` | FLOW-27 |
| UF-28 | 후보 거절, 관찰 보존 | `POST /v3/studio/learning-candidates/{candidateId}/reject` | `LearningRejectAction` | `learning_candidates`, `learning_observations` | FLOW-28 |
| UF-29 | 되돌리기, 원본 보존·앞 방 항목 추가 | `POST /v3/studio/productions/{jobId}/forks` | `RollbackBanner` | `production_jobs`, `provenance_events` | FLOW-29 |
| UF-30 | 새 브랜드·언어·취향용 작업 공간 복제 | `POST /v3/studio/workspaces/{workspaceId}/copies` | `WorkspaceCopyAction` | `workspaces`, `layer_items`, `layer_revisions` | FLOW-30 |
| UF-31 | 정본 판·삭제를 투영 서비스로 밀기 | `POST /v3/studio/sync/events` | `SyncStatusBadge` | `sync_outbox`, `sync_inbox`, `sync_mappings` | FLOW-31 |
| UF-32 | 작업 공간 삭제 전파와 새 작업 차단 | `DELETE /v3/studio/workspaces/{workspaceId}` | `WorkspaceDeleteConfirm` | `workspaces`, `sync_outbox`, `projection_sync_states` | FLOW-32 |
...
브라우저가 `member_id`, `workspace_id`, `workspace_id`를 보냈다는 이유로 신뢰하지 않는다.
...
| 품질 | 자극 | 환경 | 응답 | 측정 |
|---|---|---|---|---|
| 격리 | 회원 A가 작업 공간 B 식별자로 조회 | 운영 동일 DB | API와 DB 모두 거부 | 반환 행 0, 감사 1 |
| 동기화 | 같은 사건 3회 도착 | 정상 | 한 판만 적용, 같은 응답 | 판 증가 1 |
| 역순 | 판 12 뒤 판 11 도착 | 정상 | 판 11 적용 거부 | applied_revision 12 유지 |
| 삭제 | 작업 공간 삭제 사건 도착 | 정상 | U3, 작업 공간 설치 X4, 해당 L5 비활성 | 신규 봉투 참조 0 |
| 가용성 | 정본 서비스 장애 | 마지막 밀기 성공 | 로컬 최신 판으로 제작 | 원격 호출 0 |
| 안전 | 금지 표현 투영 실패가 설정 감지 창 초과 | 실패 이력 있음 | 영향 작만 차단 | 다른 작업 공간 작 성공 |
| 부분 성공 | 후보 3개 중 1개 실패 | 공급자 장애 | 성공 2개 보존 | 결과 2, 실패 1 |
| 비용 | 누적 비용이 상한 도달 | 실행 중 | 추가 호출 중단 | 상한 초과 청구 0 |
| 추적 | 결과 조회 | 정상 | 항목 판과 스킬 판 표시 | 누락 0 |
| 독립성 | openclaw 중단 | 독립 Studio | 가입부터 생성까지 진행 | openclaw 호출 0 |

## 18. 상류 갭과 보정 결과 <a id="회수"></a>

### 18.1 최신 정본으로 닫은 갭

| 이전 갭 | 최신 근거 | 이 판의 보정 |
|---|---|---|
| 층 이름과 스킬 위치가 문서마다 달랐음 | 사업계획 v1.2 §3.4, R96~R99 | `S0 S1 U2 U3 X4 L5 R6`만 사용하고 X4를 조립 층으로 고정 |
| 브랜드를 별도 층과 개체로 두었음 | 회장 반려 지적, 사업계획 v1.2 | 브랜드·취향 분리는 작업 공간 여러 개로만 해결 |
| 채널별 문구의 소유자가 두 서비스에 섞였음 | R88이 예전 R31을 폐기 | Studio 편집실이 제목·소개·해시태그·첫 댓글 생성, openclaw는 규격 검증·발행 |
| 첫 사용에 채널 연결을 강제함 | R89 | 갈래만 고르고 생성. 연결은 발행 시점으로 이동 |
| 학습 후보의 출현 임계가 비었음 | 사업계획 v1.2 §3.4 | 같은 선택 3회 또는 비교 가능 성과 5건. 근거 부족 표시와 승낙 후 적용 |

### 18.2 보정한 수용 기준

예전 PRD의 원문 바이트 동일 판정은 정규화된 봉투와 역할이 다르다. 이 판은 다음 세 증거로 보존성을 판정한다.

1. 접수 원문의 암호화 전 해시와 저장 직전 해시가 같다.
2. 정규화 단계·판·입력 지문으로 엔진 입력을 재현한다.
3. 봉투의 원문 조각은 저장 원문의 범위 참조와 해시로 검증한다.

### 18.3 남은 매핑 갭

사용자 흐름 32단계의 엔드포인트, 화면 구성요소, 저장 대상, 시험 빈칸은 0개다. 정보 구조·비용·권리·배포에 관한 새 선택이 등장하면 build를 자동으로 진행하지 않고 최상단 §0으로 회수한다.
...
그래도 사업계획 v1.2와 회장 확정 요구가 독립 판매 가능성을 불변조건으로 만들었으므로 설계에서 제거할 수 없다.
...
세 번째 가정은 작업 공간 표현 규칙과 사실·권리를 구조화해도 사용자에게 과한 입력 부담을 주지 않는다는 것이다.

수정은 첫 사용에는 갈래와 최소 U3만 받고, 편집실에서 필요할 때 사실·표현·금지·권리 필드를 점진적으로 여는 것이다. 자유 서술은 보조 입력으로 보존한다.
...
수정: 이 구조는 작업 공간 오염과 삭제 실패를 막는 최소 일관성 경계다. 대신 별도 사건 저장소나 복잡한 전역 버스는 요구하지 않고 PostgreSQL 안에서 시작한다.

### 20.3 까다로운 고객 관점 공격

공격: `내가 목소리를 적었는데 스킬이 왜 다른 도입부를 넣었는지 모르겠다.`

수정: 결과에 필드별 사용자 값, 스킬 제안, 최종 승자, 이유를 보여 주게 했다.

공격: `작업 공간 A에서 쓴 금지 표현이 작업 공간 B에 섞이면 끝이다.`

수정: API, DB, 봉투, 결과 계보, 캐시 키의 다섯 격리선과 별도 작업 공간 격리 시험을 필수로 했다.
...
| Studio 자체 회원과 층 | §4, §6 | sessions, profile, workspaces, workspaces | members, layer_items | STANDALONE-01 |
| 정본과 투영본 | §6, §8 | sync events, comparisons | sync_mappings, layer_revisions | SYNC-01부터 08 |
| 작업 공간 개체와 세 범위 | §6 | workspaces, workspaces | workspaces, workspaces | WORKSPACE-01부터 08 |
| X4 스킬 층 | §7 | layer items, skills | layer_items, skills, skill_versions | SKILL-01부터 10 |
| 작업 공간 표현 규칙 | §7 | workspace voice | voice_profiles | VOICE-01부터 08 |
...
| 작업 공간 격리 | §12 | 전 API | 모든 범위 테이블 | WORKSPACE-ISO 전수 |
...
- `docs/사업계획-osmu-v1.0.md` 내부 판 v1.2, 특히 §3.4
...
WEAKEST_LINE: 감지 창·보유 기간·호환 제거 시점은 실측 전이므로 숫자를 설계 계약으로 고정하지 않았다.
