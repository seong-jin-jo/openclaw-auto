...
function resolveConfig(api: OpenClawPluginApi): { apiBaseUrl: string; adminToken: string } {
  ...
  const adminToken =
    ...
    process.env.SAMPLE_ADMIN_TOKEN ||
    "";
  if (!apiBaseUrl) {
    throw new Error(
      "Sample API base URL not configured. Set SAMPLE_API_BASE_URL env var or configure in plugin settings.",
    );
  }
  if (!adminToken) {
    throw new Error(
      "Sample admin token not configured. Set SAMPLE_ADMIN_TOKEN env var or configure in plugin settings.",
    );
  }
...
export function createSampleBlogTool(api: OpenClawPluginApi) {
  return {
    ...
    label: "Sample Blog Publish",
    description:
      "Publish a column article to Sample site (example.com) via admin API. The article is published immediately as APPROVED. Provide title with SEO keyword, HTML content (1500-3000 chars), and tags (10-15 for SEO).",
    ...
    async execute(_toolCallId: string, rawParams: Record<string, unknown>) {
      ...
      if (!resp.ok) {
        ...
        throw new Error(`Sample API failed (${resp.status}): ${err}`);
      }
