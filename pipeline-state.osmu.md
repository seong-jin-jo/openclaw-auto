## 2026-08-30 01:40 진행상태 갱신 (Claude, osmu 라인)

stage: qa

★ 승낙 없는 삭제 경로 봉인 완료(ef84ccd2). 컨트롤러 직접 확인:
  활성 소스 DELETE 호출 0건(주석 2줄만), 운영 빌드 사본(openclaw/) 관련 언급 0건.
  ⇒ 운영은 이미 안전. 긴급 재배포 불필요. PR #36 배포 때 자연 최신화.
  회귀 테스트가 수정 전 소스에서 FAIL 하는 것까지 확인해 포착 능력 증명됨.
  크론 정의는 .gitignore 라 레포 밖. 현재 호출 여부는 레포에서 확인 불가로 기록.

★ 현재 전체 테스트에 studio-publish-ui 29건 실패. 발행실 조(ae325754)가 그 영역
  작업 중이라 회수 후 재확인할 것. 그 전에 "전체 통과" 주장 금지.

가동중 한 조: 발행실 대화창과 깊이(opus, ae325754).

게이트:
- 30건 대조표 재실행으로 판정 갱신하기 전까지 qa 통과 주장 금지.
- studio-publish-ui 29건이 0으로 돌아오기 전까지 배포 승인 주장 금지.
...
★ 격리 계약 테스트 실패를 컨트롤러가 직접 재현했다.
  tests/isolation/tenant-api-attack-script.contract.test.ts TENANT-READ-01
  "/api/performance/learned-rules 읽기 경로가 실앱 공격 목록에서 빠졌습니다."
  원인 커밋 822fa94a(성과실 조). 수리 조 가동중(a7117e87).
  ★"1,474 전부 통과"는 그 커밋 이전 시점의 사실이었다. 이후 깨졌다. 정정한다.

생성실·편집실 회수(f9feb84e). 9건 중 8건 구현.
자유 입력칸 0개 달성, 헤더 학습 정보 복귀, 편집실 규격 미리보기, 임시저장,
새로 시작/이어서 하기, 큐 문구 제거.

네 방 단추 수 실측: create 24 / edit 26 / publish 5 / performance 25.
발행실만 얇다. 발행실 조 발주(ae325754).

가동중 네 조: 발행실(opus), 안 터진 글 정리(sonnet), 격리 구멍(sonnet),
그리고 대조표 재실행은 조 회수 후.

게이트: 30건 대조표 재실행으로 판정 갱신하기 전까지 qa 통과 주장 금지.
격리 테스트가 초록으로 돌아오기 전까지 배포 승인 주장 금지.
...
회장 피드백 30건 전수 대조 결과: 해결 3 · 부분 6 · 미해결 12 · 설계만 9.
정본 docs/qa/회장-피드백-대조표-2026-08-29.md.

미해결 해소를 위해 두 조 가동:
- 생성실·편집실 (opus): 헤더 학습 정보 복원, 카드 문답(주관식 0), 만들 종류 중복 선택,
  편집실 실제 미리보기, 임시저장, 새로 시작/이어서 하기, 큐 문구.
- 성과실 (sonnet): 성과실 대화창과 L5 승낙 경로, 플랫폼 클릭 흐름, 캘린더 기본 보기,
  자동 좋아요·댓글 관리·안 터진 글 삭제 화면, 생성 큐 문구.

게이트: 두 조 회수 후 대조표 재실행으로 판정 갱신하기 전까지 qa 통과 주장 금지.

배포: PR #36 회장 머지 대기. 머지 후 순서는
docs/releases/2026-08-29-배포-교착-해소-순서.md (audit → expand-guard → Deploy → expand-member).

검증 상태: 전체 테스트 199파일 1,474 통과 실패 0(컨트롤러 직접 실행).
무료 몫 검증 13/13 통과(컨트롤러 직접 실행). 운영 환경은 배포 전이라 전부 미검증.
