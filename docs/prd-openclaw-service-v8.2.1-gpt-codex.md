...
created_at: 2026-08-15 19:49 KST
model: gpt-codex/gpt-5.6
agent: prd-architect
skills: 없음
evidence: 제품구조 결정서 §9.6 회장 확정 D1~D5, PRD v8.1.1 승인 통과본, 레드팀 GO-with-changes, 두 서비스 경계 wiki, 구현현황·pipeline 승인 핀, planning·doc-review·benchmarks·artifact-stamp 품질헌법, EU·YouTube·TikTok·Meta·Paddle·Lemon Squeezy·국내 다국어 경쟁 서비스 실검색
deliberation: v8.1.1의 18개 본장과 FR28·AC28을 모두 보존했다. 고객면 한국어와 출력 다국어를 분리하고, 취향은 언어별로 격리하며, 후보는 저해상도 3개 뒤 선택된 1개만 고해상도로 승격했다. 글로벌 self-serve와 studio 소비자 단독 판매 등 레드팀이 죽인 10개 범위는 비범위로 잠갔다. Merchant of Record는 기술설계 재검토 항목이라 공급자를 임의 확정하지 않았다.
-->

# openclaw-service PRD v8.2.1
...
- [3.9 시장·언어·국가 준비 계약](#39-시장언어국가-준비-계약)
- [3.10 레드팀 비범위 잠금](#310-레드팀-비범위-잠금)
- [4. 용어 정의](#4-용어-정의)
- [5. 핵심 페르소나](#5-핵심-페르소나)
...
- [12.4 v8.2.1 지정 외부 조사와 제품 반영](#124-v821-지정-외부-조사와-제품-반영)
- [13. 운영 부하](#13-운영-부하)
- [14. 리스크·레드팀·셀프심문](#14-리스크레드팀셀프심문)
- [15. 성공지표·kill-criteria](#15-성공지표kill-criteria)
- [16. 확정 반영과 회장 결정 필요](#16-확정-반영과-회장-결정-필요)
- [17. 기획 7원칙·게이트 판정](#17-기획-7원칙게이트-판정)
- [18. 개정 이력](#18-개정-이력)

## 0. TL;DR

openclaw-service의 첫 임무는 외부 고객 수를 늘리는 것이 아니다. openclaw-service의 기존 계정·예약·발행·성과 경로를 닫고 headless studio-service API를 연결해, 내부 첫 사용자 해줘단타와 제로원 인사이트, 그리고 openclaw-service 자신의 마케팅 시리즈를 제작·발행하는 것이다. 그렇게 공개된 콘텐츠와 발행 결과가 제품의 첫 증거물이 된다. 외부 유료 수요 kill-criteria는 삭제하지 않고 이 자체 사용 proof 뒤에 실행한다.

첫 외부 고객은 자기가 만든 제품을 알리고 싶은 바이브 코더·예비 창업자와, 조회수 수익·쿠팡 파트너스 수익을 만들려는 부업 크리에이터다. 두 집단은 계속 발행해야 하고 성과가 돈과 직결되며, 좋은 예시를 보면 고를 수 있지만 업계 벤치마크·콘텐츠 형식·제작 방식·프롬프트를 백지에서 정의하지 못한다. 제품은 업계·목적을 받은 뒤 잘 되는 채널과 영상을 모아 보여주고, 저해상도 후보 3개에 예상 비용 범위·소요시간·근거를 붙여 선택하게 한다. 선택된 1개만 고해상도로 완성하며, 미선택 후보의 고해상도 승격은 추가 과금이다.

고객이 보는 가입·결제·지원·랜딩·문서는 한국어 하나다. 출력 콘텐츠는 영어·일본어·태국어 등을 요청할 수 있는 다국어이며, 출력 언어는 취향이 아니라 요청 제약이다. 취향 프로파일은 언어별로 분리하고, 제품이 품질을 보증하는 언어는 한국어와 영어뿐이다. 그 밖의 언어는 생성 가능하되 원어민 검수 미제공을 실행 전과 결과에 표시한다.

미디어·창작 경계는 절대적이다. openclaw-service는 미디어 바이트를 읽거나 쓰거나 변형하지 않고, 브랜드 톤을 읽는 캡션·제목·해시태그·첫 댓글도 만들지 않는다. 크롭, 자르기, 재인코딩, 자막 합성, 재렌더, 롱폼 분할과 모든 창작 텍스트는 studio-service가 수행한다. openclaw-service는 studio가 만든 완성 파일·창작 텍스트의 불변 참조와 예약시각·계정·승인·발행 proof만 소유해 정확히 한 번 발행한다.

## 0.5 1차 목표와 출시 순서

> **1차 목표: openclaw-service를 완성하고 studio-service API를 구현해 해줘단타·제로원 인사이트·openclaw-service 자체 마케팅의 OSMU 콘텐츠를 제작·발행한다. 우리가 첫 사용자이며, 공개된 콘텐츠와 발행 증거가 제품의 첫 proof다.**
...
- 해줘단타·제로원 인사이트뿐 아니라 openclaw-service 자신의 마케팅도 같은 계보로 돌린다.
- 고객면은 한국어 하나로 운영하고 출력 언어만 요청별 다국어로 제공한다.
- 저해상도 후보 3개 중 하나를 고른 뒤 선택본만 고해상도로 완성한다.
...
| 3 | 내부 OSMU proof | 해줘단타3편·제로원 인사이트3편을 각각 2개 이상 승인 채널에 발행하고 openclaw-service 자체 마케팅3편도 동일 계보로 발행 | 공개 permalink 또는 provider external ID, source label, 비용·시간 예측 대 실제값, internal-test tenant 표식 | 9편의 제작→발행 proof 전량 |
...
외부 유료 수요 검증은 폐기하지 않는다. 자체 사용 3개 트랙의 공개 proof를 만든 뒤 바이브 코더·예비창업자와 부업 크리에이터에게 유료 concierge를 제안한다. 내부 사용은 작동 증거이고 외부 결제는 수요 증거이므로 같은 지표로 합치지 않는다.
...
- openclaw-service 자신의 마케팅을 세 번째 내부 테스트 tenant로 운영하되 외부 고객 집계와 분리한다.
- 한국어 고객면 하나를 유지하면서 요청 단위로 다국어 콘텐츠를 만들고 언어별 취향을 섞지 않는다.
...
- 다국가 동시 self-serve 결제, 24시간 다국어 지원, 모든 페르소나를 동시에 겨냥한 GTM.

## 2. 현재 구현 상태

위키, release·handoff 기록, `pipeline-state.osmu.md` 승인 핀, `docs/구현현황.md`를 1차 진실원으로 확인했다. 위키가 구현 여부를 기록하지 않은 AI disclosure와 global 준비 필드만 좁게 코드 검색했다. 코드 전체 검색은 하지 않았다.

| 영역 | 현재 상태 | 1차 정본·근거 | v8.2.1 처리 |
|---|---|---|---|
...
| openclaw-service 자체 마케팅 proof | **미구현** | 제품구조 결정서 §9.6 D3, `wiki/marketing/gtm-plan.md` | 세 번째 내부 테스트 tenant로 제작·발행·성과 계보를 축적 |
| 고객면 한국어·출력 다국어 분리 | **미구현** | 제품구조 결정서 §9.6 D1, `wiki/architecture/two-service-boundary.md` | 고객 UI·지원은 한국어, 출력 언어는 요청 제약으로 신설 |
| 언어별 취향 프로파일 | **미구현** | 제품구조 결정서 §9.6 D1 | 언어 간 선택·성과 신호 혼합0으로 신설 |
| locale·currency·timezone·tax region·policy version | **미구현 또는 현황 미기록** | 제품구조 결정서 §9.6 D4 | 1단계 데이터 계약에 포함. 필드 타입·테이블은 eng-design에서 합의 |
| 저해상도 후보3→고해상도 선택본1 | **미구현·해상도 실측 전** | 제품구조 결정서 §9.6 D5 | 수치 확정 없이 상태·과금·선택 계약만 신설 |
| AI 생성 표시·정책 버전 추적 | **부분구현** | 위키 미기록으로 `dashboard/src`만 좁게 검색. TikTok `is_aigc` 입력·UI는 존재하고 YouTube·Instagram 공통 policy version 계보는 확인되지 않음. 레드팀 LEGAL-03, 2026 공식 정책 조사 | 기존 TikTok 입력을 보존하고 플랫폼별 disclosure 상태와 적용 policy version을 publication proof까지 확장 |
...
> ⛔ 회수 필요: 위키가 TikTok의 기존 `is_aigc` 입력과 AI 생성 표시 구현 현황을 기록하지 않았다. PRD 승인과 함께 `wiki/reference/channel-status.md` 또는 해당 기능 정본에 기존 구현·미지원 플랫폼·policy version 부재를 기록해야 한다.
...
- openclaw-service 자신의 마케팅 OSMU proof. 내부 테스트 tenant로 표시해 외부 고객 성과와 분리한다.
- 고객면 한국어, 출력 다국어, 언어별 취향 프로파일, 한국어·영어 품질 보증 경계.
- 저해상도 후보 3개, 선택된 고해상도 완성본 1개, 미선택 후보 고해상도 추가 과금 경로.
- locale·currency·timezone·tax region·policy version의 1단계 제품 계약.
- AI 생성·변형 콘텐츠의 플랫폼 disclosure 상태와 적용 정책 version을 publication proof에 연결.
...
- studio-service 소비자 단독 판매와 자체 소비자 화면.
- 글로벌 동시 self-serve 결제, 국가 무제한 판매, 24시간 또는 다국어 지원 SLA.
- 모든 페르소나를 한 랜딩·한 메시지로 동시에 마케팅하는 것.
- 후보 3개를 모두 고해상도 완성본으로 기본 생성하는 것.
- 풀 영상을 기본 제작방식으로 지정하는 것.
- 자율 발행을 기본값으로 켜는 것. 자율 발행 옵션 자체는 기존 회장 확정을 보존하되 기본값은 승인 후 발행이다.
- 채널 무제한 지원을 첫 release 약속으로 판매하는 것.
- 외부 표본과 출처가 없는 성과 점수·바이럴 점수·수익 예측.
- 외부 롱폼 역분석·음악 제작·범용 텍스트 상품의 1단계 구현.
- 고급 BI·세그먼트·예측 운영자 대시보드. 잔액·실패·큐·토큰 만료·고객 막힘·환불 근거만 유지한다.
...
| 캡션·제목·해시태그·첫 댓글 | studio | openclaw가 실어 보낸 채널 규격 제약 안에서 생성하고 결과 참조를 반환 |
| 예약시각·계정·승인 | openclaw | 창작 내용을 바꾸지 않고 publication intent에 연결 |
...
`채널 적응`이라는 용어는 v8.1.1에서 폐기한다. 창작 텍스트는 studio가 생성하고, openclaw가 직접 조정하는 발행 메타데이터는 예약시각·계정·승인·disclosure 선택뿐이다. 미디어 hash와 studio가 반환한 창작 텍스트 version은 openclaw 흐름 전후에 동일해야 한다.
...
|---|---|---|---|---|
| 생성 | 주제, 선택, 언어별 취향 프로파일, 자료 version, 출력 언어 | 저해상도 후보3을 만들고 선택된1만 고해상도로 승격 | 후보별 선택·견적·추가 승격 비용·상태·완성 파일 참조 표시 | 필수 |
...
우리 산출물 편집은 작은 수정마다 소재를 다시 생성해 원가가 커지는 것을 막는 1단계 필수 기능이다. 수정 지시는 취향의 약한 신호로 기록하되, 후보3 중 선택과 같은 강한 신호로 취급하지 않는다. 외부 롱폼 쪼개기·자동 구간 탐색은 OpusClip·Vizard와 성숙 시장에서 정면 경쟁하므로 2단계로 미룬다. 단, 그 기능이 생길 때도 모든 미디어 바이트 처리는 studio가 소유한다.

#### 3.8.1 후보 해상도와 과금 상태

후보 3개는 선택을 위한 저해상도 또는 짧은 프리뷰다. 해상도·길이·codec 숫자는 기술설계 전 실물 비교가 없으므로 이 PRD에서 확정하지 않는다. 제품 계약은 다음 상태 전이와 비용 고지다.

| 단계 | 사용자에게 보이는 것 | 생성 범위 | 과금 계약 | 실패·경계 |
|---|---|---|---|---|
| 후보 준비 | 같은 brief의 저해상도 후보 3개, 후보별 근거·예상 완성 비용·예상 시간 | preview 3개 | 후보 원가만 견적에 포함 | 3개 미만이면 부족 사유와 재시도 비용을 표시하고 몰래 복제하지 않음 |
| 선택 | 정확히 1개 선택 또는 새 후보 요청 | 고해상도 생성0 | 선택 전 완성본 차감0 | 판단 불가 프리뷰면 유료 승격을 강제하지 않음 |
| 선택본 승격 | 선택된 1개의 고해상도 완성본 | high-resolution 1개 | 승인된 완성본 원가 차감 | quality gate 반려는 무과금, 원장에 반려 사유 저장 |
| 미선택본 추가 승격 | 남은 후보 중 고객이 명시 선택한 것 | 요청 수만큼 high-resolution | 후보별 추가 비용을 사전 확인 후 차감 | 묵시적 자동 승격0, 묶음 숨은 차감0 |

저해상도 프리뷰가 스타일·구도·가독성을 판단할 수 있는지는 미검증이다. 기술설계 전에 동일 brief로 저해상도·고해상도 실물을 만들어 회장이 눈으로 판정한다. 판정 전에는 `720p`, `480p`, 초 수, 파일 크기 같은 수치를 PRD·가격표·마케팅에 넣지 않는다.

### 3.9 시장·언어·국가 준비 계약

#### 3.9.1 고객면과 출력면을 분리한다

| 층 | 1단계 계약 | 품질·운영 경계 |
|---|---|---|
| 고객면 | 가입·결제·지원·랜딩·도움말·운영 공지는 한국어 하나 | 다국어 UI·24시간 지원·국가별 지원 SLA 없음 |
| 출력 콘텐츠 | 요청마다 한국어·영어·일본어·태국어 등 지원 가능한 언어를 선택 | 지원 목록은 studio capability로 표시하고 미지원 언어를 성공으로 위장하지 않음 |
| 품질 보증 | 한국어·영어는 제품 품질 게이트와 사람이 읽을 수 있는 검수 기준 제공 | 그 외 언어는 생성 가능하되 원어민 검수 미제공을 실행 전과 결과에 표시 |
| 마케팅 | 국내의 바이브 코더·예비창업자와 부업 크리에이터를 첫 얼굴로 사용 | 해외 동시 GTM, 전 페르소나 랜딩, 모든 언어 성과 약속 금지 |

출력 언어는 취향이 아니라 요청 제약이다. 같은 고객이 한국어·일본어 콘텐츠를 만들 수 있지만, 한국어에서 선택한 훅·말투·성과를 일본어 취향으로 자동 이식하지 않는다. 언어별 취향은 해당 언어의 선택·수정·성과만 근거로 version을 올린다. 언어가 다른데 근거가 없으면 공통 브랜드 사실과 해당 언어 기본 프로파일만 사용하고 `근거 부족`을 표시한다.

#### 3.9.2 글로벌 준비 필드

1단계부터 locale, currency, timezone, tax region, policy version을 제품 데이터 계약에 포함한다. 이는 글로벌 판매 확정이 아니라 나중의 파괴적 마이그레이션을 피하기 위한 준비다.

| 필드 개념 | 사용하는 제품 결정 | 금지되는 추론 |
|---|---|---|
| locale | 고객면 표시·날짜·숫자 포맷 | locale만으로 출력 언어를 자동 결정하지 않음 |
| currency | 견적·차감·환불 표시 통화 | 표시 통화만 보고 세금 관할을 결정하지 않음 |
| timezone | 예약·지원시간·정책 적용 시각 | 브라우저 추정값을 확인 없이 영구 저장하지 않음 |
| tax region | 판매 허용·세금 처리 경로 선택 | 법률 검토 없이 국가별 세율을 자체 확정하지 않음 |
| policy version | 생성·disclosure·권리·환불·발행 당시 적용 정책의 계보 | 현재 정책으로 과거 publication을 덮어쓰지 않음 |

필드의 table, type, nullable, enum, migration, source of truth는 eng-design 대화 게이트에서 정한다. 다국가 self-serve 결제와 24시간 지원은 비범위다. Merchant of Record는 Paddle·Lemon Squeezy 등의 실제 수수료·한국 사업자 onboarding·충전식 크레딧 지원·환불·webhook·세금 책임을 비교해 eng-design에서 재검토하며, 본 PRD는 공급자를 선택하지 않는다.

### 3.10 레드팀 비범위 잠금

레드팀 GO-with-changes가 죽이라고 한 10개 범위를 구현 백로그로 되살리지 않는다. 각 항목은 해금 증거가 생길 때 새 plan 재개정과 승인을 받아야 한다.

| 죽인 범위 | 1단계에서 금지되는 형태 | 해금에 필요한 최소 증거 |
|---|---|---|
| studio-service 소비자 단독 판매 | 별도 가입·결제·랜딩·소비자 화면 | openclaw 외부 cohort와 선택 학습 kill-criteria 통과 뒤 별도 PRD |
| 글로벌 동시 self-serve | 전 세계 공개 결제·다국어 SLA·국가 무제한 판매 | 국가별 세금·지원 runbook과 유료 cohort 증거 |
| 전 페르소나 마케팅 | 대학생·자영업자·아트·가상 인플루언서를 한 랜딩에서 동시 공략 | 첫 wedge 반복구매 뒤 세그먼트별 별도 GTM 실험 |
| 완성본 A/B/C | 후보 3개 모두 고해상도 기본 생성 | 저해상도 선택 실험이 실패하고 새 원가·가격 승인 |
| 풀 영상 기본 | 모든 컷 고비용 영상 생성 | 고객 명시 선택·선결제·원가 상한 승인 |
| 자율 발행 기본 | 신규 tenant·채널·유형이 처음부터 자동 발행 | 승인 발행 실측과 별도 승격 정책. 옵션은 존재하되 기본 OFF |
| 무제한 채널 | 채널 수를 핵심 약속으로 판매 | 같은 신규 채널에 유료 수요 3곳과 실제 발행 proof |
| 근거 없는 성과 점수 | 표본·기간·출처 없는 바이럴·수익 점수 | 승인된 최소 표본, 근거 ID, 한계, blind validation |
| 외부 롱폼 역분석·음악·범용 텍스트 | 1단계 studio 공개 기능 | One Thing 직접 기여와 별도 시장·원가 검증 |
| 고급 BI 대시보드 | 예측·세그먼트·코호트 탐색 중심 운영 화면 | 필수 운영 카드로 해결되지 않는 반복 의사결정 증거 |

이 잠금은 제품 기능을 영구 금지하는 법적 선언이 아니다. 1인이 내부 proof와 첫 유료 cohort를 운영할 수 있게 지금의 제품을 하나로 유지하는 plan gate다.
...
| 창작 텍스트 | 캡션·제목·해시태그·첫 댓글. 브랜드 톤을 사용하므로 studio가 생성·versioning함 |
| 발행 메타데이터 | 예약시각, 대상 계정, 승인, AI disclosure, 적용 policy version. openclaw가 소유하는 발행 계약 |
| 출력 언어 | 요청마다 지정하는 콘텐츠 언어 제약. 취향 값이 아니며 고객면 locale과도 다름 |
| 언어별 취향 프로파일 | 같은 tenant 안에서도 출력 언어마다 분리되는 선택·수정·성과 근거의 versioned 집계 |
| 저해상도 후보 | 고해상도 원가를 쓰기 전에 선택을 가능하게 하는 후보 3개. 실제 해상도 수치는 실물 실험 뒤 기술설계에서 확정 |
...
| first proof | 해줘단타·제로원 인사이트·openclaw-service 자체 마케팅의 실제 제작·발행 결과와 그 계보. 외부 수요 증거와 분리 |
...
도윤은 제품 매출이 안정되기 전까지 쿠팡 파트너스와 조회수 수익형 채널도 시험한다. 여기서도 문제는 같다. 어떤 주제를 영어·일본어로 확장할지, 같은 한국어 훅을 번역하면 되는지, 세 후보 중 어느 구도가 클릭을 부르는지 판단할 근거가 없다. 한국어 운영 화면에서 영어 결과를 만들 수 있으면 좋지만, 제품이 검수하지 못한 태국어를 전문가 수준이라고 약속하는 것은 원하지 않는다. 수익이 걸린 콘텐츠가 플랫폼의 AI 표시를 빠뜨리거나 반복형 저품질로 분류되는 것도 두렵다. 그래서 결과물마다 출력 언어, 적용 취향 version, AI disclosure, 정책 version, 실제 발행 proof가 함께 남기를 원한다.
...
| 구분 | 내용 |
|---|---|
| 긴급 pain | 제품 또는 부업 채널은 만들었지만 업계 벤치마크·콘텐츠 구성·출력 언어·제작비·발행 운영을 백지에서 정하지 못함 |
...
### 5.1 첫 외부 고객 cohort와 단일 페르소나의 관계

본 PRD의 상세 페르소나는 박도윤 1명으로 유지한다. 다만 첫 외부 모집은 두 cohort로 나누어 같은 One Thing이 성립하는지 비교한다.

| cohort | 돈과 연결되는 성과 | 공통 pain | 별도 관찰 지표 |
|---|---|---|---|
| 바이브 코더·예비창업자 | 제품 인지도·waitlist·문의 | 제품은 있으나 런칭 콘텐츠를 정의하지 못함 | 3편 proof 완료, 자료 근거 정확성, paid repeat |
| 부업 크리에이터 | 조회수 수익·쿠팡 파트너스 클릭·수익 | 반복 소재는 필요하나 주제·스타일·다국어 확장을 정의하지 못함 | 발행 지속률, 정책 위반0, 실제 수익 데이터의 수집 가능 여부 |

두 cohort를 한 성과 평균으로 섞지 않는다. 부업 크리에이터의 수익 지표가 플랫폼 API에서 직접 수집되지 않으면 `미수집`으로 두며, 근거 없는 수익 점수를 만들지 않는다.
...
| “전 세계 누구나 모든 언어 콘텐츠를 자동 발행한다” | 탈락 | 고객면 언어·세금·지원·플랫폼 정책을 동시에 열어 1인 운영 한계를 깨뜨림 |
| “고해상도 후보 3개를 만들어 고르게 한다” | 탈락 | 선택 전에 고비용 원가를 3배 쓰고 미선택 산출물 비용을 고객에게 숨기게 됨 |

### 6.2 One Thing

> **제품이나 부업 채널을 알리려는 한국인 사용자가 근거·비용·시간이 붙은 저해상도 후보만 고르면, 선택한 언어의 시리즈가 완성되어 정확한 계정에 발행되고 검증된 결과가 다음 선택을 바꾸는 제품.**

### 6.3 MVP 기능 5

| MVP | One Thing 연결 | 고객 출력 | 합격 기준 |
|---|---|---|---|
| M1 근거 있는 시작 | 업계·내부 자료·출력 언어 | 업계·목적·출력 언어 선택, 자료 근거함, active knowledge | raw prompt 필수0, source/version coverage100%, conflict hold, 고객면 한국어100% |
| M2 벤치마크·저비용 선택 견적 | 사례와 비용·시간이 붙은 선택 | 채널·영상 사례, 저해상도 후보3, 추천1, 후보·승격 비용·시간 범위 | 사례 provenance100%, 고해상도 미선택 후보0, hidden cost0, blank first screen0 |
| M3 studio 생성·편집 완성본 | 선택한 언어의 시리즈가 제작되고 작은 수정이 재생성 없이 반영됨 | 선택본 고해상도1, provider-ready 이미지·영상·창작 텍스트, 편집 지시, rights·quality·revision | openclaw creative/media mutation0, studio completion reference1, 우리 산출물 수정 시 소재 재생성0, 언어별 profile mix0 |
| M4 정확하고 정책에 맞는 발행 | 정확한 계정에 발행 | studio 창작 텍스트·예약·계정·AI disclosure·policy version 검수, proof·recovery | wrong-account0, duplicate0, false-published0, required disclosure omission0 |
| M5 근거가 바뀌는 반복 | 결과가 다음 선택을 바꿈 | 성과·트렌드·선택 근거와 언어별 다음 추천 diff | private/cross-tenant0, language-profile cross-write0, false-zero0, evidence ID+changed recommendation1 |
...
```mermaid
flowchart TD
  A[업계·목적·출력 언어 선택] --> B[내부 자료·소재 반입]
  B --> C[공개 채널·영상 벤치마크]
  C --> D[저해상도 후보3·비용·시간 비교]
  D --> E{직접 선택 또는 추천}
  E --> F[선택본1만 고해상도 제작·모든 창작 처리]
  F --> M{수정 필요}
...
모든 화면은 happy, empty, loading, source-conflict, estimate-changed, preview-insufficient, unsupported-output-language, non-guaranteed-language, studio-timeout, rights-unknown, disclosure-required, policy-version-stale, wrong-account, approval-missing, uncertain-publish, unsupported-metric 상태를 가진다. `studio 재요청`은 openclaw 자체 변환이 아니라 새 완성본을 요청하는 경로다.
...
고객면 문구는 전 과정 한국어다. 출력 언어가 한국어·영어가 아니면 후보 생성 전에 `생성은 지원하지만 원어민 품질 검수는 제공하지 않음`을 보여주고 사용자가 확인한다. 선택과 수정은 해당 출력 언어의 취향 프로파일에만 기록한다. 발행 직전에는 플랫폼·정책 version·콘텐츠 특성에 따라 AI disclosure가 필요한지 확인하고, 필요한 값을 publication proof와 함께 보존한다.
...
| FR-OS82-029 | 가입·결제·지원·랜딩·도움말의 고객면 언어는 한국어 하나로 제공한다 | openclaw | 미구현·정책 신설 | 제품구조 결정서 §9.6 D1, two-service-boundary wiki | customer-facing non-Korean default0, 한국어 누락 critical screen0 |
| FR-OS82-030 | 출력 콘텐츠 언어는 제작 요청마다 명시하는 제약이며 고객면 locale·취향 값과 분리한다 | shared | 미구현 | 제품구조 결정서 §9.6 D1 | output language missing submit0, locale implicit override0 |
| FR-OS82-031 | 취향 프로파일과 선택·수정·성과 신호는 출력 언어별로 분리하고 교차 언어 자동 이식을 금지한다 | studio | 미구현 | 제품구조 결정서 §9.6 D1 | language-profile cross-write0, evidence-language coverage100% |
| FR-OS82-032 | 한국어·영어만 품질 보증 대상으로 두고 다른 언어는 원어민 검수 미제공을 생성 전·결과에 표시한다 | shared | 미구현 | 제품구조 결정서 §9.6 D1 | non-guaranteed disclosure coverage100%, false quality guarantee0 |
| FR-OS82-033 | locale·currency·timezone·tax region·policy version을 1단계 제품 데이터 계약에 포함한다 | shared | 미구현 또는 wiki 미기록 | 제품구조 결정서 §9.6 D4 | required global-prep value coverage100%, past-policy overwrite0 |
| FR-OS82-034 | 해줘단타·제로원과 함께 openclaw-service 자신의 마케팅3편을 내부 테스트 tenant로 dogfood하고 외부 고객 지표와 분리한다 | shared | 미구현 | 제품구조 결정서 §9.6 D3, gtm-plan wiki | service-marketing proof3/3, internal-test attribution100%, external aggregate contamination0 |
| FR-OS82-035 | 동일 brief에서 저해상도 후보를 정확히 3개 제시하고 선택 전 고해상도 완성본을 생성하지 않는다 | studio | 미구현·실물 실험 전 | 제품구조 결정서 §9.6 D5 | preview3, preselection high-resolution0 |
| FR-OS82-036 | 선택된 후보 1개만 고해상도로 승격하고 미선택 후보의 고해상도 승격은 개별 사전확인·추가 과금한다 | shared billing | 미구현 | 제품구조 결정서 §9.6 D5 | default high-resolution1, implicit extra charge0, upgrade consent100% |
| FR-OS82-037 | 품질 게이트 반려 후보·완성본은 고객에게 과금하지 않고 반려 원가·사유를 운영 루프에 기록한다 | shared billing | 미구현 | 제품구조 결정서 §9.5, 레드팀 반드시 살릴 것 | rejected customer charge0, rejection reason coverage100% |
| FR-OS82-038 | 자율 발행과 승인 후 발행 옵션은 tenant·채널·콘텐츠 유형별로 보존하되 신규 범위의 기본값은 승인 후 발행이다 | openclaw | 부분구현·정책 확장 | 제품구조 결정서 §9.5, 레드팀 죽일 범위 | new-scope autonomous default0, policy visibility100% |
| FR-OS82-039 | 발행 전 AI 생성·변형 disclosure 필요 여부와 적용 platform policy version을 확인하고 publication proof에 보존한다 | openclaw | 미구현 또는 wiki 미기록 | EU Article50·YouTube·TikTok·Meta 공식 정책, 레드팀 LEGAL-03 | required disclosure omission0, policy-version coverage100% |
| FR-OS82-040 | 첫 외부 모집은 바이브 코더·예비창업자와 조회수·쿠팡 파트너스 부업 크리에이터로 제한하고 cohort 성과를 분리한다 | product·ops | 미구현 | 제품구조 결정서 §9.6 D2 | first-pilot other persona0, cohort-mixed KPI0 |
| FR-OS82-041 | studio 소비자 단독 판매·글로벌 동시 self-serve·전 페르소나 마케팅·완성본 A/B/C·풀영상 기본·자율 발행 기본·무제한 채널·근거 없는 성과점수·외부 롱폼 역분석·음악·범용 텍스트·고급 BI를 1단계 비범위로 잠근다 | product gate | 미구현 정책 | 레드팀 GO-with-changes, §3.10 | forbidden-scope release item0 |
| FR-OS82-042 | 다국가 self-serve 결제와 24시간 지원을 열지 않으며 Merchant of Record 선택은 eng-design에서 Paddle·Lemon Squeezy 등 공식 계약을 재검토한 뒤 결정한다 | product gate | 미결정·기술설계 회수 | 제품구조 결정서 §9.6 D4, Paddle·Lemon Squeezy 공식 문서 | predecision provider lock-in0, unsupported-country self-serve0, 24h SLA promise0 |
| FR-OS82-043 | 저해상도 프리뷰의 해상도·길이·파일크기는 실물 비교 전 확정하지 않고, 기술설계 전에 회장 눈판정 실험을 통과한다 | shared gate | 미검증 | 제품구조 결정서 §9.6 D5 | pre-experiment numeric promise0, paired preview evidence1 |
| FR-OS82-044 | FR-OS81-028의 검수 흐름을 보존하되 공통·채널별 캡션·제목·해시태그·첫 댓글은 studio가 만들고 openclaw는 창작 텍스트를 직접 생성·절단·변형하지 않는다 | shared boundary | 부분구현·경계 이관 | 제품구조 결정서 §3.6·§3.9, two-service-boundary wiki | openclaw creative write0, studio creative version coverage100% |
...
| NFR-OS82-14 | 언어 격리 | output language·profile language·evidence language를 전 계보에서 구분하고 언어 누락 시 fail-closed | language ambiguity0, cross-language profile mutation0 |
| NFR-OS82-15 | 정책 추적성 | 생성·권리·AI disclosure·환불·발행 당시 적용 policy version을 불변 계보로 보존 | publication policy trace100%, retroactive overwrite0 |
| NFR-OS82-16 | 국제화 준비 | locale·currency·timezone·tax region을 표시·예약·결제·지원 판단에 쓰되 서로 대리값으로 추론하지 않음 | implicit jurisdiction inference0, unconfirmed timezone scheduling0 |
| NFR-OS82-17 | 원가 통제 | 후보3·선택본1·추가 승격·품질반려를 별도 원장 사건으로 대사 | hidden candidate cost0, unconsented upgrade0, rejected charge0 |
| NFR-OS82-18 | 운영 범위 | 한국어 고객면·정해진 지원시간·필수 운영 카드만 제공하고 24시간·고급 BI를 약속하지 않음 | after-hours SLA promise0, advanced BI route0 |
...
| 029 | AC-029 한국어 고객면 | Given 신규·기존 고객, When 가입→결제→도움말→지원 흐름을 돈다, Then 모든 critical 고객 문구는 한국어이고 언어 선택을 요구하지 않는다. |
| 030 | AC-030 출력 언어 요청 제약 | Given 고객 locale=ko-KR, When 출력 언어=영어를 선택해 brief를 제출, Then 영어가 명시된 요청 1건이 생성되고 locale이나 기존 취향이 이를 한국어로 덮지 않는다. |
| 031 | AC-031 언어별 취향 격리 | Given 같은 tenant의 한국어 선택·일본어 수정·영어 성과, When profile을 갱신, Then 각 신호는 같은 언어 profile에만 반영되고 cross-write0이다. |
| 032 | AC-032 품질 보증 경계 | Given 영어와 태국어 후보 생성, When 실행 전과 결과를 본다, Then 영어는 보증 기준이 표시되고 태국어에는 원어민 검수 미제공 안내가 2곳 모두 보이며 보증 문구0이다. |
| 033 | AC-033 글로벌 준비 필드 | Given workspace·brief·publication·charge fixture, When 생성·예약·발행·원장 조회, Then locale·currency·timezone·tax region·policy version의 필요한 값이 계보에 있고 대리추론0이다. |
| 034 | AC-034 자체 마케팅 dogfood | Given 해줘단타·제로원·openclaw 자체 tenant, When first release와 외부 KPI를 집계, Then openclaw 자체 마케팅3편의 source→choice→publication proof가 있고 세 tenant는 internal-test로 식별되며 외부 aggregate row에 포함되지 않는다. |
| 035 | AC-035 저해상도 후보3 | Given 완결 brief 1건, When 후보 준비가 끝나기 전·후 원장과 asset을 검사, Then preview3이고 high-resolution0이며 후보 identity는 서로 다르다. |
| 036 | AC-036 선택본1·추가 승격 | Given 후보3 중 2번 선택, When 기본 완성을 실행, Then high-resolution은 2번 1개뿐이다. 이후 1번 승격은 별도 견적·명시 확인 뒤 추가 차감1로 끝난다. |
| 037 | AC-037 품질반려 무과금 | Given 후보 또는 완성본이 quality gate에서 반려, When 원장을 조회, Then 고객 차감0, 운영 원가·반려 사유1, 재시도와 원요청의 계보1이다. |
| 038 | AC-038 승인 발행 기본 | Given 신규 tenant·채널·콘텐츠 유형, When 첫 발행 정책을 조회, Then 승인 후 발행이 기본이고 자율 발행은 명시 선택 전 외부 호출0이다. |
| 039 | AC-039 AI disclosure·정책 version | Given 현실적으로 보이는 AI 영상과 policy fixture, When YouTube·TikTok·Instagram 발행을 준비, Then required disclosure와 적용 policy version이 검수·요청·proof에 보존되고 누락 시 외부 호출0이다. |
| 040 | AC-040 첫 cohort 분리 | Given 바이브 코더·부업 크리에이터·기타 persona 지원자, When 첫 pilot 등록과 KPI 집계, Then 앞 두 cohort만 활성이고 기타는 waitlist이며 cohort 간 성과 평균 혼합0이다. |
| 041 | AC-041 비범위 잠금 | Given release manifest·route·feature flag, When 1단계 범위를 감사, Then §3.10의 금지 항목10은 고객 활성 route·기본값·판매 카피에서 0개다. |
| 042 | AC-042 글로벌 판매·MoR 회수 | Given plan 승인 전 상태, When 결제·지원 설정과 설계 문서를 감사, Then 다국가 self-serve0, 24시간 SLA0, 특정 MoR 확정0이며 eng-design decision record가 없으면 build 계약도 없다. |
| 043 | AC-043 프리뷰 실물 비교 gate | Given 같은 brief의 저해상도·고해상도 비교 산출물, When 회장이 둘을 눈으로 판정, Then 선택 가능 여부와 비용 차이 증거가 기록된다. 그 전 제품 문구의 해상도 숫자는 0개다. |
| 044 | AC-044 창작 텍스트 소유 경계 | Given destination2 이상과 studio creative text version, When 채널별 검수·발행을 완료, Then 공통값·채널별 파생·override 차이는 보존되고 openclaw creative generate·truncate·rewrite event0이며 발행 proof의 creative version은 studio 결과와 같다. |
...
| 고객면 한국어·출력 다국어·언어별 취향 | 029~032 | TC-029~032 | 회장 확정·위키 반영, 구현 미검증 |
| 글로벌 준비 필드·자체 마케팅 dogfood | 033~034 | TC-033~034 | 회장 확정, 현재 데이터 모델·집계 미검증 |
| 저해상도 후보3·선택본1·품질반려 무과금 | 035~037, 043 | TC-035~037, 043 | 회장 확정, 프리뷰 실물 비교 전 미검증 |
| 승인 발행 기본·AI disclosure·정책 추적 | 038~039 | TC-038~039 | 옵션 정책·공식 규정 근거 확인, 제품 경로 미검증 |
| 첫 외부 cohort·비범위 잠금·글로벌 판매 gate | 040~042 | TC-040~042 | 회장 확정·레드팀 근거 확인, 외부 cohort·eng-design 미실행 |
| 창작 텍스트 소유 경계 | 028, 044 | TC-028, 044 | 기존 검수 UX 보존, studio 소유 경계는 제품 결정 근거 확인·연결 미검증 |
...
- 후보 비용: 저해상도 후보3의 원가와 선택본1의 고해상도 원가를 분리한다. 미선택 후보 고해상도 승격은 추가 비용을 사전 확인한다.
...
### 11.4 Merchant of Record 재검토 경계

Paddle과 Lemon Squeezy는 모두 공식 문서에서 자신이 Merchant of Record라고 밝히며, 최종 구매자에게 법적 판매자가 되어 결제·판매세·VAT·GST·환불·chargeback 등 거래 책임의 상당 부분을 처리한다고 설명한다. 이것은 다국가 self-serve를 나중에 열 때 1인 운영의 세무 표면을 줄일 수 있다는 근거다. 다만 MoR을 쓰더라도 한국 사업자의 payout 소득세·회계·약관·충전식 크레딧 규율이 사라진다고 단정하지 않는다.

| 비교 질문 | Paddle 공식 확인 | Lemon Squeezy 공식 확인 | 이 PRD의 판정 |
|---|---|---|---|
| 판매자 지위 | 구매자에게 MoR로 판매하고 VAT·GST·sales tax를 처리 | MoR로 판매하고 sales tax·VAT를 처리 | 둘 다 후보. 특정 공급자 확정 금지 |
| 세금 처리 | 등록·계산·징수·신고·납부와 compliant invoice를 설명 | 세금 계산·징수·납부, 주문·invoice 반영을 설명 | 글로벌 판매 재검토 근거로만 사용 |
| 거래 책임 | 결제·구독·세금·compliance 범위를 제공 | 결제·환불·chargeback·PCI 책임을 설명 | 환불·크레딧 원장 계약과 실제 API 검증 필요 |
| 아직 모르는 것 | 한국 사업자 onboarding, 실제 수수료, credit pack·usage billing 적합성, payout 회계 | 같은 항목 | eng-design에서 공식 계약·sandbox·세무 자문으로 회수 |

기술설계의 비교 기준은 총수수료만이 아니다. ①한국 사업자 승인 가능 여부 ②충전팩·부분 환불·잔액 조정 ③idempotent webhook과 invoice ④tax region·currency·refund 원장 ⑤데이터 export·provider migration ⑥고객지원 책임 ⑦국내 PG와 병행 가능성이다. 이 판단이 끝나기 전 다국가 결제 route와 특정 provider schema를 만들지 않는다.
...
### 12.4 v8.2.1 지정 외부 조사와 제품 반영

조회일은 2026-08-15 KST다. 아래 네 축은 기억이나 2차 요약이 아니라 실제 WebSearch로 공식 원문을 조회했다. 법률 적용과 플랫폼 집행 결과는 공식 문서 확인과 별개이므로, 출시 전 전문 검토와 실제 계정 테스트를 남긴다.

| 지정 조회 축 | 실제 검색어 | 공식 URL에서 확인한 사실 | 차용·변경·차별화 |
|---|---|---|---|
| EU AI Act Article 50 | `EU AI Act Article 50 transparency obligations generated content labeling official` | [EU 집행위원회 Article 50 가이드라인](https://digital-strategy.ec.europa.eu/en/library/guidelines-transparency-obligations-providers-and-deployers-ai-systems)은 Article 50 투명성 의무가 2026-08-02부터 적용되며, 생성형 AI 제공자의 machine-readable marking과 deepfake 및 공익 정보 AI 텍스트에 대한 배포자 disclosure를 설명한다. [AI 생성 콘텐츠 실무규약](https://digital-strategy.ec.europa.eu/en/policies/code-practice-ai-generated-content)은 marking·labeling의 공통 실무를 준비한다 | policy version과 disclosure 상태를 brief→검수→publication request→proof에 보존한다. EU 판매를 지금 여는 근거로 쓰지 않고, 국내 고객이 EU 대상 콘텐츠를 만들 때도 적용 가능성 검토 gate로 쓴다 |
| YouTube·TikTok·Instagram AI disclosure | `YouTube TikTok Instagram AI generated content disclosure policy 2026 official` 및 각 플랫폼 공식 도메인 검색 | [YouTube 공식 도움말](https://support.google.com/youtube/answer/14328491)은 현실적으로 보이는 altered or synthetic content에 creator disclosure를 요구하며 label을 표시한다고 설명한다. [YouTube 2026 공지](https://blog.youtube/news-and-events/improving-ai-labels-viewers-creators/)는 C2PA 등 신호에 따른 label 개선을 설명한다. [TikTok 공식 도움말](https://support.tiktok.com/en/using-tiktok/creating-videos/ai-generated-content?authuser=0)은 현실적인 AIGC label과 auto-label을 설명한다. [Meta 공식 공지](https://about.fb.com/news/2024/04/metas-approach-to-labeling-ai-generated-content-and-manipulated-media/)는 industry signal 또는 self-disclosure에 따른 `AI info` label과 현실적인 AI video·audio disclosure를 설명한다 | 플랫폼별 boolean 하나로 일반화하지 않는다. 정책 version·content class·required disclosure·provider response를 보존하고 누락 시 fail-closed한다. 자율 발행 기본은 금지한다 |
| Paddle 대 Lemon Squeezy MoR | `Paddle vs Lemon Squeezy merchant of record SaaS tax official` | [Paddle 공식 VAT 문서](https://www.paddle.com/help/sell/tax/how-paddle-handles-vat-on-your-behalf)는 MoR로서 100개 이상 관할권의 세금 등록·계산·징수·신고·납부와 invoice를 설명한다. [Paddle 구조 문서](https://developer.paddle.com/get-started/how-paddle-works/)는 구매자에게 법적 판매자가 되는 구조를 설명한다. [Lemon Squeezy 공식 MoR 문서](https://docs.lemonsqueezy.com/help/payments/merchant-of-record)와 [세금 문서](https://docs.lemonsqueezy.com/help/payments/sales-tax-vat)는 결제·sales tax·VAT·refund·chargeback·PCI 책임을 설명한다 | 둘 다 기술설계 비교 후보로만 남긴다. 한국 사업자 onboarding, 실제 수수료, credit pack·부분 환불·원장 적합성, payout 회계가 확인되기 전 provider와 schema를 확정하지 않는다 |
| 다국어 마케팅 콘텐츠 생성 SaaS | `다국어 마케팅 콘텐츠 생성 SaaS 경쟁 국내`, `국내 다국어 SNS 콘텐츠 자동 생성 마케팅 SaaS` | [나누다](https://www.nanuda.kr/)는 블로그·Threads·Instagram과 글로벌 영어 콘텐츠 자동화를 공개한다. [화이트동키 ADo2](https://www.whitedonkey.kr/)는 다국어 자동번역과 글로벌 마케팅을 설명한다. [마케티](https://markety.co.kr/)는 한국어 고객면에서 콘텐츠 생성·SNS 발행을 제시한다. [Market4U](https://www.market4u.ai/)는 K-beauty 판매자의 채널별 캡션과 광고 표기 지원을 설명한다 | `다국어 생성` 자체를 해자로 주장하지 않는다. 고객 조작면 한국어, output language 명시 요청, 언어별 취향 격리, KO·EN만 품질 보증, 후보 비용·정책 provenance를 한 계약으로 묶어 차별화한다 |

외부 조사에서 가장 중요한 수정은 `글로벌`을 번역 기능으로 오해하지 않는 것이다. 다국어 출력은 1단계 요청 능력이지만 다국가 self-serve 판매, 현지 세무, 24시간 지원, 모든 언어 품질 보증은 별도 운영 능력이다. 따라서 D1과 D4를 함께 적용해 생성 범위는 넓히되 판매·지원 약속은 잠근다.
...
| 다국어 품질 | 언어별 말투 혼입·저품질 번역·원어민 검수 오인 | language profile cross-write, KO·EN 기준 미달, 비보증 언어 문의 | cross-write1이면 다국어 실행 OFF. KO·EN 외는 비보증 표시·재확인 |
| 정책 변화 | 플랫폼 AI label·tax·약관 규칙 변경 | policy version stale, provider reject | version 없는 발행0. policy owner가 월1회 공식문서 재검토 |
| 후보 원가 | 저해상도3과 고해상도 승격의 원가·환불 혼동 | preview당 원가, rejected charge, 추가 승격 문의 | 품질반려 차감1이면 유료 제작 HOLD. 추가 승격은 별도 견적 |
| 국가·세무 | locale·currency·timezone·tax region 불일치 | invoice·예약시각·세금 지역 분쟁 | 1단계부터 필드 보존. 다국가 self-serve와 특정 MoR은 설계 결정 전 OFF |
...
| 시장 | 바이브 코더와 부업 크리에이터를 한 메시지로 뭉개 둘 다 이탈 | cohort별 activation·repeat 격차 | 제품 One Thing은 공통, acquisition copy·proof·KPI는 cohort별 분리 |
| 시장 | `다국어`가 글로벌 판매·현지 품질 보증으로 오해됨 | 해외 결제·현지 지원·원어민 품질 문의 | 한국어 고객면과 출력 언어를 분리하고 KO·EN 외 비보증 고지 |
...
| 기술 | locale·currency·timezone·tax region·policy version을 뒤늦게 붙여 과거 데이터 계보가 깨짐 | null·추론값·timezone drift | 1단계부터 명시 저장하고 누락 상태를 거부. schema는 eng-design에서 합의 |
| 비용 | style 후보와 재시도가 원가 폭증 | 상한 초과·refund 증가 | 선택 전 범위·후보 수·budget cap·재확인 |
| 비용 | 후보3 모두 고해상도로 만들어 기본 원가가 3배가 됨 | high-resolution asset>선택수 | 기본은 저해상도3·선택본1. 미선택 승격은 별도 확인·차감 |
...
| 규제 | AI 생성 콘텐츠 label·machine-readable marking 누락 | platform reject·신고·EU 대상 배포 문의 | policy version·disclosure 계보, required 누락 시 publication0, 법률 검토 |
| 세무 | MoR을 쓰면 국내 회계·소득세·약관 책임까지 사라진다고 오해 | payout·invoice·refund 원장 불일치 | MoR은 eng-design 비교 후보. 다국가 self-serve는 비범위 |
...
세 번째 실패 시나리오는 `다국어` 한 단어가 제품·결제·지원의 글로벌 동시 출시로 번지는 것이다. 일본어·태국어 품질 문의와 해외 세금·환불이 동시에 들어오고, 1인 운영자는 시차별 24시간 지원을 약속한 적도 없는데 책임져야 한다. 이를 막기 위해 고객면은 한국어로 고정하고, 출력 언어는 명시 요청으로만 열며, KO·EN만 품질 보증한다. 다국가 self-serve·24시간 지원·특정 MoR 확정은 비범위다.

네 번째 실패 시나리오는 저해상도 후보3이 고객에게 선택 가능한 품질이 아니거나, 반대로 내부에서는 후보3을 전부 고해상도로 만들어 원가만 숨기는 것이다. 출시 전에 동일 brief의 저해상도·고해상도 실물을 회장이 비교하고, 선택 가능한 최소 품질과 비용 차이를 직접 승인해야 한다. 실물 비교 전에는 해상도 수치를 마케팅 문구나 API 계약으로 고정하지 않는다.
...
네 번째 이유는 영어·일본어·태국어 결과가 만들어진다는 사실과 각 시장에서 설득력 있는 결과가 나온다는 사실이 다르기 때문이다. 그래서 출력 언어 지원과 품질 보증을 분리했다. 한국어·영어는 언어별 acceptance fixture와 수정률을 측정하고, 그 밖의 언어는 원어민 검수 미제공을 명시하며 언어별 취향 신호를 섞지 않는다.
...
| 반복가치 | 미측정 | 자체 사용3/3 완료 뒤 external3/5가 외부 pilot D+28 내 loop2회 | proof→next recommendation→proof | 자체 proof9 후 외부 D+28 |
...
| 자체 사용 선행 | 해줘단타 EC0147 1편 외 통합 미검증 | 해줘단타3·제로원3의 기존 proof6 + openclaw 자체 마케팅3, 모두 source→choice→publication proof | tenant별 manifest, internal-test 격리 | 외부 pilot 전 |
| 언어별 품질 | 미구현 | 한국어·영어 acceptance fixture 통과100%, 언어 profile cross-write0 | 언어별 blind review·profile audit | 첫 언어별10 jobs |
| 후보 원가·선택성 | 미검증 | low-resolution3·selected high-resolution1, 품질반려 고객차감0 | asset·cost·charge ledger + 회장 실물 비교 | 외부 유료 pilot 전 |
| 정책 disclosure | 미구현 | required disclosure 누락0, policy version coverage100% | publication request·provider proof audit | 지원 플랫폼별 첫10건 |
| cohort 적합성 | 미측정 | 바이브 코더·예비창업자와 부업 크리에이터를 분리 집계하고 cohort별 paid repeat 확보 | cohort-tagged funnel·support·repeat | 외부 pilot D+28 |
...
- openclaw 자체 마케팅3이 같은 흐름으로 source→choice→publication proof를 남기지 못하면 외부 pilot을 시작하지 않는다.
- kill-criteria 측정 시점은 자체 사용 proof9가 모두 닫힌 뒤 첫 외부 cohort가 시작된 날 D0로 미룬다. 기준을 삭제하거나 내부 traffic으로 통과시키지 않는다.
- 자체 사용 proof9 뒤 external3/5 미만이 외부 pilot D+28 안에 loop2회를 완료하면 선택형 marketing agency를 내려놓고 자료 기반 composer·publishing workbench로 축소한다.
- 바이브 코더·예비창업자와 부업 크리에이터 중 어느 cohort도 pilot 참여2·paid repeat1을 만들지 못하면 `전 페르소나 마케팅`으로 넓히지 않고 해당 cohort의 pain·가격·획득경로를 재검토한다.
- decision30 뒤 사례·비용·근거 열람이 선택 변화나 시간 단축과 연결되지 않으면 8단계를 축소하고 fast default를 우선한다.
- 같은 brief의 저해상도 후보3이 회장 실물 비교에서 선택 불가 판정을 받거나 실제 원가가 기본 판매가 가설을 깨면 외부 유료 pilot을 HOLD하고 후보 수·품질·가격을 재설계한다.
- 언어별 job10에서 한국어 또는 영어 acceptance 미달1, language profile cross-write1이면 해당 언어 자동 실행을 OFF한다. 일본어·태국어 등 비보증 언어의 생성 가능성을 품질 보증으로 승격하지 않는다.
...
### 16.0 2026-08-15 레드팀 회수 결정 5건

아래 다섯 건은 제품구조 결정서 §9.6의 레드팀 회수 ID D1~D5이며 회장 확정사항이라 다시 묻지 않는다. 뒤의 기존 미결정 ID `D-01`~`D-10`과는 별도 번호 체계다. design·eng-design·build가 임의로 넓히거나 축소할 수 없는 plan 입력이다.

| 확정 ID | 확정값 | PRD 반영 | 남은 검증, 결정 아님 |
|---|---|---|---|
| D1 | 고객 조작면은 한국어 하나. 출력 콘텐츠 언어는 다국어 요청 파라미터. 취향 프로파일은 언어별 분리. 품질 보증은 KO·EN만 | §3.9, FR029~032, AC029~032, NFR014~015 | KO·EN acceptance fixture와 비보증 언어 고지 문구의 실제 사용성 |
| D2 | 첫 외부 고객은 바이브 코더·예비창업자와 조회수 수익·쿠팡 파트너스형 부업 크리에이터 | §5.1, FR040, AC040, cohort KPI | 두 cohort의 acquisition copy와 표본 수는 pilot 운영계획에서 정함 |
| D3 | 해줘단타·제로원·openclaw 자체 마케팅을 먼저 사용. kill-criteria는 삭제하지 않고 외부 pilot 뒤로 측정시점 이동 | §0.5, FR034, AC024·034, §15 | 자체 사용 proof9 직접 관찰과 외부 pilot D0 기록 |
| D4 | locale·currency·timezone·tax region·policy version을 1단계부터 포함. 다국가 self-serve·24시간 지원 비범위. MoR은 eng-design 재검토 | §3.9.2, FR033·039·042, AC033·039·042, §11.4 | field type·schema·MoR 선택은 eng-design 대화 게이트 |
| D5 | 저해상도 후보3을 제시하고 선택본1만 고해상도. 미선택 후보 고해상도는 추가 과금 | §3.8.1, FR035~037·043, AC035~037·043 | 저해상도 실물 품질·비용 비교를 회장이 직접 승인 |
...
### 16.2 채널 연결 실패와 토큰 만료를 화면에서 다루는 선택지
...
### 16.3 제작 크레딧 사용 뒤 발행 실패의 환불 선택지
...
| 2 | 구체화 | 8단계, MVP5, FR44, AC44, 자체 proof9, source label3, 저해상도 후보3·선택본1 | PASS |
| 3 | 입출력 분리 | 자료·소재·선택·brief·완성 파일·메타데이터·proof·signal을 분리 | PASS |
| 4 | 정합성 | 제품구조 결정서 §0.5·§3.6·§3.7·§9.6과 v8.1.1 전수 대조, 회장 확정 D1~D5 반영 | PASS |
...
| 7 | 논리 영역 | media mutation0, provenance100%, wrong-account0, false-zero0, language cross-write0, proof9/9 | PASS |

### 17.2 INVEST·5단 스펙 경계

- 요구는 독립 ID와 Fit Criterion을 가지며 FR44와 AC44까지 1:1이다.
...
| 항목 | 상태 | 이유 |
|---|---|---|
| PRD v8.2.1 | **작성됨·검증 대기** | v8.1.1의 FR28·AC28 문장 전량 보존, D1~D5와 FR029~044·AC029~044 추가, 원본790줄보다 긴 문서 |
| current-state 정본 확인 | **통과** | wiki·release/pipeline 승인 핀·구현현황에서 기존 계정·예약·발행·성과·운영자 기능은 부분구현으로 분류. 위키가 비어 있던 AI disclosure·global 필드만 좁게 검색해 TikTok 입력 부분구현과 나머지 미구현을 확인 |
| 회장 확정 D1~D5 | **통과** | §16.0과 FR·AC·NFR·KPI·비범위에 모두 연결 |
| 남은 회장 결정 | **미통과** | §16.1의 이름·Notion·가격·환불·목소리·실원가·연결 복구 UX·제작 후 발행 실패 크레딧·채널별 메타데이터 미결. D-02는 확정 이력이라 제외 |
| 저해상도 실물 gate | **미통과** | 동일 brief의 후보·선택본을 회장이 직접 비교한 증거 없음. 수치 고정과 외부 유료 pilot 금지 |
| 독립 plan-critic | **미실행** | v8.2.1 기준 독립 리뷰와 MAJOR0 증거 없음 |
| plan 승인 | **미통과** | `/approve plan` 미실행, approved pin은 v7.3.5 |
| design 진입 | **불가** | 이 문서가 아직 승인 핀에 없고, DESIGN v24는 D1~D5·후보3→선택본1·정책 version을 반영하지 않음 |
| eng-design 진입 | **불가** | PRD·design 승인과 API·DB·port·delivery 티키타카 전 |
| build 진입 | **불가** | 승인된 v8.2.1 PRD·DESIGN·FDD 없음 |
...
| v8.2.1 | 2026-08-15 | §9.6 회장 확정 D1~D5 반영. 한국어 고객면·다국어 출력·언어별 취향, 외부 고객2 cohort, 자체 proof9 선행과 kill 시점 이동, 글로벌 준비5필드·MoR 재검토, 저해상도3→선택본1, 레드팀 금지10개, 정책·MoR·국내 다국어 경쟁 공식 조사 추가 | in-review, 승인 대기 |
...
- 내부 proof와 외부 시장수요 증거를 구분하고, 해줘단타3·제로원3·openclaw 자체 마케팅3의 proof9 전 외부 pilot을 막았다. kill-criteria는 삭제하지 않고 외부 pilot D0 뒤로 측정시점만 이동했다.
...
- studio-service 소비자 단독 판매, 글로벌 동시 self-serve, 전 페르소나 마케팅, 완성본 A/B/C, 풀 영상 기본, 자율 발행 기본, 무제한 채널, 근거 없는 성과 점수, 외부 롱폼 역분석·음악·범용 텍스트, 고급 BI 대시보드를 비범위로 잠갔다.
- 플랫폼 disclosure는 policy version과 함께 proof까지 보존하고, KO·EN 외 언어는 생성 가능과 품질 보증을 분리했다.

RUBRIC_SCORE: 완결성=5/5 정밀성=5/5 벤치마크=5/5 추적성=5/5 전문성=5/5 total=25/25
WEAKEST_LINE: "저해상도 후보의 선택 가능 품질은 회장 실물 비교 전 수치로 확정하지 않는다." D5의 구조는 확정됐지만 실제 최소 해상도·원가·품질선은 아직 직접 관찰되지 않았다.

SKILLS_USED: 없음
SKILLS_SKIPPED: pipeline - Stage Controller를 실행해 상태를 바꾸는 과업이 아니라 plan worker의 PRD 개정이며, 승인 게이트는 부모 컨트롤러 소유 / brand-positioning-kit - 승인 전 기존 포지셔닝을 보존하는 PRD 확장으로 신규 브랜드 정의 과업이 아님 / openclaw-creative-brief - 실행 에이전트용 브리프가 아니라 제품 요구사항 문서이며 schema는 eng-design 전 확정 금지

🏷 STAMP | line: openclaw-service | 생성: 2026-08-15 19:49 KST | model: gpt-codex/gpt-5.6 | agent: prd-architect

고민: 다국어 출력을 글로벌 판매와 동일시하면 1인 운영에 세무·지원·품질 책임이 한꺼번에 열린다. 그래서 고객면 한국어와 output language를 분리하고, locale·currency·timezone·tax region·policy version은 처음부터 보존하되 다국가 self-serve·24시간 지원·특정 MoR 확정은 잠갔다. D5는 후보 다양성을 주면서도 고해상도 원가를 선택본1에만 지우도록 원장·과금·실물 gate까지 연결했다.
...
- `/Users/sj/sj_code_master/postAGI/AGENTS.md`
- `CLAUDE.md`
- `AGENTS.md`
- `README.md`
- `docs/제품구조-결정-2026-08-15.md`
- `docs/prd-openclaw-service-v8.1.1-gpt-codex.md`
- `docs/레드팀-제품구상-2026-08-15-gpt-codex.md`
...
- `wiki/marketing/gtm-plan.md`
- `wiki/ops/session-state.md`
- `docs/구현현황.md`
- `pipeline-state.osmu.md`
- `pipeline-state.studio.md`
- `dashboard/src/lib/tiktok.ts`와 `dashboard/src/app/api/video/publish/route.ts`, `dashboard/src/app/videos/page.tsx`의 `is_aigc` 좁은 코드 검색 결과
- §12.1~12.3의 시그마인·마케티·CREAGEN·Higgsfield·OpusClip·Vizard·Ayrshare·Buffer와 국내 법률 citation은 승인 통과본 v8.1.1에서 문장 그대로 보존했으며 이번 개정에서 재조회하지 않음
- 이번 v8.2.1 개정에서 실제 WebSearch한 공식·경쟁 출처:
- [EU 집행위원회 Article 50 가이드라인](https://digital-strategy.ec.europa.eu/en/library/guidelines-transparency-obligations-providers-and-deployers-ai-systems)
- [EU AI 생성 콘텐츠 실무규약](https://digital-strategy.ec.europa.eu/en/policies/code-practice-ai-generated-content)
- [YouTube altered or synthetic content disclosure](https://support.google.com/youtube/answer/14328491)
- [YouTube 2026 AI label 개선 공지](https://blog.youtube/news-and-events/improving-ai-labels-viewers-creators/)
- [TikTok AI-generated content 도움말](https://support.tiktok.com/en/using-tiktok/creating-videos/ai-generated-content?authuser=0)
- [Meta AI content label 정책](https://about.fb.com/news/2024/04/metas-approach-to-labeling-ai-generated-content-and-manipulated-media/)
- [Paddle VAT 처리](https://www.paddle.com/help/sell/tax/how-paddle-handles-vat-on-your-behalf)
- [Paddle MoR 구조](https://developer.paddle.com/get-started/how-paddle-works/)
...
- [나누다](https://www.nanuda.kr/)
- [화이트동키 ADo2](https://www.whitedonkey.kr/)
- [Market4U](https://www.market4u.ai/)
- 이번 v8.2.1 개정에서 실제 실행한 검색어:
- WebSearch query: `EU AI Act Article 50 transparency obligations generated content labeling official`
- WebSearch query: `YouTube TikTok Instagram AI generated content disclosure policy 2026 official`
- WebSearch query: `Paddle vs Lemon Squeezy merchant of record SaaS tax official`
- WebSearch query: `다국어 마케팅 콘텐츠 생성 SaaS 경쟁 국내`
- WebSearch query: `국내 다국어 SNS 콘텐츠 자동 생성 마케팅 SaaS`
