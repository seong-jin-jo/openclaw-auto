# OSMU Marketing Agent PRD — v7.3.1 Product Requirements Document

<!--
STAMP
created_at: 2026-08-07 13:13 KST
...
evidence: PRD v7.3.0, critic v7.3.0 NO-GO major3/minor2, final audit96/P0-8, agency blueprint14, current brand/Studio code, Jasper/HubSpot/GitHub official docs
deliberation: 기존 84개 계약을 보존하고 source5 단일 진실 수렴, thin-data hold 정책, audit96·P0-8·campaign14 원자 release gate만 추가한다.
-->

| 항목 | 값 |
|---|---|
| 버전 | **v7.3.1** — critic MAJOR3/MINOR2 surgical RETAKE |
| 작성일 | 2026-08-07 |
...
| 상류 입력 | PRD v7.3.0; critic v7.3.0; final audit96/P0-8; agency blueprint14; current brand/wiki/onboarding/Studio code audits |
...
- 범용 CMS/문서 suite: 임의 파일 협업, comments/tasks, public wiki publishing, page-layout builder, general database, LMS/CRM 기능. 허용 knowledge 작업은 brand fact/source/tone/taboo/proof의 ingest·create/edit·version·confirm·archive·recover뿐이다.
- 광고 집행 제품: ad account 연결, bidding, paid-media spend execution, billing API. v7.3.1 build scope의 paid media는 `없음/외부 관리/미정` 기록과 cap 경고만 허용한다.

## 2. 현재 상태·증거 경계

| Area | Current judgement | Wiki/current source | Defect | v7.3.1 target |
|---|---|---|---|---|
...
### 5.2.1 Source5 convergence·exclusive truth invariant

source5의 차이는 **ingestion adapter와 provenance뿐**이다. GitHub folder, specific tone file, 6-question setup, direct paste, in-product wiki는 모두 아래 logical model과 tenant/role/version/approval/retrieval 규칙으로 수렴한다. 이는 제품 계약이며 DB table/API encoding은 eng-design에서 합의한다.

```mermaid
flowchart LR
  S[Source5 adapters] --> R[Knowledge source/page revision]
...
| Current Studio action | v7.3.1 target copy | Canonical command | Immediate result | Management projection |
|---|---|---|---|---|
...
| FR-MA731-085 | source5 모두를 공통 knowledge source/revision→candidate→active knowledge→active guide→generation snapshot model로 normalize한다. | convergence5/5 | 미구현 |
...
| FR-MA731-095 | audit96/P0-8/campaign14와 source5/failure7/guide6/learning cases를 독립 release ID로 양방향 추적하고 aggregate-only PASS를 금지한다. | audit96/P0-8/orphan0 | plan |
...
| FR-MA731-085 | AC-MA731-085 source convergence | **MA731-TC-085:** Given semantically identical brand fixture ingested independently by GitHub folder/tone file/6Q/paste/native wiki, When normalize, Then canonical candidate field set·status·tenant/role/version policy·guide eligibility diff0, provenance source type/locator만 다름; N5/M5. |
...
| FR-MA731-095 | AC-MA731-095 atomic RTM | **MA731-TC-095:** Given appendix RTM/release catalog, When bidirectional audit, Then audit96/96, P0 8/8, campaign14/14, source5/5, failure7/7, guide6/6, learning fixtures all unique release IDs; orphan/duplicate ownership/composite-only PASS0; one injected case failure fails that case+aggregate. |

### 16.1 Coverage

- FR/AC/base TC: **95/95/95**, 1:1, orphan0. 기존 MA72-001~067·MA73-068~084 삭제·재번호·약화0, MA731-085~095만 additive다.
...
#### 17.5.1 Source convergence matrix 5×4 — N20/M0
...
| Source | State normalization | Tenant/role | Version/approval | Retrieval/lineage |
|---|---|---|---|---|
...
Mutation rule: 한 source의 role/version/retrieval adapter를 의도적으로 다르게 만들면 해당 cell FAIL, convergence aggregate FAIL이며 다른 19개를 PASS로 위장하지 않는다.
...
Brand/campaign/closed-loop 신규 atomic catalog는 **60/60**(convergence20+failure7+guide6+campaign14+format3+learning10)이다. 현재 plan M=0. Design은 customer action·state·recovery를, build/QA는 persistence·authority·provider call/readback을 별도 증거 등급으로 검증한다.

#### 17.5.5 P0 exact 8/8 coverage

| P0 | Product contract | FR/AC | Independent release evidence |
|---|---|---|---|
| P0-01 Knowledge Center | source5 ingest/create/edit/sync/diff/last-good | 068~076, 085~089 | KCONV20+KFAIL7 |
...
| Surface | Current | v7.3.1 target | Forbidden |
|---|---|---|---|
...
|---|---|---|
| R0 Plan | v7.3.1 | verifier PASS+re-critic MAJOR0+approve |
...
| R2 Eng-design | storage/API/command encoding | dialogue+FR95+audit96 trace |
...
6개월 뒤 실패했다면 `wiki_docs`와 `brand_guides` 중 어느 것이 active인지 갈려 오래된 가격을 게시하고, 게시물 한 건을 adequate라 부르며 톤·CTA를 동시에 바꾸고, campaign 전체 click trace 하나로 14단계 PASS를 선언했을 가능성이 크다. 방어는 active knowledge1+guide1, KCONV20, baseline-before-hold+LEARN10, CAMPAIGN-01~14와 AUD96 unique cases다.
...
### 23.1 v7.3.0 critic MAJOR3·MINOR2 closure

| Finding | v7.3.1 closure | FR/AC/TC | Exit evidence |
|---|---|---|---|
| MAJOR1 source5 convergence/exclusive truth | §5.2.1 common model, active knowledge1+guide1, human precedence/conflict/rollback, legacy dual-read0 | 085~089 | KCONV20+KFAIL7+GUIDE6 |
...
| MAJOR3 audit atomicity | audit96 unique RTM, P0 8/8, CAMPAIGN-01~14, mutation aggregate rule | 095 | AUD96+P0-8+new atomic60 |
...
### 23.2 Audit96 bidirectional atomic RTM — N96/M0
...
| Audit | Retained/new FR→AC | Design evidence | Build/QA evidence | Release case |
|---|---|---|---|---|
| A01 | 011→AC011 | login success/cancel/error/retry | fresh identity semantic+callback state | RC-AUD-A01 |
| A02 | 002·011→AC002/011 | create/join workspace choice | two fresh identities tenant isolation | RC-AUD-A02 |
| A03 | 002·015·062→AC002/015/062 | active workspace switch+role | all projections switch/no stale rows | RC-AUD-A03 |
| A04 | 002→AC002 | forbidden deep-link state | two-tenant read/write/external adversarial | RC-AUD-A04 |
| A05 | 003·062→AC003/062 | customer/approver/operator action matrix | forged role action rejects | RC-AUD-A05 |
| A06 | 013·064·065→AC013/064/065 | timeout/401/403/500 recovery | false-ready0+safe correlation | RC-AUD-A06 |
| B01 | 068·072·085→AC068/072/085 | non-GitHub import preview | exact docs/source refs/tenant | RC-AUD-B01 |
| B02 | 069·075·085→AC069/075/085 | repo/ref/path/permission states | selected files only+success after fetch | RC-AUD-B02 |
| B03 | 073·085·089→AC073/085/089 | new wiki first page/save | reload+retrieval+tenant owner | RC-AUD-B03 |
| B04 | 073·076·087→AC073/076/087 | fact/inference/unverified editor | confirmed update eligible; unverified block | RC-AUD-B04 |
| B05 | 020·076·088→AC020/076/088 | claim inspector+citation state | supported/unsupported claim exact badges | RC-AUD-B05 |
| B06 | 069·075·087→AC069/075/087 | sync/freshness/hash state | idempotent sync+eligibility transition | RC-AUD-B06 |
| B07 | 075·087→AC075/087 | last-good+repair state | fault inject partial overwrite0 | RC-AUD-B07 |
| B08 | 074·075·087→AC074/075/087 | added/changed/removed diff | reject keeps active; accept new version | RC-AUD-B08 |
| B09 | 078·085→AC078/085 | source-scope→guide draft | deterministic cited fields+auto-active0 | RC-AUD-B09 |
| B10 | 078→AC078 | structured guide editor | save/reload+actor diff audit | RC-AUD-B10 |
| B11 | 074·088→AC074/088 | version list/diff/rollback | v1 immutable; rollback=v3 | RC-AUD-B11 |
| B12 | 078·086→AC078/086 | approve/reject/change state | forged activation denied | RC-AUD-B12 |
| B13 | 077·088→AC077/088 | artifact used-guide chip | v1 artifact unchanged after v2 | RC-AUD-B13 |
| B14 | 078·085→AC078/085 | example-selected tone draft/confidence | insufficient example needs review | RC-AUD-B14 |
| B15 | 078→AC078 | plain-language tone/editor preview | rule delta visible, external0 | RC-AUD-B15 |
| B16 | 074·078·088→AC074/078/088 | tone draft/active/retired history | v1/v2 binding+mutation invalidation | RC-AUD-B16 |
| B17 | 077·088→AC077/088 | applied tone version/rules | two tone versions inspectable delta | RC-AUD-B17 |
| C01 | 079·080·082→AC079/080/082 | idea→campaign with rationale | source/time retained through brief | RC-AUD-C01 |
| C02 | 079→AC079 | explicit measurable goal/hold | empty vs valid goal branch | RC-AUD-C02 |
| C03 | 079·080→AC079/080 | audience/offer/CTA/message fields | one-field edit propagates variants | RC-AUD-C03 |
| C04 | 079·094→AC079/094 | budget taxonomy/no-spend | no ad/billing call, 미정 hold | RC-AUD-C04 |
| C05 | 079→AC079 | dates/timezone/overdue conflict | cross-timezone/past deadline | RC-AUD-C05 |
| C06 | 080→AC080 | brief draft/change/approved/stale | upstream change invalidates fan-out | RC-AUD-C06 |
| C07 | 076·078·081→AC076/078/081 | safety/taboo/claim panel | prohibited rule blocks exact claim | RC-AUD-C07 |
| C08 | 020·076·081→AC020/076/081 | claim/evidence/resolution list | unsupported price/guarantee block | RC-AUD-C08 |
| C09 | 054·061·081→AC054/061/081 | all-media rights inspector | unknown/expired right blocks all families | RC-AUD-C09 |
| C10 | 054·061·081→AC054/061/081 | consent/disclosure/N-A reason | person/AI fixtures propagate/block | RC-AUD-C10 |
| C11 | 025·077·082·088→AC025/077/082/088 | clickable campaign lineage | one ID/version idea→learning | RC-AUD-C11 |
| D01 | 022·081→AC022/081 | Text→Photo/Card→Video numbered order | DOM order at three viewports | RC-AUD-D01 |
| D02 | 021~024→AC021~024 | per-social destination capability cards | unsupported adapter calls0 | RC-AUD-D02 |
| D03 | 023·061·081→AC023/061/081 | real card/slides/alt/rights flow | one/many slide save/publish projection | RC-AUD-D03 |
| D04 | 023·024·048~054·081→matching AC | script/render/metadata/rights states | job success/failure+provider U label | RC-AUD-D04 |
| D05 | 027→AC027 | Social→Messaging→Social video | semantic nav order 3 viewports | RC-AUD-D05 |
| D06 | 037·038→AC037/038 | explicit handoff/default OFF | untouched default causes API0 | RC-AUD-D06 |
| D07 | 023·039~041·057→matching AC | video account/job/result/native owner | capability fixture+readback label | RC-AUD-D07 |
| D08 | 019→AC019 | safe-disabled MJ explanation | user-token/Discord call0 | RC-AUD-D08 |
| D09 | 008~010·015→matching AC | Header7 every owner | shared server projection diff0 | RC-AUD-D09 |
| D10 | 014·015→AC014/015 | exact account chooser/confirm | two-account switch/reload/target ID | RC-AUD-D10 |
| D11 | 012·013→AC012/013 | Authorized→Ready ladder | fault each step, premature ready0 | RC-AUD-D11 |
| D12 | 015·024·042→AC015/024/042 | shared capability version/stale state | revoke propagates all surfaces | RC-AUD-D12 |
| D13 | 024·034·047·057→matching AC | Threads account/publish/result/insight | adapter+proof/native truth | RC-AUD-D13 |
| D14 | 024·034·047·057→matching AC | X account/honest capability | no invented analytics/action | RC-AUD-D14 |
| D15 | 024·034·047·057→matching AC | Facebook Page text/image readiness | Page ID exact+missing analytics truth | RC-AUD-D15 |
| D16 | 024·039·054·057·061→matching AC | IG Feed/Card/Reels/rights/native | professional account+format split | RC-AUD-D16 |
| D17 | 024·034·047·057→matching AC | Bluesky handle/capability | only implemented adapter actions | RC-AUD-D17 |
| D18 | 039·048~054·057→matching AC | YouTube channel/Shorts job/metadata | progress/result/native+rights | RC-AUD-D18 |
| D19 | 039·048~054·057→matching AC | TikTok creator/privacy/disclosure | exact creator/result/status | RC-AUD-D19 |
| D20 | 036~038→AC036~038 | Telegram bot/chat handoff OFF | approved target only call1 | RC-AUD-D20 |
| D21 | 036~038→AC036~038 | Discord bot/webhook/guild/channel OFF | explicit approval only | RC-AUD-D21 |
| D22 | 036~038→AC036~038 | Slack workspace/app/channel OFF | explicit approval only | RC-AUD-D22 |
| D23 | 019→AC019 | MJ non-account capability copy | account/publish-ready state0 | RC-AUD-D23 |
| D24 | 004·010·024·060·067→matching AC | Naver long-form draft/preview owner | unsupported direct-publish call0 | RC-AUD-D24 |
| D25 | 004·010·024·060·067→matching AC | Medium draft/preview/capability | honest connection/publish branch | RC-AUD-D25 |
| D26 | 004·010·024·060·067→matching AC | Substack newsletter preview/export | unsupported send implication0 | RC-AUD-D26 |
| E01 | 026·030→AC026/030 | per-card dirty/save/error/revision | sibling hashes unchanged+reload | RC-AUD-E01 |
| E02 | 021·026·043→matching AC | compatible bulk impact/exclusions | invalid cross-platform mutation0 | RC-AUD-E02 |
| E03 | 031~035·044→matching AC | exact approval snapshot detail | post-approval mutation stale+call0 | RC-AUD-E03 |
| E04 | 031~036·043·044→matching AC | itemized bulk approvals/blockers | 2 approved+1 blocked not 3 success | RC-AUD-E04 |
| E05 | 031·037→AC031/037 | fresh default OFF consequence | generate/save external calls0 | RC-AUD-E05 |
| E06 | 032·035·044→matching AC | schedule timezone/DST/conflict | no duplicate across DST/cancel | RC-AUD-E06 |
| E07 | 031·043·044→matching AC | publish preflight+repair action | each blocker adapter call0 | RC-AUD-E07 |
| E08 | 033·045→AC033/045 | command/idempotency inspector | double/concurrent call external≤1 | RC-AUD-E08 |
| E09 | 034~042→matching AC | same-ID cross-surface links | one mutation projects everywhere | RC-AUD-E09 |
| E10 | 039~042→matching AC | artifact→render→publish chain | failed render publish-job0 | RC-AUD-E10 |
| E11 | 046→AC046 | confirmed failure+attempt history | retry once/draft preserved | RC-AUD-E11 |
| E12 | 046→AC046 | uncertain/reconcile/retry disabled | accepted timeout second call0 | RC-AUD-E12 |
| E13 | 043·046→AC043/046 | per-destination partial result | repair failed target only | RC-AUD-E13 |
| E14 | 047→AC047 | provider ID/permalink/readback drawer | missing/malformed proof≠published | RC-AUD-E14 |
| E15 | 063·065→AC063/065 | safe error+correlation action | 4xx/5xx/timeout secret scan | RC-AUD-E15 |
| F01 | 055→AC055 | whitelist definition/period/coverage | prohibited native sum0 | RC-AUD-F01 |
| F02 | 056·057→AC056/057 | native provider names/units | similarly named metrics stay separate | RC-AUD-F02 |
| F03 | 058→AC058 | six metric states | missing never rendered zero | RC-AUD-F03 |
| F04 | 025·057·058→matching AC | account/revision/publication/window links | filter changes context+value together | RC-AUD-F04 |
| F05 | 059·090·091→matching AC | eligibility/confidence/alternative | thin/biased hold; eligible hypothesis only | RC-AUD-F05 |
| F06 | 084·092→AC084/092 | one-variable approval before/after | exact input lineage+variable1 | RC-AUD-F06 |
| F07 | 084·088·092→matching AC | applied-result chip+diff+undo | only approved variable changes | RC-AUD-F07 |
| F08 | 031·037·091~093→matching AC | observe/draft/approve/execute boundary | forbidden reply/delete/repost/spend0 | RC-AUD-F08 |
| G01 | 062→AC062 | Settings8 customer variant | secret/operator fields absent | RC-AUD-G01 |
| G02 | 062→AC062 | Settings9 operator variant | customer forbidden+audit | RC-AUD-G02 |
| G03 | 003·062→AC003/062 | bounded support mode/no publish | forged customer publish denied | RC-AUD-G03 |
| G04 | 004·005·067→matching AC | route25/destination26 purpose/action | inventory no removal/generic flatten | RC-AUD-G04 |
| G05 | 063→AC063 | plain Korean/customer copy | banned internal string scan | RC-AUD-G05 |
| G06 | 006·007→AC006/007 | 390/1024/1440 no overlap | screenshot+interaction+occlusion0 | RC-AUD-G06 |
| G07 | 007→AC007 | focus/labels/errors/reduced motion | axe+keyboard/manual focus | RC-AUD-G07 |
| G08 | 066·095→AC066/095 | evidence-grade matrix | semantic vs provider proof separate | RC-AUD-G08 |
| G09 | 016~018·065→matching AC | masked write-only credentials | canary scan DOM/URL/log/export0 | RC-AUD-G09 |
| H01 | 008·031·044·063→matching AC | decision-point truth stays visible | 3 viewport decision without hidden help | RC-AUD-H01 |
| H02 | 013·064→AC013/064 | persisted-evidence progress only | fault step never advances completion | RC-AUD-H02 |
| H03 | 017·031·037→matching AC | publish/message OFF+reversible opt-in | fresh tenant external calls0 | RC-AUD-H03 |
| H04 | 027·068·081·089→matching AC | categorized novice+expert paths | no valid option deleted by psychology claim | RC-AUD-H04 |

Audit ownership count는 A6+B17+C11+D26+E15+F8+G9+H4=`96`이다. `RC-AUD-*` unique96, upstream FR 없는 row0, release 없는 row0, duplicate release ID0이다. Design evidence는 prototype-observed, build/QA는 code-tested/sandbox-tested/production-observed로 분리하며 provider 미관찰은 U로 남긴다.

### 23.3 v7.2 critic MINOR4 closure
...
### 23.4 v7.1 critic closure regression
...
### 23.5 v7 original closure regression check
...
### 23.7 Quantitative RTM

- User ledger45/45; latest11/11.
- v7.1 critic MAJOR2/2·MINOR4/4, v7.2 critic MINOR4/4, v7.3.0 critic MAJOR3/3·MINOR2/2 document closure.
- FR/AC/base TC **95/95/95**; legacy84 preserved+MA731 additive11.
- Audit RTM **96/96**, P0 **8/8**, campaign **14**, source convergence **20**, failure **7**, guide **6**, format **3**, learning **10**; orphan/duplicate release ID0.
- R3 **30**, R4 **53**, OAuth **18**, navigation **75**, owner-state **60**, header **11**, 신규 atomic catalog **60**; current M=0.
...
Storage table/event design, command endpoint/transaction, idempotency key derivation, projection SLA/repair engine, tenant scope encoding, logical knowledge entities의 물리 encoding, campaign/revision/metric lineage encoding을 합의한다. Active knowledge1+guide1 exclusive truth, human precedence/conflict hold, baseline-before-hold, Studio action·copy·policy semantics, source5/campaign14 의미, record authority1, native8 denominator는 변경할 수 없다.
...
두 번째 이유는 native8 중 많은 case가 unsupported라 성과 환류 가치가 약할 수 있다는 점이다. 그래서 기준 숫자를 발명하지 않고 channel×metric measured baseline과 product/data owner 승인 전 모두 hold한다. Social5 permanent unsupported≥3이거나 threshold를 승인할 근거가 없으면 판매 문구를 내린다. 세 번째는 최소 wiki가 general CMS로 팽창할 위험이다. 범위를 브랜드 fact/source/tone/taboo/proof의 ingest/create/edit/version/confirm/archive/recover로 제한하고 협업 문서 suite는 out of scope로 유지한다.

### 24.5 기획 7원칙 판정

| # | 원칙 | v7.3.1 evidence | 판정 |
|---:|---|---|---|
| 1 | 용어 통일 | authority/view/initiation, policy/readiness, collected/unsupported schema 분리 | PASS |
| 2 | 구체화 | FR95, audit96, P0-8, campaign14, new atomic60, R3/R4/N75/N60/N11 | PASS |
| 3 | 입출력 분리 | source·setting·review 입력과 record·publication·truth 출력 구분 | PASS |
| 4 | 정합성 | audit96 각 행→FR/AC→design/build evidence→unique release case 양방향 대조 | PASS |
...
- v7.3.1 RETAKE 작성+verifier PASS; independent re-critic 전 in-review.
- v7.3.0 critic MAJOR3/MINOR2는 문서상5/5 closure; 재비평 대상이다.
...
WEAKEST_LINE: channel×metric threshold의 숫자는 baseline 전 hold로 닫았지만 pilot에서 충분한 비교표본을 만들지 못하면 Measure-to-Create 가치 자체를 내려야 한다.
...
SOURCES: `docs/openclaw-auto-marketing-agent-prd-v7.3.0-gpt-codex.md`; `tasks/marketing-agent-plan-critic-v7.3.0.output`; `tasks/marketing-agent-v18-agency-blueprint.output`; `tasks/marketing-agent-v18-completeness-audit.output`; current RepoConnect/BrandSetupWizard/sync-wiki/wiki-retrieve/onboarding/Studio; planning/doc-review/benchmarks/branding/writing/marketing standards; Buffer/Sprout/Later/Hootsuite official docs; Jasper Brand Voice/Knowledge Base/Style Guide; HubSpot Campaigns; GitHub Contents API; RFC9700; Google OAuth; TikTok Login Kit.
MODEL: gpt-codex/gpt-5.6-sol
