# openclaw-auto OSMU PRD v4.2.1 — partial generation·retry taxonomy PATCH

> **상태:** v4.2.0 critic residual MAJOR 2 closure PATCH · 승인 전 · DESIGN v6 입력 사용 금지  
...
- 사용자 확정 기본 UI는 **초안생성 1회 → text platform card 8개 → 카드별 edit/save → 카드별 Publish → 같은 카드 status/permalink**다. 전체/선택 발행은 별도 명시 action이며 개별 Publish가 기본이다.
- Threads·Instagram·X initial safety bet은 외부 발행 검증 우선순위일 뿐 8-card 초안 UI를 3개로 축소하는 근거가 아니다.
...
#### 7.1.1 Studio/API as-is drift와 v4.2 target

| 층 | As-is count·대상 | 의미 | v4.2 판단 |
|---|---|---|---|
| Studio preview | 7: Threads, X, Facebook, Instagram, Shorts, Reels, TikTok | text/feed와 video surface가 섞인 렌더 preview | 현재 구현. 8-card target의 근거로 오인 금지 |
| Studio direct Publish | 4: Threads, X, Facebook, Instagram | card/selectable direct path | 현재 구현. Bluesky/Telegram/Discord/Slack card Publish는 target |
| Next text API·schedule | 8: Threads, X, Facebook, Instagram, Bluesky, Telegram, Discord, Slack | backend text adapter/schedule 범위 | target 8-card platform set의 코드 기반, 운영 readiness는 별도 |
| Target draft UI | 8 text cards | 생성·편집·저장은 readiness와 무관, Publish만 readiness gate | **미구현 target**, 기존 구현 주장 금지 |
...
### 8.1 사용자 확정 8-card 기본 작업모델

```mermaid
flowchart LR
  G[초안생성 1회] --> C[8 platform cards]
  C --> T[Threads]
  C --> X[X]
  C --> F[Facebook]
  C --> I[Instagram]
  ...
  C --> D[Discord]
  C --> S[Slack]
  T & X & F & I & B & TG & D & S --> E[각 카드 edit/save]
  E --> P[그 카드의 Publish]
  P --> R[같은 카드 status + 실제 permalink]
```

- `초안생성`은 source 한 번으로 고정 platform ID `threads,x,facebook,instagram,bluesky,telegram,discord,slack` card 8개를 항상 만든다. 각 card는 `editable_payload` 또는 `generation_failed{reason,regenerate_action}`를 가져야 한다. payload가 빈 문자열이면 card는 남겨 수동 edit/save할 수 있지만 Publish는 disabled다.
- 한 card `Regenerate`는 그 platform payload/generation state만 쓰고 다른 7개 payload·saved/status write는 0이다. partial generation 때문에 card 자체를 누락하거나 성공한 7개를 다시 생성하지 않는다.
- card generation/edit 상태는 `generating → draft_nonempty|draft_empty|generation_failed → saved`다. `draft_empty`는 편집 가능·Publish disabled, `generation_failed`는 reason+Regenerate를 제공한다. readiness가 없으면 payload와 무관하게 `disabled`이며 draft/edit/save는 유지하고 credential/review/account 조건을 표시한다.
- publish 상태는 `ready → publishing` 뒤 아래 taxonomy 중 하나로만 전이한다.

| Provider/persistence 관찰 | Card state/action | Adapter invocation 규칙 |
|---|---|---|
| provider-confirmed failure | `failed_confirmed`, `Retry Publish` 허용 | 사용자가 Retry할 때 해당 card adapter=1, 다른 7=0 |
| provider success + persistence failure | `repair_required`, `Repair record` 허용 | persistence repair invocation=1, provider adapter=0, 다른 7=0 |
...
- card Publish는 그 card의 adapter publish invocation 1회만 만든다. Threads처럼 provider protocol이 container-create→publish의 다단계 HTTP를 요구해도 하나의 adapter invocation/dispatch intent로 묶으며 다른 7 adapter invocation은 0, 외부 게시물 결과는 최대 1이다.
- `선택 발행`과 `전체 발행`은 card Publish와 분리된 명시 action이다. 기본 selection은 0이고, target review·confirmation 없이는 실행되지 않으며 화면 진입·초안생성·save가 bulk dispatch를 유발하지 않는다.
...
| FR-28 | 초안생성 1회로 고정 platform ID card 8개를 항상 만들고 partial failure·empty payload·단일 Regenerate를 card별 격리한다 | 부분: preview7/direct4 | Studio/constants audit | 8 IDs exactly; payload 또는 generation_failed; empty Publish disabled; regenerate other7 write0 |
| FR-29 | card Publish/Retry/Repair/Reconcile은 관찰 taxonomy에 따라 선택 card만 안전하게 처리한다 | 부분: direct4, universal group 없음 | Studio/publish audit | confirmed-failure만 adapter retry1; persistence repair adapter0; unknown terminal 전 adapter0 |
| FR-30 | readiness 없는 card도 draft/edit/save하되 Publish disabled+enable 조건을 표시하고 bulk는 별도 명시 action으로 둔다 | 미구현 target | DESIGN-010 | draft 8/8, false-enabled 0, implicit bulk 0 |
...
| NFR-11 Card isolation & uncertainty | generation/edit/publish/retry/repair/reconcile이 platform card별 독립이고 unknown을 success/failure로 추정하지 않음 | 한 card action의 다른7 mutation0; empty publish0; terminal 전 adapter retry0; duplicate0 |
...
| R3 Initial dispatch safety | **외부 Publish 검증은 Threads·Instagram Feed·X 우선**, draft UI는 text cards 8개 전부 | 세 target credential concurrent/retry/permalink E2E + card drafts 8/8 | 세 target별 Publish flag off 가능; 다른 5 card는 draft/edit/save 유지, Publish disabled |
...
|---|---|---|
| AC-01 / OSMU-V421-TC-001 | FR-01 | Given repo inventory, When matrix를 재생성하면, Then text=8/video=3/extensions=15이고 중복 범주가 분리된다. |
| AC-02 / OSMU-V421-TC-002 | FR-02,NFR-08 | Given 현재 shell, When v4 UI를 비교하면, Then sidebar/route/settings/provider-tab orphan·무승인 rename·invented top-level nav가 0이다. |
| AC-03 / OSMU-V421-TC-003 | FR-19,NFR-04 | Given 미검증·장애 target, When 고객이 readiness를 보면, Then enabled=false와 reason/action/owner/evidence time이 보인다. |
| AC-04 / OSMU-V421-TC-004 | FR-01,18 | Given root extensions 15, When code-derived audit를 실행하면, Then 15/15에 runtime loader·credential source·tenant scope·accepted media·external result·permalink·queue compatibility·disposition 8필드가 있고 queue-compatible은 threads/x/instagram 3개뿐이다. |
| AC-05 / OSMU-V421-TC-005 | FR-08 | Given 동일 signed state와 동일 nonce cookie를 가진 callback 20개, When barrier로 동시에 실행하면, Then nonce atomic consume·provider token endpoint 호출·account write가 각각 exactly 1이고 나머지 19개는 token endpoint 전 replay error로 거절된다. |
| AC-06 / OSMU-V421-TC-006 | FR-09 | Given 연결 account, When four surfaces를 보면, Then identity/scope/state/verified-at/CTA diff가 0이다. |
| AC-07 / OSMU-V421-TC-007 | FR-09,NFR-01 | Given provider에 같은 브라우저 세션의 계정 2개, When 목표를 선택·연결하면, Then 저장·review·발행 account가 목표와 같고 cross-tenant call 0이다. |
| AC-08 / OSMU-V421-TC-008 | FR-03 | Given Studio/queue/schedule/video source, When 생성하면, Then tenant-scoped stable source identity와 provenance가 있다. |
| AC-09 / OSMU-V421-TC-009 | FR-04 | Given Studio draft, When Inbox/Calendar 대상이 되면, Then 명시적 bridge 하나가 같은 source를 가리키며 복제 orphan이 없다. |
| AC-10 / OSMU-V421-TC-010 | FR-05 | Given M0 snapshot, When M1 backfill→M2 dual-write→M3 shadow-read→M4 7일 parity→M5 canary→M6 canonical→M7 writer freeze→M8 fallback removal을 진행하면, Then 각 단계의 writer/read authority가 §13.1과 일치하고 exit 전 다음 단계 진입이 0이다. |
| AC-11 / OSMU-V421-TC-011 | FR-06,23,NFR-06 | Given M2~M7 각 단계 한쪽 write failure와 M5 이후 신규 write 100건, When reverse replay 뒤 JSON legacy read로 rollback하면, Then 100/100 source·state·result가 조회되고 record loss·duplicate external call·silent drift가 0이다. |
| AC-12 / OSMU-V421-TC-012 | FR-07 | Given multi-target source, When 한 variant만 수정/승인하면, Then 다른 target payload/approval overwrite가 0이다. |
| AC-13 / OSMU-V421-TC-013 | FR-10,11,NFR-02 | Given no-draft 포함 동일 intent 20개, When 동시에 dispatch하면, Then provider external result가 최대 1개다. |
| AC-14 / OSMU-V421-TC-014 | FR-12,NFR-03 | Given provider success 후 persistence failure, When repair하면, Then republish 0이고 기존 provider result가 저장된다. |
| AC-15 / OSMU-V421-TC-015 | FR-13 | Given timeout/unknown, When reconcile하면, Then enabled text adapter 8개 각각 provider ID·permalink 또는 actionable terminal reason을 회수한다. |
| AC-16 / OSMU-V421-TC-016 | FR-14 | Given 3개 target의 mixed result, When source를 열면, Then 하나의 group에서 target별 status/provider ID/link/updated time이 독립 표시된다. |
| AC-17 / OSMU-V421-TC-017 | FR-15 | Given success 2·failed 1, When retry를 누르면, Then failed 1만 호출되고 success duplicate 0이다. |
| AC-18 / OSMU-V421-TC-018 | FR-21,NFR-04 | Given provider ID/permalink 없음, When 결과를 표시하면, Then provider home URL을 게시물 링크로 표시하지 않는다. |
| AC-19 / OSMU-V421-TC-019 | FR-16 | Given accepted video, When terminal 전 조회하면, Then processing이고 published가 아니다; terminal 뒤만 실제 link와 final status다. |
| AC-20 / OSMU-V421-TC-020 | FR-17 | Given YouTube upload success/timeout, When 저장·reconcile하면, Then TikTok/Reels와 같은 result fields와 duplicate 0을 충족한다. |
| AC-21 / OSMU-V421-TC-021 | FR-16,17 | Given video targets 3, When capability E2E를 실행하면, Then 각 target은 enabled proof 또는 disabled reason/action 중 하나이고 false-success 0이다. |
| AC-22 / OSMU-V421-TC-022 | FR-18 | Given F3, When extensions를 판정하면, Then 15/15 disposition과 owner/proof가 있고 root-only 미검증 고객 노출 0이며 한 번에 승인된 1개만 repair한다. |
| AC-23 / OSMU-V421-TC-023 | FR-20,22,NFR-07 | Given revoke 또는 incident, When dispatch/trace하면, Then 새 external call 0, evidence 보존, source→provider 단계·owner·action 식별이 된다. |
| AC-24 / OSMU-V421-TC-024 | FR-24,NFR-01,05 | Given 2 tenants×2 accounts와 synthetic private payload, When read/write/publish/log/analytics를 검사하면, Then cross-tenant와 raw private/secret leak가 0이다. |
| AC-25 / OSMU-V421-TC-025 | FR-27,F4/BM,NFR-10 | **Start:** Given §14.5 회장 승인, When 운영자가 명시적 `Start F4`를 한 번 실행하면, Then immutable `f4_started_at` exactly 1이고 그 전 contact/collect/pay는 0이다. **Branch A:** Given delivery=`delivered`인 qualified contacted count와 `f4_started_at`, When delivered unique 100 소진 또는 `f4_started_at+30d` 중 먼저 도달하고 consented workspace <10이면, Then F1·F2·F3·paid acquisition `stopped`, 신규 work/예산 0, count/start/cutoff/flag snapshot이 종료증거이며 first qualified evidence/lock time 사용은 0이다. fixture `day30, qualified=0, consented=0`도 동일 stop이다. **Branch B:** Given consented 10, When 성숙한 activation/repeat/payment 창을 평가하면, Then activation≥3/repeat≥3/actual-paid≥2 중 하나라도 미달 시 동일 stop이고 전부 충족해도 자동 다음-bet 진입 0이다. 각 attempt outcome은 append되며 failed/bounced/blocked는 contacted 분모 0, audit 보존이다. |
| AC-26 / OSMU-V421-TC-026 | 전체 | Given PLAN-007·DESIGN-006·v4 sources, When RTM을 검사하면, Then orphan 0이고 v3.1.1/DESIGN v6가 target 근거로 연결된 행은 0이다. |
| AC-27 / OSMU-V421-TC-027 | FR-25,NFR-09 | Given 회장이 §14.5를 승인하고 unchecked consent를 한 외부 고객, When export 또는 해지/삭제를 요청하면, Then raw 30일·evidence 180일 범위와 provider/AI/subprocessor/책임을 사전 표시하고 active store는 7일 내 삭제되며 철회 후 dispatch 0이다. 삭제 시각부터 30일이 지난 backup snapshot을 restore/read하면 대상 raw/evidence가 없거나 cryptographic erasure로 unreadable하고, legal hold 예외는 사전 통지된 범위·근거·기간과 access audit가 있다. 승인 전 cohort create/collect/pay는 0이다. |
| AC-28 / OSMU-V421-TC-028 | FR-26,NFR-10 | Given 52h appetite와 running work item이 누적 1,679분에서 시작, When active minute가 1,680에 도달하면, Then circuit breaker가 다음 provider/API/migration batch/file mutation 전 발동하고 current atomic transaction을 commit/rollback한 뒤 safe checkpoint 5필드를 기록한다. 신규 R0~R3와 F1~F4 `stopped`, 1,681분 side effect 0이며 ledger planned/actual/evidence·Threads/IG/X 범위·X readiness도 유지된다. |
| AC-29 / OSMU-V421-TC-029 | FR-28,NFR-11 | Given generation fixture가 6 valid payload·1 generation error·1 empty payload를 반환, When `초안생성`을 1회 실행하면, Then 고정 IDs `threads,x,facebook,instagram,bluesky,telegram,discord,slack` card exactly 8개가 존재한다. 각 card는 editable payload 또는 `generation_failed+reason+Regenerate`이며 empty payload card Publish는 disabled다. 실패 card 하나를 Regenerate하면 그 card write=1, 다른 7 payload/saved/status write=0이고 edit/save도 동일 격리를 유지한다. |
| AC-30 / OSMU-V421-TC-030 | FR-29,NFR-02,11 | Given card별 adapter/repair/reconcile spy, When (A) provider-confirmed failure card의 Retry를 누르면 선택 adapter=1·다른7=0, (B) provider success+persistence failure에서 Repair하면 repair invocation=1·provider adapter=0·다른7=0, (C) timeout/unknown이면 reconcile invocation≥1·terminal 전 provider adapter=0이고 성공/확정실패 판정 뒤에만 published/failed_confirmed로 전이한다. 최초 card Publish는 선택 adapter=1·다른7=0이며 published card Retry는 기존 result reconcile·adapter=0·duplicate=0이다. |
| AC-31 / OSMU-V421-TC-031 | FR-30,NFR-04,11 | Given readiness가 있는 initial 3과 readiness 없는 나머지 card fixture, When 초안을 생성하면, Then draft/edit/save card는 8/8이고 unavailable card Publish만 disabled+credential/review/account 조건을 표시한다. 화면 진입·generate·save의 bulk call은 0이며 `선택 발행`/`전체 발행`은 별도 action·target review·confirm 뒤에만 실행된다. |
...
| DESIGN-010 user-confirmed work model | FR-28~30,NFR-11 | 029~031 | 8 cards, independent save/publish/result, explicit bulk |
| as-is Studio/API drift | FR-01,28~30 | 001,029~031 | preview7/direct4/API8를 현재와 target으로 분리 |
| v4.2.0 critic partial generation | FR-28,NFR-11 | 029 | fixed IDs8, 6+failed+empty fixture, single regenerate isolation |
| v4.2.0 critic retry taxonomy | FR-29,NFR-02,11 | 030 | confirmed failure retry / persistence repair / unknown reconcile 분리 |
...
|---:|---|---|---|---|
| 1 | 용어 통일 | **PASS — v4.1.2 closure 보존** | §14.4 timer/cohort 정의 + §8.1 card/Publish/selected/bulk 정의 | card Publish와 bulk Publish, preview와 draft card를 별도 용어로 고정 |
...
| 4 | 정합성 | **PASS** | initial safety 3, target draft cards 8, Studio preview7/direct4/API8, video3/extensions15, AC/TC 31쌍을 분리 | safety 우선순위를 UI card 범위 축소로 사용하지 않고 as-is/target count 혼합 0 |
| 5 | 정책 상세 | **PASS — v4.2.0 FAIL→v4.2.1 closure** | 기존 정책 + partial generation/empty/Regenerate와 confirmed-failure/repair/unknown taxonomy | AC-29~30에 6+1+1 fixture와 adapter/repair/reconcile call count가 생긴 뒤에만 PASS |
| 6 | 추출 철저 | **PASS** | source→8 cards→partial failure regenerate→edit/save→Publish→retry/repair/reconcile→status/permalink가 FR-28~30·AC-29~31에 연결 | failure·unknown step orphan 0; rejected DESIGN v6 target link 0 |
...
SKILLS_SKIPPED: `brand-positioning-kit` 등은 본 과업이 브랜드 포지셔닝이 아니라 UI failure-state 계약 기반 PRD PATCH이므로 미적용
