...
<!--
STAMP
created_at: 2026-08-06 22:30 KST
model: gpt-codex/gpt-5.6-sol
agent: prd-architect / marketing_agent_prd_v6
...
deliberation: 발행기 기능을 버리지 않으면서도 발견→학습→행동의 고객가치 폐루프를 One Thing 하나로 묶는 것이 핵심이었다.
-->

| 항목 | 값 |
|---|---|
| 버전 | v6.0.0 (MAJOR — 제품 목적을 Publisher에서 Marketing Agent로 재정의) |
| 작성일 | 2026-08-06 |
...
OSMU는 코딩·자동화 역량이 없는 1인 사업자가 자기 브랜드의 **검증된 사실**과 시장·성과 신호를 바탕으로 다음 7일의 마케팅 기회를 고르고, 원본 하나를 채널별 콘텐츠로 만들고, 사람이 승인한 것만 발행하며, 실제 결과를 다음 실험으로 되돌리는 **Marketing Agent**다. 제품 루프는 `Discover → Plan → Create → Review → Publish → Measure → Learn → Act`이며, 기존 25개 라우트·26개 사이드바·provider별 화면·text 8 + video 3·OSMU 발행/예약/복구 기능을 제거하지 않고 상위에 Agent Command Center와 Weekly Marketing Plan을 더한다. 현행 구현은 브랜드 wiki 생성, Studio, 발행·예약, Threads 중심 반응/성장/인기/스타일 학습이 부분 존재하지만, tenant-scoped 단일 학습 폐루프와 운영 provider E2E는 아직 미검증이다.
...
- Agent Command Center: 오늘/이번 주 목표, 기회, 승인 대기, 장애, 성과, 추천 행동을 한곳에서 요약한다.
- Weekly Marketing Plan: 목표·audience·offer·campaign과 7일 실행안을 근거·가정·승인상태와 함께 관리한다.
- Brand Truth onboarding: website/GitHub URL sync와 GitHub 없는 고객의 fact editor를 모두 지원한다.
- Opportunity Inbox: trend, keyword, popular content, owned analytics, customer signal을 출처·수집시각·표본·신뢰도와 함께 제시한다.
- Cross-platform OSMU campaign workspace: 한 campaign brief에서 text 8과 video 3 산출물을 만들고, provider별 capability를 숨기지 않는다.
- Review/approval, 즉시/예약 발행, permalink/result, 502·timeout·persistence failure의 복구 계약.
- Campaign analytics: provider metric의 정의·출처·수집시각·결측 이유를 보존하고 campaign goal에 연결한다.
- Insight → next experiment: 성과 패턴을 fact/inference/unverified로 구분하여 다음 실험을 추천하고 승인 후 계획에 반영한다.
- Alerts와 safe actions: reversible action은 정책 범위에서 자동화 가능하되, irreversible action은 명시 승인한다.
- 기존 route·sidebar·asset·theme·role·provider family의 fidelity 보존 및 390/1024 접근성 회귀 방지.
...
### 2.3 Preservation contract — additive, not replacement
...
| “매주 하나의 근거 있는 마케팅 판단을 실행하고 다음 판단으로 학습” | 폐루프와 고객 결과를 하나로 묶음 | 초기 신호가 적을 때 추천 품질이 낮을 수 있음 | 채택; 표본 부족을 명시하고 보류 허용 |

### 4.2 One Thing

> **OSMU는 고객이 매주 하나의 근거 있는 마케팅 판단을 승인·실행하고, 실제 결과를 다음 주 더 나은 판단으로 되돌리게 한다.**

“그리고” 없이 판단→실행→학습이라는 하나의 반복 단위를 정의한다. 콘텐츠 수와 채널 수는 이 반복을 위한 수단이다.
...
MVP는 기능 수가 아니라 One Thing을 한 번 완결하는 5개 capability다.

| MVP | One Thing 연결 | 포함 | 제외/제약 | 종료증거 |
|---|---|---|---|---|
| M1 Brand Truth + Marketing Brief | “근거 있는 판단”의 입력 | website/GitHub URL, no-GitHub fact editor, fact/inference/unverified, goal/audience/offer/campaign | 자동으로 사실 확정 금지 | confirmed facts와 citations를 쓴 brief 1건, cross-tenant leak 0 |
| M2 Opportunity Inbox + Weekly Plan | 매주 “하나의 판단” | source truth를 가진 opportunity와 7일 plan, plan 승인 | enterprise listening 전체 범위 제외 | 45분 내 opportunity→approved plan 1회 |
| M3 OSMU Campaign Workspace | 판단을 “실행”으로 번역 | text8 + video3 inventory, provider-specific variant/edit/validation, current surfaces deep-link | 모든 11개 direct publish 약속 금지 | 11 output truth + supported CTA만 노출, unsupported 0 |
| M4 Approval→Publish→Result/Recovery | 승인한 판단을 안전하게 실행 | canonical account/status, now/schedule, permalink, partial/uncertain/repair/reconcile, 502 trace | ads/DM/delete 자동화 제외 | wrong-account 0, same intent external call≤1, terminal result |
| M5 Campaign Analytics→Experiment | 결과를 “다음 판단”으로 환류 | metric provenance, campaign report, tenant learning, next experiment, safe alerts/actions | 인과 추정 확정·가짜 metric 금지 | insight 또는 sample-hold 1개와 next experiment 승인 1개 |
...
| FR-MA-002 | website/GitHub URL/Paste/Fact Editor 4가지 brand source onboarding | 부분구현; brand-grounding repo/paste/wizard | 4 source type가 source health·updated_at과 함께 표시 | Must |
...
| FR-MA-010 | irreversible action은 content/account/time/channel hash 건별 승인 | 미구현 | hash delta 시 execution0·approval invalidated | Must |

### 9.2 Discover and Plan

| FR | 요구 | 현재/출처 | Fit Criterion | Priority |
|---|---|---|---|---|
| FR-MA-011 | Opportunity Inbox가 trend/keyword/popular/owned signal을 통합 | 부분구현; search/keyword/analytics 분리 | 4 source family를 source·time·scope·confidence와 표시 | Must |
...
| FR-MA-015 | Weekly Plan이 7일, campaign 1~3개, hypothesis·owner·status를 관리 | 미구현 | approved plan은 7 dates와 each campaign hypothesis 1개 | Must |
...
| FR | 요구 | 현재/출처 | Fit Criterion | Priority |
|---|---|---|---|---|
| FR-MA-018 | campaign source에서 text8+video3 output inventory 생성 | 부분구현; Studio/API/video | 11 named outputs; capability false는 disabled/external/handoff | Must |
...
| FR-MA-021 | source→brief→variant→approval→result campaign lineage 유지 | 미구현 | 모든 output/result가 campaign/source/approval deep-link 가능 | Must |
...
| FR-MA-023 | Inbox legacy queue와 campaign approvals를 origin label로 구분 | 부분구현; surface map | legacy/campaign origin 100% 표시, 가짜 merge 0 | Must |
...
| FR | 요구 | 현재/출처 | Fit Criterion | Priority |
|---|---|---|---|---|
| FR-MA-025 | approved intent만 now/schedule execute | 부분구현 | unapproved/mutated/expired intent adapter call0 | Must |
...
| FR-MA-028 | partial 결과를 card/account별 독립 terminal state로 표시 | 부분구현 | 3 targets 중 1 fail 시 2 success 보존, failure actionable | Must |
...
| FR-MA-032 | schedule create/change/cancel/due는 timezone/CAS/lease/revoke를 처리 | 부분구현 | DST gap/fold confirmation; cancel/revoke due call0; race≤1 | Must |
...
| FR-MA-035 | provider간 metric은 같은 정의일 때만 비교·집계 | 미구현 | incompatible metric aggregation0, separate labels | Must |
...
| FR-MA-038 | Threads style/popular learning을 tenant-scoped store로 격리 | 미구현; legacy local files | A edits/performance affect B generation 0/100; current legacy label | Must |
...
| FR-MA-041 | alert를 severity·owner·action·expiry로 분류 | 부분구현; alert routes | every alert has next action and terminal ack/resolve | Should |
...
|---|---|---|
| FR-MA-001 | AC-MA-001 tenant isolation | **MA-V6-TC-001 H:** Given A workspace facts/signals/campaigns, When A reads/learns/publishes, Then A rows only. **F:** Given B IDs, When A attempts read/write/learn/publish, Then 403, adapter0, leak0. **E:** DB/RLS trace+API body+DOM+audit. |
| FR-MA-002 | AC-MA-002 source onboarding4 | **MA-V6-TC-002 H:** Given valid website/GitHub/paste/manual fact, When each connects, Then four sources show health/time. **F:** Given unsupported URL/empty fact, When saved, Then actionable error, partial source not active. **E:** browser video+network+source record. |
| FR-MA-003 | AC-MA-003 truth classes | **MA-V6-TC-003 H:** Given confirmed fact, When drafting, Then claim may be used with citation. **F:** Given inference/unverified claim, When approval attempted, Then blocked until confirmation/removal. **E:** fact-state audit+review DOM. |
| FR-MA-004 | AC-MA-004 citations | **MA-V6-TC-004 H:** Given 3 source facts, When content/recommendation renders, Then every factual claim links source/path. **F:** Given missing citation, Then unverified blocker and publish disabled. **E:** claim-citation matrix+DOM links. |
| FR-MA-005 | AC-MA-005 source errors | **MA-V6-TC-005 H:** Given valid repo/token/ref, When sync, Then files and timestamp. **F:** Given 403/404/rate-limit/bad ref/no-md/decrypt/network/secret-key faults, Then 8 distinct reason/action, raw secret0. **E:** fixture responses+log scan+screens. |
| FR-MA-006 | AC-MA-006 account truth diff0 | **MA-V6-TC-006 H:** Given canonical B connected, When Settings/Studio/channel/Review load, Then B/state/time diff0. **F:** Given stale projection, Then refresh/reconnect_required, never false connected. **E:** four-surface screenshots+API diff. |
| FR-MA-007 | AC-MA-007 wrong account | **MA-V6-TC-007 H:** Given provider session A and target B, When user reauthenticates and callback completes, Then canonical B, B adapter1, A0. **F:** cancel/mismatch retains prior state and shows reconnect action. **E:** secret-window video+provider handle+trace. |
| FR-MA-008 | AC-MA-008 manual recovery | **MA-V6-TC-008 H:** Given OAuth unavailable and user opens Advanced, When valid token verifies, Then one canonical account masked. **F:** invalid token produces failed reason, raw token0, OAuth truth unchanged. **E:** DOM+network+redaction scan. |
| FR-MA-009 | AC-MA-009 autonomy audit | **MA-V6-TC-009 H:** Given L2 workspace, When agent drafts, Then internal draft allowed and audit records requested/effective level. **F:** publish attempt at L2 yields adapter0. **E:** policy matrix tests+audit. |
| FR-MA-010 | AC-MA-010 approval hash | **MA-V6-TC-010 H:** Given approved account/content/time/channel hash, When L3 executes before expiry, Then exactly approved intent. **F:** mutate any field/expire approval, Then invalidated and adapter0. **E:** approval record+hash diff+adapter spy. |
| FR-MA-011 | AC-MA-011 opportunity inbox | **MA-V6-TC-011 H:** Given four healthy source families, When refresh, Then cards include source/time/scope/confidence. **F:** one source down, Then others remain and failed source is named. **E:** source fixtures+Opportunity DOM. |
| FR-MA-012 | AC-MA-012 no false zero | **MA-V6-TC-012 H:** Given native metric=0, When shown, Then zero with source/time. **F:** permission/stale/rate-limit/NA, Then named state not numeric zero. **E:** 5 fixture screenshots+API payloads. |
| FR-MA-013 | AC-MA-013 explainable rank | **MA-V6-TC-013 H:** Given goal/audience/offer and 10 opportunities, When ranked, Then every card has evidence and relevance reason. **F:** evidence missing, Then card unverified and cannot top-rank. **E:** rank table+citations. |
| FR-MA-014 | AC-MA-014 brief completeness | **MA-V6-TC-014 H:** Given all 7 brief fields, When approve, Then immutable approved version. **F:** any required field blank, Then field error and approval0. **E:** browser form+record diff. |
| FR-MA-015 | AC-MA-015 weekly plan | **MA-V6-TC-015 H:** Given approved brief/opportunities, When plan generated, Then 7 dates, 1–3 campaigns, hypothesis/owner/status. **F:** over3 campaign or outside window, Then revise required. **E:** plan artifact+DOM. |
| FR-MA-016 | AC-MA-016 honest hold | **MA-V6-TC-016 H:** Given sufficient confirmed facts, When plan drafts, Then cited claims only. **F:** required price/outcome unverified, Then plan holds and requests confirmation, invented value0. **E:** generated JSON+fact audit. |
| FR-MA-017 | AC-MA-017 command center | **MA-V6-TC-017 H:** Given due approval/failure/result/action, When dashboard loads, Then priority cards deep-link to owners. **F:** target missing/permission denied, Then terminal message not dead-end. **E:** click/key route log+screens. |
| FR-MA-018 | AC-MA-018 output11 truth | **MA-V6-TC-018 H:** Given campaign brief, When create opens, Then text8+video3 names and capability states. **F:** provider unsupported/unconnected, Then disabled/external/handoff, direct CTA0. **E:** inventory DOM+capability snapshot. |
| FR-MA-019 | AC-MA-019 validation no mutation | **MA-V6-TC-019 H:** Given valid variants, When validate, Then per-provider pass. **F:** overlimit/media fault, Then only that card blocked and original unchanged. **E:** byte diff+validation UI. |
| FR-MA-020 | AC-MA-020 isolated edits | **MA-V6-TC-020 H:** Given 11 variants, When one edits/saves/reloads, Then one diff and persistence. **F:** stale version save, Then conflict terminal, overwrite0. **E:** before/after payload+DOM. |
| FR-MA-021 | AC-MA-021 lineage | **MA-V6-TC-021 H:** Given source→campaign→variant→approval→result, When any node opens, Then bidirectional navigation. **F:** orphan node, Then reconcile label, fabricated parent0. **E:** lineage graph+route log. |
| FR-MA-022 | AC-MA-022 review completeness | **MA-V6-TC-022 H:** Given ready card, When review opens, Then citations/account/capability/time/preview present. **F:** stale account/capability, Then approval disabled with remediation. **E:** review checklist DOM+API timestamp. |
| FR-MA-023 | AC-MA-023 inbox origins | **MA-V6-TC-023 H:** Given legacy and campaign items, When Inbox filters, Then origin labels and correct owner links. **F:** missing lineage, Then reconcile state not campaign success. **E:** seeded UI+route logs. |
| FR-MA-024 | AC-MA-024 video handoff | **MA-V6-TC-024 H:** Given video3 draft, When handoff, Then `/videos` receives script/asset and returns readiness. **F:** provider/TTS unavailable, Then no publish CTA and actionable reason. **E:** browser route/state trace. |
| FR-MA-025 | AC-MA-025 approved execute | **MA-V6-TC-025 H:** Given valid approval, When now/due runs, Then only approved targets dispatched. **F:** missing/mutated/expired approval, Then job blocked and adapter0. **E:** ledger+adapter trace. |
| FR-MA-026 | AC-MA-026 idempotency20 | **MA-V6-TC-026 H:** Given one intent, When 20 concurrent requests, Then one external call and shared result. **F:** replay after success, Then call0 and stored result. **E:** concurrency trace+provider fixture count. |
| FR-MA-027 | AC-MA-027 published proof | **MA-V6-TC-027 H:** Given provider success+readback, When result renders, Then external ID/permalink/time. **F:** response lacks proof, Then not “published”; uncertain/failed label. **E:** provider response+DOM link. |
| FR-MA-028 | AC-MA-028 partial truth | **MA-V6-TC-028 H:** Given 3 targets with 2 success/1 fail, When result loads, Then two links retained and one actionable fail. **F:** aggregate success label attempted, Then contract test fails. **E:** result payload+screen. |
| FR-MA-029 | AC-MA-029 confirmed retry | **MA-V6-TC-029 H:** Given pre-dispatch 502 proven, When user approves retry, Then one new attempt. **F:** phase unknown, Then retry disabled and adapter0. **E:** phase trace+button state+call count. |
| FR-MA-030 | AC-MA-030 reconcile first | **MA-V6-TC-030 H:** Given timeout/uncertain, When reconcile finds provider post, Then link recorded without repost. **F:** user presses retry before reconcile, Then blocked/call0. **E:** provider lookup+ledger+call count. |
| FR-MA-031 | AC-MA-031 repair no repost | **MA-V6-TC-031 H:** Given provider success/DB fail, When repair, Then existing external result persisted. **F:** repair path tries publish adapter, Then test fail/call0. **E:** fault injection+DB row+adapter spy. |
| FR-MA-032 | AC-MA-032 schedule safety | **MA-V6-TC-032 H:** Given valid IANA time, When create/change/due, Then latest version executes once. **F:** DST gap/fold/cancel/revoke/lease loss, Then confirmation or adapter0. **E:** clock/lease fixtures+trace. |
| FR-MA-033 | AC-MA-033 incident trace | **MA-V6-TC-033 H:** Given 502, When surfaced, Then safe customer message+correlation; operator sees phase/upstream/version/impact. **F:** secret/external body appears, Then redaction test fails release. **E:** screen+structured log+secret scan. |
| FR-MA-034 | AC-MA-034 metric provenance | **MA-V6-TC-034 H:** Given collected metric, When displayed, Then native name/definition/source/time. **F:** any provenance field missing, Then metric hidden or NA. **E:** report schema+DOM. |
| FR-MA-035 | AC-MA-035 comparison integrity | **MA-V6-TC-035 H:** Given compatible metrics, When compared, Then definition and normalization visible. **F:** incompatible views/reach, Then aggregate disabled and separate rows. **E:** metric dictionary test+screen. |
| FR-MA-036 | AC-MA-036 campaign analytics | **MA-V6-TC-036 H:** Given campaign publications, When report opens, Then goal metric/window and per-post lineage. **F:** orphan metric, Then excluded with reason. **E:** campaign report+lineage query. |
| FR-MA-037 | AC-MA-037 sample-aware insight | **MA-V6-TC-037 H:** Given threshold-met sample, When learn runs, Then evidence/confidence/limitation. **F:** below threshold, Then “판정 보류”, causal claim0. **E:** sample fixtures+generated insight audit. |
| FR-MA-038 | AC-MA-038 tenant learning | **MA-V6-TC-038 H:** Given A edit/performance, When A next draft, Then A evidence may influence. **F:** When B drafts, Then A markers0/100. **E:** seeded A/B generation diff+query trace. |
| FR-MA-039 | AC-MA-039 experiment fields | **MA-V6-TC-039 H:** Given insight, When experiment drafts, Then hypothesis/baseline/target/window/one variable. **F:** two changed variables/missing rule, Then approval blocked. **E:** experiment record+form errors. |
| FR-MA-040 | AC-MA-040 loop closure | **MA-V6-TC-040 H:** Given approved experiment, When next plan drafts, Then experiment linked and scheduled. **F:** rejected/expired experiment, Then no automatic plan mutation. **E:** bidirectional links+audit. |
| FR-MA-041 | AC-MA-041 alert lifecycle | **MA-V6-TC-041 H:** Given severe incident, When alert creates, Then severity/owner/action/expiry and ack/resolve. **F:** duplicate same incident≤30m, Then coalesced. **E:** alert records+notification screenshot. |
| FR-MA-042 | AC-MA-042 safe actions only | **MA-V6-TC-042 H:** Given approved internal plan update/metric refresh, When policy permits, Then executed/audited. **F:** auto reply/DM/delete/spend/account change, Then blocked/call0. **E:** policy test+external adapter spies. |
| FR-MA-043 | AC-MA-043 route preservation | **MA-V6-TC-043 H:** Given 1024 shell, When 26 destinations click+keyboard, Then current 25-route owners resolve. **F:** missing/renamed/collapsed owner, Then manifest test fails. **E:** route manifest+video+logs. |
| FR-MA-044 | AC-MA-044 additive IA | **MA-V6-TC-044 H:** Given Command Center action, When opened, Then deep-links existing owner with context. **F:** generic replacement panel or dead-end, Then fidelity test fails. **E:** before/after owner map+route log. |
| FR-MA-045 | AC-MA-045 visual/role fidelity | **MA-V6-TC-045 H:** Given light/dark and three roles, When surfaces render, Then icons/tokens/shells/tabs8/9 parity. **F:** customer sees operator Video/TTS or generic icon, Then fail. **E:** paired screenshots+DOM inventory. |
| FR-MA-046 | AC-MA-046 responsive access | **MA-V6-TC-046 H:** Given 390/1024, When 25 routes and 26 nav exercised, Then overflow0, target≥44, focus visible. **F:** hidden nav/overflow/dead-end, Then release blocked. **E:** Playwright measurements+videos. |
| FR-MA-047 | AC-MA-047 terminal states | **MA-V6-TC-047 H:** Given loading/empty/error/permission/disabled/external fixtures, When route loads, Then named terminal/action. **F:** loading exceeds timeout/no action, Then fail. **E:** route-state matrix+screens. |
| FR-MA-048 | AC-MA-048 production gate | **MA-V6-TC-048 H:** Given secret-window new customer, When connect→plan→draft→approve→real publish→measure, Then exact account, permalink, console/5xx0. **F:** any wrong account/false-success/duplicate/502 unresolved, Then release blocked. **E:** timestamped video+provider permalink+logs+deployment SHA. |
...
- V6 test plans are registered as `MA-V6-TC-001..048` in `docs/qa-tracker.md`; all begin ⬜ and must not inherit v5.2 PASS.
...
| Weekly Marketing Plan | 7-day plan, campaign 1–3, hypothesis, status | Calendar를 대체하지 않고 scheduled actions로 deep-link |
...
### 12.2 Platform capability contract
...
### 12.3 Responsive, theme, asset, role hard gates
...
범위는 internal 1 workspace + opt-in external 3 workspaces 이하, text direct-supported cohort 우선이다. Week 1은 M1–M3으로 brand truth→weekly plan→review-ready campaign을 완결한다. Week 2는 M4–M5로 실제 지원 provider 한 곳 이상을 secret-window에서 publish→permalink→metric/hold→next experiment까지 닫는다. video3는 truthful handoff와 readiness까지만 cohort success에 포함하며, 외부 실발행 capability가 증명된 provider만 확장한다.

### 13.3 Roadmap

| Phase | Scope | Entry | Exit |
|---|---|---|---|
| R0 Plan | PRD v6 + critic | current file | MAJOR0 + chairman decisions + approve plan |
| R1 Trust foundation | brand truth classes/citations, canonical accounts, autonomy/approval | plan approved | A/B leak0, false-success0, review truth |
| R2 Weekly loop | command center, opportunity inbox, weekly plan, campaign lineage | R1 | plan≤45m and campaign review ready |
| R3 Safe execution | publish/schedule/result/recovery/502 trace | R2 | real provider permalink, duplicate/wrong0 |
| R4 Learning | campaign analytics, tenant learning, experiment, alerts/safe actions | R3 | one complete weekly loop per cohort |
| R5 Expansion | more direct providers/video publish/listening depth | kill criteria pass | provider-by-provider production evidence |
...
### 18.1 회장 결정 필요 — critic 전에 임의 확정 금지

| # | 결정 | 추천안 | 선택 시 결과 | 미선택 리스크 |
|---|---|---|---|---|
| O-1 | Command Center를 current `/` 진화로 할지 새 route로 둘지 | `/` additive evolution | 26 sidebar 유지, existing home metrics 재사용 | 새 route는 sidebar 27 여부와 preservation 충돌 |
| O-2 | 첫 external cohort provider | Threads 1개 + Instagram OAuth truth 병행 검증 | legacy learning과 가장 가까우며 wrong-account risk 직접 해소 | 다중 provider 동시 시작 시 2주 loop 증명 실패 가능 |
| O-3 | L3 기본값 | default L2, workspace opt-in L3 | 신뢰 확보 후 실행 확대 | default L3는 false-success/502 open 상태와 충돌 |
| O-4 | website source 수집 범위 | 홈페이지+사용자 선택 URL ≤20개 | 저작권/비용/오염 통제 | 무제한 crawl은 사실 신뢰·운영비 악화 |
| O-5 | metric sample threshold | provider별 metric dictionary를 eng-design에서 합의 | fake insight 차단 | 단일 임의 숫자는 채널 차이 왜곡 |

### 18.2 Revision history

| 버전 | 날짜 | 변경 | 작성자 |
|---|---|---|---|
| v6.0.0 | 2026-08-06 | publisher 중심 v5.2를 Marketing Agent 8-stage loop로 MAJOR 재정의; 원요구·truth·48 RTM 복원 | prd-architect/gpt-codex |
...
- 회장 decisions O-1..O-5: **미결**.
- design/eng-design/build/QA/ship: **진입 불가**.
- V6 TC 48: **계획 등록만, 전부 미실행**.

---

RUBRIC_SCORE: 완결성=5/5 정밀성=5/5 벤치마크=5/5 추적성=5/5 전문성=4/5 total=24/25
WEAKEST_LINE: “Command Center를 current `/` 진화로 할지 새 route로 둘지” — IA 선택은 회장·design 합의 전이라 의도적으로 미결이다.
