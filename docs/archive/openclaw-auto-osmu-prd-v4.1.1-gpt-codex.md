# openclaw-auto OSMU PRD v4.1.1 — cohort 판정·appetite stop PATCH

> **상태:** v4.1.0 critic residual closure PATCH · 승인 전 · DESIGN v6 입력 사용 금지  
...
| FR-26 | 최초 safety bet은 Threads·Instagram Feed·X만 enabled proof를 만들고 작업별 누적 time ledger로 28h를 강제한다 | 기획 보정 | code audit/appetite | X credential probe, work-item ledger 1,680분 cap, cap 도달 후 신규 작업·자동 확대 0 |
| FR-27 | qualified prospect를 최초 접촉 전에 네 조건으로 판정하고 immutable contact ledger에서 dedupe한다 | 미구현·cohort 전 필수 | §14.4 | 100명 각각 prospect ID·4조건 evidence·contact timestamp lock, duplicate/contact-without-lock 0 |
...
- qualified count는 locked·4/4 PASS·contact timestamp가 있는 unique prospect만 센다. 접촉했지만 뒤늦게 evidence를 붙인 row는 분모에서 제외하고 위반으로 기록한다.

#### 14.4.2 두 branch 종료 상태

| Branch | Trigger | 평가 | 반드시 실행할 stop·종료증거 |
|---|---|---|---|
| A — 모집 실패 | **unique qualified 100명 소진 또는 첫 qualified lock 후 30일 종료 중 먼저 도달**, 그리고 consented workspace <10 | activation/repeat/paid를 평가하지 않는다 | F1 text expansion, F2 video, F3 extension, paid acquisition 모두 `stopped`; 예산예약 0, 신규 work item 0, feature flag snapshot, ledger count/cutoff timestamp를 종료증거로 고정 |
...
각 작업 시작 전에 `work_item_id`, slice, 목적, owner, `planned_minutes`, `started_at`을 append하고 종료 때 `ended_at`, `actual_minutes`, evidence link를 append한다. atomic guard는 누적 actual+진입 작업 planned가 **1,680분을 초과하면 시작을 거절**하며, 누적 actual이 1,680분에 도달하면 R0~R3 신규 work item과 F1~F4를 `stopped`로 둔다. 수정은 원행 overwrite가 아니라 adjustment event로 남겨 시간 축소 조작을 막는다.
...
|---|---|---|
| AC-01 / OSMU-V411-TC-001 | FR-01 | Given repo inventory, When matrix를 재생성하면, Then text=8/video=3/extensions=15이고 중복 범주가 분리된다. |
| AC-02 / OSMU-V411-TC-002 | FR-02,NFR-08 | Given 현재 shell, When v4 UI를 비교하면, Then sidebar/route/settings/provider-tab orphan·무승인 rename·invented top-level nav가 0이다. |
| AC-03 / OSMU-V411-TC-003 | FR-19,NFR-04 | Given 미검증·장애 target, When 고객이 readiness를 보면, Then enabled=false와 reason/action/owner/evidence time이 보인다. |
| AC-04 / OSMU-V411-TC-004 | FR-01,18 | Given root extensions 15, When code-derived audit를 실행하면, Then 15/15에 runtime loader·credential source·tenant scope·accepted media·external result·permalink·queue compatibility·disposition 8필드가 있고 queue-compatible은 threads/x/instagram 3개뿐이다. |
| AC-05 / OSMU-V411-TC-005 | FR-08 | Given 동일 signed state와 동일 nonce cookie를 가진 callback 20개, When barrier로 동시에 실행하면, Then nonce atomic consume·provider token endpoint 호출·account write가 각각 exactly 1이고 나머지 19개는 token endpoint 전 replay error로 거절된다. |
| AC-06 / OSMU-V411-TC-006 | FR-09 | Given 연결 account, When four surfaces를 보면, Then identity/scope/state/verified-at/CTA diff가 0이다. |
| AC-07 / OSMU-V411-TC-007 | FR-09,NFR-01 | Given provider에 같은 브라우저 세션의 계정 2개, When 목표를 선택·연결하면, Then 저장·review·발행 account가 목표와 같고 cross-tenant call 0이다. |
| AC-08 / OSMU-V411-TC-008 | FR-03 | Given Studio/queue/schedule/video source, When 생성하면, Then tenant-scoped stable source identity와 provenance가 있다. |
| AC-09 / OSMU-V411-TC-009 | FR-04 | Given Studio draft, When Inbox/Calendar 대상이 되면, Then 명시적 bridge 하나가 같은 source를 가리키며 복제 orphan이 없다. |
| AC-10 / OSMU-V411-TC-010 | FR-05 | Given M0 snapshot, When M1 backfill→M2 dual-write→M3 shadow-read→M4 7일 parity→M5 canary→M6 canonical→M7 writer freeze→M8 fallback removal을 진행하면, Then 각 단계의 writer/read authority가 §13.1과 일치하고 exit 전 다음 단계 진입이 0이다. |
| AC-11 / OSMU-V411-TC-011 | FR-06,23,NFR-06 | Given M2~M7 각 단계 한쪽 write failure와 M5 이후 신규 write 100건, When reverse replay 뒤 JSON legacy read로 rollback하면, Then 100/100 source·state·result가 조회되고 record loss·duplicate external call·silent drift가 0이다. |
| AC-12 / OSMU-V411-TC-012 | FR-07 | Given multi-target source, When 한 variant만 수정/승인하면, Then 다른 target payload/approval overwrite가 0이다. |
| AC-13 / OSMU-V411-TC-013 | FR-10,11,NFR-02 | Given no-draft 포함 동일 intent 20개, When 동시에 dispatch하면, Then provider external result가 최대 1개다. |
| AC-14 / OSMU-V411-TC-014 | FR-12,NFR-03 | Given provider success 후 persistence failure, When repair하면, Then republish 0이고 기존 provider result가 저장된다. |
| AC-15 / OSMU-V411-TC-015 | FR-13 | Given timeout/unknown, When reconcile하면, Then enabled text adapter 8개 각각 provider ID·permalink 또는 actionable terminal reason을 회수한다. |
| AC-16 / OSMU-V411-TC-016 | FR-14 | Given 3개 target의 mixed result, When source를 열면, Then 하나의 group에서 target별 status/provider ID/link/updated time이 독립 표시된다. |
| AC-17 / OSMU-V411-TC-017 | FR-15 | Given success 2·failed 1, When retry를 누르면, Then failed 1만 호출되고 success duplicate 0이다. |
| AC-18 / OSMU-V411-TC-018 | FR-21,NFR-04 | Given provider ID/permalink 없음, When 결과를 표시하면, Then provider home URL을 게시물 링크로 표시하지 않는다. |
| AC-19 / OSMU-V411-TC-019 | FR-16 | Given accepted video, When terminal 전 조회하면, Then processing이고 published가 아니다; terminal 뒤만 실제 link와 final status다. |
| AC-20 / OSMU-V411-TC-020 | FR-17 | Given YouTube upload success/timeout, When 저장·reconcile하면, Then TikTok/Reels와 같은 result fields와 duplicate 0을 충족한다. |
| AC-21 / OSMU-V411-TC-021 | FR-16,17 | Given video targets 3, When capability E2E를 실행하면, Then 각 target은 enabled proof 또는 disabled reason/action 중 하나이고 false-success 0이다. |
| AC-22 / OSMU-V411-TC-022 | FR-18 | Given F3, When extensions를 판정하면, Then 15/15 disposition과 owner/proof가 있고 root-only 미검증 고객 노출 0이며 한 번에 승인된 1개만 repair한다. |
| AC-23 / OSMU-V411-TC-023 | FR-20,22,NFR-07 | Given revoke 또는 incident, When dispatch/trace하면, Then 새 external call 0, evidence 보존, source→provider 단계·owner·action 식별이 된다. |
| AC-24 / OSMU-V411-TC-024 | FR-24,NFR-01,05 | Given 2 tenants×2 accounts와 synthetic private payload, When read/write/publish/log/analytics를 검사하면, Then cross-tenant와 raw private/secret leak가 0이다. |
| AC-25 / OSMU-V411-TC-025 | FR-27,F4/BM | **Branch A:** Given 접촉 전 4조건 evidence·prospect ID·contact timestamp가 lock/dedupe된 qualified ledger, When unique qualified 100명을 소진하거나 첫 lock 후 30일 cutoff 중 먼저 도달하고 consented workspace <10이면, Then F1·F2·F3·paid acquisition이 `stopped`, 신규 work/예산 0이며 count·cutoff·flag snapshot이 종료증거다. **Branch B:** Given consented workspace 10개 확보, When 각 21일 activation 창과 activation 후 14일 repeat 창 및 payment record를 평가하면, Then activation=연결+승인+실 permalink ≥3, repeat=다른 두 번째 source 실 permalink ≥3, 7일 환불 가능한 실제결제 ≥2 중 하나라도 미달 시 동일 stop을 실행하고 전부 충족해도 자동 다음-bet 진입은 0이다. |
| AC-26 / OSMU-V411-TC-026 | 전체 | Given PLAN-007·DESIGN-006·v4 sources, When RTM을 검사하면, Then orphan 0이고 v3.1.1/DESIGN v6가 target 근거로 연결된 행은 0이다. |
| AC-27 / OSMU-V411-TC-027 | FR-25,NFR-09 | Given 회장이 §14.5를 승인하고 unchecked consent를 한 외부 고객, When export 또는 해지/삭제를 요청하면, Then raw 30일·evidence 180일 범위와 provider/AI/subprocessor/책임을 사전 표시하고 active store는 7일 내 삭제되며 철회 후 dispatch 0이다. 삭제 시각부터 30일이 지난 backup snapshot을 restore/read하면 대상 raw/evidence가 없거나 cryptographic erasure로 unreadable하고, legal hold 예외는 사전 통지된 범위·근거·기간과 access audit가 있다. 승인 전 cohort create/collect/pay는 0이다. |
| AC-28 / OSMU-V411-TC-028 | FR-26,NFR-10 | Given 52h appetite와 empty work-item ledger, When initial bet 작업을 시작·종료하면, Then enabled proof 대상은 Threads·Instagram Feed·X뿐이고 모든 work item에 planned/actual minutes와 evidence가 append된다. 누적 actual+next planned가 1,680분을 초과하거나 actual이 1,680분에 도달하면 신규 R0~R3 작업과 F1~F4가 `stopped`, 1,681번째 분 시작 0이며 X credential·비용·실 permalink 중 하나라도 실패하면 X disabled다. |
...
| residual qualified prospect/branches | FR-27,NFR-10 | 025 | 4조건 pre-contact lock+dedupe, A/B stop evidence |
| residual backup expiry | FR-25,NFR-09 | 027 | deletion+30d backup restore/read assertion |
| residual time cap | FR-26,NFR-10 | 028 | work-item cumulative ledger, 1,680분 stop |
...
- F4 Branch A에서 4조건을 접촉 전에 증명한 unique qualified 100명을 소진하거나 30일 cutoff 중 먼저 도달했는데 consented workspace가 10개 미만이면 F1~F3·paid acquisition을 `stopped`로 전환한다. contact-ledger lock 없는 prospect는 100명 분모에 넣지 않는다.
...
|---:|---|---|---|---|
| 1 | 용어 통일 | **PASS — v4.1.0 FAIL→v4.1.1 closure** | §3 taxonomy, §13.1 authority, §14.4 qualified/consented/activation/repeat/paid intent의 사전 판정식 | qualified를 단순 연락목록으로 부르던 공백을 4조건+locked unique ledger로 닫은 뒤에만 PASS |
...
| 5 | 정책 상세 | **PASS — v4.1.0 FAIL→v4.1.1 closure** | OAuth replay, migration rollback, Branch A/B, backup 30일 expiry, 1,680분 cap | AC-25/27/28에 stop action·종료증거가 생긴 뒤에만 PASS; branch/backup/time-ledger 중 하나 빠지면 즉시 FAIL |
