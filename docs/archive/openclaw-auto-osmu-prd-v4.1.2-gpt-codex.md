# openclaw-auto OSMU PRD v4.1.2 — F4 timer·circuit-breaker final PATCH

> **상태:** v4.1.1 critic residual final closure PATCH · 승인 전 · DESIGN v6 입력 사용 금지  
...
|---|---|---|
| AC-01 / OSMU-V412-TC-001 | FR-01 | Given repo inventory, When matrix를 재생성하면, Then text=8/video=3/extensions=15이고 중복 범주가 분리된다. |
| AC-02 / OSMU-V412-TC-002 | FR-02,NFR-08 | Given 현재 shell, When v4 UI를 비교하면, Then sidebar/route/settings/provider-tab orphan·무승인 rename·invented top-level nav가 0이다. |
| AC-03 / OSMU-V412-TC-003 | FR-19,NFR-04 | Given 미검증·장애 target, When 고객이 readiness를 보면, Then enabled=false와 reason/action/owner/evidence time이 보인다. |
| AC-04 / OSMU-V412-TC-004 | FR-01,18 | Given root extensions 15, When code-derived audit를 실행하면, Then 15/15에 runtime loader·credential source·tenant scope·accepted media·external result·permalink·queue compatibility·disposition 8필드가 있고 queue-compatible은 threads/x/instagram 3개뿐이다. |
| AC-05 / OSMU-V412-TC-005 | FR-08 | Given 동일 signed state와 동일 nonce cookie를 가진 callback 20개, When barrier로 동시에 실행하면, Then nonce atomic consume·provider token endpoint 호출·account write가 각각 exactly 1이고 나머지 19개는 token endpoint 전 replay error로 거절된다. |
| AC-06 / OSMU-V412-TC-006 | FR-09 | Given 연결 account, When four surfaces를 보면, Then identity/scope/state/verified-at/CTA diff가 0이다. |
| AC-07 / OSMU-V412-TC-007 | FR-09,NFR-01 | Given provider에 같은 브라우저 세션의 계정 2개, When 목표를 선택·연결하면, Then 저장·review·발행 account가 목표와 같고 cross-tenant call 0이다. |
| AC-08 / OSMU-V412-TC-008 | FR-03 | Given Studio/queue/schedule/video source, When 생성하면, Then tenant-scoped stable source identity와 provenance가 있다. |
| AC-09 / OSMU-V412-TC-009 | FR-04 | Given Studio draft, When Inbox/Calendar 대상이 되면, Then 명시적 bridge 하나가 같은 source를 가리키며 복제 orphan이 없다. |
| AC-10 / OSMU-V412-TC-010 | FR-05 | Given M0 snapshot, When M1 backfill→M2 dual-write→M3 shadow-read→M4 7일 parity→M5 canary→M6 canonical→M7 writer freeze→M8 fallback removal을 진행하면, Then 각 단계의 writer/read authority가 §13.1과 일치하고 exit 전 다음 단계 진입이 0이다. |
| AC-11 / OSMU-V412-TC-011 | FR-06,23,NFR-06 | Given M2~M7 각 단계 한쪽 write failure와 M5 이후 신규 write 100건, When reverse replay 뒤 JSON legacy read로 rollback하면, Then 100/100 source·state·result가 조회되고 record loss·duplicate external call·silent drift가 0이다. |
| AC-12 / OSMU-V412-TC-012 | FR-07 | Given multi-target source, When 한 variant만 수정/승인하면, Then 다른 target payload/approval overwrite가 0이다. |
| AC-13 / OSMU-V412-TC-013 | FR-10,11,NFR-02 | Given no-draft 포함 동일 intent 20개, When 동시에 dispatch하면, Then provider external result가 최대 1개다. |
| AC-14 / OSMU-V412-TC-014 | FR-12,NFR-03 | Given provider success 후 persistence failure, When repair하면, Then republish 0이고 기존 provider result가 저장된다. |
| AC-15 / OSMU-V412-TC-015 | FR-13 | Given timeout/unknown, When reconcile하면, Then enabled text adapter 8개 각각 provider ID·permalink 또는 actionable terminal reason을 회수한다. |
| AC-16 / OSMU-V412-TC-016 | FR-14 | Given 3개 target의 mixed result, When source를 열면, Then 하나의 group에서 target별 status/provider ID/link/updated time이 독립 표시된다. |
| AC-17 / OSMU-V412-TC-017 | FR-15 | Given success 2·failed 1, When retry를 누르면, Then failed 1만 호출되고 success duplicate 0이다. |
| AC-18 / OSMU-V412-TC-018 | FR-21,NFR-04 | Given provider ID/permalink 없음, When 결과를 표시하면, Then provider home URL을 게시물 링크로 표시하지 않는다. |
| AC-19 / OSMU-V412-TC-019 | FR-16 | Given accepted video, When terminal 전 조회하면, Then processing이고 published가 아니다; terminal 뒤만 실제 link와 final status다. |
| AC-20 / OSMU-V412-TC-020 | FR-17 | Given YouTube upload success/timeout, When 저장·reconcile하면, Then TikTok/Reels와 같은 result fields와 duplicate 0을 충족한다. |
| AC-21 / OSMU-V412-TC-021 | FR-16,17 | Given video targets 3, When capability E2E를 실행하면, Then 각 target은 enabled proof 또는 disabled reason/action 중 하나이고 false-success 0이다. |
| AC-22 / OSMU-V412-TC-022 | FR-18 | Given F3, When extensions를 판정하면, Then 15/15 disposition과 owner/proof가 있고 root-only 미검증 고객 노출 0이며 한 번에 승인된 1개만 repair한다. |
| AC-23 / OSMU-V412-TC-023 | FR-20,22,NFR-07 | Given revoke 또는 incident, When dispatch/trace하면, Then 새 external call 0, evidence 보존, source→provider 단계·owner·action 식별이 된다. |
| AC-24 / OSMU-V412-TC-024 | FR-24,NFR-01,05 | Given 2 tenants×2 accounts와 synthetic private payload, When read/write/publish/log/analytics를 검사하면, Then cross-tenant와 raw private/secret leak가 0이다. |
| AC-25 / OSMU-V412-TC-025 | FR-27,F4/BM,NFR-10 | **Start:** Given §14.5 회장 승인, When 운영자가 명시적 `Start F4`를 한 번 실행하면, Then immutable `f4_started_at` exactly 1이고 그 전 contact/collect/pay는 0이다. **Branch A:** Given delivery=`delivered`인 qualified contacted count와 `f4_started_at`, When delivered unique 100 소진 또는 `f4_started_at+30d` 중 먼저 도달하고 consented workspace <10이면, Then F1·F2·F3·paid acquisition `stopped`, 신규 work/예산 0, count/start/cutoff/flag snapshot이 종료증거이며 first qualified evidence/lock time 사용은 0이다. fixture `day30, qualified=0, consented=0`도 동일 stop이다. **Branch B:** Given consented 10, When 성숙한 activation/repeat/payment 창을 평가하면, Then activation≥3/repeat≥3/actual-paid≥2 중 하나라도 미달 시 동일 stop이고 전부 충족해도 자동 다음-bet 진입 0이다. 각 attempt outcome은 append되며 failed/bounced/blocked는 contacted 분모 0, audit 보존이다. |
| AC-26 / OSMU-V412-TC-026 | 전체 | Given PLAN-007·DESIGN-006·v4 sources, When RTM을 검사하면, Then orphan 0이고 v3.1.1/DESIGN v6가 target 근거로 연결된 행은 0이다. |
| AC-27 / OSMU-V412-TC-027 | FR-25,NFR-09 | Given 회장이 §14.5를 승인하고 unchecked consent를 한 외부 고객, When export 또는 해지/삭제를 요청하면, Then raw 30일·evidence 180일 범위와 provider/AI/subprocessor/책임을 사전 표시하고 active store는 7일 내 삭제되며 철회 후 dispatch 0이다. 삭제 시각부터 30일이 지난 backup snapshot을 restore/read하면 대상 raw/evidence가 없거나 cryptographic erasure로 unreadable하고, legal hold 예외는 사전 통지된 범위·근거·기간과 access audit가 있다. 승인 전 cohort create/collect/pay는 0이다. |
| AC-28 / OSMU-V412-TC-028 | FR-26,NFR-10 | Given 52h appetite와 running work item이 누적 1,679분에서 시작, When active minute가 1,680에 도달하면, Then circuit breaker가 다음 provider/API/migration batch/file mutation 전 발동하고 current atomic transaction을 commit/rollback한 뒤 safe checkpoint 5필드를 기록한다. 신규 R0~R3와 F1~F4 `stopped`, 1,681분 side effect 0이며 ledger planned/actual/evidence·Threads/IG/X 범위·X readiness도 유지된다. |
...
|---:|---|---|---|---|
| 1 | 용어 통일 | **PASS — v4.1.1 FAIL→v4.1.2 closure** | §14.4의 `f4_started_at`, qualified, contacted=`delivered`, consented, activation/repeat/paid intent | first qualified timestamp와 F4 timer를 분리하고 delivery failure 분모를 정의한 뒤에만 PASS |
...
| 5 | 정책 상세 | **PASS — v4.1.1 FAIL→v4.1.2 closure** | explicit F4 start, day30 zero fixture, delivery outcome, backup expiry, active 1,680분 circuit breaker | AC-25/27/28에 timer origin·failure denominator·safe checkpoint가 생긴 뒤에만 PASS; 하나 빠지면 즉시 FAIL |
...
SKILLS_SKIPPED: `brand-positioning-kit` 등은 본 과업이 브랜드 포지셔닝이 아니라 기존 런타임 감사 기반 PRD PATCH이므로 미적용
