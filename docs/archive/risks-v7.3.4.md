# OSMU Marketing Agent — Risks v7.3.4

<!-- STAMP | created_at: 2026-08-07 17:05 KST | model: gpt-codex/gpt-5.6-sol | agent: prd-architect | skills: brand-positioning-kit, openclaw-creative-brief | evidence: PRD v7.3.4, critic v7.3.3, updated audit96 v1.2+independent H/E/F288, current credential/brand/Studio/analytics | deliberation: 행수/hash/critic 침묵 자기인증을 제거하고 exact observable assertion과 mutation teeth를 named kill gate로 둔다. -->

| 항목 | 값 |
|---|---|
| 버전·정본 | v7.3.4 · [PRD v7.3.4](openclaw-auto-marketing-agent-prd-v7.3.4-gpt-codex.md) |
...
제품 신뢰를 무너뜨리는 사건은 브랜드 근거 덮어쓰기, 대행처럼 보이지만 캠페인 책임이 없는 흐름, workspace·계정·승인·중복, 거짓 성과와 가짜 학습이다. v7.3.4는 source family5·adapter6→guide→brief→artifact→proof→metric→experiment의 version lineage를 hard gate로 둔다.
...
SOURCES: `openclaw-auto-marketing-agent-prd-v7.3.4-gpt-codex.md`; critic v7.3.3; updated audit96 v1.2; independent ABC/DE/FGH H/E/F288 manifest; mutation fixture; agency blueprint14; current credential/RepoConnect/sync-wiki/wiki-retrieve/Studio/analytics; Jasper/HubSpot/Buffer/Sprout; RFC9700.
