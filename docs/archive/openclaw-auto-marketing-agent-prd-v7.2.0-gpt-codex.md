# OSMU Marketing Agent PRD — v7.2 Product Requirements Document

<!--
STAMP
created_at: 2026-08-06 18:15 KST
...
evidence: PRD v7.1.0, independent critic v7.1 MAJOR2/MINOR4, current Studio/publish/analytics/metrics code, wiki/current manifests, official Buffer/Sprout workflow
deliberation: canonical record를 화면 한 곳과 동일시하지 않고 Studio의 Save·지금 게시·예약을 같은 command contract의 initiation surface로 보존하며, native analytics truth를 destination-format 8개 전수 release case로 고정한다.
-->

| 항목 | 값 |
|---|---|
| 버전 | **v7.2.0** — v7.1 critic MAJOR2·MINOR4 surgical closure |
| 작성일 | 2026-08-06 |
| 작성자/모델 | prd-architect / gpt-codex/gpt-5.6-sol |
| 상태 | **in-review** — verifier PASS+independent critic MAJOR0+`/approve plan` 전 downstream 금지 |
| 상류 입력 | PRD v7.1.0; `tasks/marketing-agent-plan-critic-v7.1.output`; current surface/code audits |
...
- Messaging의 record authority는 하나의 logical `message delivery`; Inbox/Calendar는 approval/schedule clients/projections다.
...
| Area | Current judgement | Wiki/current source | Defect | v7.2 target |
|---|---|---|---|---|
...
| Telegram | community message | optional message | bot | message delivery | Inbox+Calendar+result | chat+message ID | explicit not-collected |
| Discord | community message | optional message | webhook/bot | message delivery | Inbox+Calendar+result | destination+message ID | explicit not-collected |
| Slack | community message | optional message | webhook/bot | message delivery | Inbox+Calendar+result | channel+timestamp | explicit not-collected |
...
기존9, rail3, preview7, direct4와 함께 다음을 삭제·이동하지 않는다: 채널별 카드 edit, Save, selected Publish, 예약 button, SchedulePanel, draft/history/wiki/RepoConnect. Management view 추가는 이 primary flow를 대체하지 않는다.
...
| Messaging | message delivery record/command | Inbox approval/detail, Calendar schedule, destination result | Studio optional destination+Inbox+Calendar | channel Queue/Analytics |
...
| Current Studio action | v7.2 target copy | Canonical command | Immediate result | Management projection |
|---|---|---|---|---|
...
- Messaging detail: message delivery edit/send/recovery/result within Inbox; Calendar manages schedule.
...
Policy는 tenant configuration/readiness에서 오며 버튼을 숨기지 않는다. copy와 설명이 실제 command를 반영해야 한다.

### 8.3 Family exposure

- Social: Studio Save가 social delivery를 만들거나 갱신하고 Queue/Inbox/Calendar가 동일 identity를 본다.
- Messaging: 선택한 destination에만 message delivery를 만들고 Inbox/Calendar clients가 승인·일정을 관리한다. channel Queue0.
...
각 case는 native definition, provider/API source, observation window, collected_at, account, publication, capability/state를 제공하고 false-zero0이어야 한다. Feed와 Reels는 Meta account가 같아도 별도 format case다.
...
| FR-MA72-031 | Studio publish copy/command/result를 tenant approval policy별 §7.3대로 분기한다. | direct/required exact | 미구현 |
| FR-MA72-032 | Studio schedule copy/command/result를 tenant approval policy별 §7.3대로 분기한다. | direct/required exact | 부분 |
...
| FR-MA72-037 | Studio/Inbox/Calendar messaging initiation·management는 same message delivery command다. | identity/version diff0 | 미구현 |
...
| FR-MA72-058 | native truth는 definition/source/window/time/account/publication/state와 false-zero0을 가진다. | provenance100% | 미구현 |
...
| FR-MA72-062 | Settings customer-owned, Admin global/support boundary를 지킨다. | customer global mutation0 | 부분 |
...
| FR-MA72-031 | AC-MA72-031 Studio publish policies | **MA72-TC-031:** Given direct/required tenants, When Publish, Then copy=`지금 게시`/`승인 요청`; external≤1/0 and same record. |
| FR-MA72-032 | AC-MA72-032 Studio schedule policies | **MA72-TC-032:** Given direct/required tenants, When 예약, Then copy=`예약`/`예약 승인 요청`; active schedule1/requested0-external. |
...
| FR-MA72-037 | AC-MA72-037 message clients | **MA72-TC-037:** Given message delivery, When Studio/Inbox/Calendar initiate/manage, Then same ID/version/approval/schedule command. |
...
| FR-MA72-058 | AC-MA72-058 native truth | **MA72-TC-058:** Given each native8 case, When render, Then definition/source/window/time/account/publication/state complete and false-zero0. |
...
| FR-MA72-062 | AC-MA72-062 settings/admin | **MA72-TC-062:** Given customer/operator, When actions, Then customer-own/operator-global and proxy publish0. |
...
| RC-R3-STUDIO | Save; publish-direct; publish-approval; schedule-direct; schedule-approval | 5 | 0 |
| RC-R3-DUAL-INIT | Studio+Queue publish same intent; Studio+Calendar schedule same intent | 2 | 0 |

R3 exact exit: **29/29**, safety incident0. `Publish now`와 `Schedule` initiation은 별도 semantic case이며 external side effect≤1이다.
...
R4 exact exit: **53/53**. Native8는 collected 또는 explicit not-collected/unsupported truth, provenance complete, false-zero0이면 PASS다.
...
| Surface | Current | v7.2 target | Forbidden |
|---|---|---|---|
...
| Messaging | setup/executor | message delivery1+clients | Inbox/Calendar two authorities |
...
|---|---|---|
| R0 Plan | v7.2 | verifier PASS+critic MAJOR0+approve |
...
| R3 Trust core | Social5+Messaging3+Studio commands+OAuth | R3 **29/29**, OAuth **18/18**, safety0 |
...
6개월 뒤 실패했다면 designer가 authority=one screen으로 오해해 Studio Publish/예약을 삭제했거나, QA가 video native3만 보고 Social analytics 빈 상태를 놓쳤을 가능성이 크다. 방어는 before→after policy table, Studio semantic7 cases, dual-init2, native8 exact N/M이다.
...
### 23.1 v7.1 critic closure

| Finding | v7.2 closure | FR/TC | Quantitative evidence |
|---|---|---|---|
| MAJOR1 authority vs Studio actions | three terms; before→after; approval policies; message authority1; same command | 028~045 | R3 Studio5+dual2, R3 total29 |
...
### 23.2 v7 original closure regression check

| Original area | v7.2 evidence |
|---|---|
...
| atomic N/M | R3 29/R4 53/OAuth18/VP75/state60/header11 |
| metric denominator/sample/cost | §13 maintained |

### 23.3 User ledger L35~L45
...
- v7.1 critic MAJOR2/2, MINOR4/4 addressed.
- FR/AC/base TC **67/67/67**.
- R3 **29**, R4 **53**, OAuth **18**, navigation **75**, owner-state **60**, header **11**; current M=0.
...
| Approval policy | direct=`지금 게시/예약`; required=`승인 요청/예약 승인 요청` |
...
| Message | logical message delivery authority1 |
| Physical IA | Social>게시물/짧은 영상; Messaging separate |
| Analytics | aggregate whitelist7+native8 truth/no-sum |
...
**이 결론이 틀렸다면 가장 그럴듯한 이유는?** approval policy가 실제 tenant 설정으로 존재하지 않아 design이 두 flow를 동시에 만들 부담이 생길 수 있다. 그래도 현재 direct action을 삭제하지 않으면서 enterprise approval을 지원하려면 policy contract가 필요하다. 초기 cohort는 하나의 policy를 기본으로 하더라도 copy/command semantics를 사칭하면 안 된다.
...
- v7.2 작성: 완료 후보/in-review.
- critic MAJOR2 closure: 문서상2/2, recritic 미실행.
- plan approval: 미승인; provider E2E M=0.
- design entry: verifier PASS+critic major_findings0+`/approve plan` 전 불가.
- v7.0/v7.1/pipeline/wiki/code 변경 없음.

---

RUBRIC_SCORE: 완결성=5/5 정밀성=5/5 벤치마크=5/5 추적성=5/5 전문성=3/5 total=23/25
WEAKEST_LINE: Approval policy 두 흐름은 Studio 보존과 enterprise safety를 동시에 만족시키는 제품 계약이지만 실제 tenant 설정·초기 cohort default는 eng-design/pilot에서 검증해야 한다.
...
SOURCES: `docs/openclaw-auto-marketing-agent-prd-v7.1.0-gpt-codex.md`; `tasks/marketing-agent-plan-critic-v7.1.output`; v7 critic/exit; current Studio/publish/analytics/metrics/Sidebar/constants/videos code; wiki surface map; planning/doc-review/benchmarks/template; Buffer Scheduling/All Channels; Sprout Approval; Later; Hootsuite; RFC9700; Google OAuth; TikTok Login Kit.
MODEL: gpt-codex/gpt-5.6-sol
