# OSMU Marketing Agent PRD — v7.3.3 Product Requirements Document

<!--
STAMP
created_at: 2026-08-07 16:20 KST
...
evidence: PRD v7.3.2, critic v7.3.2 NO-GO major2/minor0, updated final audit96/P0-8, agency blueprint14, current credential code/wiki, Buffer/Jasper/Notion official docs
deliberation: 기존 105개를 보존하고 audit5의 원 predicate와 credential class별 lifecycle을 신규 atomic contract로 폐쇄하며 all96 source predicate manifest를 검증한다.
-->

| 항목 | 값 |
|---|---|
| 버전 | **v7.3.3** — critic v7.3.2 MAJOR2/MINOR0 surgical RETAKE |
| 작성일 | 2026-08-07 |
| 작성자/모델 | prd-architect / gpt-codex/gpt-5.6-sol |
| 상태 | **in-review** — verifier·independent re-critic·`/approve plan` pending; downstream 금지 |
| 상류 입력 | PRD v7.3.2; critic v7.3.2; updated final audit96/P0-8; agency blueprint14; current credential/brand/wiki/onboarding/Studio/Settings code audits |
...
- 광고 집행 제품: ad account 연결, bidding, paid-media spend execution, billing API. v7.3.3 build scope의 paid media는 `없음/외부 관리/미정` 기록과 cap 경고만 허용한다.

## 2. 현재 상태·증거 경계

| Area | Current judgement | Wiki/current source | Defect | v7.3.3 target |
|---|---|---|---|---|
...
| Current Studio action | v7.3.3 target copy | Canonical command | Immediate result | Management projection |
|---|---|---|---|---|
...
- FR/AC/base TC: **112/112/112**, 1:1, orphan0. 기존 MA72-001~067·MA73-068~084·MA731-085~095·MA732-096~105 삭제·재번호·약화0, MA733-106~112만 additive다.
...
| Surface | Current | v7.3.3 target | Forbidden |
|---|---|---|---|
...
|---|---|---|
| R0 Plan | v7.3.3 | verifier PASS+re-critic MAJOR0+approve |
...
| R2 Eng-design | storage/API/command encoding | dialogue+FR112+audit96 source-predicate trace |
...
### 23.1 v7.3.2 critic MAJOR2·MINOR0 closure

| Finding | v7.3.3 correction | Exact evidence |
|---|---|---|
...
### 23.2 v7.3.1 critic MAJOR1·MINOR1 closure

| Finding | v7.3.3 closure | FR/AC/TC | Exit evidence |
|---|---|---|---|
...
### 23.3 v7.3.0 critic MAJOR3·MINOR2 closure
...
### 23.4 Audit96 bidirectional semantic RTM — N96/M0
...
|---|---|---|---|---|
| A01 | 011→AC011→TC011 | login success/cancel/error/retry | SOURCE PREDICATE — fresh identity semantic+callback state | RC-AUD-A01 |
| A02 | 096·097→AC096/097→TC096/097 | first login create or invited join with explicit role | SOURCE PREDICATE — create owner1 or invited membership1; cancel/error phantom/membership0 | RC-AUD-A02 |
| A03 | 098→AC098→TC098 | explicit multi-workspace switch | SOURCE PREDICATE — target all-surface transition; failure retains prior; stale tenant row/command0 | RC-AUD-A03 |
| A04 | 002→AC002→TC002 | forbidden deep-link state | SOURCE PREDICATE — two-tenant read/write/external adversarial | RC-AUD-A04 |
| A05 | 003·062→AC003/062→TC003/062 | customer/approver/operator action matrix | SOURCE PREDICATE — forged role action rejects | RC-AUD-A05 |
| A06 | 013·064·065→AC013/064/065→TC013/064/065 | timeout/401/403/500 recovery | SOURCE PREDICATE — false-ready0+safe correlation | RC-AUD-A06 |
| B01 | 099→AC099→TC099 | non-GitHub existing wiki/file export import | SOURCE PREDICATE — scope preview+exact eligible/selected/skipped docs+per-document source pointer; direct paste completion0 | RC-AUD-B01 |
| B02 | 069·075·085→AC069/075/085→TC069/075/085 | repo/ref/path/permission states | SOURCE PREDICATE — selected files only+success after fetch | RC-AUD-B02 |
| B03 | 073·085·089→AC073/085/089→TC073/085/089 | new wiki first page/save | SOURCE PREDICATE — reload+retrieval+tenant owner | RC-AUD-B03 |
| B04 | 073·076·087→AC073/076/087→TC073/076/087 | fact/inference/unverified editor | SOURCE PREDICATE — confirmed update eligible; unverified block | RC-AUD-B04 |
| B05 | 020·076·088→AC020/076/088→TC020/076/088 | claim inspector+citation state | SOURCE PREDICATE — supported/unsupported claim exact badges | RC-AUD-B05 |
| B06 | 069·075·087→AC069/075/087→TC069/075/087 | sync/freshness/hash state | SOURCE PREDICATE — idempotent sync+eligibility transition | RC-AUD-B06 |
| B07 | 075·087→AC075/087→TC075/087 | last-good+repair state | SOURCE PREDICATE — fault inject partial overwrite0 | RC-AUD-B07 |
| B08 | 074·075·087→AC074/075/087→TC074/075/087 | added/changed/removed diff | SOURCE PREDICATE — reject keeps active; accept new version | RC-AUD-B08 |
| B09 | 078·085→AC078/085→TC078/085 | source-scope→guide draft | SOURCE PREDICATE — deterministic cited fields+auto-active0 | RC-AUD-B09 |
| B10 | 078→AC078→TC078 | structured guide editor | SOURCE PREDICATE — save/reload+actor diff audit | RC-AUD-B10 |
| B11 | 074·088→AC074/088→TC074/088 | version list/diff/rollback | SOURCE PREDICATE — v1 immutable; rollback=v3 | RC-AUD-B11 |
| B12 | 078·086→AC078/086→TC078/086 | approve/reject/change state | SOURCE PREDICATE — forged activation denied | RC-AUD-B12 |
| B13 | 077·088→AC077/088→TC077/088 | artifact used-guide chip | SOURCE PREDICATE — v1 artifact unchanged after v2 | RC-AUD-B13 |
| B14 | 106→AC106→TC106 | selected writing example→tone draft with source/time/rationale, inferred anchors/sliders/phrases and confidence basis | SOURCE PREDICATE — insufficient examples force needs-review, active-tone mutation0 and confident-copy0 | RC-AUD-B14 |
| B15 | 078→AC078→TC078 | plain-language tone/editor preview | SOURCE PREDICATE — rule delta visible, external0 | RC-AUD-B15 |
| B16 | 074·078·088→AC074/078/088→TC074/078/088 | tone draft/active/retired history | SOURCE PREDICATE — v1/v2 binding+mutation invalidation | RC-AUD-B16 |
| B17 | 077·088→AC077/088→TC077/088 | applied tone version/rules | SOURCE PREDICATE — two tone versions inspectable delta | RC-AUD-B17 |
| C01 | 107→AC107→TC107 | opportunity/idea card→`캠페인 시작`, preserving source URL/time/chosen rationale into campaign, approved brief and Studio snapshot | SOURCE PREDICATE — missing source/cancel/duplicate/persist failure creates campaign0 or named hold; lineage loss0 | RC-AUD-C01 |
| C02 | 079→AC079→TC079 | explicit measurable goal/hold | SOURCE PREDICATE — empty vs valid goal branch | RC-AUD-C02 |
| C03 | 079·080→AC079/080→TC079/080 | audience/offer/CTA/message fields | SOURCE PREDICATE — one-field edit propagates variants | RC-AUD-C03 |
| C04 | 079·094→AC079/094→TC079/094 | budget taxonomy/no-spend | SOURCE PREDICATE — no ad/billing call, 미정 hold | RC-AUD-C04 |
| C05 | 079→AC079→TC079 | dates/timezone/overdue conflict | SOURCE PREDICATE — cross-timezone/past deadline | RC-AUD-C05 |
| C06 | 080→AC080→TC080 | brief draft/change/approved/stale | SOURCE PREDICATE — upstream change invalidates fan-out | RC-AUD-C06 |
| C07 | 076·078·081→AC076/078/081→TC076/078/081 | safety/taboo/claim panel | SOURCE PREDICATE — prohibited rule blocks exact claim | RC-AUD-C07 |
| C08 | 020·076·081→AC020/076/081→TC020/076/081 | claim/evidence/resolution list | SOURCE PREDICATE — unsupported price/guarantee block | RC-AUD-C08 |
| C09 | 108→AC108→TC108 | photo/card-news/video rights inspector with source/license-or-permission/territory/expiry/owner/proof attachment | SOURCE PREDICATE — unknown/expired rights block approval+publish for media families3 and adapter calls0 | RC-AUD-C09 |
| C10 | 054·061·081→AC054/061/081→TC054/061/081 | consent/disclosure/N-A reason | SOURCE PREDICATE — person/AI fixtures propagate/block | RC-AUD-C10 |
| C11 | 025·077·082·088→AC025/077/082/088→TC025/077/082/088 | clickable campaign lineage | SOURCE PREDICATE — one ID/version idea→learning | RC-AUD-C11 |
| D01 | 022·081→AC022/081→TC022/081 | Text→Photo/Card→Video numbered order | SOURCE PREDICATE — DOM order at three viewports | RC-AUD-D01 |
| D02 | 021~024→AC021~024→TC021~024 | per-social destination capability cards | SOURCE PREDICATE — unsupported adapter calls0 | RC-AUD-D02 |
| D03 | 023·061·081→AC023/061/081→TC023/061/081 | real card/slides/alt/rights flow | SOURCE PREDICATE — one/many slide save/publish projection | RC-AUD-D03 |
| D04 | 023·024·048~054·081→AC023/024/048~054/081→TC023/024/048~054/081 | script/render/metadata/rights states | SOURCE PREDICATE — job success/failure+provider U label | RC-AUD-D04 |
| D05 | 027→AC027→TC027 | Social→Messaging→Social video | SOURCE PREDICATE — semantic nav order 3 viewports | RC-AUD-D05 |
| D06 | 037·038→AC037/038→TC037/038 | explicit handoff/default OFF | SOURCE PREDICATE — untouched default causes API0 | RC-AUD-D06 |
| D07 | 023·039~041·057→AC023/039~041/057→TC023/039~041/057 | video account/job/result/native owner | SOURCE PREDICATE — capability fixture+readback label | RC-AUD-D07 |
| D08 | 019→AC019→TC019 | safe-disabled MJ explanation | SOURCE PREDICATE — user-token/Discord call0 | RC-AUD-D08 |
| D09 | 008~010·015→AC008~010/015→TC008~010/015 | Header7 every owner | SOURCE PREDICATE — shared server projection diff0 | RC-AUD-D09 |
| D10 | 014·015→AC014/015→TC014/015 | exact account chooser/confirm | SOURCE PREDICATE — two-account switch/reload/target ID | RC-AUD-D10 |
| D11 | 012·013→AC012/013→TC012/013 | Authorized→Ready ladder | SOURCE PREDICATE — fault each step, premature ready0 | RC-AUD-D11 |
| D12 | 015·024·042→AC015/024/042→TC015/024/042 | shared capability version/stale state | SOURCE PREDICATE — revoke propagates all surfaces | RC-AUD-D12 |
| D13 | 024·034·047·057→AC024/034/047/057→TC024/034/047/057 | Threads account/publish/result/insight | SOURCE PREDICATE — adapter+proof/native truth | RC-AUD-D13 |
| D14 | 024·034·047·057→AC024/034/047/057→TC024/034/047/057 | X account/honest capability | SOURCE PREDICATE — no invented analytics/action | RC-AUD-D14 |
| D15 | 024·034·047·057→AC024/034/047/057→TC024/034/047/057 | Facebook Page text/image readiness | SOURCE PREDICATE — Page ID exact+missing analytics truth | RC-AUD-D15 |
| D16 | 024·039·054·057·061→AC024/039/054/057/061→TC024/039/054/057/061 | IG Feed/Card/Reels/rights/native | SOURCE PREDICATE — professional account+format split | RC-AUD-D16 |
| D17 | 024·034·047·057→AC024/034/047/057→TC024/034/047/057 | Bluesky handle/capability | SOURCE PREDICATE — only implemented adapter actions | RC-AUD-D17 |
| D18 | 039·048~054·057→AC039/048~054/057→TC039/048~054/057 | YouTube channel/Shorts job/metadata | SOURCE PREDICATE — progress/result/native+rights | RC-AUD-D18 |
| D19 | 039·048~054·057→AC039/048~054/057→TC039/048~054/057 | TikTok creator/privacy/disclosure | SOURCE PREDICATE — exact creator/result/status | RC-AUD-D19 |
| D20 | 036~038→AC036~038→TC036~038 | Telegram bot/chat handoff OFF | SOURCE PREDICATE — approved target only call1 | RC-AUD-D20 |
| D21 | 036~038→AC036~038→TC036~038 | Discord bot/webhook/guild/channel OFF | SOURCE PREDICATE — explicit approval only | RC-AUD-D21 |
| D22 | 036~038→AC036~038→TC036~038 | Slack workspace/app/channel OFF | SOURCE PREDICATE — explicit approval only | RC-AUD-D22 |
| D23 | 019→AC019→TC019 | MJ non-account capability copy | SOURCE PREDICATE — account/publish-ready state0 | RC-AUD-D23 |
| D24 | 100→AC100→TC100 | Naver title/body/tags/category/preview draft+export | SOURCE PREDICATE — fields exact; unverified direct publish control/call0 | RC-AUD-D24 |
| D25 | 101→AC101→TC101 | Medium publication/title/subtitle/body/tags/canonical/preview draft+export | SOURCE PREDICATE — capability truth; unsupported/unverified adapter call0 | RC-AUD-D25 |
| D26 | 102→AC102→TC102 | Substack subject/preview/body/CTA/audience/send choice draft+export | SOURCE PREDICATE — send default OFF; unverified/unapproved send call0 | RC-AUD-D26 |
| E01 | 026·030→AC026/030→TC026/030 | per-card dirty/save/error/revision | SOURCE PREDICATE — sibling hashes unchanged+reload | RC-AUD-E01 |
| E02 | 021·026·043→AC021/026/043→TC021/026/043 | compatible bulk impact/exclusions | SOURCE PREDICATE — invalid cross-platform mutation0 | RC-AUD-E02 |
| E03 | 031~035·044→AC031~035/044→TC031~035/044 | exact approval snapshot detail | SOURCE PREDICATE — post-approval mutation stale+call0 | RC-AUD-E03 |
| E04 | 031~036·043·044→AC031~036/043/044→TC031~036/043/044 | itemized bulk approvals/blockers | SOURCE PREDICATE — 2 approved+1 blocked not 3 success | RC-AUD-E04 |
| E05 | 031·037→AC031/037→TC031/037 | fresh default OFF consequence | SOURCE PREDICATE — generate/save external calls0 | RC-AUD-E05 |
| E06 | 032·035·044→AC032/035/044→TC032/035/044 | schedule timezone/DST/conflict | SOURCE PREDICATE — no duplicate across DST/cancel | RC-AUD-E06 |
| E07 | 031·043·044→AC031/043/044→TC031/043/044 | publish preflight+repair action | SOURCE PREDICATE — each blocker adapter call0 | RC-AUD-E07 |
| E08 | 033·045→AC033/045→TC033/045 | command/idempotency inspector | SOURCE PREDICATE — double/concurrent call external≤1 | RC-AUD-E08 |
| E09 | 034~042→AC034~042→TC034~042 | same-ID cross-surface links | SOURCE PREDICATE — one mutation projects everywhere | RC-AUD-E09 |
| E10 | 039~042→AC039~042→TC039~042 | artifact→render→publish chain | SOURCE PREDICATE — failed render publish-job0 | RC-AUD-E10 |
| E11 | 046→AC046→TC046 | confirmed failure+attempt history | SOURCE PREDICATE — retry once/draft preserved | RC-AUD-E11 |
| E12 | 109→AC109→TC109 | provider-accepted timeout→uncertain→reconcile by provider/account/time/content hash before any retry | SOURCE PREDICATE — found post binds proof and second publish0; not-found remains uncertain with retry0 until evidenced failed-confirmed | RC-AUD-E12 |
| E13 | 043·046→AC043/046→TC043/046 | per-destination partial result | SOURCE PREDICATE — repair failed target only | RC-AUD-E13 |
| E14 | 047→AC047→TC047 | provider ID/permalink/readback drawer | SOURCE PREDICATE — missing/malformed proof≠published | RC-AUD-E14 |
| E15 | 063·065→AC063/065→TC063/065 | safe error+correlation action | SOURCE PREDICATE — 4xx/5xx/timeout secret scan | RC-AUD-E15 |
| F01 | 055→AC055→TC055 | whitelist definition/period/coverage | SOURCE PREDICATE — prohibited native sum0 | RC-AUD-F01 |
| F02 | 056·057→AC056/057→TC056/057 | native provider names/units | SOURCE PREDICATE — similarly named metrics stay separate | RC-AUD-F02 |
| F03 | 058→AC058→TC058 | six metric states | SOURCE PREDICATE — missing never rendered zero | RC-AUD-F03 |
| F04 | 025·057·058→AC025/057/058→TC025/057/058 | account/revision/publication/window links | SOURCE PREDICATE — filter changes context+value together | RC-AUD-F04 |
| F05 | 059·090·091→AC059/090/091→TC059/090/091 | eligibility/confidence/alternative | SOURCE PREDICATE — thin/biased hold; eligible hypothesis only | RC-AUD-F05 |
| F06 | 084·092→AC084/092→TC084/092 | one-variable approval before/after | SOURCE PREDICATE — exact input lineage+variable1 | RC-AUD-F06 |
| F07 | 084·088·092→AC084/088/092→TC084/088/092 | applied-result chip+diff+undo | SOURCE PREDICATE — only approved variable changes | RC-AUD-F07 |
| F08 | 031·037·091~093→AC031/037/091~093→TC031/037/091~093 | observe/draft/approve/execute boundary | SOURCE PREDICATE — forbidden reply/delete/repost/spend0 | RC-AUD-F08 |
| G01 | 103→AC103→TC103 | Customer Settings8 exact groups/owner/action/state | SOURCE PREDICATE — all eight groups complete; operator field/raw secret0 | RC-AUD-G01 |
| G02 | 104·111→AC104/111→TC104/111 | operator OAuth app Client Secret DB-source timed reveal after exact reauth+intent/reason; env imports first | SOURCE PREDICATE — public/customer/wrong-operator/cross-tenant/expired-reauth/env-direct raw0; no-store, clear≤30s, audit actor/reason/time | RC-AUD-G02 |
| G03 | 105→AC105→TC105 | owner-approved timed support with reason/audit | SOURCE PREDICATE — no approval/expiry access0; publish/schedule/approval/delete/secret/role mutation0 | RC-AUD-G03 |
| G04 | 004·005·067→AC004/005/067→TC004/005/067 | route25/destination26 purpose/action | SOURCE PREDICATE — inventory no removal/generic flatten | RC-AUD-G04 |
| G05 | 063→AC063→TC063 | plain Korean/customer copy | SOURCE PREDICATE — banned internal string scan | RC-AUD-G05 |
| G06 | 006·007→AC006/007→TC006/007 | 390/1024/1440 no overlap | SOURCE PREDICATE — screenshot+interaction+occlusion0 | RC-AUD-G06 |
| G07 | 007→AC007→TC007 | focus/labels/errors/reduced motion | SOURCE PREDICATE — axe+keyboard/manual focus | RC-AUD-G07 |
| G08 | 066·095·112→AC066/095/112→TC066/095/112 | evidence-grade matrix plus source-predicate semantic validator | SOURCE PREDICATE — design semantics and provider proof stay separate; missing noun/action/failure token makes row+aggregate FAIL | RC-AUD-G08 |
| G09 | 016~018·065·111→AC016~018/065/111→TC016~018/065/111 | five credential classes keep distinct plaintext/readback/recovery rules | SOURCE PREDICATE — customer OAuth/BYOK raw0; tenant issue1 then reveal0; env HTTP raw0; operator only G02 path; all leak surfaces0 | RC-AUD-G09 |
| H01 | 008·031·044·063→AC008/031/044/063→TC008/031/044/063 | decision-point truth stays visible | SOURCE PREDICATE — 3 viewport decision without hidden help | RC-AUD-H01 |
| H02 | 110→AC110→TC110 | onboarding/readiness progress completes only from persisted server-read verified evidence | SOURCE PREDICATE — route visit/callback without identified persisted account/local draft each add completion0; fault/reload/stale cache cannot celebrate | RC-AUD-H02 |
| H03 | 017·031·037→AC017/031/037→TC017/031/037 | publish/message OFF+reversible opt-in | SOURCE PREDICATE — fresh tenant external calls0 | RC-AUD-H03 |
| H04 | 027·068·081·089→AC027/068/081/089→TC027/068/081/089 | categorized novice+expert paths | SOURCE PREDICATE — no valid option deleted by psychology claim | RC-AUD-H04 |

Audit ownership count는 A6+B17+C11+D26+E15+F8+G9+H4=`96`이다. `RC-AUD-*` unique96, upstream FR/AC/TC 없는 row0, contract summary 공백0, release 없는 row0, duplicate release ID0이다. 숫자96과 `SOURCE PREDICATE` 라벨은 의미 합격 증거가 아니다. exact source contract/action/failure와 mapping은 별도 pinned manifest에 보존하며, v7.3.2 critic이 지적한 B14/C01/C09/E12/H02와 updated-audit G02/G09은 v7.3.3 외부 재비평 전 semantic PASS를 주장하지 않는다. Design evidence는 prototype-observed, build/QA는 code-tested/sandbox-tested/production-observed로 분리하며 provider 미관찰은 U로 남긴다.
...
### 23.5 v7.2 critic MINOR4 closure
...
### 23.6 v7.1 critic closure regression
...
### 23.7 v7 original closure regression check
...
- FR/AC/base TC **112/112/112**; 기존105 preserved+MA733 additive7.
...
### 23.10 Audit96 source-predicate validator contract
...
- 생성 evidence: `tasks/marketing-agent-prd-v7.3.3-audit96-source-manifest.output` — source row96, mapped row96, missing0. 각 행은 audit ID, 원문 customer contract(noun), required screen/action, required failure evidence, exact FR→AC→TC, release ID를 한 줄에 고정한다.
- validator rule: row count, ID existence, `MATCH`/`SOURCE PREDICATE` 문자열, aggregate count는 semantic score0이다. 원문 predicate 하나라도 삭제·변경하거나 mapping/release가 빠지면 해당 row와 aggregate가 FAIL이다.
- manual evidence: v7.3.2 critic이 전수 대조 후 의미 불일치로 지목하지 않은 89행은 해당 critic 기록을 수동 evidence로 연결한다. B14/C01/C09/E12/H02와 updated-audit에서 의미가 바뀐 G02/G09은 `PENDING v7.3.3 re-critic`이며 self-PASS0이다.
- mutation evidence: MA733-TC-112는 predicate 하나를 의도적으로 제거/변경한 fixture가 그 row와 aggregate를 모두 실패시키는지 검증한다. deterministic sample seed, reviewer, reviewed_at, rationale가 없는 수동 예외는 PASS하지 않는다.
...
- v7.3.3 RETAKE 작성; local structure/render·quality verifier PASS, independent re-critic 전 in-review.
- v7.3.2 critic MAJOR2/MINOR0은 문서상2/2 closure; B14/C01/C09/E12/G02/G09/H02 재비평 대상이다.
...
SOURCES: `docs/openclaw-auto-marketing-agent-prd-v7.3.2-gpt-codex.md`; `tasks/marketing-agent-plan-critic-v7.3.2.output`; `tasks/marketing-agent-v18-agency-blueprint.output`; `tasks/marketing-agent-v18-completeness-audit.output` v1.2; `tasks/marketing-agent-prd-v7.3.3-audit96-source-manifest.output`; current credential/RepoConnect/BrandSetupWizard/sync-wiki/wiki-retrieve/onboarding/Studio/Settings; planning/doc-review/benchmarks/branding/writing/marketing standards; Notion export official help; Medium draft/import/publication official help; Substack draft/publish/export official help; Jasper Knowledge Base; HubSpot Campaigns; GitHub Contents API; OWASP Secrets Management; MDN Cache-Control; RFC9700; Google OAuth; TikTok Login Kit.
MODEL: gpt-codex/gpt-5.6-sol
