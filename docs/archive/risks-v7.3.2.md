# OSMU Marketing Agent — Risks v7.3.2

<!-- STAMP | created_at: 2026-08-07 15:25 KST | model: gpt-codex/gpt-5.6-sol | agent: prd-architect | skills: brand-positioning-kit, openclaw-creative-brief | evidence: PRD v7.3.2, critic v7.3.1, audit96, current brand/Studio/analytics | deliberation: 이중 truth와 thin-data eligibility를 named kill gate로 승격한다. -->

| 항목 | 값 |
|---|---|
| 버전·정본 | v7.3.2 · [PRD v7.3.2](openclaw-auto-marketing-agent-prd-v7.3.2-gpt-codex.md) |
...
제품 신뢰를 무너뜨리는 사건은 브랜드 근거 덮어쓰기, 대행처럼 보이지만 캠페인 책임이 없는 흐름, workspace·계정·승인·중복, 거짓 성과와 가짜 학습이다. v7.3.2는 source family5·adapter6→guide→brief→artifact→proof→metric→experiment의 version lineage를 hard gate로 둔다.
...
| customer/operator settings leak | customer가 shared secret을 보거나 operator가 tenant setting을 임의 변경 | Customer8/Operator9 exact owner/action/state+raw secret0 |
...
SOURCES: `openclaw-auto-marketing-agent-prd-v7.3.2-gpt-codex.md`; critic v7.3.1; audit96; agency blueprint14; current RepoConnect/sync-wiki/wiki-retrieve/Studio/analytics; Jasper/HubSpot/Buffer/Sprout; RFC9700.
