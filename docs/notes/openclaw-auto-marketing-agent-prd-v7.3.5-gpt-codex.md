# OSMU Marketing Agent PRD — v7.3.5 Product Requirements Document

<!--
STAMP
created_at: 2026-08-07 15:12 KST
...
evidence: PRD v7.3.4, critic v7.3.4 NO-GO major2, updated audit96 v1.2/P0-8, independent H/E/F288, agency blueprint14, actual emitted manifest baseline+mktemp mutation, current credential code/wiki, Buffer/Jasper/OWASP/MDN official docs
deliberation: 기존124개를 그대로 보존하고 의미 FAIL 8행을 observable TC로 닫으며 validator가 실제 emitted manifest를 읽게 해 정본 오염을 구조적으로 검출한다.
-->

| 항목 | 값 |
|---|---|
| 버전 | **v7.3.5** — critic v7.3.4 MAJOR2 actual-manifest integrity RETAKE |
| 작성일 | 2026-08-07 |
| 작성자/모델 | prd-architect / gpt-codex/gpt-5.6-sol |
| 상태 | **in-review** — actual-manifest baseline·mktemp mutation·render·quality verifier PASS; independent re-critic·`/approve plan` pending; downstream 금지 |
| 상류 입력 | PRD v7.3.4; critic v7.3.4; updated audit96 v1.2/P0-8; independent H/E/F288; agency blueprint14; v7.3.4 manifest; current credential/brand/wiki/onboarding/Studio/Settings code audits |
...
- 광고 집행 제품: ad account 연결, bidding, paid-media spend execution, billing API. v7.3.5 build scope의 paid media는 `없음/외부 관리/미정` 기록과 cap 경고만 허용한다.

## 2. 현재 상태·증거 경계

| Area | Current judgement | Wiki/current source | Defect | v7.3.5 target |
|---|---|---|---|---|
...
| Current Studio action | v7.3.5 target copy | Canonical command | Immediate result | Management projection |
|---|---|---|---|---|
...
| FR-MA735-125 | fresh identity A와 B가 초대 없이 각각 workspace를 생성하면 tenant ID는 서로 달라야 하고 membership·source·draft·account·job·metric의 cross-identity 연결은 0이어야 한다. | fresh2/distinct tenant1/cross-membership0 | 미구현 |
| FR-MA735-126 | workspace 전환 성공 시 source·draft·account·Queue·analytics가 같은 target workspace로 원자 전환되고, 하나라도 이전 workspace를 가리키면 전환 전체가 실패한다. | projections5/target parity100%/stale0 | 미구현 |
| FR-MA735-127 | non-GitHub import 완료는 문서 수·tenant scope·source pointer뿐 아니라 Studio claim inspector에서 imported claim의 citation이 원문 locator로 resolve되어야 성립한다. | imported citation resolve1/false success0 | 미구현 |
| FR-MA735-128 | artifact guide lineage chip은 applied guide version·rules와 함께 `applied_at`을 저장·reload·표시하며 재생성 시 새 적용시각을 별도 revision에 기록한다. | version-rules-time3/reload100% | 미구현 |
| FR-MA735-129 | Studio의 `Text→Photo/Card→Video` rail은 1440·1024·390 각각에서 DOM 순서 1→2→3, keyboard/jump target, incomplete/blocked truth를 동일하게 유지한다. | viewport3/order3/jump-truth parity | 미구현 |
| FR-MA735-130 | Telegram handoff는 bot identity·bot health·target chat·preview·approval·schedule·result를 같은 record에 보이고, unhealthy bot이나 stale approval에서는 send/schedule call0이다. | fields7/health-schedule truth/call0 | 미구현 |
| FR-MA735-131 | bulk edit impact preview와 confirmation은 selected count와 exact selected account list를 함께 표시하고, selection 변경·권한 상실·mixed capability 발생 시 재확인 전 mutation0이다. | count+accounts exact/reconfirm/mutation0 | 미구현 |
...
| FR-MA735-125 | AC-MA735-125 cross-identity tenant distinctness | **MA735-TC-125:** Happy—Given fresh identities A and B without invites, When each creates a workspace, Then exactly two non-equal tenant IDs and owner memberships exist. Edge—Given A and B choose the same display name, Then IDs remain distinct and each active workspace resolves only its own membership. Failure—Given either identity querying the other's tenant membership, source, draft, account, job or metric, Then rows0, mutation0, external calls0 and A02+Audit96 aggregate FAIL. |
| FR-MA735-126 | AC-MA735-126 workspace-switch projection parity | **MA735-TC-126:** Happy—Given workspace A and B with distinct source, draft, account, Queue item and analytics fixtures, When the user confirms A→B, Then all five projections show B and the same active workspace ID after server readback. Edge—Given one projection is still loading or stale, Then switch status remains partial/reconciling and commands are disabled with the stale surface named. Failure—Given any source, draft, account, Queue or analytics record still points to A, Then success copy0, B command execution0 and A03+Audit96 aggregate FAIL; cancel/failure retains A atomically. |
| FR-MA735-127 | AC-MA735-127 imported citation usability | **MA735-TC-127:** Happy—Given a non-GitHub export with three imported documents and a claim anchored in document2, When import completes and Studio claim inspector opens, Then the citation resolves to document2's exact source locator/span and retrieval can use it. Edge—Given one skipped or malformed document, Then eligible/selected/skipped counts and the unusable citation reason are visible while valid citations still resolve. Failure—Given count/scope/pointer metadata exists but the imported claim citation cannot resolve or opens the wrong tenant/document/span, Then import usable-success0 and B01+Audit96 aggregate FAIL. |
| FR-MA735-128 | AC-MA735-128 guide applied-at lineage | **MA735-TC-128:** Happy—Given approved guide v2 applied to artifact R1, When saved and reloaded, Then the lineage chip shows guide v2, exact applied rules and immutable `applied_at`; regeneration creates R2 with its own later `applied_at`. Edge—Given rollback-created guide v3, Then old R1 keeps v2/time while new generation binds v3/new time. Failure—Given missing/changed `applied_at`, inferred current time, or lineage chip pointing to the active guide instead of the used snapshot, Then B13+Audit96 aggregate FAIL and publication eligibility0. |
| FR-MA735-129 | AC-MA735-129 Studio rail order at three viewports | **MA735-TC-129:** Happy—Given 1440, 1024 and 390 viewports, When Studio DOM and keyboard traversal are inspected, Then Text is position1, Photo/Card position2 and Video position3 at every viewport, with jump targets reaching the same rail. Edge—Given one incomplete or blocked rail, Then its position remains fixed and exact incomplete/blocker truth is visible without reordering. Failure—Given visual CSS order, DOM order, keyboard order or jump target differs at any viewport, Then D01 and Audit96 aggregate FAIL; a general route screenshot contributes PASS0. |
| FR-MA735-130 | AC-MA735-130 Telegram bot health and scheduled handoff | **MA735-TC-130:** Happy—Given a healthy identified bot, selected chat, approved preview and future time, When Telegram schedule confirms, Then one record preserves bot identity, health, chat, approval snapshot, schedule and terminal result. Edge—Given degraded/revoked bot health, changed chat, stale approval or past time, Then the exact state and repair/reapproval action appear and draft remains. Failure—Given unhealthy/unknown bot, unapproved content or stale target, Then send/schedule calls0; false scheduled/success result0 and D20+Audit96 aggregate FAIL. |
| FR-MA735-131 | AC-MA735-131 bulk selected-account confirmation | **MA735-TC-131:** Happy—Given three selected cards across exact accounts A/B/C, When impact preview and confirmation open, Then selected count3 and ordered account list A/B/C appear with compatible field, exclusions and before/after impact. Edge—Given selection, account permission or capability changes after preview, Then confirmation becomes stale, shows the exact diff and requires a new preview. Failure—Given hidden/wrong account, count/list mismatch or confirmation from a stale selection, Then bulk mutation0, sibling hash changes0 and E02+Audit96 aggregate FAIL. |

### 16.1 Coverage

- FR/AC/base TC: **131/131/131**, 1:1, orphan0. 기존 MA72-001~067·MA73-068~084·MA731-085~095·MA732-096~105·MA733-106~112·MA734-113~124 삭제·재번호·약화0, MA735-125~131만 additive다.
...
| Surface | Current | v7.3.5 target | Forbidden |
|---|---|---|---|
...
|---|---|---|
| R0 Plan | v7.3.5 | verifier PASS+re-critic MAJOR0+approve |
...
| R2 Eng-design | storage/API/command encoding | dialogue+FR131+Audit96 QA assertion288 trace |
...
### 23.1 v7.3.4 critic MAJOR2 closure

| Finding | v7.3.5 correction | Exact evidence |
|---|---|---|
| MAJOR1 — Audit96 의미 88/96, A02·A03·B01·B10·B13·D01·D20·E02 FAIL | A02/A03/B01/B13/D01/D20/E02에 atomic FR/AC/TC125~131을 추가하고 8행 모두 source literal과 observable H/E/F가 같은 의미를 검증하도록 재작성했다. B10은 TC114를 유지하되 clean action predicate로 재생성했다 | RTM 수정8행; MA735-125~131; emitted QA-AUD 8행 |
| MAJOR2 — validator가 actual manifest를 읽지 않아 B10 오염을 놓침 | validator 입력을 emitted manifest로 바꾸고 SOURCE/PRD SHA, schema9, rows96, mapping/release와 H/E/F exact equality를 실제 파일에서 검사한다 | `tasks/marketing-agent-v7.3.5-audit96-semantic-fixture.mjs`; baseline aggregate PASS |
| Mutation이 normative artifact를 오염 | `mktemp -d` 사본의 B10 happy action만 제거해 row+aggregate FAIL을 확인하고 canonical SHA before/after 동일을 별도 기록했다 | `/tmp/marketing-agent-v735-semantic-mutation.json`; `/tmp/marketing-agent-v735-manifest-integrity.json` |

### 23.1a v7.3.3 critic MAJOR1·MINOR1 closure

| Finding | v7.3.5 correction | Exact evidence |
|---|---|---|
| MAJOR — 이전 critic 침묵·행수·hash를 Audit96 semantic PASS로 잘못 승격 | 그 근거를 전부 폐기하고 Audit96 96행 각각에 source contract/action/failure 원문과 독립 happy/edge/failure assertion 3개를 부여했다 | `tasks/marketing-agent-prd-v7.3.5-audit96-source-manifest.output`; QA-AUD-*-H/E/F 288 assertions |
...
| Validator self-certification | source literal·행수·hash는 의미 증거0으로 유지하고 v7.3.5 actual-manifest 검사와 독립 re-critic을 분리했다 | `tasks/marketing-agent-v7.3.5-audit96-semantic-fixture.mjs`; §23.11 |
...
| Finding | v7.3.5 closure | FR/AC/TC | Exit evidence |
|---|---|---|---|
...
| A02 | 096·097·125→AC096/097/125→TC096/097/125 + QA-AUD-A02-H/E/F | first login create/join plus two fresh identities with distinct tenant IDs | SOURCE PREDICATE — owner/invited membership exact; fresh A/B tenant IDs differ; cross-membership/source/draft/account/job/metric0 | RC-AUD-A02 |
| A03 | 098·126→AC098/126→TC098/126 + QA-AUD-A03-H/E/F | explicit multi-workspace switch across source/draft/account/Queue/analytics | SOURCE PREDICATE — five target projections share workspace ID; partial/stale blocks commands; cancel/failure retains prior atomically | RC-AUD-A03 |
...
| B01 | 099·127→AC099/127→TC099/127 + QA-AUD-B01-H/E/F | non-GitHub import with exact counts/pointers and Studio-usable citation | SOURCE PREDICATE — imported claim resolves exact tenant/document/span; unresolved or wrong citation means usable-success0 | RC-AUD-B01 |
...
| B13 | 077·088·128→AC077/088/128→TC077/088/128 + QA-AUD-B13-H/E/F | artifact used-guide chip with version/rules/applied_at | SOURCE PREDICATE — save/reload keeps used snapshot time; regeneration/rollback creates new revision time; history rewrite0 | RC-AUD-B13 |
...
| D01 | 022·081·129→AC022/081/129→TC022/081/129 + QA-AUD-D01-H/E/F | Text→Photo/Card→Video DOM/keyboard/jump order at 1440/1024/390 | SOURCE PREDICATE — every viewport order1→2→3; incomplete/blocked truth stays in place; general route screenshot PASS0 | RC-AUD-D01 |
...
| D20 | 036~038·130→AC036~038/130→TC036~038/130 + QA-AUD-D20-H/E/F | Telegram bot identity/health/chat/preview/approval/schedule/result handoff OFF | SOURCE PREDICATE — healthy+approved exact target only call1; degraded/stale/past repair; unsafe send/schedule0 | RC-AUD-D20 |
...
| E02 | 021·026·043·131→AC021/026/043/131→TC021/026/043/131 + QA-AUD-E02-H/E/F | bulk impact with selected count+exact account list+exclusions | SOURCE PREDICATE — selection/account/capability change stales confirmation; count/list mismatch or hidden account mutation0 | RC-AUD-E02 |
...
Audit ownership count는 A6+B17+C11+D26+E15+F8+G9+H4=`96`이다. `RC-AUD-*` unique96, upstream FR/AC/TC 없는 row0, contract summary 공백0, release 없는 row0, duplicate release ID0이다. 숫자96·hash·`SOURCE PREDICATE` 라벨·이전 critic 침묵은 의미 합격 증거0이다. 독립 ABC/DE/FGH 감사가 96행 모두에 작성한 observable H/E/F 288 assertions를 원문 contract/action/failure와 함께 QA-AUD ID로 고정하고 RTM의 mapped TC에 연결했다. 구조 validator는 source retention과 mutation teeth만 증명하며 semantic acceptance는 v7.3.5 independent critic에게 남긴다. Design evidence는 prototype-observed, build/QA는 code-tested/sandbox-tested/production-observed로 분리하며 provider 미관찰은 U로 남긴다.
...
| v7.3.4 critic failed rows exact semantic fixture | 8/8 | 8/8 |
...
- FR/AC/base TC **131/131/131**; 기존124 byte-preserved+MA735 additive7.
...
- normative QA TC catalog: `tasks/marketing-agent-prd-v7.3.5-audit96-source-manifest.output`. 각 행은 exact source contract/action/failure, exact base FR→AC→TC, `QA-AUD-<ID>-H/E/F`, release ID를 한 줄에 고정한다. 이 catalog는 PRD RTM이 직접 참조하는 mapped TC 산출물이다.
- validator rule: `tasks/marketing-agent-v7.3.5-audit96-semantic-fixture.mjs`는 generator의 in-memory row를 PASS시키지 않는다. 실제 `--manifest=<emitted file>`을 parse해 SOURCE/PRD SHA, schema9, rows96, source contract/action/failure, exact mapping/release, exact H/E/F, mutation placeholder0을 검증한다. 행수/hash/라벨/이전 critic 침묵은 semantic score0이다.
- clean baseline evidence: `/tmp/marketing-agent-v735-semantic-baseline.json` — actual manifest rows96, assertions288, supplemented rows8, header/schema/row failure0, aggregate PASS; exact manifest SHA는 evidence JSON에 기록한다.
- immutable mutation evidence: `mktemp -d` 사본 `/tmp/marketing-agent-v735-final-mutation.*/manifest.output`의 B10 happy action만 `[REMOVED_ACTION_PREDICATE]`로 바꿨다. `/tmp/marketing-agent-v735-semantic-mutation.json`은 `happy_assertion_exact`, `source_action_in_happy`, `mutation_placeholder_absent` FAIL, failed_rows1, aggregate FAIL, process exit1이다.
- canonical integrity evidence: `/tmp/marketing-agent-v735-manifest-integrity.json` — canonical SHA before/after exact equality와 mutation 사본 SHA를 기록하고, 정본에는 `[REMOVED_*]` 0개다.
- independent semantic gate: v7.3.5 critic이 96행을 다시 대조해 반례0을 내기 전 semantic acceptance는 PENDING이다. actual-manifest validator PASS는 독립 의미 승인이 아니다.
...
| # | 원칙 | v7.3.5 evidence | 판정 |
|---:|---|---|---|
| 1 | 용어 통일 | authority/view/initiation, policy/readiness, source family5/adapter6, collected/unsupported schema 분리 | PASS |
| 2 | 구체화 | FR131, Audit96 source predicates+H/E/F288, P0-8, campaign14, new atomic64, R3/R4/N75/N60/N11 | PASS |
| 3 | 입출력 분리 | source·setting·review 입력과 record·publication·truth 출력 구분 | PASS |
| 4 | 정합성 | actual emitted manifest의 source contract/action/failure→FR→AC→TC→unique release case 양방향 대조; v7.3.5 외부 재비평 대기 | PASS(구조) / PENDING(의미 재비평) |
...
- v7.3.5 RETAKE 작성; local structure·mutation·render와 quality verifier는 PASS. independent re-critic MAJOR0과 회장 `/approve plan` 전 in-review.
- v7.3.4 critic MAJOR2와 이전 critic findings는 문서상 closure; Audit96 전체 96행 재비평 대상이다.
...
SOURCES: `docs/openclaw-auto-marketing-agent-prd-v7.3.4-gpt-codex.md`; `tasks/marketing-agent-plan-critic-v7.3.4.output`; `tasks/marketing-agent-v18-agency-blueprint.output`; `tasks/marketing-agent-v18-completeness-audit.output` v1.2; independent `tasks/audit96-{abc,de,fgh}-exact-assertions.output`; `tasks/marketing-agent-prd-v7.3.5-audit96-source-manifest.output`; actual baseline/mktemp mutation/canonical-integrity evidence; current credential/RepoConnect/BrandSetupWizard/sync-wiki/wiki-retrieve/onboarding/Studio/Settings; planning/doc-review/benchmarks/branding/writing/marketing standards; Notion export official help; Medium draft/import/publication official help; Substack draft/publish/export official help; Jasper Knowledge Base; HubSpot Campaigns; GitHub Contents API; OWASP Secrets Management; MDN Cache-Control; RFC9700; Google OAuth; TikTok Login Kit.
MODEL: gpt-codex/gpt-5.6-sol
