...
실사 종료 뒤 다른 워커가 공유 작업 트리의 `/api/metrics` GET에 수집 커버리지를 추가했다. 이 후속 변경은 대상 커밋 `d5ac3d1c`에 포함되지 않지만 최신 dev 앱에서 즉시 재호출했다. HTTP 200, `posts` 배열, `coverage.version`, `coverage.source`, `coverage.platforms`, 오류 없음으로 관찰됐다. 후속 집중 회귀 4/4와 현재 TypeScript도 통과해 전수 실사의 HTTP 500 0건 결론은 유지된다.
