<!--
STAMP
파일: docs/prd-openclaw-운영-v1.0.md
라인: osmu / openclaw-service (운영 범위)
판: v1.0 (신설). 정본 PRD v8.2.1의 하위 분할판이며 그것을 대체하지 않는다. R37 "범위를 쪼갠다" 확정에 따라 생성은 studio PRD, 운영은 이 문서가 맡는다
생성: 2026-08-21 KST
model: claude-opus-5[1m]
agent: prd-architect
skills: 없음 호출 (SKILLS_SKIPPED는 문서 끝)
품질헌법: /Users/sj/.claude/standards/planning.md 실제 Read 완료. §2 7원칙 판정표는 §9에 첨부
기반 산출물 (버전핀, 전부 실제 Read):
  - docs/사업계획-osmu-v0.9.md (§1.1~1.6, §2.4~2.7, §3.3 openclaw 기술과제, §3.4 일곱 칸, §4.2~4.6)
  - docs/requests/회장-확정-요구사항-대장.md (R01~R37 전량)
  - wiki/research/2026-08-competitor-and-production-research.md (경쟁 55곳, 원가, 어뷰징 사고)
  - wiki/architecture/two-service-boundary.md (경계 판정 3기준, 소유권 표, 접점 5개)
  - docs/user-flow.md (네 방, 상태 12개, 되돌리기 규칙, PRD 빈틈 17개)
  - wiki/reference/channel-status.md (채널 현황, readiness 계약, capability SSOT)
  - CLAUDE.md (아키텍처 도구 목록, 멀티채널 발행 구조, 큐 스키마 v2, 채널 상태 표, 크론잡 표)
  - docs/prd-openclaw-service-v8.2.1-gpt-codex.md (§2 현재 구현 상태, §8 FR-OS81/82, §9 NFR, §16 미결)
벤치마크: WebSearch 예산 소진(세션 200/200)으로 이번 판은 실검색을 못 했다. 대신 7차례 웹 조사가 전부 verify를 통과해 기록된 wiki/research/2026-08-competitor-and-production-research.md를 벤치마크 원천으로 썼다. 빌린 것 = Planable의 승인 보관함 배치, SocialBee의 전 요금제 생성 무제한 대비 Later의 인색한 AI 크레딧(우리는 상품별 크레딧 분리로 중간을 택함). 버린 것 = Sprout Social식 자리당 과금(1인 사용자에 무의미). 이 판의 벤치마크는 2차 출처 의존이므로 다음 판에서 실검색으로 갱신해야 한다
고민 한 줄: 이 문서에서 값이 있는 부분은 발행 기능 목록이 아니라 "이미 돌아가는 크론 발행 구조 위에 사람이 신뢰할 수 있는 예약과 대행을 어떻게 얹는가"이고, 그래서 지면을 상태 이름과 안전장치에 몰았다
-->

# openclaw-service 운영 PRD v1.0 — 발행 · 채널 운영 · 성과

> **One Thing:** 만들 줄은 아는데 알리질 못하는 사람이, 완성물을 넘긴 그 화면에서 언제 어디로 나갈지 정하고 나간 뒤 무슨 일이 벌어졌는지까지 근거와 함께 보는 곳.

**한 줄 결론:** 이 PRD는 이미 돌아가는 채널 구현과 크론 발행 위에 세 가지를 얹는다. ①발행 세 갈래(지금·보관·예약)와 채널별 문구를 사람이 검수하는 자리 ②맡긴 범위 안에서만 도는 댓글·좋아요 대행과 그 정지 장치 ③성과를 꼬리표에 붙여 무엇이 통했는지 근거와 함께 사람에게 보여주는 것. **자동 학습은 이 판에서 약속하지 않는다.**
...
| 범위 밖 | 생성과 편집의 내부 동작 (studio PRD 소유). 단 경계는 §11에 명시 |
| 전제 | **studio API가 이미 준비돼 있다.** 개발 순서는 studio가 먼저이고 이 문서는 그 다음이다 (사업계획 §4.6 로드맵 1~3단계) |
| 진입 순서와의 관계 | 파는 순서는 운영을 묶은 상품이 먼저다 (사업계획 §4.6 5단계). 만드는 순서와 파는 순서가 반대라는 것을 혼동하지 않는다 |
| 첫 고객 | 회장이 운영하는 **바이브 코딩 커뮤니티**. 만들 줄은 아는데 마케팅에서 막힌 사람들이 이미 모여 있다 (사업계획 §1.1, §4.5 3단계) |
| 시장 근거 | 국내에 발행·응대·성과를 묶은 구독형 도구가 조사 범위(55곳)에서 안 보였다. 그 빈칸이 우리 진입 지점이다 (연구 기록 §1.5) |
| 정본 관계 | PRD v8.2.1의 FR-OS81-001~028, FR-OS82-029~044를 대체하지 않는다. 이 문서의 FR-OP는 그중 운영에 해당하는 조항을 **화면 계약 수준으로 내린 것**이며 대응은 각 FR에 표기했다 |
...
| G2 | 발행 결과가 참인지 거짓인지 사용자가 헷갈리지 않는다 | 증거 없는 발행을 "게시됨"으로 표시한 건수 0 |
| G3 | 예약이 조용히 실패하지 않는다 | 예약 실패가 발생한 뒤 사용자가 그것을 알기까지 걸린 시간의 p95가 1시간 이내 |
| G4 | 대행 응대가 맡긴 범위를 넘지 않는다 | 위임 범위 밖 자동 발화 0 |
| G5 | 무엇이 통했는지 사람이 근거를 보고 판단할 수 있다 | 성과 상위 항목마다 꼬리표와 관찰 기간이 붙어 있는 비율 100퍼센트 |
| G6 | 쓰는 만큼 나가는 원가가 요금을 넘지 않는다 | 계정별 상한을 넘긴 사용 0, 최상급 모델이 요금제 포함분으로 소비된 건수 0 |
...
| 발행실에서 창작 텍스트를 직접 타이핑해 고치는 것 | 경계가 깨진다. 대신 재요청 경로를 둔다 (§4.2 FR-OP-014, 미결 M3) |
...
**박도윤, 32세, 혼자 SaaS를 만든 바이브 코더** (PRD v8.2.1 §5의 페르소나를 이 문서도 그대로 쓴다. 새 인물을 만들면 두 문서가 다른 사람을 향하게 된다.)
...
**근거 (본인 직관 외):** 사업계획 §1.1이 "만드는 건 하는데 알리질 못하는 사람"을 별도 행으로 세우고 그 행에 "우리가 운영 중인 바이브 코딩 커뮤니티가 정확히 이 사람들"이라고 적었다. 즉 관찰 가능한 실제 모집단이 이미 우리 손에 있다. 연구 기록 §4.4는 이 사람이 혼자 하려면 월 약 57만원의 도구값이 든다는 것을 항목별로 적었다. 지불 의사 자체는 미검증이며 이것이 kill-criteria의 대상이다 (§10).
...
**1차 진실원은 위키다.** `wiki/reference/channel-status.md`, 루트 `CLAUDE.md`, PRD v8.2.1 §2를 읽어 채웠다. 코드 검색은 하지 않았다.

| 영역 | 현재 상태 | 정본 출처 | 이 PRD의 처리 |
|---|---|---|---|
| 채널 발행 도구 | **이미구현.** Threads, X, Instagram 전용 도구 + 채널별 publish extension 14종 | `CLAUDE.md` 아키텍처 절 | 그대로 쓴다. 새 발행기 만들지 않는다 |
| 채널 연결·검증 | **이미구현.** OAuth 9채널 코드 완료, 수동 입력 3채널, `verify_channel` 실호출 검증 | channel-status wiki, ADR-004 | 그대로 쓴다. 상태 표시만 §4.1에서 정교화 |
| 채널 readiness 상태 계약 | **이미구현.** `connected · not_connected · opening_soon · publish_pending · error` 5상태 | channel-status wiki | 그대로 쓴다. 발행실이 이 값을 읽는다 |
| 채널 capability SSOT | **이미구현.** `channel-capabilities.ts`가 단일 소스, `constants.ts`가 재수출 | channel-status wiki | **채널 규격 카탈로그(§4.2)는 이것의 확장이다.** 새 소스 만들지 않는다 |
| 멀티채널 발행 크론 | **이미구현.** `multi-channel-publish` 2시간 주기, 채널별 상태 갱신, cleanup 포함 | `CLAUDE.md` 크론 절 | 그대로 쓴다. **단 2시간 주기가 분 단위 예약을 못 지킨다. 이것이 이 PRD의 실제 갭이다 (FR-OP-009)** |
| 큐 스키마 v2 채널별 상태 | **이미구현.** 글마다 채널별 `status · mediaId · publishedAt · error` | `CLAUDE.md` 큐 절 | 그대로 쓴다. 발행 증거의 자리가 이미 있다 |
| 채널별 가이드·키워드 | **이미구현.** 공통 파일 + 채널 전용 파일, 복사 버튼, 채널별 API | `CLAUDE.md` | 그대로 쓴다. 채널별 문구 작성(§4.2)의 입력이 된다 |
| 반응 수집·댓글 좋아요·저조 삭제 | **이미구현(Threads).** `threads-collect-insights` 6시간 주기 | `CLAUDE.md` 크론 절 | **재구현 금지.** 이 PRD가 얹는 것은 기능이 아니라 **위임 범위와 정지 장치**다 (§4.3) |
| 외부 인기글 수집 | **이미구현(Threads).** `threads-fetch-trending` 주 1회 | `CLAUDE.md` | 그대로 쓴다. 성과실 둘러보기의 소스 |
| 팔로워 추적 | **이미구현(Threads).** `threads-track-growth` 매일 | `CLAUDE.md` | 그대로 쓴다 |
| Queue · Inbox · Calendar · Videos 화면 | **부분구현.** 화면은 있고 동일 정본 연결이 미완 | PRD v8.2.1 §2 | 같은 작업물의 다른 창으로 재배선 |
| 발행 증거·복구 | **부분구현** | PRD v8.2.1 §2 | 1차 목표의 최우선 완결 대상 |
| 성과 수집의 미수집 표시 | **부분구현.** 미수집을 0으로 표시하는 문제 | PRD v8.2.1 §2 | §4.4에서 상태를 이름으로 분리 |
| 발행 세 갈래 문구 | **미구현** | R17 | 신설 |
| 채널별 제목·소개·해시태그·첫 댓글의 발행실 소유 | **미구현(경계 이동 직후)** | R31, 경계 wiki (2026-08-19 이동) | 신설. 단 생성 주체는 studio (§4.2) |
| 상품별 크레딧 분리·계정별 상한·최상급 모델 격리·봇 방어 | **미구현** | 사업계획 §4.4 | 신설 (§4.7) |
| 꼬리표를 성과에 잇는 계보 | **미구현** | 사업계획 §1.5, §5 | 신설 (§4.5) |

> ⛔ **회수 필요: 채널 개수가 문서마다 다르다**
> - 배경: 경계 wiki는 "이미 완성된 채널 30종 구현"이라 쓰고, channel-status wiki는 Live 1 + Connected 1 + 구현완료 미연결 13으로 15종을 센다. 루트 CLAUDE.md는 "14개 채널 publish extensions"라 쓴다. 이 PRD를 쓰면서 세 숫자가 다 다른 것을 확인했다.
> - 무엇을 정하나: 발행 가능 채널의 공식 개수와 그 세는 기준(발행 extension 기준인가, 연결 가능 provider 기준인가).
> - 옵션 A(추천): channel-status wiki를 정본으로 삼고 `channel-capabilities.ts`의 항목 수를 기계로 세어 wiki에 박는다 → 고르면: 숫자가 코드에서 나오므로 다시 어긋나지 않는다 / 안 고르면: 랜딩과 요금제 문구에 서로 다른 숫자가 나간다.
> - 옵션 B: 경계 wiki의 30종을 정본으로 두고 나머지를 맞춘다 → 트레이드오프: 30의 근거를 못 찾았다. 근거 없는 숫자를 마케팅에 쓰게 된다.
> - 추천 근거: capability SSOT가 이미 코드 단일 소스로 확정돼 있다(channel-status wiki). 숫자를 사람이 세는 순간 그 SSOT를 우회하는 것이다.

---

## 4. 기능 요구사항과 수용 기준

표기 규칙. **소유**는 화면과 계약의 주인이고, **대응**은 PRD v8.2.1의 상위 조항이다. AC는 Given/When/Then으로 쓰고 경계와 실패를 반드시 포함한다.

### 4.1 화면과 계정 (사용자가 오는 유일한 입구)
...
소유 openclaw · 현재 미구현 · 근거 user-flow 빈틈 12 (이 PRD가 답한다)
...
**FR-OP-007 | 채널별 제목·소개·해시태그·첫 댓글은 발행실이 소유하고 발행 시점 정보로 갱신한다**
소유 openclaw(소유) / studio(생성) · 현재 미구현 · 근거 R31, 경계 wiki 2026-08-19 이동, FR-OS82-044

```gherkin
Given 완성본과 내보낼 채널 3개
When  발행실이 채널별 문구를 준비할 때
Then  공통 문구 1벌과 채널별 차이가 나란히 보이고 무엇이 다른지 표시된다
And   각 채널의 글자수 상한·해시태그 개수 상한이 함께 보이고 초과분은 잘리기 전에 경고된다
And   상한 초과를 사용자 확인 없이 자동으로 잘라내지 않는다
And   문구의 실제 생성은 studio에 요청하며 openclaw가 창작 텍스트를 직접 짓지 않는다
And   발행 직전에 문구가 최신 상태(지금 뜨는 태그·최근 반응·발행 시각)로 갱신됐는지 표시한다
```
...
소유 openclaw · 현재 **부분구현.** 기존 `multi-channel-publish`가 2시간 주기 · 근거 CLAUDE.md 크론 절, 사업계획 §3.3 기술과제
...
소유 openclaw · 현재 미구현 · 근거 R13, user-flow §2.3 빈틈 8 (이 PRD가 답한다)
...
소유 openclaw · 현재 미구현 · 근거 user-flow 빈틈 6 (**이 PRD는 답을 확정하지 않는다. §10 M3**)
...
소유 product gate · 현재 정책 신설 · 근거 이 위임의 명시 지시, 연구 기록 §6 "성과가 학습으로 실제로 이어지는지 우리 실측 전"
...
소유 shared · 현재 미구현 · 근거 경계 wiki §접점 5개, 사업계획 §4.6 1단계
...
소유 openclaw · 현재 부분구현·이관 필요 · 근거 R32·R34, 위임 명시 지시
...
And   공통 값과 워크스페이스 값이 충돌하면 자동으로 고르지 않는다 (§10 M2 결정 전까지 제작을 멈추고 묻는다)
```
...
**격리 원칙 두 가지.** ①워크스페이스가 다르면 어떤 것도 서로 읽거나 연결되지 않는다. ②출력 언어가 다른 신호는 같은 프로파일로 섞이지 않는다.
...
## 7. Steelman 반론 (가장 강한 반대를 내가 만든다)

**반론 1. "발행과 응대는 해자가 아니다. 이걸 묶어 판다는 것은 이미 성숙한 두 시장의 얇은 조합을 파는 것이다."**
이 반론이 맞는 부분이 크다. 연구 기록 §2.3이 확인했듯 Blotato는 월 29달러에 20~40계정 자동 배포와 댓글 관리를 이미 하고, Postiz는 셀프 설치하면 공짜다. 댓글 응대 전용 도구는 월 육천 원짜리도 있다. 즉 우리가 발행실과 채널 운영으로 얻는 기능적 우위는 사실상 없다. 심지어 우리 발행 구현의 대부분은 제3자 오픈소스 위에 있어서, 경쟁자가 같은 것을 벤더링하면 며칠 만에 따라온다. 그렇다면 우리가 파는 것이 "발행 기능"이라면 우리는 이미 진 싸움을 하는 것이다. **우리 답은 꼬리표다.** 도구 두 개를 붙이면 만들 때의 정보와 나간 뒤의 반응 사이에서 계보가 끊긴다. FR-OP-023이 그 끊김을 막는 유일한 조항이고, 소급이 불가능하다는 점이 시간이 갈수록 벌어지는 차이를 만든다. 다만 이것은 아직 가설이며 §10 kill-criteria K2가 그 검증이다.

**반론 2. "자동 학습을 약속하지 않으면 이 제품은 그냥 대시보드다. 사람은 대시보드를 보지 않는다."**
날카롭다. 성과실이 "무엇이 통했는지 근거와 함께 보여준다"에서 멈추면, 그것을 보고 다음 행동을 하는 것은 결국 사람이다. 그런데 우리 타깃은 정확히 "그 판단을 하기 싫거나 못 하는 사람"이다. 즉 우리가 만든 화면이 우리 타깃이 안 하는 일을 요구하게 된다. 이 모순은 실재하고, 자동 학습을 약속하지 않기로 한 결정이 그 모순을 만든다. 우리가 이 반론에 대는 답은 FR-OP-022다. 성과실의 값은 보는 데 있지 않고 "비슷하게 만들기" 버튼 하나로 다음 작업물이 되는 데 있다. 판단을 요구하지 않고 클릭 하나로 잇는다. 그럼에도 근거 표시가 장식이 될 위험은 남으며, 이것이 §10 M6이다.

---

## 8. Premortem (1년 뒤 이 제품이 실패했다면)

**시나리오 A. 조용한 실패가 신뢰를 죽였다.** 2027년 봄, 사용자 마흔 명이 붙어 있었다. 그런데 인스타그램이 토큰 정책을 바꿨고, 우리 갱신 경로가 그 변경을 못 따라가 예약 발행이 2주 동안 조용히 실패했다. 화면에는 "예약됨"이 그대로 떠 있었다. 사용자는 자기 글이 나간 줄 알고 있다가 2주 뒤에야 알았다. 그 뒤로는 우리 화면을 믿지 않고 매번 채널에 직접 들어가 확인했다. 확인할 거면 우리를 쓸 이유가 없다. 이 시나리오의 방어가 FR-OP-010, FR-OP-011, NFR-OP-02, NFR-OP-03이며, 그중 가장 중요한 것은 "증거 없이 게시됨이라 말하지 않는다"는 단 한 줄이다.

**시나리오 B. 대행 답글 하나가 사고를 냈다.** 자영업 고객의 인스타에 환불 관련 항의 댓글이 달렸고, 우리 대행이 정형 답글로 "확인 후 처리해 드리겠습니다"를 붙였다. 고객은 그것을 환불 약속으로 받아들였고 분쟁이 됐다. 사업주는 자기가 하지 않은 말이 자기 계정 이름으로 나간 것에 격분했고, 그 캡처가 커뮤니티에 돌았다. 신규 가입이 멈췄다. 이 시나리오의 방어가 FR-OP-017의 넘김 조건 목록과 전체 정지 스위치이며, 특히 **모든 위임 항목의 기본값이 꺼짐**이라는 것이 마지막 안전선이다. 사고는 기능이 있어서가 아니라 기본값이 켜져 있어서 난다.

**시나리오 C. 원가가 요금을 먹었다.** 프로 요금제 고객 셋이 최상급 영상에 몰아 썼고, 우리는 상품별 크레딧 분리를 나중에 하기로 미뤄 뒀다. 세 계정이 그달 전체 마진을 지웠다. FR-OP-028·029가 이 방어이고, 이것은 출시 후에 붙이면 늦는다. 크레딧 구조는 출시 시점에 이미 분리돼 있어야 한다.
...
| # | 원칙 | 판정 질문 | 판정 | 근거 |
|---|---|---|---|---|
| 1 | 용어 통일 | '정보'·'관리' 같은 추상어가 하나의 정의로 고정됐나 | PASS | "학습 정보"는 §4.6 FR-OP-027에서 원본(openclaw 보관)과 취향 프로파일(studio 계산)로 분리 정의. "채널 운영"은 §4.3의 네 항목(좋아요·정형 답글·자유 답글·내리기)으로만 정의. "성과"는 D10의 네 요소로 고정 |
| 2 | 구체화 | 개수·한도가 숫자로 박혔나 | PASS | 발행 세 갈래, 상태 12개, 위임 4항목, readiness 5상태, 하드캡 1.5배, 정지 60초, 규격 확인일 90일, 미연결 알림 3건, 재시도 20회 |
| 3 | 입출력 분리 | 입력=실질 데이터 / 출력=엔드유저 관점으로 나눠 썼나 | PASS | 각 FR의 AC가 Given(입력 상태)과 Then(사용자가 보는 것)으로 분리. §5는 의미만, 화면은 §4 |
| 4 | 정합성 | 기능 간 서술이 앞뒤 모순 없나 | PASS | FR-OP-007(발행실 소유)과 FR-OP-014(직접 편집 미확정)의 충돌 가능 지점을 M3으로 명시 표면화. FR-OP-015가 기존 크론 동작과 충돌하지 않게 "편입"으로 처리 |
| 5 | 정책 상세 | 경계 케이스 명시했나 | PASS | 예약 중 되돌리기(012), 연결 없는 완성본(004), 증거 미확보(010), 꼬리표 없는 발행(023), 미수집과 미지원 구분(019), 이미 삭제된 글 복구 불가(018) |
| 6 | 추출 철저 | 유저플로우 각 스텝에 대응 기능이 있나 | **부분 PASS** | user-flow의 발행·성과 구간은 전부 대응. 생성·편집 구간은 이 문서 범위 밖(studio PRD 소유)이라 의도적 미대응. 빈틈 17개 중 이 문서가 답한 것은 8·12번이고 1·2·3·4·5·6번은 §10에 미결로 이월 |
| 7 | 논리 영역 | 감상·희망 표현이 판정 가능 문장으로 바뀌었나 | PASS | "친절하게"·"편리하게" 0건. 목표 6개 전부 관측 가능한 계수로 기술 |

---

## 10. 미결 항목

### 10.1 kill-criteria (측정 가능)

| ID | 조건 | 그러면 |
|---|---|---|
| K1 | 커뮤니티에 연 뒤 **8주 안에 유료 전환 10명 미만**이면 | 운영 묶음 상품의 지불 의사 가설을 기각하고 만들기 단독 상품 순서를 앞당긴다 |
| K2 | 발행 100건이 쌓인 시점에 **꼬리표가 붙은 비율이 80퍼센트 미만**이거나, 성과 상위 10건 중 **꼬리표로 설명되는 건이 3건 미만**이면 | 계보 차별점 가설(steelman 1의 답)을 기각하고 성과실을 단순 대시보드로 축소한다 |
| K3 | 출시 후 4주 동안 **조용한 발행 실패(사용자가 1시간 안에 인지 못 한 실패)가 5건 이상** 발생하면 | 예약 자동 실행을 끄고 승인 후 수동 발행만 남긴다 |
| K4 | 대행 응대에서 **위임 범위 밖 발화가 1건이라도** 나오면 | 자유 답글 기능을 즉시 회수하고 정형 답글만 남긴다 |
| K5 | 첫 30일 사용 데이터에서 **상위 10퍼센트 계정의 원가가 그 계정 요금의 100퍼센트를 넘으면** | 요금제를 재설계하기 전까지 신규 가입을 멈춘다 |

### 10.2 회장 결정이 필요한 것

| ID | 무엇을 정하나 | 옵션과 결과 | 세션 추천 |
|---|---|---|---|
| M1 | 채널 개수 정본 (§3의 회수 항목) | A: capability 코드에서 세어 wiki에 박음 → 다시 안 어긋남 / B: 30종을 정본으로 → 근거를 못 찾음 | **A** |
| M2 | 공통 정보와 워크스페이스 값이 충돌할 때 | A: 제작을 멈추고 사람에게 묻는다(자료 충돌 규칙과 동일) → 일관되지만 마찰 / B: 워크스페이스가 공통을 덮는다 → 매끄럽지만 금지선이 조용히 뚫릴 수 있다 | **A.** 금지선이 걸린 문제라 조용한 통과가 더 비싸다 |
| M3 | 발행실에서 캡션을 직접 타이핑하게 할 것인가 | A: 재요청만 허용 → 경계가 깨끗하나 오타 하나에 왕복 / B: 직접 편집 허용하고 그 판을 openclaw 소유로 → 편하지만 창작 텍스트 소유가 둘로 갈린다 | **B의 제한판.** 직접 편집은 허용하되 그 순간 그 문구를 "사람이 고침"으로 표시하고 studio 신호에서 제외. 다만 이것은 경계 원칙의 예외라 회장 확정 없이 화면에 넣지 않는다 |
| M4 | 예약 정밀도 (FR-OP-009) | A: 크론 주기를 짧게 → 정밀하지만 호출량과 비용 증가 / B: 2시간 주기 유지하고 범위를 정직하게 표시 → 싸지만 "저녁 7시 정각"을 못 준다 | **B로 출시하고 K3 결과를 보고 A 검토.** 발행 시각이 성과에 얼마나 영향을 주는지 우리 실측 전이다 |
| M5 | 되돌린 작업물의 크레딧 (user-flow 빈틈 3) | A: 이미 쓴 것은 복구 없음, 재승격은 새 과금 → 단순하고 원가 정합 / B: 일정 기간 안 재승격은 무과금 → 친절하나 어뷰징 표면 | **A.** 단 되돌리기 확인 화면에 반드시 그 사실을 적는다(FR-OP-012에 이미 반영) |
| M6 | 성과 근거 표시가 실제로 다음 행동을 만드는가 | 측정 설계가 필요하다. "비슷하게 만들기" 클릭률을 성과실 방문 대비로 잰다 | 출시와 함께 계측을 넣고 4주 뒤 판정 |
| M7 | user-flow 빈틈 1·2 (PRD를 4실 구조로 개정할 것인가) | 이 문서는 4실을 전제로 썼다. 상위 PRD v8.2.1은 여전히 선형 8단계다 | **개정.** 이 문서와 studio 생성 PRD가 나온 시점에 v8.2.1을 상위 개요로 재정렬 |
...
| 30초에 맞추려고 다시 만드는 것 | studio 생성실 | 새로 만드는 것이므로 |
| 캡션·제목·해시태그·첫 댓글의 **실제 생성** | studio | 창작 텍스트다. openclaw는 소유하고 검수하고 요청한다 |
...
## 12. 자기심문 — 이 PRD가 틀렸다면 어디서 틀렸나

**가장 그럴듯한 오류는 "이미 구현된 것을 전제로 썼다"는 말이 실은 위키를 믿은 것이지 동작을 확인한 것이 아니라는 점이다.** channel-status wiki 자신이 "source presence does not prove production operation"이라 적고 있고, OAuth 거짓 성공·인스타 토큰 UI 중복·설정 상태 누락·502가 미해결 관찰로 남아 있다고 명시한다. 즉 §3의 "이미구현" 열 다수가 실제로는 "코드가 있다"에 가깝다. 이 PRD가 그 위에 발행실과 대행을 얹으면, 아래층이 흔들리는 채로 위층을 짓는 것이 된다. **이 오류를 §10 K3에 반영했고**, 더해 기술설계 착수 조건으로 다음을 건다: 발행실 개발 전에 Threads와 Instagram 각 1건씩 실제 발행 증거를 재현해 아래층이 서 있음을 확인한다. 확인 못 하면 이 PRD의 §4.2는 착수하지 않는다.

**두 번째 오류 후보는 예약 정밀도를 M4로 미룬 것이다.** "저녁 7시에 올려 주세요"는 이 제품에서 사용자가 가장 자주 할 요청인데, 우리는 그것을 2시간 범위로 답하기로 했다. 이것이 틀렸다면 첫 주에 이탈로 드러난다. 그래서 M4의 판정을 K3와 같은 4주 창에 묶었다.

**세 번째.** FR-OP-025는 자동 학습을 약속하지 말라고 하지만, FR-OP-024는 성과 신호를 studio에 전달한다. 사용자 눈에는 이 둘이 같아 보일 수 있다. "보내긴 보내는데 안 배운다"는 설명이 제품 화면에서 성립하는지는 검증 전이며, 성립하지 않으면 신호 전달 자체를 1단계에서 빼는 편이 정직하다. 이것을 M6 판정에 함께 건다.

---

SKILLS_USED: 없음
SKILLS_SKIPPED: postagi-prd(대상 레포가 postAGI가 아니라 스코프 밖), document-generate(신규 문서 1건 작성이라 누락 문서 생성 스킬 미해당), spec(대화형이라 메인세션 전용), plan-ceo-review·plan-eng-review(대화형 리뷰이며 이 위임은 PRD 산출 1건을 지정)
RUBRIC_SCORE: 해당 없음 (콘텐츠·카피 산출물 아님. 제품 요구 명세)
PRESENTATION_CHECK: 툴콜 태그 잔재 없음 확인 / 마크다운 구조 육안 확인 / em dash 0건
SOURCES/MODEL: claude-opus-5[1m] | docs/사업계획-osmu-v0.9.md(§1.1~1.6·§2.4~2.7·§3.3·§3.4·§4.2~4.6) · docs/requests/회장-확정-요구사항-대장.md(R01~R37 전량 + 미결 2건) · wiki/research/2026-08-competitor-and-production-research.md(§1 경쟁 55곳·§2 실물 분석·§3 어뷰징 사고·§4 원가·§6 미확인) · wiki/architecture/two-service-boundary.md(경계 3기준·소유권 표·접점 5개·과금) · docs/user-flow.md(네 방·상태 12개·되돌리기·빈틈 17개 전량) · wiki/reference/channel-status.md(Live/Connected/구현완료 현황·readiness 5상태·capability SSOT) · CLAUDE.md(아키텍처 도구 목록·멀티채널 발행 구조·큐 스키마 v2·채널 상태 표·크론잡 7건) · docs/prd-openclaw-service-v8.2.1-gpt-codex.md(§2 현재 구현 상태·§8 FR-OS81-001~028/FR-OS82-029~044·§9 NFR·§16 미결) · ~/.claude/standards/planning.md | 웹 검색: **미수행(세션 WebSearch 예산 200/200 소진).** 경쟁·원가 근거는 전부 위 연구 기록의 2차 인용이며 실검색 갱신은 다음 판의 과제다 (unsourced 아님, 출처는 verify 통과 기록)
