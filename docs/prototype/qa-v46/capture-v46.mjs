...
const port = process.env.V46_CDP_PORT || '9335';
const outputDir = new URL('./', import.meta.url);
const prototypeUrl = pathToFileURL(new URL('../openclaw-auto-4room-v46.html', import.meta.url).pathname).href;
...
for (const target of [
  { vp: '390', id: 'v44-flow-390-light', file: 'openclaw-auto-v46-390.png' },
  { vp: '1024', id: 'v44-flow-create', file: 'openclaw-auto-v46-1024.png' },
  { vp: '1440', id: 'v44-flow-1440-dark', file: 'openclaw-auto-v46-1440.png' },
]) {
...
results.sidebar.expanded = await evaluate(`(() => {
  localStorage.removeItem('openclaw-v46-sidebar-collapsed');
  ...
  return { collapsed: root.classList.contains('sidebar-collapsed'), width: Math.round(side.getBoundingClientRect().width), label: document.querySelector('[data-action="sidebar-toggle"]').getAttribute('aria-label') };
})()`);
results.sidebar.collapsed = await evaluate(`(() => {
  ...
  const side = root.querySelector('.sidebar');
  return { collapsed: root.classList.contains('sidebar-collapsed'), width: Math.round(side.getBoundingClientRect().width), label: document.querySelector('[data-action="sidebar-toggle"]').getAttribute('aria-label'), persisted: localStorage.getItem('openclaw-v46-sidebar-collapsed') };
})()`);
await sleep(200);
await screenshot('openclaw-auto-v46-1024-sidebar-collapsed.png');
results.captures.push('openclaw-auto-v46-1024-sidebar-collapsed.png');
...
await sleep(200);
await screenshot('openclaw-auto-v46-card-space-a-1024.png');
results.captures.push('openclaw-auto-v46-card-space-a-1024.png');
...
await sleep(200);
await screenshot('openclaw-auto-v46-card-space-b-1024.png');
results.captures.push('openclaw-auto-v46-card-space-b-1024.png');
...
await screenshot('openclaw-auto-v46-screen-selection.png');
results.captures.push('openclaw-auto-v46-screen-selection.png');
...
results.summary = {
  ...
  sidebarToggleWorks: results.sidebar.expanded.width === 224 && results.sidebar.collapsed.width === 56 && results.sidebar.collapsed.persisted === 'true',
