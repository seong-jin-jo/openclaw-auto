...
## 2026-09-03 NG: v68 생성실 담당 패널 접근성 계약 회귀

v68 생성실 개편에서 담당 패널의 시맨틱 요소가 `aside`에서 접근성 이름이 있는 `section`으로
바뀌었다. 화면에는 같은 이름이 보이지만 접근성 역할이 `complementary`에서 `region`으로 바뀌어
화면 낭독기 랜드마크 계약과 기존 계약 테스트 두 건이 깨졌다.

| 검증 | 판정 | 종료 증거 |
|---|---|---|
| 생성실 담당 랜드마크 | NG | `FE3-CREATE-01`, `FE6-CREATE-01` 실패. `생성 담당 대화창`이 `region`으로 노출됨 |
| 다른 방 담당 랜드마크 | 확인 중 | 편집실, 발행실, 성과실의 담당 패널 역할과 접근성 이름 전수 대조 필요 |
| 전체 회귀 | 미검증 | `npx vitest run` 전체 실행 전 |
