...
## 2026-08-28 ❌ NG: 첫 콘텐츠 전에 채널 연결을 요구하는 온보딩
...
**현재 종료조건:** 실제 초기 작업 공간에서 업종 선택 → 올릴 갈래 선택 →
채널 연결 없이 생성실 진입 → 첫 후보 생성을 직접 누른 캡처와 HTTP 응답을
`docs/prototype/qa-fe7/`에 남긴다. 발행 시점에서만 사이드바의 기존 채널 연결
화면으로 이동해야 한다.
