...
## 2026-09-01 NG: 테넌트 접속 기록 부재

고객 신원을 확인해도 `tenants`에 마지막 접속 시각을 남기지 않고, 접속 이력 표도 없어 운영자가
마지막 접속, 최근 30일 접속 일수, 가입 뒤 미접속 여부를 확인할 수 없다. AI 사용량 전용
`usage_events`를 접속 이력으로 오인하지 않는다. 아래 종료 증거가 모두 성립하기 전까지 NG다.

| 검증 | 판정 | 종료 증거 |
|---|---|---|
| 15분 접속 합치기 | NG | 실제 PostgreSQL에서 15분 안 재접속 뒤 접속 이력 1행 |
| 인증 가용성 | NG | 접속 기록 쓰기 실패를 주입해도 고객 테넌트 식별 성공 |
| 운영자 결측 표시 | NG | 기록 없는 작업 공간에 `접속 기록 없음`, 접속 일수 0 표기 없음 |
| 개인정보 최소화 | NG | 접속 이력 표에 테넌트와 시각 외 IP, 지문, 위치, 사용자 에이전트 컬럼 없음 |
| 전체 회귀 | 미검증 | TypeScript와 전체 Vitest 실패 0, design lint 위반 0 |
