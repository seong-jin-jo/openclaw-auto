...
## 2026-09-03 NG: 운영 화면 문구·언어·개인정보 노출
...
긴 대시가 있으며, 발행실 상단에는 내부 실행 도구 이름이 노출된다. 수정 후 계약 테스트와 같은
화면의 직접 관찰 증거를 모두 확보하기 전까지 이 항목은 NG다.

| 검증 | 판정 | 직접 근거·종료 조건 |
|---|---|---|
| 작업 공간 이름과 개인정보 | NG | `live-studio-1024.png`, `live-perf2-1024.png`에서 이메일 제목과 성과 요약 노출. 이름이 없으면 `기본 작업 공간`, 이메일 화면 노출 0건 |
| 쿠키 동의 배너와 담당 패널 | NG | 네 캡처 모두 우하단 입력·전송 영역 가림. 1024 화면에서 두 조작 영역 겹침 0건 |
| 채널 화면 한국어 | NG | `live-connect-1024.png`에서 영문 탭·정보·자동화·설정 라벨 재현. 고객 라벨 영문 0건 |
| UI 긴 대시 | NG | 연결 가이드의 고급 입력 안내에서 긴 대시 재현. 고객 UI 문구 전체 긴 대시 0건을 계약 테스트로 고정 |
| OAuth 기본 흐름의 고객 언어 | NG | 수동 토큰 발급과 권한 코드가 주 안내로 노출. OAuth 연결 단추는 활성 유지하고 수동 입력은 접힌 보조 수단으로 표시 |
| 내부 실행 도구 이름 | NG | `live-studio-1024.png` 상단에 내부 실행 도구 이름 노출. 고객 화면 내부 도구명 0건 |
| 전체 회귀 | 미검증 | TypeScript, 전체 Vitest, design lint, 변경 화면 직접 관찰을 모두 통과해야 함 |
