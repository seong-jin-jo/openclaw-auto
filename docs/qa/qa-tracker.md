...
## 2026-08-28 ❌ NG: 여덟 컨셉을 한 공장 실행으로 운영할 수 없음
...
**현재 종료조건:** 한 작업 공간에서 컨셉 8개를 한 번에 시작하고, 설정한 동시 실행 한도를
넘지 않으며, 의도적으로 실패시킨 컨셉 1개가 있어도 나머지 7개가 성공해야 한다. 실행 조회는
컨셉마다 대기, 실행 중, 성공, 실패와 Studio 생성 작업 번호 또는 오류를 보여야 한다. 다른 작업
공간에서는 실행과 컨셉 행이 보이지 않아야 한다.
