...
## 2026-08-28 ❌ NG → 🔧: Studio 생성 장부가 프로세스 메모리에만 존재
...
**🔧 수정 범위:** 기존 `schema.sql`, `rls.sql`, `withTenant()` 규약으로 생성 작업, 멱등 키,
무료 재생성 사용 기록을 영속화한다. 회원의 현지 날짜 의미와 기존 HTTP 상태·응답 모양은 유지한다.

**종료조건:** Studio E2E 10건, 전체 Vitest와 TypeScript를 통과하고, 앱을 재시작한 뒤 재시작 전에
만든 작업이 같은 회원에게 HTTP 200으로 조회되는 것을 실제 DB 경로에서 관찰한다.
