...
## 2026-09-03 NG: 승인 인박스의 공백 본문 승인 차단 불완전

운영 1024 화면에서 본문을 판단할 내용이 비어 있는데 승인 단추가 활성이고 본문 누락 경고도
나오지 않는 상태를 직접 관찰했다. 현재 화면 코드에는 `trim()` 판정이 있으나 승인 API는 저장된
본문과 선택적 제목을 검증하지 않고 상태를 승인으로 바꾼다. 화면과 API가 같은 공백 판정을 쓰고,
공백과 줄바꿈만 있는 본문에서 경고와 비활성 상태, API 거절을 모두 재현하기 전까지 NG다.

| 검증 | 판정 | 종료 증거 |
|---|---|---|
| 운영 승인 인박스 | NG | `docs/qa/osmu-live-walk-20260903/v70-inbox-1024.png`에서 판단할 본문 없이 승인 단추 활성 관찰 |
| 화면 공백 본문 | 미검증 | 공백과 줄바꿈만 있는 본문에서 경고 노출, 승인 단추 비활성, A 단축키 요청 0건 |
| 승인 API 공백 본문 | 미검증 | 공백과 줄바꿈만 있는 저장 항목의 승인 요청 거절과 상태 불변 |
| 선택적 제목 | 미검증 | 제목 필드가 있는 항목은 공백 제목을 승인하지 않고 정상 제목은 표시 |
