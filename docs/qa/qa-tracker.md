...
## 2026-09-02 NG: v67 발행 계약과 초안 복원 경로 불일치

회장 요청 원문과 v67 디자인 정본을 현재 코드에 대조했다. 텍스트 편집 형식이 카드뉴스로 저장되고,
플랫폼별 제목·캡션·해시태그·선택 계정이 초안 API에서 유실되며, 표시 이름을 발행실에서 직접
수정하거나 일괄 복사할 수 있다. Facebook에 확인되지 않은 상한을 표시하고 발행 전 하드 한도
차단도 없다. 아래 종료 증거를 모두 다시 관찰하기 전까지 v67 build는 NG다.

| 검증 | 판정 | 종료 증거 |
|---|---|---|
| 텍스트 편집 형식 | NG | `{ kind: "text" }` 저장·복원 계약 테스트와 새로고침 실측 |
| 발행 필드 복원 | NG | 제목·캡션·해시태그·선택 계정 저장·복원 통합 테스트 |
| 계정 표시 | NG | 연결 계정의 표시 이름·사용자 이름 읽기 전용, 로딩·오류·미연결 상태 실측 |
| 플랫폼별 필드·한도 | NG | v67 정본의 필드 분기와 하드 한도 정상·거절 계약 테스트 |
| 전체 흐름 | 미검증 | 1024·390에서 연결→생성→편집→발행→성과 실제 클릭 관찰 |
