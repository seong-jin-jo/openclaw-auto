...
## 2026-09-02 NG: v67 발행실 플랫폼 집중 필터 누락

v67 디자인 정본에는 `전체 7곳`과 일곱 플랫폼 집중 필터가 있으나 현재 발행실 소스에는
필터 UI와 상태가 없다. 기존 일곱 플랫폼 미리보기, 입력값, 발행 대상 선택은 보존한 채 보기만
좁히는 계약을 추가하고 1024와 390 실제 렌더로 다시 확인하기 전까지 이 항목은 NG다.

| 검증 | 판정 | 종료 증거 |
|---|---|---|
| 전체 보기 | NG | `PUB-FOCUS-01`에서 기본 상태 일곱 플랫폼 카드 표시 |
| 한 플랫폼 집중 | NG | `PUB-FOCUS-02`에서 플랫폼 칩 선택 뒤 해당 카드 한 장만 표시 |
| 편집·선택 보존 | NG | `PUB-FOCUS-03`에서 필터 왕복 뒤 캡션과 발행 선택 유지 |
| 실제 화면 | 미검증 | 1024와 390에서 칩 줄, 한 장 집중, 전체 복귀를 직접 관찰 |
...
| 플랫폼별 필드·한도 | PASS | `PUB-FIELD-01`, `PUB-LIMIT-01`, `PUB-LIMIT-API-01·02`, `PUB-LIMIT-UI-01` 정상·경계·거절 통과. X 280가중 문자 경계는 손실 없이 발행 |
