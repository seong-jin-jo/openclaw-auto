...
## 2026-08-28 NG: 작업 공간 간 테넌트 격리 공격 검증 미완료

**반려 관찰:** 여섯 사업체가 같은 시스템을 쓰지만, 실제 작업 공간 두 개와 서로 다른
고객 토큰을 사용해 모든 읽기·수정·삭제 경로를 교차 공격한 최신 증거가 없다. 토큰 없음,
만료 토큰, 몸통의 다른 작업 공간 ID가 인증된 작업 공간 경계를 바꾸지 못하는지도 아직
실앱에서 전수 관찰되지 않았다.

**현재 종료조건:** 실제 Postgres에 작업 공간 A와 B 및 구분 데이터를 만들고, A 토큰으로
B의 모든 테넌트 읽기 경로와 수정·삭제 경로를 호출한다. 무토큰·만료 토큰·몸통 위조도
같이 실행한다. 누출은 코드와 RLS를 함께 고치고 정상 1건, 거절 1건 이상의 회귀 테스트를
남긴다. `localhost:3456` HTTP 응답과 DB 잔여를 직접 관찰한 표, 전체 Vitest,
TypeScript, production build까지 통과해야 PASS로 바꾼다.
...
**현재 종료조건:** 같은 로컬 앱과 작업 공간에서 다섯 화면의 첫 그림 시간을 수정 전후로
각각 기록한다. 느린 원인을 번들, 요청 수, 순차 호출 관점에서 특정하고 수정한다. 키보드만으로
주요 흐름이 끝까지 이어지고, 자동 대비 검사 위반이 0이며, 실제 HTTP 상태와 브라우저 화면을
함께 관찰해야 PASS로 바꾼다.
