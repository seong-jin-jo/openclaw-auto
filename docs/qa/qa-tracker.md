...
## 2026-08-28 ❌ NG: 댓글 본문과 후속 행동 계약 부재

**반려 관찰:** 현행 `GET /api/metrics`는 답글 개수만 반환한다. 댓글·반응 본문 목록,
답글 초안, 답글 전송, 댓글 좋아요, 나중 처리, 댓글의 편집실 인계 경로가 없어 사용자가
반응 내용을 읽고 후속 행동을 끝낼 수 없다.

**현재 종료조건:** 채널 capability가 허용하는 동작만 노출하고, 지원하지 않는 채널은 이유를
응답에 명시한다. 계약 단위·통합 테스트와 `localhost:3456` 실제 HTTP 요청에서 정상 경로,
거절 입력, tenant 격리, 보관 상태를 직접 관찰한 뒤 🔧 상태로 전환한다.

## 2026-08-28 ❌ NG: v63 생성실·편집실 실제 앱 준수 부족
...
**수정 종료조건:** `localhost:3456`에서 생성실·편집실을 390, 768, 1024, 1440 네 폭으로
캡처한다. 생성실은 디스플레이 선택 단추 0개와 대화창 후보 선택을, 편집실은 왼쪽
목차, 아래 대사, 아이콘 조작, 삭제·복원 시 길이 변화를 직접 관찰한다. 401·콘솔 오류·가로 넘침은
0건이어야 하며, 전체 Vitest·TypeScript·design lint·production build를 통과한다.
