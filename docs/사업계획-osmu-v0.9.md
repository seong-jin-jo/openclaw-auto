<!--
STAMP | 사업계획-osmu | v0.9 | 개정 2026-08-21 KST | model: claude-opus-5[1m] | agent: main(컨트롤러)
...
정본은 `docs/학습정보-층계-계약-v1.0.md`입니다. 아래는 그 요약입니다.

| 칸 | 무엇 | 직접 할 때 이것 | 누가 채우나 |
|---|---|---|---|
| `R6` 이번 요청 | 주제, 채널, 비용 상한 | 채팅창에 친 프롬프트 | 카드 클릭. 저장하지 않습니다 |
| `L5` 학습된 규칙 | 선택과 성과에서 자란 것 | 기억해 둔 노하우 | 자동으로 자라되 승낙해야 실립니다 |
| `X4` 스킬 | 제작법 파일 | 스킬 파일 | 우리가 등록하고 사용자도 올립니다 |
| `U3` 작업 공간 | 타깃, 컨셉, 반입 문서 요약 | 프로젝트별 지침 파일 | 공간 만들 때 |
| `U2` 계정 공통 | 브랜드 사실, 금지 표현, 말투, 언어 | 루트 지침 파일 | 가입 때 한 번 |
| `S1` 시장·모델 지식 | 시장별 문법, 모델별 말투 | 경험으로 아는 것 | 우리 |
| `S0` 시스템 고정 | 안전선, 표기 의무 | 도구가 박아 둔 규칙 | 아무도 못 바꿉니다 |

**앞글자가 누가 바꿀 수 있는지를 뜻합니다.** S는 우리 쪽, U는 사용자, X는 둘이 섞임, L은 자라남, R은 이번 한 번입니다.

**숫자는 쌓이는 순서이자 겹쳤을 때 이기는 순서입니다.** 위가 아래를 덮되 두 가지는 못 덮습니다. 시스템 고정 전부와 사용자가 적은 금지 표현입니다.

**이 순서는 원가이기도 합니다.** 안 바뀌는 것을 아래에 두면 모델에 보내는 글의 앞부분이 매번 같아져 값이 크게 깎입니다. 순서를 흩으면 같은 내용인데 값만 몇 배가 됩니다.

**스킬은 자기 영역 안에서만 아래를 덮습니다.** 구조와 순서와 길이는 정하고, 말투와 어휘와 브랜드 사실과 금지 표현은 못 건드립니다.
...
### 3.5 다른 팀에서 배운 비용 최적화 (2026-08-21 실물 분석)
