...
function renderGenericChannel(key) {
  ...
  const setupGuides = {
    ...
    instagram: { fields: ["accessToken", "userId"], labels: ["Graph API Access Token", "Instagram Business User ID"],
      quick: ["Instagram을 Business 또는 Creator 계정으로 전환 (프로필 > 설정 > 프로페셔널 계정)", "Facebook Page 생성 후 Instagram 계정과 연결", "developers.facebook.com > 앱 만들기 (비즈니스 유형)", "Instagram Graph API + Instagram Content Publishing 제품 추가", "테스터 등록: 앱 역할 > Instagram Testers에 자기 계정 추가 → Instagram에서 수락", "Graph API Explorer에서 instagram_basic + instagram_content_publish 권한으로 토큰 생성", "User ID: Graph API Explorer에서 GET /me?fields=user_id 호출하여 확인"],
      detail: "Instagram Graph API는 Meta 플랫폼입니다. App Secret(시크릿 코드)은 장기 토큰 교환 시 필요하지만, 대시보드에서는 Access Token만 입력하면 됩니다. 테스터 모드에서는 App Review 없이 자기 계정에 발행 가능. 프로덕션(다른 계정)은 App Review 필요. 토큰 유효기간: 단기 1시간, 장기 60일. 캐러셀(카드뉴스) 발행 지원." },
