    ...
    <aside class="w-56 border-r border-gray-800/50 flex flex-col h-screen sticky top-0" style="background:#0e0e0e">
      ...
      <nav class="flex-1 py-3">
        ...
        <button data-nav="blog" class="sidebar-item ${S.page === "blog" ? "active" : ""} w-full text-left px-4 py-2 text-sm text-gray-300 flex items-center gap-3">
          <span class="w-4 h-4 rounded bg-blue-900 flex items-center justify-center text-[9px] font-bold text-blue-300">B</span>
          Blog (D-Edu)
          <span class="ml-auto text-[10px] px-1.5 py-0.5 rounded-full bg-blue-900/50 text-blue-400">SEO</span>
        </button>
...
function renderBlog() {
  ...
  return `
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="text-xl font-bold text-white">Blog Queue</h2>
        <p class="text-xs text-gray-500 mt-1">Threads → SEO 블로그 글 자동 생성 파이프라인</p>
      </div>
      <span class="text-sm text-gray-500">${posts.length} posts</span>
    </div>
    ${posts.length === 0 ? `
      <div class="card p-8 text-center">
        <p class="text-gray-500 text-sm">블로그 글이 없습니다.</p>
        <p class="text-gray-600 text-xs mt-1">Threads에서 발행된 글이 자동으로 블로그 글로 변환됩니다.</p>
      </div>
    ` : ""}
    <div class="space-y-3">
    ${posts.map(p => `
      <div class="card p-4">
        <div class="flex items-start justify-between mb-2">
          <div class="flex items-center gap-2">
            ...
            ${p.blogPostUrl ? `<a href="${esc(p.blogPostUrl)}" target="_blank" class="text-[10px] text-blue-400 hover:underline">View →</a>` : ""}
          </div>
