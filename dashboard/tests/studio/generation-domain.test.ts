...
import { InMemoryGenerationService } from "@/lib/studio/generation/service";
...
describe("Studio 생성 도메인 계약", () => {
  ...
  it("GEN-CANDIDATE-01 한국어 설명: 후보 A B C 세 장에 형식 근거 비용 채널을 모두 반환한다", () => {
    const service = new InMemoryGenerationService();
    const response = service.create("member-1", "create-1", parseGenerationRequest(generationRequestFixture()));
  ...
  it("GEN-IDEMPOTENCY-01 한국어 설명: 같은 키와 같은 본문은 같은 작업을 돌려준다", () => {
    const service = new InMemoryGenerationService();
    const request = parseGenerationRequest(generationRequestFixture());
    const first = service.create("member-1", "same-key", request);
    const second = service.create("member-1", "same-key", request);

    expect(second).toEqual(first);
  });

  it("GEN-IDEMPOTENCY-02 한국어 설명: 같은 키에 다른 본문은 거절한다", () => {
    const service = new InMemoryGenerationService();
    service.create("member-1", "same-key", parseGenerationRequest(generationRequestFixture()));
    const changed = generationRequestFixture();
    changed.learning_context.r6.topic = "다른 주제";

    expect(() => service.create("member-1", "same-key", parseGenerationRequest(changed))).toThrowError(expect.objectContaining({
      code: "IDEMPOTENCY_CONFLICT",
      status: 409,
    }));
  });

  it("GEN-RETRY-01 한국어 설명: 후보 셋 거절 뒤 첫 재생성은 회원에게 하루 한 번 무료다", () => {
    const service = new InMemoryGenerationService();
    const original = service.create("member-1", "create-1", parseGenerationRequest(generationRequestFixture()));
    const retried = service.regenerate("member-1", original.jobId, new Date("2026-08-27T01:00:00.000Z"));
  ...
  it("GEN-RETRY-02 한국어 설명: 같은 회원의 두 번째 재생성은 비용 승인 전 거절한다", () => {
    const service = new InMemoryGenerationService();
    const original = service.create("member-1", "create-1", parseGenerationRequest(generationRequestFixture()));
    const now = new Date("2026-08-27T01:00:00.000Z");
    service.regenerate("member-1", original.jobId, now);
  ...
  it("GEN-RETRY-03 한국어 설명: 동시에 두 번 재생성해도 무료 처리는 한 건만 성공한다", async () => {
    ...
    const settled = await Promise.allSettled([
      Promise.resolve().then(() => service.regenerate("member-1", original.jobId, now)),
      Promise.resolve().then(() => service.regenerate("member-1", original.jobId, now)),
    ]);
