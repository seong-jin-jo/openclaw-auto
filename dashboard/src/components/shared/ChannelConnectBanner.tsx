...
export function ChannelConnectBanner() {
  ...
  const unconnected = IMPLEMENTED_PLUGINS.filter((k) => {
  ...
  return (
    <Link
      ...
      className="flex items-center justify-between gap-3 mb-4 px-4 py-3 rounded-xl border border-amber-700/40 bg-amber-900/15 hover:bg-amber-900/25 transition"
    >
      ...
      <span className="text-xs px-3 py-1.5 rounded-lg bg-amber-600 text-white whitespace-nowrap">연결하기 →</span>
    </Link>
  );
}
