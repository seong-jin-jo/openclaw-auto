...
export function RepoConnect({ workspace, onSynced, onClose }: { workspace: Workspace; onSynced?: () => void; onClose: () => void }) {
  ...
  const tab = (m: "wiki" | "files", label: string) => (
    <button onClick={() => changeMode(m)} className={`flex-1 py-stack-tight text-caption rounded-control ${mode === m ? "bg-accent text-accent-fg" : "bg-surface-2 text-subtle hover:bg-surface-2"}`}>{label}</button>
  );

  return (
    <div className="fixed inset-0 bg-player-surface/60 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-surface border border-border rounded-surface p-stack-section w-[480px] max-w-[92vw]" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-micro">
          <h3 className="text-body font-bold bg-gradient-to-r from-accent to-accent-hover bg-clip-text text-transparent">📚 레포 위키 연동</h3>
          <button onClick={onClose} className="text-subtle hover:text-muted text-body-sm">✕</button>
        </div>
        <p className="text-caption text-subtle mb-stack">GitHub 위키를 가져옵니다. <b className="text-muted">폴더 전체</b>=문서 다 저장→생성 시 사실 참조 / <b className="text-muted">특정 파일</b>=브랜드 톤만 증류.</p>

        <div className="flex gap-stack-tight mb-stack">{tab("wiki", "📚 폴더 전체 (위키)")}{tab("files", "📄 특정 파일 (톤)")}</div>

        {mode === "wiki" && wikiCount > 0 && (
          <div className="mb-stack px-stack py-stack-tight rounded-control bg-accent-soft border border-accent text-caption text-muted">
        ...
        <div className="space-y-stack">
          <div>
            ...
            <input value={repo} onChange={(e) => changeRepo(e.target.value)} placeholder="https://github.com/owner/repo"
              className="w-full mt-micro px-stack py-stack-tight text-body-sm bg-surface border border-border rounded-control text-muted focus:border-accent outline-none" />
            {repo.trim() && (
              normalizedRepo.ok ? (
                <div className="mt-stack-tight rounded-control border border-border bg-surface-2 px-stack py-stack-tight text-caption text-subtle">
                  <span className="mr-stack-tight">확인된 연결</span>
                  <span className="text-muted">{normalizedRepo.repo}</span>
                  <span className="mx-stack-tight">·</span>
                  <span>브랜치 </span>
                ...
                <p className="mt-stack-tight text-caption text-danger">{normalizedRepo.error}</p>
              )
            )}
          </div>
          <div>
        ...
        {msg && <p className={`text-caption mt-stack ${tone === "ok" ? "text-success" : "text-danger"}`}>{msg}</p>}

        <button onClick={sync} disabled={busy || !repo.trim()}
          className="w-full mt-pad-inset py-stack text-body-sm bg-accent text-accent-fg rounded-control disabled:opacity-50">
          {busy ? "동기화 중… (claude)" : mode === "wiki" ? "📚 위키 폴더 전체 동기화" : "🔄 파일 가져와 톤 갱신"}
        </button>
        <p className="text-caption text-subtle mt-stack-tight">자동 동기화(cron/webhook)는 후속. pgvector 시맨틱 검색 업그레이드 여지.</p>
      </div>
    </div>
  );
}
