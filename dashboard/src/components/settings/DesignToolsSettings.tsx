    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="card p-5">
          ...
          <div className="mb-3">
            ...
            <details className="mt-2 text-[10px]">
              ...
              <div className="mt-2 p-3 rounded bg-surface/50 text-subtle space-y-1.5">
                ...
                <p className="font-medium text-subtle mt-2">Scopes 설정</p>
                <p>앱 설정 페이지 좌측 메뉴 <strong>Scopes</strong> 클릭 -- <strong>Reading and writing</strong> 섹션에서 체크:</p>
                <p className="pl-2"><code className="bg-surface-2 px-1 rounded">design:content</code> Read and Write -- 디자인 생성/수정</p>
                <p className="pl-2"><code className="bg-surface-2 px-1 rounded">design:meta</code> Read -- 디자인 메타데이터</p>
                <p className="pl-2"><code className="bg-surface-2 px-1 rounded">asset</code> Read and Write -- 이미지 업로드</p>
                <p className="pl-2"><code className="bg-surface-2 px-1 rounded">brandtemplate:meta</code> Read -- 템플릿 읽기</p>
                <p className="pl-2"><code className="bg-surface-2 px-1 rounded">brandtemplate:content</code> Read -- 템플릿 내용</p>
                <p className="pl-2"><code className="bg-surface-2 px-1 rounded">profile</code> Read -- 프로필 정보</p>
                <p className="font-medium text-subtle mt-2">OAuth Redirect URL (앱 페이지 &gt; Authentication 탭)</p>
                <p>URL 1 필드에 입력: <code className="bg-surface-2 px-1 rounded">{`https://대시보드주소/api/canva/callback`}</code></p>
                <p>Return navigation 스위치 ON -- Return URL도 동일하게 설정</p>
                <p className="font-medium text-subtle mt-2">앱 유형</p>
          ...
          {canvaEditable && (
            <div className="flex gap-2 mt-4">
              <button onClick={saveCanva} className="flex-1 py-2 bg-accent text-text text-sm rounded hover:bg-accent-hover">
                {canvaConnected ? "Update" : "Connect"}
              </button>
              {canvaConnected && editingCanva && (
                <button onClick={() => setEditingCanva(false)} className="px-4 py-2 bg-surface-2 text-muted text-sm rounded hover:bg-surface-2">Cancel</button>
              )}
            </div>
          )}
        </div>
        ...
        <div className="card p-5">
          ...
          <div className="mb-3">
            ...
            <details className="mt-3 text-[10px]">
              ...
              <div className="mt-2 p-3 rounded bg-surface/50 text-subtle space-y-2">
                ...
                <p className="text-muted font-medium mt-3">Remote MCP 서버 (권장 -- 설치 불필요)</p>
                <p>Figma가 호스팅하는 서버에 연결. 별도 프로그램 설치 없이 URL만 등록하면 됩니다.</p>

                <p className="text-subtle font-medium mt-2">연결 방법 -- Claude Code에서:</p>
                <div className="p-2 rounded bg-surface-2 font-mono mt-1 space-y-1">
                ...
                <p className="text-muted font-medium mt-3">자동화 흐름</p>
            ...
            <details className="mt-2 text-[10px]">
              ...
              <div className="mt-2 p-3 rounded bg-surface/50 text-subtle space-y-1.5">
                ...
                <p className="font-medium text-subtle mt-2">Scopes (권한) 상세</p>
                <p><code className="bg-surface-2 px-1 rounded">file_content:read</code> -- 파일 노드/레이어 읽기, PNG Export에 필수</p>
                <p><code className="bg-surface-2 px-1 rounded">files:read</code> -- 파일 목록 접근</p>
                <p><code className="bg-surface-2 px-1 rounded">file_dev_resources:write</code> -- 개발 리소스 쓰기 (선택)</p>
                <p className="font-medium text-subtle mt-2">지원 MCP 클라이언트</p>
          ...
          {figmaEditable && (
            <div className="flex gap-2 mt-4">
              <button onClick={saveFigma} className="flex-1 py-2 bg-accent text-text text-sm rounded hover:bg-accent-hover">
                {figmaConnected ? "Update" : "Connect"}
              </button>
              {figmaConnected && editingFigma && (
                <button onClick={() => setEditingFigma(false)} className="px-4 py-2 bg-surface-2 text-muted text-sm rounded hover:bg-surface-2">Cancel</button>
              )}
            </div>
          )}
